...
{
    public class IEDataOptimizationService
    {
        ...
        private RMDiscoveryOptimizeDataSettingDto _dataOptimizeSettingDto;
        ...
        private IRMDiscoveryOffice365FileExtensionDao _FileExtensionDao = new RMDiscoveryOffice365FileExtensionDao();

        public IEDataOptimizationService(RMDiscoveryOptimizeDataSettingDto setting, bool checkVersionRule,SourceFlag sourceFlag,bool isProcessDuplicateDatas)
        {
                    ...
                    if (dataList != null && dataList.Count>0)
                    {
                        ...
                        foreach (var item in dataList)
                        {
                            ...
                            var fileData = new RMDiscoveryFileDataInfo
                            {
                                ...
                                HistoryVersionsCount = GetValue<long>(dataObj, "HistoryVersionsCount"),
                                HistoryVersionsSize = GetValue<long>(dataObj, "HistoryVersionsSize"),
                            ...
                            if(tagRuleIds != null && tagRuleIds.Any())
                            {
                                ...
                                {
                                    if (dataObj.TryGet(tagId) is object tagValue && tagValue != null){
                                        fileData.Tags.Add(tagId, tagValue);
                                    }
                                }
                            }
        ...
        public async IAsyncEnumerable<List<RMDiscoveryFileDataInfo>> GetAllDuplicateFilesAsync(string siteId, string webId, string listId, List<string> ObjectIds, int pageSize = 1000)
        {
            ...
            foreach (var objectIdBatch in ObjectIds.Chunk(DuplicateObjectIdBatchSize))
            {
                int offset = 0;
                long maxItemId = 0L;
                var extensionString = $"ObjectId in ('{string.Join("','", objectIdBatch)}')";
                _logger.Info($"Query duplicate files in batch. SiteId:{siteId}, WebId:{webId}, ListId:{listId}, BatchSize:{objectIdBatch.Length}");
        ...
        private async Task InitProcessDuplicateDatasSettingsAsync()
        {
            List<string> filters = new List<string>();
