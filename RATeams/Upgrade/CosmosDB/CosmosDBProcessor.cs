...
namespace RATeams.Upgrade.CosmosDB
{
    public class CosmosDBProcessor
    {
        private static readonly RALogger s_logger = RALogger.GetInstance(typeof(CosmosDBProcessor));
        ...
        public async Task<bool> ChangeBaseRemoteNode(RMRemoteNode changeInfo, Func<Record, RMRemoteNode, Record> func)
        {
            try
            {
                var items = SearchItemsAsync(item => item.SourceFlag == (int)SourceFlag.SharePoint && changeInfo.ObjectId.Equals(item.ScopeId.ToString(), StringComparison.OrdinalIgnoreCase));
                await foreach (var item in items)
                {
                    var processedItem = func(item, changeInfo);
                    await _concurrentAction.Replace(processedItem);
                }
                return true;
            }
            catch (Exception e)
            {
                s_logger.Error($"An error occurred while process change source flag operation of Cosmos DB. While process [{changeInfo.Id}]. Error: {e}");
                return false;
            }
        }
