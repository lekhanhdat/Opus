...
namespace AvePoint.ObjectModel.ServerSE
{
    class AveWorkflowManager : IAveWorkflowManager
    {
        ...
        /// <returns></returns>
        public IAveWorkflow StartWorkflow(object parentItem, IAveWorkflowAssociation association, string eventData, int initiatorId, AveWorkflowRunOptions options)
        {
            ...
            AveAssemblyUtility.SetPropertyValue(spWorkflowEventInstance, "EventType", typeof(ISharePointService).GUID);
            AveAssemblyUtility.SetPropertyValue(spWorkflowEventInstance, "EventName", "OnWorkflowActivated");
            AveAssemblyUtility.SetPropertyValue(spWorkflowEventInstance, "IsActivation", true);
            AveAssemblyUtility.SetPropertyValue(spWorkflowEventInstance, "RunAsUserId", initiatorId);

            if (parentItem is AveListItem)
            {
                object[] objArray = new object[3];
                objArray[0] = eventData;
                objArray[2] = association.AssociationData;
                AveAssemblyUtility.SetPropertyValue(spWorkflowEventInstance, "EventData", objArray);
            }
            else
            {
                AveAssemblyUtility.SetPropertyValue(spWorkflowEventInstance, "EventData", new object[] { eventData, association, association.AssociationData });
            }
            ...
            return new AveWorkflow(association, (SPWorkflow)AveAssemblyUtility.InvokeMethod(mWorkflowManager, "StartWorkflow",
               new Type[] { typeof(object), typeof(SPWorkflowAssociation), typeof(SPListItem).Assembly.GetType("Microsoft.SharePoint.Workflow.SPWorkflowEvent"), typeofSPWorkflowRunOptions },
               new object[] { parentSPItem, (association as AveWorkflowAssociation).WorkflowAssociation, spWorkflowEventInstance, valueofSPWorkflowRunOptions }));
        }
