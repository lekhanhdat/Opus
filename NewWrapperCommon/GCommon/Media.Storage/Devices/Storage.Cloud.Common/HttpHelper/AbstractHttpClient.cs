...
namespace AvePoint.Media.Storage.Cloud.Common
{
    ...
    abstract class AbstractHttpClient : IHttpClient
    {
        ...
        private static StorageLogger logger = StorageLogger.GetInstance(typeof(AbstractHttpClient));
        ...
        private void SetProxySetting(HttpWebRequest request)
        {
            var proxy = default(WebProxy);
            var credential = default(NetworkCredential);
            if (!string.IsNullOrEmpty(this.OpenParam.ProxyIp))
            {
                proxy = new WebProxy(this.OpenParam.ProxyIp, this.OpenParam.ProxyPort);
                if (!string.IsNullOrEmpty(this.OpenParam.ProxyUsername) && !string.IsNullOrEmpty(this.OpenParam.ProxyPassword))
                {
                    credential = new NetworkCredential(this.OpenParam.ProxyUsername, this.OpenParam.ProxyPassword);
                }
            }
            SetUpProxy(request, proxy, credential);
        }
        ...
        public void AddHeaders(HttpWebRequest request, SortedList<HttpRequestHeader, object> headers)
        {
            if (headers == null)
            {
                return;
            ...
            foreach (KeyValuePair<HttpRequestHeader, object> item in headers)
            {
                HttpRequestHeader key = item.Key;
                object value = item.Value;

                if (key == HttpRequestHeader.ContentType)
                {
                    request.ContentType = value as string;
                    continue;
                }

                if (key == HttpRequestHeader.ContentLength)
                {
                    request.ContentLength = Convert.ToInt64(value);
                    continue;
                }

                if (key == HttpRequestHeader.Range)
                {
                    string range = value as string;
                    string[] ranges = range.Split(new Char[] { ',' });

                    int offset = Convert.ToInt32(ranges[0]);
                    int length = Convert.ToInt32(ranges[1]);

                    request.AddRange(offset, offset + length - 1);
                    continue;
                }

                request.Headers.Add(key, value as string);
            }
        }
        #endregion
        ...
        private void CombiningHeaders(HttpWebRequest request, Dictionary<string, string> headers)
        {
