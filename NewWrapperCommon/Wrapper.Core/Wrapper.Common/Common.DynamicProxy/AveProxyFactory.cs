...
namespace AvePoint.Wrapper.Common
{
	/// <summary>
	/// </summary>
	public class AveProxyFactory
	{
        private static AveProxyFactory instance;
        ...
        private AveProxyFactory()
        {

        }

        public static AveProxyFactory GetInstance()
        {
            if (instance == null)
            {
                CreateInstance();
        ...
        private static void CreateInstance()
        {
            lock (lockObj)
            {
                if (instance == null)
                {
                    instance = new AveProxyFactory();
                }
            }
        }

        public Object Create(IAveProxyInvocationHandler handler, Type objType, bool isObjInterface)
        {
        ...
        public Object Create(IAveProxyInvocationHandler handler, Type objType)
        {
            return Create(handler, objType, false);
        }

        private Type CreateType(IAveProxyInvocationHandler handler, Type[] interfaces, string dynamicTypeName)
        {
            Type retVal = null;
            ...
            if (handler != null && interfaces != null)
            {
                ...
                Type handlerType = typeof(IAveProxyInvocationHandler);
        ...
        {
            AveMetaDataFactory.Add(interfaceType);
            ...
            if (interfaceMethods != null)
            {
                ...
                for (int i = 0; i < interfaceMethods.Length; i++)
                {
                    ...
                    OpCode resultLocalStore = OpCodes.Stloc_0;
                    OpCode resultLocalLoad = OpCodes.Ldloc_0;
                    if (numOfParams > 0)
                    {
                        methodIL.DeclareLocal(typeof(System.Object[]));
                        resultLocalStore = OpCodes.Stloc_1;
                        resultLocalLoad = OpCodes.Ldloc_1;
                    }
                    ...
                    methodIL.Emit(OpCodes.Call,
                        typeof(AveMetaDataFactory).GetMethod(
                    ...
                    methodIL.Emit(OpCodes.Callvirt,
                        typeof(IAveProxyInvocationHandler).GetMethod("Invoke"));
                    ...
                    if (!methodInfo.ReturnType.Equals(typeof(void)))
                    {
                        ...
                        // store the result
                        methodIL.Emit(resultLocalStore);
