...
{
    public class SharePointSettingsJobDetailWorker : AbstractDetailWorker
    {
        public override List<List<ReportCell>> ConvertReportData(IEnumerable<JMJobDetails> details)
        {
            ...
            {
                if (detail is JMGlobalSettingJobDetails)
                {
                    List<ReportCell> reports = new List<ReportCell>();
                    JMGlobalSettingJobDetails reportnfo = detail as JMGlobalSettingJobDetails;
                    reports.Add(new ReportCell() { Key = "ObjectName", Value = reportnfo.ObjectName });
                    reports.Add(new ReportCell() { Key = "SourceURL", Value = reportnfo.SourceURL });
                    reports.Add(new ReportCell() { Key = "ColumnName", Value = reportnfo.ColumnName });
                    reports.Add(new ReportCell() { Key = "Action", Value = reportnfo.Action });
                    reports.Add(new ReportCell() { Key = "Classification", Value = reportnfo.SettingScopeId });
        ...
        public override void InitCreateTableSQLString()
        {
            ...
            CREATE_TABLE_SQL = string.Format(JobMonitorConstants.CREATE_TABLE_GLOBAL_SETTING, TABLE_NAME);
            INSERT_DATA_SQL = string.Format(JobMonitorConstants.INSERT_DATA_GLOBAL_SETTING, TABLE_NAME);
        }
    }
}
