...
namespace AvePoint.GCommon.Utility.Cryptography
{
    public static class CspCommunicationWrapper
    {
        ...
        private static byte[] staticCommunicationEncryptionKeyHash = HashAlgorithmFactory.CreateHashAlgorithm(HashAlgorithm.SHA1, new byte[0]).ComputeHash(staticCommunicationEncryptionKey);
        ...
        /// <summary>
        /// encrypt by default AES key
        /// </summary>
        /// <returns></returns>
        public static byte[] WrapKeyByDefault(byte[] password)
        {
            CheckAvailable();
            if (password != null)
            {

                IEncryption en = EncryptionFactory.GetEncryption(EncryptionAlgorithm.AES_ENCRYPTION, staticCommunicationEncryptionKey, staticCommunicationEncryptionKeyHash);
                byte[] key = en.EncryptBinary(password);
                return key;
            }
            return null;
        ...
        /// </summary>
        /// <returns></returns>
        public static string WrapKeyToBase64StringByDefault(byte[] password)
        {
            if (password != null)
            {

                byte[] result = WrapKeyByDefault(password);
                return Convert.ToBase64String(result);
            }
            return null;
        }
