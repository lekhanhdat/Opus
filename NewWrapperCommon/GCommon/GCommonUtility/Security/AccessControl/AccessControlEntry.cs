...
namespace AvePoint.GCommon.Security.AccessControl
{
    public class AccessControlEntry : ICollection<AceRights>
    {
        ...
        public override string ToString()
        {
            ...
            for (int flag = 0x01; flag <= (int)AceFlags.FailedAccess; flag = flag << 1)
            {
        ...
        public string DetailInfo()
        {
            ...
            sb.AppendFormat("AceType:{0}, AceFlags:{1}, AceRights:{2}, ObjectGuid:{3}, InheritObjectGuid:{4}, ", this.aceType, this.flags, this.rights, this.objectGuid, this.inheritObjectGuid);

            if (this.accountSID != null)
            {
                sb.AppendFormat("Account:{0}, SID:{1}, WellKnownSidType:{2}", accountSID.Name, accountSID.SID, accountSID.WellKnownSidType);
            }
            else
            {
                sb.AppendFormat("Account:NULL");
            }
        ...
        public static AccessControlEntry AccessControlEntryFromString(string aceString)
        {
            ...
            {
                int aceTypeValue = Array.IndexOf<string>(AccessControlEntry.aceTypeStrings, aceMatch.Groups["ace_type"].Value.ToUpperInvariant());
            ...
            {
                string aceFlagsValue = aceMatch.Groups["ace_flags"].Value.ToUpperInvariant();
            ...
            {
                string rightsValue = aceMatch.Groups["rights"].Value.ToUpperInvariant();
            ...
            {
                ace.accountSID = SecurityIdentity.SecurityIdentityFromString(aceMatch.Groups["account_sid"].Value.ToUpperInvariant(), false);
            }
            else throw new FormatException("Invalid ACE String Format");
