...
{
    public abstract class BaseStartWithFilterBuilder : IFilterBuilder
    {
        public Query Filter(Query query, ExplorerFilterOptionV2 filterOption)
        {
            if (CanFilter(filterOption))
            {
                return query.WhereStarts(GetFilterColumnName().FormatColumnName(), GetFilterValue(filterOption));
            }
            return query;
        }
