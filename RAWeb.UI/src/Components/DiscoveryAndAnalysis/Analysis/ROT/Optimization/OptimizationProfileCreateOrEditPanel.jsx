...
import GroupedRule from "../../Components/GroupedRule";
...
const InactiveOptimizationProfileCreateOrEditPanel = (
    ...
    const onRuleInfoChange = (queryParameter) => {
        const clonedProfileInfo = _.cloneDeep(profileInfo);
        clonedProfileInfo.ruleIds =
            queryParameter.rotRuleQueryParameter.ruleIds;
        setProfileInfo(clonedProfileInfo);
        if (validateInfoes.has("rule")) {
            const clonedValidateInfoes = _.cloneDeep(validateInfoes);
            clonedValidateInfoes.delete("rule");
            setValidateInfoes(clonedValidateInfoes);
        }
    };
    ...
    const onSave = useStableCallback(async () => {
        ...
        if (_.isEmpty(profileInfo.name)) {
            clonedValidateInfoes.set(
                "name",
                RMResx.RM_PRM_PRE_ColumnValid_RequireText
            );
        }
        ...
        if (validateNameLength) {
            clonedValidateInfoes.set(
                "name",
                RMResx.RM_PF_profileNametooLong
            );
        }

        if (
            existsProfileInfoes.some(
                (item) =>
                    item.name.toLowerCase().trim() === profileInfo.name?.toLowerCase().trim()&&
                    item.id !== profileInfo.id
            )
        ) {
            clonedValidateInfoes.set("name", RMResx.RM_DA_Profile_ProfileName_Exists);
        }

        if (profileInfo.ruleIds.length === 0) {
            clonedValidateInfoes.set(
                "rule",
                RMResx.RM_PRM_PRE_ColumnValid_RequireMultipleChoice
            );
        }

        if (clonedValidateInfoes.size > 0) {
            setValidateInfoes(clonedValidateInfoes);
            return false;
        }
        ...
        if (actionType == DiscoveryActionType.Create) {
            const res = await ProfileRequester.addRotProfileInfo(
                clonedProfileInfo
            );
            if (res.MessageType == 1) {
                ...
                showToast.success(
                    <$g.I18NProvider
                        msg={
                            RMResx.RM_DA_Profile_ProfileSave
                        }
                    >
            ...
            const res = await ProfileRequester.updateRotProfileInfo(
                clonedProfileInfo
            );
            if (res.MessageType == 1) {
                showToast.error(res.ErrorMessage);
                ...
                showToast.success(
                    <$g.I18NProvider
                        msg={
                            RMResx.RM_DA_Profile_ProfileUpdated
                        }
                    >
                        <a className="ra-link-a" href="/Root/JM/Index">
                            {RMResx.RM_JS_JM_Title}
            ...
            <div className="reco-inactive-profile-panel">
                ...
                <div className="reco-profile-field-item">
                    ...
                    <div className="reco-profile-field-input">
                        ...
                        <div
                            className="reco-profile-field-validate-message"
                            hidden={!validateInfoes.has("rule")}
                            tabIndex="0"
                        >
                            {validateInfoes.get("rule")}
                        </div>
                    </div>
                </div>
                <div className="reco-profile-field-item">
