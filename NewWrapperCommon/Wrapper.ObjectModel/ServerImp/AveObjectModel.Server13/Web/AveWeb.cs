...
namespace AvePoint.ObjectModel.Server13
{
    public class AveWeb : AveSecurableObject, IAveWeb
    {
        ...
        {
            return new AveFolder(this, mWeb.GetFolder(serverRelativeUrl));
        }

        public IAveFile GetFile(string serverRelativeUrl)
        {
            return new AveFile(this, mWeb.GetFile(serverRelativeUrl));
        }
            ...
            try
            {
                ...
                if (webSettingInfo.ThemedTitle != null && webSettingInfo.ThemedTitle.IsAvailable)
                {
                    ...
                    if (colorFile != null && IfRestoreThemeAgain(themeCssFolderUrl, mWeb.ThemedCssFolderUrl))
                    {
                        ...
                        string newThemedCssFolderUrl = AveAssemblyUtility.InvokeMethod(currentTheme, "ApplyToInternal", new Type[] { typeof(SPWeb), typeof(bool), typeof(bool) }, new object[] { mWeb, isSharedTheme, true }) as string;
                ...
                else
                {
                    ...
                    if (useRestoreCssFolder)
                    {
                        ...
                        if (rootWeb.GetFolder(folderPath).Exists)
                        {
        ...
        private SPFile GetThemeFile(string fileRelativeUrl)
        {
            ...
            {
                file = mWeb.Site.RootWeb.GetFile(fileRelativeUrl);
            }
            else
            {
                file = mWeb.GetFile(fileRelativeUrl);
            }
        ...
        private SPFolder GetThemeFolder(string folderRelativeUrl)
        {
            ...
            {
                return mWeb.Site.RootWeb.GetFolder(folderRelativeUrl);
            }
            else
            {
                return mWeb.GetFolder(folderRelativeUrl);
            }
        }
        ...
        private static void RotateContextHashTables()
        {
            try
            {
                Type pushContext = Type.GetType("Cobalt.PushContext, Microsoft.CobaltCore, Version=15.0.0.0, Culture=neutral, PublicKeyToken=71e9bce111e9429c");
        ...
        {
            string assemblyName = "Microsoft.SharePoint.Portal, Version=15.0.0.0, Culture=neutral, PublicKeyToken=71e9bce111e9429c";
        ...
        public Guid PublishNintexWorkflow(Stream stream, string publishName, string listName, Guid listId)
        {
