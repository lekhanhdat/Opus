    ...
    class LookupDataFormat : BaseDataFormat
    {
        ...
        public override object CheckFieldValue(object value)
        {
            if (value != null)
            {
                ...
                AveLookupObject obj;
                mItem.ParentSite.MappingManager.SiteMappingManager.TryGetValueFromLookupFieldMapping(mItem.ParentList.SPList.ID, destField.ID, out obj);
                ...
                if (needCache)
                {
                    CacheLookupColumnValue(sourceLookupValue, destLookupField, obj);
                }
            }
            return string.Empty;
        }
        ...
        {
            int itemId = -1;
            //如果TPGuid不为空，说明需要通过item的TPGuid来获取item的RowId.
            itemId = mItem.ParentSite.MappingManager.SiteMappingManager.GetMappingItemId(lookupListId, sourceItemId);

            if (itemId <= 0 && itemGuid != Guid.Empty)
            {
                itemId = mItem.GetLookupIdByGUID(lookupWebId, lookupListId, itemGuid);
            }
            if (itemId <= 0 && WrapperConfiguration.GetLookupItemByLeafNameDuringRestore && !string.IsNullOrEmpty(leafName))
            {
                itemId = mItem.ParentSite.GetLookupItemIdByDisplayValue(lookupWebId, lookupListId, "FileLeafRef", leafName);
            }
            return new LookupItemValue { ItemRowId = itemId };
        }

        private void CacheLookupColumnValue(ArrayList lookupFieldValue, IAveFieldLookup destLookupField, AveLookupObject obj)
        {
            var lookupFieldIdValue = new AveLookupFieldInfo
            {
                LookupListID = obj != null ? new Guid(obj.SourceListId) : new Guid(destLookupField.LookupList),
                LookupFieldID = destField.ID,
                Version = originalVesrion,
