...
namespace AvePoint.Common.FilterEngine
{
    ...
    internal class PhysicalFolderFilterEngine : FilterEngineBase
    {
        public PhysicalFolderFilterEngine(FilterOption option) : base(option)
        {
        }

        protected override PolicyLevel Level { get { return PolicyLevel.PhysicalFile; } }
        protected override bool IsQualified(ObjectInfoBase objectInfo, FilterPolicy policy)
        {
            PhysicalFolderInfo physicalFileInfo = objectInfo as PhysicalFolderInfo;

            if (policy.Rule is NameAndExtentionRule)
            {
                return StringConditionChecker.IsQualified(policy.Condition, physicalFileInfo.Title, policy.Value);
            }
            ...
            else if (policy.Rule is CreatedByRule)
            {
                if (policy.Condition == PolicyCondition.DoesNotContains)
                {
                    return StringConditionChecker.IsQualified(policy.Condition, physicalFileInfo.CreatedByLogonName, policy.Value) && StringConditionChecker.IsQualified(policy.Condition, physicalFileInfo.CreatedByTitle, policy.Value) && StringConditionChecker.IsQualified(policy.Condition, physicalFileInfo.CreatedByLogonName, policy.Value);
                }
                else
                {
                    return StringConditionChecker.IsQualified(policy.Condition, physicalFileInfo.CreatedByLogonName, policy.Value) || StringConditionChecker.IsQualified(policy.Condition, physicalFileInfo.CreatedByTitle, policy.Value) || StringConditionChecker.IsQualified(policy.Condition, physicalFileInfo.CreatedByLogonName, policy.Value);
