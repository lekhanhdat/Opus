...
{
    public class ArchiverRetentionJobHandler
    {
        ...
        private IRMSODashboardMonthlySnapshotDao MonthlySnapshotDao => PlatformWindsorManager.GetService<IRMSODashboardMonthlySnapshotDao>();
        ...
        private ChunkedCsvWriter<MigrationRecord>? _reportWriter { get; set; }
        private string _tenantId = string.Empty;

        public ArchiverRetentionJobHandler(string jobId, JobType jobType)
        {
            SubJobId = jobId;
            mJobType = jobType;
        ...
        public async Task RunAsync()
        {
            ...
            try
            {
                ...
                _tenantId = subJobWithContext.O365TenantId
                    ?? TenantLocalValue.LogonGroupId?.ToString()
                    ?? string.Empty;
                ...
                foreach (var archiverPruningJob in archiverPruningJobs)
                {
                    ...
                    try
                    {
                        ...
                        if (archiverPruningJob.IsSimulateJob)
                        {
                            ...
                            {
                                mLog.Warn($"Dot not run simulate retention job for other retention action: jobid:{archiverPruningJob.JobId}, RetentionAction:{archiverPruningJob.RetentionAction}");
                                continue;
                            }
                        }
                            ...
                            try
                            {
                                ...
                                switch (archiverPruningJob.RetentionAction)
                                {
                                    case MediaArchiverRetentionAction.DeleteData:
                                        if (archiverPruningJob.IsSimulateJob)
                                        {
                                            SimulateDeleteSubJobData(archiverPruningJob);
                                        }
                                        else
                                        {
                                    ...
                                    case MediaArchiverRetentionAction.MoveData:
                                        if ((_reportWriter is null || _reportWriter.IsDisposed)
                                            && mKeyValueDao.IsEnableExtendedMoveActionForRetention())
                                        {
            ...
            finally
            {
                ...
                {
                    mLog.Error($"Error while completing report writer for jobId: {SubJobId}, details: {ex}");
                }
        ...
        private async Task<RARetentionJobTelemetry> BuildRARetentionJobTelemetry(ArchiverPruningJob archiverPruningJob)
        {
            ...
            try
            {
                ...
                telemetry.JobType = ((int)JobType.ArchiverRetention).ToString();
        ...
        public async Task RunDeleteOrphanDatasAsync()
        {
            ...
            try
            {
                ...
                JobContextSetting = subJobWithContext.JobContext?.Settings;
                var archiverPruningJobs = SerializerHelper.DeserializeByDataContractSerializer<List<ArchiverPruningJob>>(JobContextSetting);
                TeamsArchiverRetentionJobHandler teamsArchiverRetentionJobHandler = new TeamsArchiverRetentionJobHandler();
                teamsArchiverRetentionJobHandler.InitTeamsRetainForOrphanBlob(SubJobId, mJobType);
                ExchangeArchiverRetentionJobHandler exchangeArchiverRetentionJobHandler = new ExchangeArchiverRetentionJobHandler();
                exchangeArchiverRetentionJobHandler.InitExchangeArchiverRetentionForOrphanBlob(SubJobId, mJobType);
                foreach (var archiverPruningJob in archiverPruningJobs)
                {
                    try
                    {
                        mLog.Info($"this delete orphan datas job:{archiverPruningJob.JobId}");
                        (bool, SampleDBLocker) lockResult;
                        if (archiverPruningJob.dataSourceForOrphanBlob == DataSourceForOrphanBlob.SharePoint)
                        {
                            lockResult = await SampleDBLocker.TryGet4IndexDBUpdater(archiverPruningJob.SiteUrl, archiverPruningJob.SiteId, SubJobId);
                        }
                        else
                        {
                            lockResult = await SampleDBLocker.TryGet4IndexDBEmail(archiverPruningJob.SiteUrl, archiverPruningJob.SiteId, SubJobId, mJobType);
                        }
                        if (lockResult.Item1)
                        {
                            using var dbLocker = lockResult.Item2;
                            try
                            {
                                ...
                                switch (archiverPruningJob.dataSourceForOrphanBlob)
                                {
                                    case DataSourceForOrphanBlob.SharePoint:
                                        DeleteSubJobData(archiverPruningJob);
                                        break;
                                    case DataSourceForOrphanBlob.Teams:
                                        teamsArchiverRetentionJobHandler.DeleteSubJobData(archiverPruningJob);
                                        teamsArchiverRetentionJobHandler.RemoveSuccessDeletedOrphanDatasSubjob();
                                        break;
                                    case DataSourceForOrphanBlob.Mailbox:
                                        exchangeArchiverRetentionJobHandler.DeleteSubJobData(archiverPruningJob);
                                        exchangeArchiverRetentionJobHandler.RemoveSuccessDeletedOrphanDatasSubjob();
                                        break;
                                }
        ...
        private void RemoveSuccessDeletedOrphanDatasSubjob()
        {
            try
            {
                foreach (string subsubJobid in successDeleteSubsubJobIds)
                {
                    ...
                    {
                        mLog.Warn($"there exsit something wrong when delete orphan datas job,so donot delete this subjob record,id:{subsubJobid}");
                    }
                    else
                    {
                        mLog.Info($"delete orphan datas job success,delete subjob record,id:{subsubJobid}");
        ...
        private bool CheckJobIsFileLevelBackup(string subSubjobId)
        {
            ...
            return ArchiverSiteMasterIndexDao.IsFileLevelBlockBackup(subjobId);
        }
        ...
        public void SimulateDeleteSubJobData(ArchiverPruningJob archiverRetentionInfo)
        {
            ...
            try
            {
                ...
                {
                    retentionInfo.IsFileLevelBlockBackup = ArchiverSiteMasterIndexDao.IsFileLevelBlockBackup(backupSubJobId);
                }
                retentionInfo.RetentionRule = AvePoint.Media.Service.DomainModel.RetentionRule.RetainArchiverJobData;
                var retentionService = (IRetentionService)PlatformWindsorManager.GetService("AvePoint.Media.Service.ArchiverBackup.ArchiverBackupRetentionService", typeof(IRetentionService));
                result = retentionService.Retain(retentionInfo, SendJobReport) as ArchiverRetentionResult;
            }
            catch (Exception e)
            {
            ...
            if (result != null && result.State == 2)
            {
                ...
                //        RetentionInfosDao.AccumulateUpdateRetentionInfo((int)SourceFlag.SharePoint, 0, result.Size);
                //    }
                //    else
                //    {
            ...
            else
            {
                if (archiverRetentionInfo.RetentionDataTimeType != KeepDateType.ModifiedTime)
                {
                    ...
                    else
                    {
                        ...
                        var dreport = new JMArchiverRententionDashboardDetails(report);
                        ReportManager.SendJobDetail(dreport);
                    }
                }
                else
                {
        ...
        public void DeleteSubJobData(ArchiverPruningJob archiverRetentionInfo)
        {
            ...
            if (result != null && result.State == 2)
            {
                try
                {
                    var subInfosCount = ArchiverIndexSubInfoDao.GetSubInfoCountAsync(backupSubJobId).GetAwaiter().GetResult();
            ...
            if (result != null && result.State == 2
               && (!archiverRetentionInfo.IsSoftDelete || archiverRetentionInfo.IsFitSoftDelete)
               && archiverRetentionInfo.RetainType != RetainType.DeleteOrphanDatas)
            {
                UpsertMonthlySnapshot(archiverRetentionInfo, result.Size);
            }
            if (result != null && result.State == 2)
            {
                if (archiverRetentionInfo.RetentionDataTimeType != KeepDateType.ModifiedTime)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            report.SrcStorageName = archiverRetentionInfo.DataLogicalDevice.Name;
                            report.JobId = archiverRetentionInfo.JobId;
                            report.Action = (!archiverRetentionInfo.IsFitSoftDelete && archiverRetentionInfo.IsSoftDelete && !archiverRetentionInfo.IsSystemStorage) ? "RM_AR_CP_GSS_Retention_SoftDelete" : "RM_JS_Common_Delete";
                            ReportManager.SendJobDetail(report);
                        }
                    }
                }
        ...
        {
            mLog.Info($"MarkSubJobDataTier : SiteUrl:{archiverRetentionInfo.SiteUrl}");
            ArchiverRetentionResult result = null;
        ...
        private void UpsertMonthlySnapshot(ArchiverPruningJob archiverRetentionInfo, long deletedSize)
        {
            try
            {
                var realSiteUrl = archiverRetentionInfo.SiteUrl;
                var subJobTenantId = _tenantId;
    ...
    ?? WrapperConfiguration.WrapperConfigurationForBPOS.O365TenantId.ToString();
                ...
                {
                    mLog.Warn($"UpsertMonthlySnapshot skipped, cannot resolve O365TenantId for subJobId:{SubJobId}");
                    return;
                }

                var period = DateTime.UtcNow.ToString("yyyyMM");
                bool isOneDrive = archiverRetentionInfo.SiteUrl.Contains("-my.sharepoint.com");

                long spoDestroyed = isOneDrive ? 0 : deletedSize;
                long odDestroyed = isOneDrive ? deletedSize : 0;

                mLog.Info($"UpsertMonthlySnapshot for retention job. " +
                          $"TenantId:{o365TenantId}, Period:{period}, IsOneDrive:{isOneDrive}, " +
                          $"SpoDestroyedArchive:{spoDestroyed}, OdDestroyedArchive:{odDestroyed}");

                MonthlySnapshotDao.UpsertMonthlySnapshotAsync(
                    o365TenantId, period,
                    spoArchivedSize: 0,
                    odArchivedSize: 0,
                    spoDestroyedFromArchiveSize: spoDestroyed,
                    odDestroyedFromArchiveSize: odDestroyed,
                    spoDestroyedFromLiveSize: 0,
                    odDestroyedFromLiveSize: 0
                ).GetAwaiter().GetResult();
                mLog.Info($"UpsertMonthlySnapshot successfully");
            }
            catch (Exception ex)
            {
                mLog.Error($"UpsertMonthlySnapshot failed for retention job, error:{ex}");
            }
        }
        private void UpdateArchiverSize(string siteUrl)
        {
            try
            {
                mLog.Info("start update archiver size for archiver info");
                ...
                ArchiveSiteInfoDao.UpdateArchiverSize(siteUrl, siteUrlAndSizeMapping[siteUrl]);

                mLog.Info($"Start updating archiveInfo for Teams. site: {siteUrl}.");
                var worker = new TeamsSODashboardWorker();
                worker.UpdateTeamsGroupRelatedSiteArchivedInfo(siteUrl).GetAwaiter().GetResult();
            }
            catch (Exception e)
            {
        ...
        private void ValidSoftDeleteIsFitRetainByModifiedTime(ArchiverPruningJob archiverRetentionInfo)
        {
            if (archiverRetentionInfo.IsSoftDelete && archiverRetentionInfo.SoftDeleteTime > 0 && !archiverRetentionInfo.IsFitSoftDelete)
            {
                ...
                {
                    archiverRetentionInfo.IsFitSoftDelete = ValidateRetentionTime(archiverRetentionInfo.SoftDeleteTime, archiverRetentionInfo.SoftDeleteKeepValue, archiverRetentionInfo.SoftDeleteDateUnit, archiverRetentionInfo.SimulateJobRunTime);
                }
                else
                {
                    archiverRetentionInfo.IsFitSoftDelete = ValidateRetentionTime(archiverRetentionInfo.SoftDeleteTime, archiverRetentionInfo.SoftDeleteKeepValue, archiverRetentionInfo.SoftDeleteDateUnit, DateTime.UtcNow.Ticks);
                }
            }
        }
        private bool ValidateRetentionTime(long retentionTimeTicks, int keepValue, DateUnit dateUnit, long runJobTime)
        {
            if (keepValue < 0)
            {
                mLog.Info($"keep value is zero,return false");
            ...
            mLog.Info($"ValidatesoftTime.RetentionTime {retentionTime.Ticks}, RunJobTime:{runJobTime}");
            return retentionTime.Ticks <= runJobTime;
        }
        private ArchiverRetentionResult RealExportDataToAnotherDevice(ArchiverPruningJob archiverRetentionInfo)
        {
            ...
            var retentionService = (IRetentionService)PlatformWindsorManager.GetService("AvePoint.Media.Service.ArchiverBackup.ArchiverBackupRetentionService", typeof(IRetentionService));
            var result = retentionService.Retain(retentionInfo, SendJobReport, SendMigrationReport) as ArchiverRetentionResult;
            return result;
        }
        ...
        {
            var reportStorageId = mKeyValueDao.GetStorageIdForArchivedDataMigrationReport();
            if (!string.IsNullOrEmpty(reportStorageId) && Guid.TryParse(reportStorageId, out _))
            {
                ...
                {
                    var maxRecordsPerFile = mKeyValueDao.GetMaxRecordsPerCSVFile();
                    ...
                    if (!string.IsNullOrEmpty(xriString))
                    {
                        var writer = new ChunkedCsvWriter<MigrationRecord>(
                            xriString: xriString,
                            folderPath: SecurityUtils.SafeCombinePath("ArchivedDataMigrationReports", SubJobId.Split('_').FirstOrDefault()),
                            groupKeySelector: record => record.SiteUrl,
                            jobIdSelector: record => SubJobId,
