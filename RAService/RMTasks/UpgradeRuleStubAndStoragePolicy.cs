...
{
    public class UpgradeRuleStubAndStoragePolicy : ITaskExecutor
    {
        private readonly RALogger Logger = RALogger.GetInstance(typeof(RuleStubAndStoragePolicyUpgrade));
        public System.Threading.Tasks.Task ExecutorAsync(TaskBase task)
        {
            ...
            Logger.Info($"Upgrade rule stub setting, total tenant count is {tenants.Count}");
