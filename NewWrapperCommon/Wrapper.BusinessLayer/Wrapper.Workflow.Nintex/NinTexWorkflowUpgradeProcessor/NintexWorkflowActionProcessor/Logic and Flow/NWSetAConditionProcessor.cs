...
{
    class NWSetAConditionProcessor : NWConditionActionBase
    {
        public NWSetAConditionProcessor(NintexWFActionProcessor workflowActionProcessor)
            : base(workflowActionProcessor)
        {
            CLASSNAME = "#ConditionalBranch";
        }
        ...
        {
            if (childActivities.Length == 2)
            {
                List<string> condintionBranchChildName = new List<string>() { "No", "Yes" };
                List<WorkflowAction> children = new List<WorkflowAction>();
                foreach (var child in childActivities)
                {
                    ...
                    childWorkflwAction.Configuration.Name = condintionBranchChildName.First();
                    condintionBranchChildName.RemoveAt(0);
                    children.Add(childWorkflwAction);
                }
                return children;
            }
            throw new Exception(string.Format("Not expected data. child count: {0}", childActivities.Length));
        }
        ...
        protected override List<Property> CreateProperties()
        {
            ...
            property.DisplayName = "Conditional Branch";
            property.ID = string.Format("ConditionalBranch_{0}", Guid.NewGuid().ToString());
