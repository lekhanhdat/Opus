...
public class RMDiscoveryGoogleContainerRotDataAnalyzer
{
    ...
    public async Task<bool> AnalysisAsync()
    {
        try
        {
            _logger.Info($"Start analysis google organization [{_googleOrganizationId}] container [{_containerId}] profile [{_profileInfo.Id} {_profileInfo.Name}] rot data.");
            ...
            await _profileDataDao.DeleteContainerRotDataByContainerIdAsync(_googleOrganizationId, _profileInfo.Id, _containerId);

            var driveDataEnumerable = _profileDataDao.GetDriveRotDataByContainerIdAsync(_googleOrganizationId, _profileInfo.Id, _containerId,
                    customColumns);

            var containerDataInfo = new RMDiscoveryGoogleProfileContainerRotData
            {
            ...
            await foreach (var driveData in driveDataEnumerable)
            {
                containerDataInfo.FileTotalSize += driveData.FileTotalSize;
                containerDataInfo.RotFileTotalSize += driveData.RotFileTotalSize;
                containerDataInfo.RCategoryFileTotalSize += driveData.RCategoryFileTotalSize;
                containerDataInfo.OCategoryFileTotalSize += driveData.OCategoryFileTotalSize;
                containerDataInfo.TCategoryFileTotalSize += driveData.TCategoryFileTotalSize;
                foreach (var customColumn in customColumns)
                {
                    var targetDataCustomColumn =
                        containerDataInfo.CustomColumns.First(item => item.Name == customColumn.Name);
                    var sourceDataCustomColumn = driveData.CustomColumns.First(item => item.Name == customColumn.Name);
                    targetDataCustomColumn.Value = Convert.ToInt64(targetDataCustomColumn.Value) +
                                                   Convert.ToInt64(sourceDataCustomColumn.Value);
                }
            }

            await _profileDataDao.AddContainerRotDataListAsync(_googleOrganizationId, _profileInfo.Id, containerDataInfo);

            _logger.Info($"End analysis google organization [{_googleOrganizationId}] container [{_containerId}] profile [{_profileInfo.Id} {_profileInfo.Name}] rot data.");

            return true;
        }
        catch (Exception e)
        {
            _logger.Error($"An error occurred while analysis google organization [{_googleOrganizationId}] container [{_containerId}] profile [{_profileInfo.Id} {_profileInfo.Name}] rot data. Error: {e}");
            return false;
        }
    }
