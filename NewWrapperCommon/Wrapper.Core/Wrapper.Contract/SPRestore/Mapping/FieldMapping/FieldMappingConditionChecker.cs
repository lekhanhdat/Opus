...
namespace AvePoint.Wrapper.Core.SPRestore.Mapping
{
    ...
    class FieldMappingConditionChecker
    {
        protected FieldMappingCondition conditionRule;

        public FieldMappingConditionChecker(FieldMappingCondition conditionRule)
        {
            this.conditionRule = conditionRule;
        }

        internal virtual bool IsQualified(FieldMappingConditionInfo conditionInfo)
        {
            return true;
        }
    }

    class ListLevelFieldMappingConditionChecker : FieldMappingConditionChecker
    {
        public ListLevelFieldMappingConditionChecker(FieldMappingCondition conditionRule) : base(conditionRule) { }

        internal override bool IsQualified(FieldMappingConditionInfo conditionInfo)
        {
            var listConditionInfo = conditionInfo as ListFieldMappingConditionInfo;
            if (listConditionInfo == null)
            {
                throw new ArgumentException("conditioninfo is not type of ListFieldMappingConditionInfo", "conditionInfo");
            }
            return IsWebLevelQualified(listConditionInfo) && IsListLevelQualified(listConditionInfo);
        }

        private bool IsWebLevelQualified(ListFieldMappingConditionInfo conditionInfo)
        {
            bool result = true;
        ...
        private bool IsListLevelQualified(ListFieldMappingConditionInfo conditionInfo)
        {
            bool result = true;
    ...
    class WebLevelFieldMappingConditionChecker : FieldMappingConditionChecker
    {
        public WebLevelFieldMappingConditionChecker(FieldMappingCondition conditionRule) : base(conditionRule) { }

        internal override bool IsQualified(FieldMappingConditionInfo conditionInfo)
        {
            var webConditionInfo = conditionInfo as WebFieldMappingConditionInfo;
            if (webConditionInfo == null)
            {
                throw new ArgumentException("conditioninfo is not type of WebFieldMappingConditionInfo", "conditionInfo");
            }
        ...
        private bool IsQualifiedInternal(WebFieldMappingConditionInfo conditionInfo)
        {
            bool result = true;
            if (this.conditionRule.SiteCondition.Count > 0)
            {
    ...
    static class FieldMappingConditionEntryExtension
    {
        internal static bool IsQualified(this FieldMappingConditionEntry condition, WebFieldMappingConditionInfo info)
        {
        ...
        private static bool CheckContentTypeCondition(FieldMappingConditionEntry condition, Dictionary<string, List<Guid>> cts, Guid fieldId)
        {
            return FieldMappingConditionHelper.CheckContentTypeCondtion(cts, fieldId, condition.ConditionValue, condition.Operation);
        }

        private static bool CheckWebURLCondition(FieldMappingConditionEntry condition, string webUrl)
        {
            return FieldMappingConditionHelper.CheckStringCondtion(webUrl, condition.ConditionValue, condition.Operation);
        }

        internal static bool IsQualified(this FieldMappingConditionEntry condition, ListFieldMappingConditionInfo info)
        {
            switch (condition.ConditionType)
            {
                case ConditionType.URL:
        ...
        private static bool CheckListTemplateID(FieldMappingConditionEntry condition, string listTemplateID)
        {
            return FieldMappingConditionHelper.CheckStringCondtion(listTemplateID, condition.ConditionValue, condition.Operation);
        }

        private static bool CheckListTitleCondition(FieldMappingConditionEntry condition, string listTitle)
        {
            return FieldMappingConditionHelper.CheckStringCondtion(listTitle, condition.ConditionValue, condition.Operation);
        }


    }

    static class FieldMappingConditionHelper
    {
