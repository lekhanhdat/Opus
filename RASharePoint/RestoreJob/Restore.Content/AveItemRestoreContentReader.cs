...
{
    public class AveItemRestoreContentReader
    {
        private static readonly AveLogger mLog = AveLogger.GetInstance(typeof(AveItemRestoreContentReader));
        ...
        public AveItemRestoreContentReader(IAveRestoreStream restoreStream, ItemRestoreConfig config, RestoreTreeNode root)
        {
        ...
        public RestoreContentDto MoveNext()
        {
            ...
            while ((head = mRestoreStream.ReadHead()) != null)
            {
                ...
                bool isCurrentVersion = GetAttributeBoolean("isCurrentVersion", rootElement, false);
        ...
        private AvePoint.GCommon.Contract.Tree.Object.NodeLevel GetNodeLevel(char aveConstants)
        {
            switch (aveConstants)
            {
                case AveConstants.TYPE_SITE:
                    return GCommon.Contract.Tree.Object.NodeLevel.SiteCollection;
                case AveConstants.TYPE_WEB:
                    return GCommon.Contract.Tree.Object.NodeLevel.Site;
                case AveConstants.TYPE_LIST:
                    return GCommon.Contract.Tree.Object.NodeLevel.List;
                case AveConstants.TYPE_FOLDER:
                    return GCommon.Contract.Tree.Object.NodeLevel.Folder;
                default:
                    mLog.Warn(@"fail get node level:", aveConstants);
                    return GCommon.Contract.Tree.Object.NodeLevel.Undefined;
            }
        }

        private bool NeedRmovePrefixSpace(char type)
        {
            return type == AveConstants.TYPE_FOLDER
                   || type == AveConstants.TYPE_DOCUMENT
                   || type == AveConstants.TYPE_LISTITEM
                   || type == AveConstants.TYPE_ATTACHMENTS
                   || type == AveConstants.TYPE_VERSION
                   || type == AveConstants.TYPE_LISTITEMVERSION;
        }
        ...
        private static RestoreTreeNode GetParentNode(RestoreTreeNode currentNode, string path, char type)
        {
        ...
        private static string GetPathFromName(RestoreTreeNode parentNode, string name, char type)
        {
