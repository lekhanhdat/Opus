...
namespace AvePoint.Media.Service.ArchiverBackup
{
    ...
    public class GDriveArchiverLifecycleRetentionService : RetentionServiceBase<ArchiverLifecycleRetentionInfo, GDriveArchiverLifecycleRetentionResult>
        , IRetentionService
    {
        ...
        private ArchiverMessage _archiverMessage;
        ...
        public GDriveArchiverLifecycleRetentionService ()
        {
            InitArchiverServices();
        }
        ...
        public IGDriveArchiverRetentionIndexService RetentionIndexService => PlatformWindsorManager.GetService<IGDriveArchiverRetentionIndexService>();
        ...
        private IRMArchiveGDriveInfoDao RMArchiveGDriveInfoDao = PlatformWindsorManager.GetService<IRMArchiveGDriveInfoDao>();
        ...
        private readonly string SelectTableNameArchiverBodyFileCount = "SELECT COUNT(*) FROM " + IndexConstants.TableNameGDriveItem + $" WHERE COL_TYPE = {(int)GDriveDataType.File} AND COL_NAME NOT LIKE '%:%'";
        private readonly string SelectTableNameArchiverBodyVersionCount = "SELECT COUNT(*) FROM " + IndexConstants.TableNameGDriveItem + $" WHERE COL_TYPE = {(int)GDriveDataType.FileVersion} AND COL_NAME LIKE '%:%'";
        ...
        public override void Open(ArchiverLifecycleRetentionInfo retentionInfo)
        {
            ...
            this.isGoogleDrive = retentionInfo.IsGoogleDrive;
            this._archiverRetentionInfo = retentionInfo;
            this._rules = retentionInfo.Rules;
            this._archiverMessage = retentionInfo.ArchiverMessage;
            this.logger.Info(MediaServiceArchiverBackupResource.ArchiverBackupRetentionServiceOpenStart, this._archiverRetentionInfo.JobId); 
            ...
            this._dataLogicalDevice = this.DeviceManager.Open(this._archiverRetentionInfo.DataLogicalDevice.GetXRIS(PhysicalDeviceUsage.Data));
            this.CacheManager.Open(retentionInfo.CacheSetting, false, true);
            this.isObjectType = this._dataLogicalDevice.StorageInterfaceType.Equals(StorageInterfaceType.Object) || this._indexLogicalDevice.StorageInterfaceType.Equals(StorageInterfaceType.Object);
            this.logger.Info(MediaServiceArchiverBackupResource.RetentionServiceOpenIndexAndDataDeviceFinished);
             
            this.OpenMainIndex(this._archiverRetentionInfo, this.indexVolume); 
        ...
        public override GDriveArchiverLifecycleRetentionResult Retain(ArchiverLifecycleRetentionInfo retentionInfo)
        {
            var retentionResult = new GDriveArchiverLifecycleRetentionResult() { sucessItem = new List<GoogleBasicIndex>(), manualSkippedItem = new List<GoogleBasicIndex>() };
            ...
            try
            {
                foreach (string ruleId in usedRuleIds)
                {
                    ...
                    if (rule != null && rule.IsEnableRetention)
                    {
                        ...
                        List<GoogleBasicIndex> manualItem = new List<GoogleBasicIndex>();
                        logger.Info("Start to process retenion in rule {0}", rule.Name);
                        long expireDAte = this.CalculateExpireDate(rule.RetentionInfo);
                        logger.Info("Queryable expire date long is {0}", expireDAte);
                        List<GoogleBasicIndex> indexes = this.RetentionIndexService.GetRetentionData(rule.Id, expireDAte);
                        logger.Info("doc count in the {0}, is {1}", rule.Name, indexes.Count);
                        List<GoogleBasicIndex> needDeleteDataAndIndex = new List<GoogleBasicIndex>();
                        ...
                        List<GoogleBasicIndex> needManualDelete = new List<GoogleBasicIndex>();
                        List<Record> needUpdateEntity = null;
                        if (manualItem.Count > 0)
                        {
                            var result = ProcessManualApproveData(manualItem, rule, out needUpdateEntity);
                        ...
                        {
                            GDriveArchiverLifecycleRetentionResult tempResult = RetainJobData(needDeleteDataAndIndex, removeStub);
                ...
                retentionResult.HasIndexRelatedToBackupJob = IsExistsIndexRelatedToJob(this._archiverRetentionInfo.JobId);
            }
            catch (Exception)
            {
                throw;
        ...
        private void GenaratAndInsertRetentionSubInfos(List<GoogleBasicIndex> IndexItem, string jobId,Guid siteGroupId,string siteUrl,string siteId)
        {
        ...
        private Tuple<List<GoogleBasicIndex>, List<GoogleBasicIndex>> ProcessManualApproveData(List<GoogleBasicIndex> indexes, Rule rule, out List<Record> needUpdateEntity)
        {
            List<GoogleBasicIndex> result = new List<GoogleBasicIndex>();
            List<GoogleBasicIndex> manualSkippedResult = new List<GoogleBasicIndex>();
            ...
            foreach (var indexs in sortIndexs)
            {
                ...
                try
                {
                    ...
                    ManualExportReportInfo manualApprovalReportInfo = RMArchiverItemConverter.ConvertToReportInfo(archiverentity, true);
                    PerProcessManualApprovalReport(manualApprovalReportInfo);
                    if (manualApprovalReportInfo.SourceFlag == (int)SourceFlag.All)
                    {
                        manualApprovalReportInfo.SourceFlag = isGoogleDrive ? (int)SourceFlag.OneDrive : (int)SourceFlag.SharePoint;
                    }
            ...
            return new Tuple<List<GoogleBasicIndex>, List<GoogleBasicIndex>>(result, manualSkippedResult);
        }

        private GoogleBasicIndex FindCurrentVersion(List<GoogleBasicIndex> indexs)
        {
            GoogleBasicIndex currentVersion = null;
        ...
        private Dictionary<string, List<GoogleBasicIndex>> SortIndexById(List<GoogleBasicIndex> indexes)
        {
            Dictionary<string, List<GoogleBasicIndex>> keyValues = new Dictionary<string, List<GoogleBasicIndex>>();
            foreach (var index in indexes)
            {
                if (!keyValues.ContainsKey(index.ItemId))
                {
                    keyValues[index.ItemId] = new List<GoogleBasicIndex>();
                }
                keyValues[index.ItemId].Add(index);
            }
            return keyValues;
        }
        ...
        {
            Rule rule = _rules.Values.FirstOrDefault(a => a.Id == ruleId);
            if (this.isGoogleDrive && rule != null)
            {
        ...
        private RMManualArchiverSharePointOnlineTableEntity GenerateEntity(GoogleBasicIndex index, Rule rule)
        {
            ...
            try
            {
                ...
                entity.SourceFlag = (int)SourceFlag.Google;
                ...
                entity.NodeID = new Guid(index.ItemId);
                ...
                spDataSource.SiteUrl = index.DriveName;
                ...
                spDataSource.SiteTitle = index.DriveName;
                spDataSource.SourceFlag = (int)SourceFlag.Google;
                #endregion
        ...
        private DateTime CalculateExpire(GoogleBasicIndex index, Rule rule)
        {
        ...
        public override void ProcessException(Exception e, GDriveArchiverLifecycleRetentionResult result)
        {
        ...
        public override void Dispose()
        {
            ...
            if (this.IndexService != null && this._archiverRetentionInfo.RetentionRule.Equals(DomainModel.RetentionRule.RetainArchiverJobData))
            {
        ...
        private GDriveArchiverLifecycleRetentionResult RetainJobData(List<GoogleBasicIndex> indexes, bool removeStub)
        {
            this.logger.Info("Start to retain data for Records lifecycle, job id {0}", this._archiverRetentionInfo.JobId);
            GDriveArchiverLifecycleRetentionResult result = this.ConvertInfoToResult(this._archiverRetentionInfo);
            List<GoogleBasicIndex> failedOnes = this.DeleteDataFromDevice(indexes, this._dataVolume, this.indexVolume, removeStub);
       ...
       /* private long GetContentFileNumber(String name)
        ...
        private GDriveArchiverLifecycleRetentionResult ConvertInfoToResult(ArchiverRetentionInfo info)
        {
            var result = new GDriveArchiverLifecycleRetentionResult();
   ...
   /*     private ArchiverPruningJob ConvertToArchiverPruningJob(ArchiverRetentionInfo archiverRetentionInfo)
        ...
        private HashSet<string> RecordDeletingDuplicatedFiles(GoogleBasicIndex item)
        {

            return new HashSet<string>();
        }

        private bool CheckIsLastDuplicatedFileWithSameCRC(GoogleBasicIndex item, HashSet<string> deletingFileIDs)
        {
            //var sql = $"SELECT * FROM {IndexConstants.TableNameArchiveBody} WHERE COL_EXTENSION_8 = @CRC;";
            //var dupFiles = this.IndexMainProcessor.ExecuteQuery<GoogleBasicIndex>(sql, new Dictionary<string, object>() { { "@CRC", item.StorageCrc64 } });
        ...
        private void RealDeleteDeuplicateFileDataFromDevice(
            GoogleBasicIndex item, Dictionary<string, long> deleteSizePairs, List<string> needUpdateSiteForDashBoard,
            List<GoogleBasicIndex> sucessOnes, List<GoogleBasicIndex> failedOnes)
        {
            logger.Info($"This is dedup file data: {item.Id}.");
            ...
            if (!CheckIsLastDuplicatedFileWithSameCRC(item, deletingFileIDs))
            {
                ...
                if (!needUpdateSiteForDashBoard.Contains(item.DriveId))
                {
                    needUpdateSiteForDashBoard.Add(item.DriveId);
                }
                sucessOnes.Add(item);
            }
            //else
        ...
        private List<GoogleBasicIndex> DeleteDataFromDevice(List<GoogleBasicIndex> indexes, String dataVolume, String indexVolume, bool removeStub)
        {
            logger.Info("Start to delete index and data from Records");
            var tempDeleteDataSize = default(Int64);
            List<GoogleBasicIndex> sucessOnes = new List<GoogleBasicIndex>();
            List<GoogleBasicIndex> failedOnes = new List<GoogleBasicIndex>(); 
            ...
            //            int index = 0;
            //            int page = 200;
            //            List<GoogleBasicIndex> temp = item.Skip(index * page).Take(page).ToList();
        ...
        private void UpdateRetentionInfo(List<KeyValuePair<string, long>> retentionInfoList)
        {
            ...
            {
                logger.Info($"Retention file info:{info.Value}, site URL: {this._archiverRetentionInfo.SiteUrl}, list URL:{info.Key}, archiver job: {this._archiverRetentionInfo.JobId}");
                var retentionSiteInfo = new RMRetentionSiteInfo()
                {
                    Id = Guid.NewGuid().ToString(),
                    ListUrl = info.Key,
                    SiteUrl = this._archiverRetentionInfo.SiteUrl,
        ...
        private void UpdateArchivedInfo(string driveName, long deleteSize, string tenantId, string driveId)
        {
            var siteUrlAndJobIdMapping = ArchiverSiteMasterIndexDao.GetAllBackupSiteCollectionDistinctJobIdMappings(new List<string>() { driveName });
            var siteUrlAndSizeMapping = ArchiverIndexSubInfoDao.GetAllArchiverIndexSubInfoBySiteUrls(siteUrlAndJobIdMapping);
            var size = siteUrlAndSizeMapping[driveName];
            RMArchiveGDriveInfoDao.UpdateGoogleArchiverInfo(driveName, GetFileCount(), GetFileVersionCount(), tenantId, driveId, size);
        }
        ...
        public void UpdateArchivedSiteInfo(List<string> driveNames, string tenantId, string driveId)
        {
            long size = 0;
            foreach (var driveName in driveNames)
            {

                if (SOArchiverJobInfoStatistics.Instance.DriveAndDeleteSize.TryGetValue(driveName, out var deleteSize))
                {
                    size = deleteSize;
                }
                UpdateArchivedInfo(driveName, size, tenantId, driveId);
            }
        }

        public void InitArchiverServices()
        {
            if (ArchiverIndexSubInfoDao == null)
            {
                ArchiverIndexSubInfoDao = PlatformWindsorManager.GetService<IArchiverIndexSubInfoDao>();
            }
            if (IndexMainProcessor == null)
            {
                IndexMainProcessor = new IndexProcessor<GDriveArchiverIndexProcessorParameter>();
            }
        }
        //private void AppendDataSize(List<GoogleBasicIndex> index, string fileName, long dataSize)
        //{
        ...
        private async Task<RARetentionJobTelemetry> BuildRARetentionJobTelemetry(string objectSubSubJobId)
        {
            ...
            try
            {
                telemetry.JobId = _archiverRetentionInfo.JobId;
                telemetry.MainJobId = _archiverRetentionInfo?.JobId?.Split("_")?.FirstOrDefault();
                telemetry.RetentionObject = _archiverRetentionInfo.SiteUrl;
                telemetry.ArchivedSubJobId = objectSubSubJobId;
                telemetry.JobType = (_archiverRetentionInfo as ArchiverLifecycleRetentionInfo)?.JobType.ToString() ?? string.Empty;
                telemetry.StorageName = _archiverRetentionInfo?.DataLogicalDevice?.Name;
                telemetry.RetentionAction = (int)_archiverRetentionInfo.RetentionAction;
