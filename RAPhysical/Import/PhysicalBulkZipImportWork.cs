...
{
    public class PhysicalBulkZipImportWork : IDisposable
    {
        private RALogger logger = RALogger.GetInstance(typeof(PhysicalBulkZipImportWork));
        #region Job Param
        ...
        //private HashSet<int> CheckDuplicateRecordsId = new();
        ...
        public PhysicalBulkZipImportWork(RMImportPhysicalZipRecordMessage msg)
        {
        ...
        private Dictionary<string, List<string[]>> ReadCSVFile(string filePath)
        {
            ...
            try
            {
                using (FileStream fs = new FileStream(filePath, FileMode.Open))
                {
                    if (filePath.EndsWith("csv"))
                    {
                        ...
                        temp.AddRange(Csv.CsvReader.ReadFromStream(fs, new Csv.CsvOptions
                        {
                            // csv文件没有标题行
                        ...
                        datas.Add(Path.GetFileName(filePath), temp);
                    }
                    else
                    {
        ...
        private async Task UpdateProcessSucceedRecord(Record record)
        {
            ...
            JMImportPhysicalRecordsJobDetail detail = new JMImportPhysicalRecordsJobDetail()
            {
                ...
                Barcode = record.CustomColumnDic.TryGetValue(DefaultColumnIDs.Barcode, out var barcode) ? barcode.Value : record.RecordsId,
                Title = record.LeafName,
                SrcLocation = record.FullPath,
                LocationFullPath = record.FullPath,
        ...
        public async Task ImportPhysicalRecordsAsync()
        {
            ...
            try
            {
                ...
                // 获取指定目录
                var downloadFolder = System.IO.Path.GetDirectoryName(physicalRecordsCSVPath) ?? "";
                if (!Directory.Exists(downloadFolder)) { Directory.CreateDirectory(downloadFolder); }

                // 得到解压的目录
                var unZipFolder = System.IO.Path.Combine(downloadFolder, System.IO.Path.GetFileNameWithoutExtension(physicalRecordsCSVPath));
                AvePoint.GCommon.ZipUtil.UnZipFile(physicalRecordsCSVPath, unZipFolder);

                var termFiles = Directory.GetFiles(unZipFolder ?? "");

                foreach (var termFile in termFiles)
                {
                    Dictionary<string, List<string[]>> datas = this.ReadCSVFile(termFile);
                    foreach (KeyValuePair<string, List<string[]>> keyValue in datas)
                    {
                        ...
                        try
                        {
                            ...
                            var bulkTask = AveTenantTasks.ExecuteAction(BulkValidateNameAndSaveAsync);
                ...
                status = Result.HasFailed
                    ...
                    : JobStatus.Finished;
                // 删除解压的zip文件夹和zip文件
                DirectoryInfo directory = new DirectoryInfo(unZipFolder);
        ...
        private void InitColumnIndexDic(string[] header, TemplateDto template)
        {
            ...
            for (int i = 0; i < header.Length; i++)
            {
                ...
                else if (I18NEntity.GetString("RM_Template_Column_Name_DataClosed").Equals(header[i], StringComparison.OrdinalIgnoreCase) || "RM_Template_Column_Name_DataClosed" == header[i])
                {
                    columnIndexDic.Add(header[i].ToLower(), i);
                }
                else
                {
        ...
        private async Task<bool> ProcessCustomAsync(List<KeyValuePair<int, string[]>> rowDataList, TemplateDto template, JMImportPhysicalRecordsJobDetail oldDetail)
        {
            async Task ProcessOneCustom(TemplateDto template, int rowNumber, JMImportPhysicalRecordsJobDetail oldDetail, string[] rowData)
            {
                ...
                try
                {
                    ...
                    bool generateNewId = string.IsNullOrEmpty(uniqueId);
                    if (generateNewId)
                    {
                        // 直接打印错误信息
                        detail.Status = JobDetailsStatus.Failed;
                        detail.Comment = "RM_Phy_Import_UniqueIdEmptyValueError";
                        Result.HasFailed = true;
                        Interlocked.Increment(ref FailedItemCount);
                        return;
                    }
                    ...
                    {
                        // 根据temId和uniqueId查询
                        rec = ExplorerDao.GetPhysicalRecordByRecordsIdAndTemId(uniqueId, template.id);
                    }

                    string leafName = rowData[columnIndexDic[Column_Name]];
                    detail.UniqueId = uniqueId;

                    bool isUpdate = this.ValidateConflictUniqueID(ref rec, uniqueId, detail, leafName);
                    rec.RecordStatus = (int)RMRecordStatus.Active;

                    await AssembleBasicColumnAsync(rowData, nodeType, template, rec, detail);
        ...
        private async Task<bool> ProcessRecordAsync(List<KeyValuePair<int, string[]>> rowDataList, TemplateDto template, JMImportPhysicalRecordsJobDetail oldDetail)
        {
            async Task ProcessOneRecord(TemplateDto template, int rowNumber, JMImportPhysicalRecordsJobDetail oldDetail, string[] rowData)
            {
                ...
                try
                {
                    ...
                    {
                        using var scope0 = new PerformanceScope($"Import One Record Prepare Save", addToStatistics: true);
                        // 根据temId和uniqueId查询
                        rec = ExplorerDao.GetPhysicalRecordByRecordsIdAndTemId(uniqueId, template.id);
                    }

                    if (!IsAdmin && !PhysicalLocationPermission.Contains(rec?.LocationId ?? Guid.Empty))
                    {
                        detail.Comment = "RM_Phy_Import_NoPermissionForLocation";
                        detail.Status = JobDetailsStatus.Failed;
                        Result.HasFailed = true;
                    ...
                    await this.AssembleBasicColumnAsync(rowData, nodeType, template, rec, detail);

                    Record folder = ExplorerDao.GetPhysicalRecordById(rec.ParentId);
                    ...
                    Dictionary<string, string> metaInfo = await this.AssembleColumnInTemplateAsync(template, rec, rowData);
        ...
        private async Task<bool> ProcessFolderAsync(List<KeyValuePair<int, string[]>> rowDataList, TemplateDto template, JMImportPhysicalRecordsJobDetail oldDetail)
        {
            ...
            if (rowDataList.Count > splitTasksNumber)
            {
                AveTenantTasks.RunParallel(rowDataList, existingItemsPerTask, new CancellationTokenSource(), rowData =>
                {
                    ProcessOneFolder(template, rowData.Key, oldDetail, rowData.Value).GetAwaiter().GetResult();
                });
            }
            else
            {
        ...
        private bool ValidateConflictUniqueID(ref Record rec, string uniqueId, JMImportPhysicalRecordsJobDetail detail, string leafName = null)
        {
            if (rec != null)
            {
                // 为了显示csv中的Name
                ...
                {
                    throw new Exception("RM_Phy_Import_LeafNameEmptyError");
                }
                return true;
            }
            else
            {
                // 查询不存在 record 直接抛异常
                throw new Exception("RM_Phy_Import_CanNotFindRecord");
            }
        }

        private async Task AssembleBasicColumnAsync(string[] rowData, RMNodeType nodeType, TemplateDto template, Record rec, JMImportPhysicalRecordsJobDetail detail)
        {
            rec.NodeId = rec.Id;
            ...
            if (rec.CreatedBy.IsNullOrEmpty())
            {
        ...
        private async Task AssembleAdditionalInfoAsync(string[] rowData, RMNodeType nodeType, TemplateDto template, Record rec, JMImportPhysicalRecordsJobDetail detail)
        {
            using var scope = new PerformanceScope($"AssembleAdditionalInfoAsync", addToStatistics: true);
            RMTerm mTerm = null;
            if (columnIndexDic.ContainsKey(Column_Classification))
            {
                if (!string.IsNullOrEmpty(rowData[columnIndexDic[Column_Classification]]))
                {
                    //从import文件的路径中获取Term
                    mTerm = this.getTermByPath(rowData[columnIndexDic[Column_Classification]]);
                    if (mTerm == null)
                    {
                        detail.Comment = "RM_Phy_Import_TermPathAnalyseError";
                        // 填写错误的时候 查询默认值
                        RMPRTreeNode treeNode = await PhysicalRecordSettingService.LoadPhysicalRecordSettingAsync(rec.LocationId);
                        rec.TermId = treeNode.DefaultTermId;
                        rec.TermName = treeNode.DefaultTermName;
                    }
                    else
                    {
            ...
            {
                throw new Exception("RM_Phy_Import_TermPathError");
            }

            await AssembleMetaInfoAsync(rowData, template, rec);
        }

        private async Task AssembleMetaInfoAsync(string[] rowData, TemplateDto template, Record rec)
        {
            Dictionary<string, string> metaInfo = await this.AssembleColumnInTemplateAsync(template, rec, rowData);
            rec.MetaInfo = JsonConvert.SerializeObject(metaInfo);
        }
        ...
        {
            long importedCreateTime = isUpdate ? rec.TimeCreated : DateTime.UtcNow.Ticks;
            ...
            {
                if (rec.CreateDate != GetCreateDate(importedCreateTime))
                {
        ...
        {
            if (columnIndexDic.ContainsKey(I18NEntity.GetString("RM_Template_Column_Name_DataClosed").ToLower()))
            {
                string modifiedTime = rowData[columnIndexDic[I18NEntity.GetString("RM_Template_Column_Name_DataClosed").ToLower()]];
        ...
        private async Task<Dictionary<string, string>> AssembleColumnInTemplateAsync(TemplateDto template, Record rec, string[] rowData)
        {
            Dictionary<string, string> metaInfo = new Dictionary<string, string>();
            // 取出历史的metainfo
            Dictionary<string, string> temMetaInfo = JsonConvert.DeserializeObject<Dictionary<string, string>>(rec.MetaInfo);
            foreach (TemplateCategoryDto cat in template.categories)
            {
                foreach (TemplateColumnDto col in cat.columns)
                {
                    ...
                    else if ("RM_Template_Column_Name_Capability" == col.columnName || I18NEntity.GetString("RM_Template_Column_Name_Capability").Equals(col.columnName, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        {
                            int nameCapability;
                            if (int.TryParse(rowData[columnIndexDic[I18NEntity.GetString("RM_Template_Column_Name_Capability")]], out nameCapability))
                            {
                                if (nameCapability < 0)
                                {
                                    // 不进行修改
                                    if (temMetaInfo.ContainsKey(col.uniqueId.ToString()))
                                    {
                                        metaInfo.Add(col.uniqueId.ToString(), temMetaInfo[DefaultColumnIDs.Capability]);
                                    }
                                }
                                else
                                {
                    ...
                    {
                        // 从历史数据中取，在进行赋值
                        if (temMetaInfo.ContainsKey(DefaultColumnIDs.HomeLocation))
                        {
                            metaInfo.Add(col.uniqueId.ToString(), temMetaInfo[DefaultColumnIDs.HomeLocation]);
                        }
                    }
                    ...
                    else if ("RM_PRM_PRE_Column_LoanBy" == col.columnName)
                    {
                        // 从历史数据中取，在进行赋值
                        if (temMetaInfo.ContainsKey(DefaultColumnIDs.LoanedBy))
                        {
                            metaInfo.Add(col.uniqueId.ToString(), temMetaInfo[DefaultColumnIDs.LoanedBy]);
                        }
                    }
                    else if ("RM_Template_Column_Name_Status" == col.columnName && columnIndexDic.ContainsKey(Column_Status))
                    {
                        ...
                        if (rec.RecordStatus == (int)RMRecordStatus.Destroyed && statusInt != (int)RMRecordStatus.Destroyed)
                        {
                            throw new GCommon.Utility.AveException("RM_PhysicalBulkUpdateImport_UpdateDestoryedRecordError");
                        }

                        if (statusInt == (int)RMRecordStatus.Destroyed || statusInt == (int)RMRecordStatus.Missing)
                        {
                            if (temMetaInfo.TryGetValue(DefaultColumnIDs.LoanedBy, out string loanedByString) && !string.IsNullOrEmpty(loanedByString))
                            {
                                throw new GCommon.Utility.AveException("RM_PhysicalBulkUpdateImport_UpdateDestoryedLoanedRecordError");
                            }
                        }
                    ...
                    else if (col.allowEdit && this.columnIndexDic.ContainsKey(I18NEntity.GetString(col.columnName)))  //allow edit说明不是默认Column
                    {
                        ...
                        else if (col.typeId == (int)AvePoint.RA.Contract.TemplateManagement.ColumnType.SingleChoice)
                        {
                            ...
                            if (columnIndexDic.ContainsKey(columnName))
                            {
                                ...
                                {
                                    if (optionVal != null && (option.Value.Equals(optionVal, StringComparison.CurrentCultureIgnoreCase) || I18NEntity.GetString(option.Value).Equals(optionVal, StringComparison.CurrentCultureIgnoreCase)))
                                    {
                                ...
                                if (!sucess && temMetaInfo.ContainsKey(col.uniqueId.ToString()) && !optionVal.IsNullOrEmpty())
                                {
                                    //没有正确的修改值 && val不为空  把历史的值得到，在赋值回去
                                    metaInfo.Add(col.uniqueId.ToString(), temMetaInfo[col.uniqueId.ToString()]);
                                    sucess = true;
                                    logger.Warn($"single choice column {col.columnName}, reset value {temMetaInfo[col.uniqueId.ToString()]}");
                                }
                            }
                            else
                            {
                        ...
                        else if (col.typeId == (int)AvePoint.RA.Contract.TemplateManagement.ColumnType.MultipleChoice)
                        {
                            ...
                            if (columnIndexDic.ContainsKey(columnName))
                            {
                                ...
                                foreach (KeyValuePair<int, string> option in options)
                                {
                                    ...
                                    {
                                        if (temp != string.Empty && (option.Value.Equals(temp, StringComparison.CurrentCultureIgnoreCase) || I18NEntity.GetString(option.Value).Equals(temp, StringComparison.CurrentCultureIgnoreCase)))
                                        {
                                            ChoiceColumnValue formatFiled = new ChoiceColumnValue() { Value = option.Key.ToString(), Name = option.Value };
                                ...
                                if (!sucess && temMetaInfo.ContainsKey(col.uniqueId.ToString()) && !optionVal.IsNullOrEmpty())
                                {
                                    ...
                                    metaInfo.Add(col.uniqueId.ToString(), temMetaInfo[col.uniqueId.ToString()]);
                                    sucess = true;
                                    logger.Warn($"multi choice column {col.columnName}, reset value {temMetaInfo[col.uniqueId.ToString()]}");
                                }
                            }
                            if (col.required && !sucess)
                            {
        ...
        readonly object termMemberDictionaryLock = new object();
