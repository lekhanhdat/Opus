...
namespace AvePoint.RA.CommonUtil
{
    ...
    public class AveRollingFileAppender : FileAppender
    {
        ...
        /// <summary>
        /// get the  previous log file's name after roll file
        /// </summary>
        ...
        /// <summary>
        /// Get The type of rolling done
        /// </summary>
        protected AveRollPoint RollPoint
        {
            get { return m_rollPoint; }
        }

        /// <summary>
        /// get or set the timestamp when we shall next recompute the filename.
        /// </summary>
        protected DateTime NextCheck
        {
            get { return m_nextCheck; }
        ...
        {
            get { return m_scheduledFilename; }
            set { m_scheduledFilename = value; }
        }
        #endregion
        ...
        private readonly object lockObj = new object();//Quality Issue
        /// <summary>
            ...
            lock (lockObj)
            {
                ...
                else
                {
                    if (LogLog.IsErrorEnabled)
                    {
                        ...
                        {
                            LogLog.Error(typeof(AveRollingFileAppender), "RollingFileAppender: INTERNAL ERROR. Append is False but OutputFile [" + fileName + "] already exists.");
                        }
                    }
                }
                ...
                if (QuietWriter is CountingQuietTextWriter)
                {
        ...
        private void DetermineCurSizeRollBackups()
        {
            ...
            LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: curSizeRollBackups starts at [" + m_curSizeRollBackups + "]");
        }
        ...
        private ArrayList GetExistingFiles(string baseFilePath)
        {
            ...
            LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Searched for existing files in [" + directory + "]");
            return alFiles;
        }
        ...
        private void RollOverIfDateBoundaryCrossing()
        {
            if (m_staticLogFileName && m_rollDate)
            {
                if (FileExists(m_baseFileName))
                {
                    ...
                    LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: [" + last.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo) + "] vs. [" + m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo) + "]");
                    ...
                    if (!(last.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo).Equals(m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo))))
                    {
                        ...
                        LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Initial roll over to [" + m_scheduledFilename + "]");
                        RollOverTime(false);
                        LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: curSizeRollBackups after rollOver at [" + m_curSizeRollBackups + "]");
                    }
                }
            }
        }
        ...
        protected void ExistingInit()
        {
            ...
            if (AppendToFile == false)
            {
                ...
                if (fileExists)
                {
                    ...
                    {
                        LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Output file [" + fileName + "] already exists. MaxSizeRollBackups is 0; cannot roll. Overwriting existing file.");
                    }
                    else
                    {
                        LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Output file [" + fileName + "] already exists. Not appending to file. Rolling existing file out of the way.");
        ...
        private void InitializeFromOneFile(string baseFile, string curFileName)
        {
            ...
            if (m_rollDate && !m_staticLogFileName)
            {
                ...
                {
                    LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Ignoring file [" + curFileName + "] because it is from a different date period");
                    return;
                }
            }
            ...
            try
            {
                ...
                if (SystemInfo.TryParse(curFileName.Substring(index + 1), out backup))
                {
                    if (backup > m_curSizeRollBackups)
                    {
                        ...
                        LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: File name [" + curFileName + "] moves current count to [" + m_curSizeRollBackups + "]");
                    }
                }
            }
            catch (FormatException)
            {
                ...
                LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Encountered a backup file not ending in .x [" + curFileName + "]");
            }
        }
        ...
        /// </remarks>
        private AveRollPoint ComputeCheckPeriod(string datePattern)
        {
            ...
            for (int i = (int)AveRollPoint.TopOfMinute; i <= (int)AveRollPoint.TopOfMonth; i++)
            {
                // Get string representation of next pattern
                string r1 = NextCheckDate(s_date1970, (AveRollPoint)i).ToString(datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo);

                LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Type = [" + i + "], r0 = [" + r0 + "], r1 = [" + r1 + "]");

                // Check if the string representations are different
                if (r0 != null && r1 != null && !r0.Equals(r1))
                {
                    // Found highest precision roll point
                    return (AveRollPoint)i;
                }
            }

            return AveRollPoint.InvalidRollPoint; // Deliberately head for trouble...
        }
        ...
        override public void ActivateOptions()
        {
            if (m_rollDate && m_datePattern != null)
            {
                ...
                if (m_rollPoint == AveRollPoint.InvalidRollPoint)
                {
            ...
            if (m_rollDate && !m_rollSize && File != null && m_scheduledFilename == null)
            {
        ...
        {
            if (m_staticLogFileName && !string.IsNullOrWhiteSpace(m_scheduledFilename))
            {
                ...
                m_scheduledFilename = AmendmentDestinationForRollFile(File, m_scheduledFilename);
        ...
        /// <summary>
        /// 当前方法用于修正根据time roll file时候File move到其他path的问题
        /// 问题原因: 
        /// RollFile时,会将log从File移动到m_scheduledFilename下
        /// 字段 File 会在Job Role进行初始化的时候进行赋值,(logs//TenantName//JobId)
        /// 但是字段m_scheduledFilename 只会在roll file 结束后进行赋值
        /// 所以会存在一种情况,当第一次roll file时,由于m_scheduledFilename 处于未初始化状态,当前值为(log//common//RevIMScheduleJob)
        /// 导致log移动到其他位置,文件丢失.
        /// </summary>
        /// <param name="from"></param>
        /// <param name="to"></param>
        /// <returns>new target path</returns>
        private string AmendmentDestinationForRollFile(string from ,string to)
        {
            var fromFolderPath = Path.GetDirectoryName(from);
            var toFolderPath = Path.GetDirectoryName(to); 
            if (fromFolderPath != toFolderPath)
            {
                var fromFileName = Path.GetFileName(from);
                var toFileName = Path.GetFileName(to);
                var toFileSplit = toFileName.Split(new string[] { ".log" }, 2, StringSplitOptions.RemoveEmptyEntries);
                var fromFileSplit = fromFileName.Split(new string[] { ".log" }, 2, StringSplitOptions.RemoveEmptyEntries);
                if (toFileSplit.Length != 0 && fromFileSplit.Length != 0)
                {
                    toFileSplit[0] = fromFileSplit[0];
                    toFileName = string.Join(".log", toFileSplit);
                    to = Path.Combine(fromFolderPath, toFileName);
                }
            }
            return to;
        }
        ...
        protected void RollFile(string fromFile, string toFile)
        {
            ...
            if (FileExists(fromFile))
            {
                ...
                try
                {
                    LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Moving [" + fromFile + "] -> [" + toFile + "]");
                    using (SecurityContext.Impersonate(this))
                    {
                        System.IO.File.Move(fromFile, toFile);
                    }
                    //文件拷贝完毕直接上传
                    RALogger.UploadLog(toFile);
                }
            ...
            {
                LogLog.Warn(typeof(AveRollingFileAppender), "RollingFileAppender: Cannot RollFile [" + fromFile + "] -> [" + toFile + "]. Source does not exist");
            }
        }
        ...
        protected void DeleteFile(string fileName)
        {
            if (FileExists(fileName))
            {
                ...
                {
                    LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Exception while moving file to be deleted [" + fileName + "] -> [" + tempFileName + "]", moveEx);
                }

                // Try to delete the file (either the original or the moved file)
                // Upload the deleted file before deleting it.
                RALogger.UploadLog(fileToDelete);
                /*
                try
                {
                    using (SecurityContext.Impersonate(this))
        ...
        public virtual void DeleteFileAfterUploading(string fileName)
        {
            try
            {
                using (SecurityContext.Impersonate(this))
                ...
                LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Deleted file [" + fileName + "]");
            }
            catch (Exception deleteEx)
            {
                // Moved the file, but the delete failed. File is probably locked.
                // The file should automatically be deleted when the lock is released.
                LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: Exception while deleting temp file [" + fileName + "]", deleteEx);
            }
        }
        ...
        protected void RollOverSize()
        {
            ...
            LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: rolling over count [" + ((CountingQuietTextWriter)QuietWriter).Count + "]");
            LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: maxSizeRollBackups [" + m_maxSizeRollBackups + "]");
            LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: curSizeRollBackups [" + m_curSizeRollBackups + "]");
            LogLog.Debug(typeof(AveRollingFileAppender), "RollingFileAppender: countDirection [" + m_countDirection + "]");
        ...
        /// </para>
        /// </remarks>
        protected DateTime NextCheckDate(DateTime currentDateTime, AveRollPoint rollPoint)
        {
        ...
        /// </summary>
        private AveRollPoint m_rollPoint;
        ...
        /// <summary>
        /// after roll file previous log file's name
        /// </summary>
        private string m_prevFileName = string.Empty;
