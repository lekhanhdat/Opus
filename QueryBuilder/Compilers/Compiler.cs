        ...
        protected virtual string CompileColumns(SqlResult ctx)
        {
            if (ctx.Query.HasComponent("aggregate", EngineCode))
            {
                ...
                if (aggregateColumns.Count == 1)
                {
                    ...
                    //cosmos db count case
                    if(sql == "1")
                    {
                        return "SELECT " + "VALUE " + aggregate.Type.ToUpperInvariant() + "(" + sql + $")";
                    }
            ...
            var top = ctx.Query.IsTop ? $"top {ctx.Query.TopNumber} " : "";

            return $"SELECT {distinct}{top}{select}";
