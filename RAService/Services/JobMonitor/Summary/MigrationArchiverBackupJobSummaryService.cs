...
{
    public class MigrationArchiverBackupJobSummaryService : MigrationBaseJobSummaryService
    {
        ...
        public override RMJobSummaryInfos GetJobSummaryInfo(BaseJobDto job, GeneralSettingModel gsm)
        {
            ...
            if (GetSummaryAttributes().Contains(STATISTICS))
            {
                RMJobSummaryItem statisticsItem = new()
                {
                    ...
                    SummaryRow = GetStaticticsForBackupRows(job)
                };
            ...
            if (GetSummaryAttributes().Contains(STATISTICSFOREXPORT))
            {
                RMJobSummaryItem statisticsItem = new()
                {
                    Title = SOI18NResource.Get("StorageOptimization.Service_0475628A-CEC2-435B-9343-E4F8160C00D4", "Export Statistics"),
                    SummaryRow = GetStaticticsForExportRows(job)
                };
                result.SummaryItem.Add(statisticsItem);
            }
            if (GetSummaryAttributes().Contains(STATISTICSFORBACKUP))
            {
                RMJobSummaryItem statisticsItem = new()
                {
                    Title = SOI18NResource.Get("StorageOptimization.Service_cd262b94-34c0-4f56-923b-9accdf0f62c3", "Backup Statistics"),
                    SummaryRow = GetStaticticsForBackupRows(job)
                };
                result.SummaryItem.Add(statisticsItem);
            }
            if (GetSummaryAttributes().Contains(STATISTICSFORDELETION))
            {
                RMJobSummaryItem statisticsItem = new()
                {
                    Title = SOI18NResource.Get("StorageOptimization.Service_5e1dd145-ce26-4efb-ab5a-0519fcb312be", "Statistics for Deletion"),
                    SummaryRow = GetStaticticsForDeletionRows(job)
                };
                result.SummaryItem.Add(statisticsItem);
            }
            if (GetSummaryAttributes().Contains(STATISTICSFORRECORDMANAGER))
            {
                RMJobSummaryItem statisticsItem = new()
                {
                    Title = SOI18NResource.Get("StorageOptimization.Service_1dc85826-3caf-4b2a-b4ff-8d51b5788f83", "Record Declaration Statistics"),
                    SummaryRow = GetStaticticsForRecordManagerRows(job)
                };
                result.SummaryItem.Add(statisticsItem);
            }
            return result;
