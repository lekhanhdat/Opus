...
namespace AvePoint.ObjectModel.Common
{
    class AveTaxonomyFieldValue : AveClientObject, IAveTaxonomyFieldValue
    {
        ...
        public int WssId
        {
            get
            {
                if (base.DataCache.GetProperty<int>("WssId")==-1)
                {                    
                    string[] array = this.taxonomyField.DefaultValue.Split(';');
                    if (array.Length > 1)
                    {
                        base.DataCache.PropertiesCache["WssId"] = Convert.ToInt32(array[0]);
                    }
                }
        ...
        public void PopulateFromLabelGuidPair(string text)
        {
            ...
            this.DataCache.AddPropertyies((this.taxonomyField as AveTaxonomyField).GetFieldValueAsTaxonomyFieldValue(text));
