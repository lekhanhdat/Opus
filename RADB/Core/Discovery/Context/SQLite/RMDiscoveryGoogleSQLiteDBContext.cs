...
{
    public class RMDiscoveryGoogleSQLiteDBContext : IDisposable
    {
        public int Timeout { get; set; } = 60 * 10;

        private readonly SQLiteConnection _connection;

        public RMDiscoveryGoogleSQLiteDBContext(string connectionString)
        {
        ...
        public Task<int> ExecuteInsertWapperAsync<T>(List<T> items, string googleOrganizationId) where T : RMDiscoveryDBTable
        {
            return ExecuteInsertAsync(items, RMDiscoveryGoogleSQLiteDBManager.GetSchemaName(googleOrganizationId));
        }

        public async Task<int> ExecuteInsertAsync<T>(List<T> items, object schema) where T : RMDiscoveryDBTable
        {
            ...
            var sql = $@"INSERT INTO {SecurityUtils.SanitizeSQLParameterName(schema.ToString())}${SecurityUtils.SanitizeSQLParameterName(tableInfo.Name)} 
