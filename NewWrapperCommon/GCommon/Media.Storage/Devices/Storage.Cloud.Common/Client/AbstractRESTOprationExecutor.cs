...
namespace AvePoint.Media.Storage.Cloud.Common
{
    ...
    abstract class AbstractRESTOprationExecutor : ICloudOprationExecutor, IHttpRequestPrepare, IHttpResponseHandler
    {
        ...
        private StorageLogger log = StorageLogger.GetInstance(MethodBase.GetCurrentMethod().DeclaringType);
        private object lockObject = new object();
        protected StorageLogger Logger
        {
            set
            {
                log = value;
        ...
        public abstract string GetFinalUrl(StorageInfo info);
        ...
        protected void InitProxySetting()
        {
            if (!string.IsNullOrEmpty(CloudOpenParam.ProxyIp))
            {
                this.Proxy = new WebProxy(CloudOpenParam.ProxyIp, CloudOpenParam.ProxyPort);
                if (!string.IsNullOrEmpty(CloudOpenParam.ProxyUsername) && !string.IsNullOrEmpty(CloudOpenParam.ProxyPassword))
                {
                    var userName = CloudOpenParam.ProxyUsername;
                    var password = CloudOpenParam.ProxyPassword;
        ...
        public HttpWebResponse DoExecute(BasicRequest request)
        {
            ...
            {
                Logger.Error("An error occurred while executing the request, detail:{0} and the response body:{1}", request.ToString(), GetStringFromResponse(we.Response as HttpWebResponse));
                ...
                else if (we.Status == WebExceptionStatus.ProtocolError)
                {
                    using (HttpWebResponse response = we.Response as HttpWebResponse)
                    {
                        ...
                        {
                            Logger.Warn("There is a exception when get response from server {0}", response.StatusDescription);
        ...
        public abstract string BuildURLWithOutQueryParams(string container);

        public abstract string BuildObjectAbsoluteURL(string container, string objectName);

        public abstract string BuildObjectAbsoluteURL(string url, string container, string objectName);
        ...
        protected virtual List<string> ListObjectWithPreFix(string xSetName, string prefix, bool isGetName)
        {
            ...
            while (true)
            {
                ...
                List<string> files = ListXstream(xSetName, prefix, MAX_OBJECT_COUNT, maker, isGetName);
        ...
        public abstract string ListAzureMetaName(string baseURL, Dictionary<string, string> queryParams, Dictionary<string, string> headers);

        public abstract void InitConfig(CloudOpenParameter prams);

        public abstract CloudOpenParameter ConveryParams(Dictionary<string, string> prams);

        public abstract List<string> ListContainers();

        public abstract bool CheckContainer(string xSetName);

        public abstract bool CreateContainer(string xSetName);

        public abstract bool DeleteContainer(string xSetName);

        public abstract List<string> ListObject(string xSetName);

        public abstract List<string> ListObject(string xSetName, string prefix);

        public abstract bool CheckObject(string xSetName, string xStreamName);

        public abstract HttpWebRequest GetUploadRequest(string xSetName, string xStreamName, string mimeType, HttpWebRequest webRequest, int blockNumber, long dataLength);

        public abstract bool CreateObject(string xSetName, string xStreamName, HttpWebRequest request, long dataLength);

        public abstract Stream OpenObject(string xSetName, string xStreamName, int rangFrom, int rangeTo);

        public abstract Stream OpenObject(string container, string objectName, int[] lengths, FileMode mode);

        public abstract bool DeleteObject(string xSetName, string xStreamName, bool isDeleteSubFile);

        public abstract CloudFileInfo GetObjectInfo(string xSetName, string xStreamName);

        public abstract bool Login(string xSetName);

        public abstract long GetContainerSize(string xSetName);

        public virtual StorageOpenValidResult GetPermissions()
        {
        ...
        class HttpRequestRetryMethod : IRetryMethod<HttpWebResponse>
        {
            private HttpWebRequest request;
            private AbstractRESTOprationExecutor executor;
        ...
        protected virtual HttpWebResponse DoExecute(HttpWebRequest request, Dictionary<string, string> headers)
        {
            ...
            {
                Logger.Error("An error occurred while executing the request whose uri is {0} and method is {1} and the response body:{2}", request.RequestUri.ToString(), request.Method, GetStringFromResponse(we.Response as HttpWebResponse));
            ...
            {
                Logger.Error("DoExecute error: {0}", e.ToString());
                try
                {
                    if (request != null)
                    {
        ...
        public bool DeleteObject(string fullURL, Dictionary<string, string> parameters, Dictionary<string, string> headers)
        {
            try
            {
                ...
                {
                    if (resp.StatusCode == HttpStatusCode.NoContent || resp.StatusCode == HttpStatusCode.OK || resp.StatusCode == HttpStatusCode.Accepted)
                    {
                        return true;
                    }
                }
        ...
        public abstract SpaceInfo GetUserAccountInfo();
        ...
        public virtual object Invoke(string methodName, object[] args)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (ex.InnerException is AuthenticationFailedException)
                {
                ...
                if (ex.InnerException is GatewayTimeoutException)
                {
                    throw ex.InnerException;
                }
        ...
        public abstract List<XDirectoryInfo> Parse2Directory(string responseXmlString, string path);

        public abstract List<XFileInfo> Parse2File(string responseXmlString);
    ...
    delegate void Parse2DirectoryDelegate(string responseXmlString);
