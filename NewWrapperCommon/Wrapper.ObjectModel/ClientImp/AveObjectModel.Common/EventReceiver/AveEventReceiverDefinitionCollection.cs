...
namespace AvePoint.ObjectModel.Common
{
    class AveEventReceiverDefinitionCollection : AveAbstractCommonCollection<IAveEventReceiverDefinition>, IAveEventReceiverDefinitionCollection
    {
        ...
        public AveEventReceiverDefinitionCollection(AveSite site, IAveRequest request, string eventReciverSource, Dictionary<string, object> eventReceiverColProperties)
        {
            mSite = site;
            mRequest = request;
            mEventReceiverSource = eventReciverSource;
            base.DataCache.AddPropertyies(eventReceiverColProperties);
            InitSiteEventReceiverCollection();
        }
        ...
        {
            List<Dictionary<string, object>> eventReceiverPropertiesList = base.DataCache.GetChildren();
            mListData = new List<IAveEventReceiverDefinition>(eventReceiverPropertiesList.Count);
            foreach (Dictionary<string, object> eventReceiverProperties in eventReceiverPropertiesList)
            {
        ...
        internal void InitSiteEventReceiverCollection()
        {
            List<Dictionary<string, object>> eventReceiverPropertiesList = base.DataCache.GetChildren();
            mListData = new List<IAveEventReceiverDefinition>(eventReceiverPropertiesList.Count);
            foreach (Dictionary<string, object> eventReceiverProperties in eventReceiverPropertiesList)
            {
                AveEventReceiverDefinition eventReceiverDefinition = new AveEventReceiverDefinition(mSite, this, mRequest, mEventReceiverSource, eventReceiverProperties);
                mListData.Add(eventReceiverDefinition);
            }
        }
        ...
        public void Add(AveEventReceiverType receiverType, string assembly, string className)
        {
            if (!IsExsit(receiverType, assembly, className))
            {
                ...
                eventReceiverDefinitionProp = this.mRequest.AddEventReceiverDefinition(this.mWeb.ServerRelativeUrl, null, this.mList == null ? null : this.mList.Title, this.mList == null ? Guid.Empty : this.mList.ID, this.mEventReceiverSource, (int)receiverType, assembly, className, string.Empty);
        ...
        {
            Dictionary<string, object> eventReceiverDefinitionProp = new Dictionary<string, object>();
            eventReceiverDefinitionProp = this.mRequest.AddEventReceiverDefinition(this.mWeb.ServerRelativeUrl, null, this.mList == null ? null : this.mList.Title, this.mList == null ? Guid.Empty : this.mList.ID, this.mEventReceiverSource, (int)receiverType, assembly, className, name);
            AveEventReceiverDefinition eventReceiverDefinition = new AveEventReceiverDefinition(this.mWeb, this.mList, this, this.mRequest, this.mEventReceiverSource, eventReceiverDefinitionProp);
            this.mListData.Add(eventReceiverDefinition);
        }
