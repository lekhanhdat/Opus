...
using AvePoint.GCommon.Utility.Cryptography.Encryption.Blowfish;

namespace AvePoint.GCommon.Utility.Cryptography.Encryption
{
    public class BlowfishEncryption : AbstractEncryption
    {


        public BlowfishEncryption()
        {
            base.Crypto = new BlowfishAlgorithm();
            this.SetKeyAndIV(null);
        }


        public BlowfishEncryption(SecureString key)
        {
            base.Crypto = new BlowfishAlgorithm();
            this.SetKeyAndIV(CryptoUtil.ConvertSecureStringToBytes(key));
        }

        public BlowfishEncryption(byte[] key)
        {
            base.Crypto = new BlowfishAlgorithm();
