...
namespace AvePoint.RA.Web.Common.WIF
{
    ...
    public class JwtValidationHandler : Attribute, IAsyncAuthorizationFilter
    {
        ...
        private static readonly HashSet<string> SkipUserIdCheckEndpoints =
        [
            "/api/googleone/initialization/init"
        ];
        ...
        public Task OnAuthorizationAsync(AuthorizationFilterContext context)
        {
            ...
            try
            {
                ...
                var tokenResult = ValidateIdentityServerToken(token, context, isInternalIdentityServer, publicKey);
        ...
        private ClaimsPrincipal ValidateIdentityServerToken(string token, AuthorizationFilterContext context, bool isInternalIdentityServer, string publicKey)
        {
            try
            {
                ...
                if (isMyhubRequest)
                {
                    var requestUrl = request.Path.ToString().ToLowerInvariant();
                    if (!WhiteList.ConvertAll(w => { return w.ToLowerInvariant(); }).Contains(requestUrl))
                    {
        ...
        private void GenerateBasicInfoByProduct(ProductType product, ClaimsPrincipal claims, HttpRequest httpRequestMessage)
        {
            ...
            switch (product)
            {
                ...
                case ProductType.RecordsAgent:
                    var tenantGroupId = GenerateBasicInfoForRecords(claims, httpRequestMessage);
                ...
                case ProductType.AOSVNext:
                    ...
                    break;
                case ProductType.OpusControlPlus:
                    GenerateBasicInfoForOpusControlPlus(claims, httpRequestMessage);
                    break;
                default:
                    GenerateBasicInfoForRecords(claims, httpRequestMessage);
                    break;
        ...
        private bool UseInternalIdentityServer(HttpRequest httpRequest, ref ProductType productName)
        {
            ...
            if (httpRequest.Headers.ContainsHeader(RequestHeadersParam.OPUS_CONTROL_PLUS))
            {
                productName = ProductType.OpusControlPlus;
            }
        ...
        private string GenerateBasicInfoForRecords(ClaimsPrincipal claimsPrincipal, HttpRequest httpRequest)
        {
            ...
            if (httpRequest.HttpContext.Request.Headers.ContainsHeader("X-MultiGeo-IP"))
            {
                string multiGeoIP = httpRequest.HttpContext.Request.Headers.GetHeaderValue("X-MultiGeo-IP");
                if (!string.IsNullOrEmpty(multiGeoIP))
                {
                    ClientRequestLocalValue.ClientIP = multiGeoIP;
                }
            }
            ...
            if (httpRequest.Headers.ContainsHeader("uid"))
            {
                userId = httpRequest.GetRequestHeadersParam("uid");
                if (string.IsNullOrEmpty(userId))
                {
                    logger.Warn($"uid is empty in request header, tenantGroupId: {tenantGroupId}, userName: {userName}");
                }
            }
        ...
        private string GenerateBasicInfoForOpusControlPlus(ClaimsPrincipal claimsPrincipal, HttpRequest httpRequest)

        {
            ...
            if (httpRequest.Headers.ContainsHeader("TimezoneId"))
            {
                timezoneId = httpRequest.GetRequestHeadersParam("TimezoneId");
            }

            var userName = GetClaimValueByCliamTypeName(claimsPrincipal.Claims, ClaimTypes.Upn);
            var callerType = GetClaimValueByCliamTypeName(claimsPrincipal.Claims, "product-identifier");

            TenantLocalValue.LogonGroupId = tenantGroupId;
            TenantLocalValue.LogonUserEmail = userName;
            TenantLocalValue.TimezoneId = timezoneId;
            TenantLocalValue.RequesterType = callerType switch
            {
                "GoogleControl" => RequesterTypeEnum.OpusControlPlus,
                "GoogleControlMyHub" => RequesterTypeEnum.OpusControlPlusMyHub,
                _ => RequesterTypeEnum.Opus,
            };

            var cultureInfo = httpRequest.GetRequestHeadersParam("X-CPlus-Language");
            if (!string.IsNullOrEmpty(cultureInfo))
            {
                cultureInfo = cultureInfo switch
                {
                    "en" => "en-US",
                    "ja" => "ja-JP",
                    _ => "en-US",
            ...
            if (!ShouldSkipUserIdCheckSafe(httpRequest) && !string.IsNullOrWhiteSpace(userName))
            {
                var activeUser = AccountDao.GetActiveUserByNameAsync(userName).GetAwaiter().GetResult();
                if (activeUser == null)
                {
                    throw new Exception($"{string.Format(I18NEntity.GetString("RM_VT_TenantGroupIdUserIdInvalid"), tenantGroupId)}");
                }
                TenantLocalValue.LogonUserId = activeUser.UserId;
                TenantLocalValue.LogonGoogleUserId = activeUser.AADId;
            }
            return tenantGroupId;
        }
        ...
        private void ValidAgentInfo(string tenantId, ClaimsPrincipal result)
        {
            try
            {
                ...
                {
                    if (!TenantInfoService.AdditionalDataSourceEnable(tenantId))
                    {
        ...
        private static bool ShouldSkipUserIdCheckSafe(HttpRequest request)
        {
            try
            {
                var requestUrl = request.Path.ToString().ToLowerInvariant() ?? "";
                return SkipUserIdCheckEndpoints.Any(endpoint =>
                    string.Equals(endpoint, requestUrl, StringComparison.OrdinalIgnoreCase));
            }
            catch (Exception ex)
            {
                logger.Error($"An error occurred while checking if UserId validation should be skipped. Exception: {ex.Message}");
                return false;
            }
        }
    }
