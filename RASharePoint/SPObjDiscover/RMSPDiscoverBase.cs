...
namespace AvePoint.RA.SharePoint.SPObjDiscover
{
    ...
    {
        private static readonly AvePoint.RA.Contract.Services.IRALogger logger = RALogger.GetInstance(typeof(RMSPDiscoverBase));
        ...
        public RMSPDiscoverBase(AveDiscoverSite aveDiscoverSite, SPTreeNodeDto treeNode, JobContext jobContext)
        {
        ...
        public virtual System.Threading.Tasks.Task RunNowAsync()
        {
        ...
        protected bool IsSystemList(AveDiscoverList list)
        {
            ...
            {
                logger.Info("skip hidden list : {0}", string.IsNullOrEmpty(list?.RootFolderUrl) ? list?.Name : list?.RootFolderUrl);
                result = true;
            }
            return result;
        }
