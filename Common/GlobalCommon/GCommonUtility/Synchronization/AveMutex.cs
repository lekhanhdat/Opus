...
namespace AvePoint.GCommon.Utility
{
    ...
    {
        private static AveLogger logger = AveLogger.GetInstance(typeof(AveMutex));
        ...
        public void WaitLocked()
        {
            if (this.mutex == null)
            {
                string mutexMd5Name = HashCodeHelper.StringHash(mutexName).ToString();
                try
                {
            ...
            catch (System.Threading.AbandonedMutexException ame)
            {
                ...
                logger.Warn($@"fail wait locked , ex:{ame}");
            }
        }
