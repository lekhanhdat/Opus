...
class CRMSPTree extends Component {
    constructor(props) {
        ...
        this.rootItem = {
            ...
            SourceFlag: props.treeSource,
            ContainerId: null,
            PageIndex: 0,
            PageSize: 15,
            ChildrenIds: [],
            Children: [],
            Name: RMResx.RM_JS_DAM_SiteCollection_Results,
        };
        this.treeContext = {
            ...
            transToTreeNodeObject(oitem) {
                let isVirtualRootNode = oitem.IsVirtualRoot === true || oitem.Level == null;
                let isLoadMoreMode = isVirtualRootNode || oitem.Level === NodeLevel.WebApplication;
                ...
                let pagerSize = 15;
                let hasNextPage = !!oitem.HasNextPage;
                ...
                return {
                    ...
                    pagerSize: pagerSize,
                    pagerIndex: !oitem.PageIndex || oitem.PageIndex * pagerSize >= children.length ? 0 : oitem.PageIndex,
                };
            },
            ...
            getViewChildren(oitem) {
                ...
                if(oitem.Children){
                    for (let child of oitem.Children) {
                        if(child.Level !== NodeLevel.SiteCollection){
                            child.Parent = oitem;
                        }
                        children.push(child);
                    }
            ...
            onLoadNodes(parentItem, funcSuccess, funcFail) {
                ...
                let isLoadMoreMode = poItem.Level === NodeLevel.Root;
                ...
                if(poItem?.Parent?.IsVirtualRoot){
                ...
                $$.fetch.post(url, postData).then((data)=>{
                    ...
                    if ("IconStatus" in mainComponent.updateProps) {
    ...
    search = async (keywords) => {
        ...
        $$.loading(true);
        let url = "/api/SPOnPremBrowse/SearchSiteCollectionLazyLoad";
        if (this.props.treeSource == SourceFlags.SP) {
            url = "/api/SPSettingApi/SearchSiteCollectionLazyLoad";
        }
        if (this.props.treeSource == SourceFlags.OneDrive) {
        ...
        let data = {
            ...
            data: {
                IsArchiverTree: isArchiverTree,
                SearchKey: keywords,
                SourceFlag: this.props.treeSource,
                ContainerId: null,
                PageSize: 15
            }
        };
