...
{
    [Serializable]
    public class AveException : Exception
    {
        ...
        public AveException(Exception innerException, string format, params object[] args)
            : base(args !=null && args.Length > 0? string.Format(format, args) : format, innerException)
        {
        }
        #endregion
        ...
        private string GetValue(string prefix)
        {
            ...
            var index = temp.IndexOf("AvePoint.GCommon.Utility.Exceptions");
            if (index >= 0)
            {
