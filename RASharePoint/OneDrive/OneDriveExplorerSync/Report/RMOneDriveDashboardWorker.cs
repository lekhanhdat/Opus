...
namespace AvePoint.RA.SharePoint.OneDriveExplorerSync.Report
{
    public class RMOneDriveDashboardWorker
    {
        private static readonly RALogger logger = RALogger.GetInstance(typeof(RMOneDriveDashboardWorker));
        ...
        public RMOneDriveDashboardWorker(JobContext jobContext)
        {
            this.jobContext = jobContext;
        }

        public async Task ProcessBoardChangedItemAsync(BoardItem item)
        {
            try
            {
                using (var performance = new PerformanceScope($"RMDashboard.ProcessChange"))
                {
                    switch (item.ChangeType)
                    {
                        case BoardChangeType.Modified:
                            await ModifiedTermUsagesAsync(item);
                            break;
                        ...
                        case BoardChangeType.Delete:
                            await TermUsagesForDeleteRecordAsync(item);
        ...
        public void ProcessTermChange()
        {
            try
            {
                foreach (var item in RMOneDriveExplorerBoardCache.Instance.TermChangedDic)
                {
                    var termId = item.Key;
                    var count = item.Value;
                    var currentTerm = TermDao.FindWithNewContext(s => s.UniqueId == termId && !s.IsRemoved);
                    var currentTermUsage = RMTermUsageDao.FindWithNewContext(s => s.TermId == termId && s.SourceFlag == (int)SourceFlag.OneDrive);
        ...
        public void ProcessCollectionChange()
        {
            try
            {
                foreach (var sc in RMOneDriveExplorerBoardCache.Instance.CollectionChangedDic)
                {
                    var scopeId = sc.Key;
                    var count = sc.Value;
        ...
        public void ProcessTotalChange()
        {
            try
            {
                var currentTotal = BoardTotalDao.FindWithNewContext(s => s.SourceFlag == (int)SourceFlag.OneDrive);
                ...
                foreach (var tempTotal in RMOneDriveExplorerBoardCache.Instance.TotalChangedDic)
                {
        ...
        public async Task ProcessFullWaitingForApprovalAsync()
        {
            try
            {
                var index = 1;
                ...
                var tempDatas = ManualApproveDao.GetDatasByPager(index, pageSize, ref totalCount, m => m.SourceFlag == (int)SourceFlag.OneDrive);
                ...
                await ProcessWaitAssignerAsync(tempDatas, userData);
                while (totalCount - index * pageSize > 0)
                {
                    index++;
                    var manualItems = ManualApproveDao.GetDatasByPager(index, pageSize, ref totalCount, m => m.SourceFlag == (int)SourceFlag.OneDrive);
        ...
        public async Task ProcessFullWaitingForApprovalNewAsync()
        {
            try
            {
        ...
        private void ProcessWaitingDataOfDateNew()
        {
            ...
            try
            {
                List<long> tempDatas = ManualApproveDao.GetAllCollectionTime(index, pageSize, ref totalCount, SourceFlag.OneDrive);
                ...
                {
                    index++;
                    List<long> manualItems = ManualApproveDao.GetAllCollectionTime(index, pageSize, ref totalCount, SourceFlag.OneDrive);
                    logger.Info("Query collection time row count {0}, total count {1}", tempDatas.Count, totalCount);
        ...
        //暂时没用上
        /*private async Task ProcessTop10AssignerNewAsync(Dictionary<string, int> top10userData, int totalCount)
        {
            var top9Count = 0;
        ...
        private void ProcessWaitingTotal()
        {
            ...
            int totalWaiting = ManualApproveDao.GetWaitingCount(SourceFlag.OneDrive);
            var currentTotal = BoardTotalDao.FindWithNewContext(s => s.SourceFlag == (int)SourceFlag.OneDrive);
        ...
        private void ProcessWaitingTimeLine(Dictionary<long, int> tempWaitForApprovalTimeDic)
        {
            try
            {
                ...
                {
                    var currentDataOfDay = RMDataOfDayDao.FindWithNewContext(s => s.Dater == timeCountPair.Key && s.SourceFlag == (int)SourceFlag.OneDrive);
        ...
        private async Task ProcessWaitAssignerAsync(List<RMManualApprove> allDatas, Dictionary<string, int> userData)
        {
            try
            {
        ...
        private async Task<List<string>> GetApprovalUsersAsync(RMManualApprove data) 
        {
        ...
        public void ProcessDataOfDateChange()
        {
            try
            {
                foreach (var tempDataOfDay in RMOneDriveExplorerBoardCache.Instance.DataOfDateChangedDic)
                {
                    ...
                    foreach (var item in tempDataOfDay.Value)
                    {
                        ...
                        var currentDataOfDay = RMDataOfDayDao.FindWithNewContext(s => s.Dater == dater && s.SourceFlag == (int)SourceFlag.OneDrive);
                        if (currentDataOfDay != null)
                        {
        ...
        private void AddOrDeleteTermUsages(BoardItem item, long count)
        {
            ...
            {
                RMOneDriveExplorerBoardCache.Instance.AddTermChange(termId, count);
            }
        ...
        private void AddDataOfDays(BoardItem item, long count)
        {
            ...
            if (createdTime != 0)
            {
                ...
                RMOneDriveExplorerBoardCache.Instance.AddDataOfDateChange(BoardRecordStatus.Creation, date, count);
            }
        ...
        {
            if (item.OldRecord != null && (item.OldRecord.RecordStatus == (int)RMRecordStatus.Destroyed || item.OldRecord.RecordStatus == (int)RMRecordStatus.Moved || item.OldRecord.RecordStatus == (int)RMRecordStatus.MoveOverwrite))
            {
                ...
                if (destroyedTime != 0)
                {
                    ...
                    RMOneDriveExplorerBoardCache.Instance.AddDataOfDateChange(BoardRecordStatus.Destruction, date, 1);
        ...
        {
            RMOneDriveExplorerBoardCache.Instance.AddTotalChange(BoardRecordStatus.ManagedRecord, count);
        ...
        private void DeleteTotals(BoardItem item)
        {
            if (item.OldRecord != null)
            {
                ...
                {
                    RMOneDriveExplorerBoardCache.Instance.AddTotalChange(BoardRecordStatus.ManagedRecord, -1);
                }
                else if (item.OldRecord.RecordStatus == (int)RMRecordStatus.Destroyed || item.OldRecord.RecordStatus == (int)RMRecordStatus.Moved || item.OldRecord.RecordStatus == (int)RMRecordStatus.MoveOverwrite)
                {
                    RMOneDriveExplorerBoardCache.Instance.AddTotalChange(BoardRecordStatus.ManagedRecord, -1);
                    RMOneDriveExplorerBoardCache.Instance.AddTotalChange(BoardRecordStatus.Destruction, 1);
                }
            }
        ...
        private void AddOrDeletCollectionCount(BoardItem item, long count)
        {
            ...
            RMOneDriveExplorerBoardCache.Instance.AddCollectionChange(scopeId, count);
        ...
        private void DeletCollectionCount(BoardItem item)
        {
            ...
            if (item.OldRecord != null)
            {
                ...
                if (item.OldRecord.RecordStatus == (int)RMRecordStatus.Active || item.OldRecord.RecordStatus == (int)RMRecordStatus.Destroyed
                     || item.OldRecord.RecordStatus == (int)RMRecordStatus.Moved || item.OldRecord.RecordStatus == (int)RMRecordStatus.MoveOverwrite)
                {
                    RMOneDriveExplorerBoardCache.Instance.AddCollectionChange(scopeId, -1);
                }
        ...
        private async Task ModifiedTermUsagesAsync(BoardItem item)
        {
            //需要判断TermId是否发生了变化...
            ...
            var tempHistories = await ClassificationHistoryDao.FindListAsync(d => d.RecordId == item.NewRecord.Id);
            ...
            if (previousTermId != currentTermId)
            {
                ...
                {
                    //Previous Term - 1
                    RMOneDriveExplorerBoardCache.Instance.AddTermChange(previousTermId, -1);
                }
                if (currentTermId != Guid.Empty)
                {
                    //Current Term + 1
                    RMOneDriveExplorerBoardCache.Instance.AddTermChange(currentTermId, 1);
                }
            }

        }

        private async Task TermUsagesForDeleteRecordAsync(BoardItem item)
        {
            ...
            if (item.OldRecord != null)
            {
                ...
                var previousTermId = Guid.Empty;
                var tempHistories = await ClassificationHistoryDao.FindListAsync(d => d.RecordId == item.OldRecord.Id);
                var tempHistory = tempHistories.OrderBy(j => j.OperationTime).FirstOrDefault();
                if (tempHistory != null)
                {
                    previousTermId = tempHistory.PreviousTermId;
