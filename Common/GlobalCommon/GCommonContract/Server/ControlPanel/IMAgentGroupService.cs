...
namespace AvePoint.GCommon.Contract.Server.ControlPanel
{
    ...
    public interface IMAgentGroupService
    {
        ...
        [OperationContract]
        List<ServiceGroupDto> GetAllTypeAgentGroups();
        [OperationContract]
        ...
        /// <summary>
        /// 根据AgentId查询Agent可用的模块.
        /// </summary>
        /// <param name="agentIds">The agent id array.</param>
        /// <returns>可用模块map，一个模块名对应多个agent name.</returns>
        [OperationContract]
        Dictionary<string, List<string>> GetAvailableModule(string[] agentIds);

        /// <summary>
        /// 获取所有可用的agent。
        /// </summary>
        /// <returns>key : farm的display name, value : agent list.</returns>
        [OperationContract]
        Dictionary<FarmDto, List<ServiceDto>> GetAvailableAgentMap();

        [OperationContract]
        ServiceGroupDto GetServiceGroupDetailForGUI(string groupId);
