...
{
    public class LoanPickStatusProcessor : BasePickStatusProcessor
    {
        private IRecordLoanAllianceDao RecordLoanAllianceDao => PlatformWindsorManager.GetService<IRecordLoanAllianceDao>();

        public LoanPickStatusProcessor(JobType jobType, string jobId) : base(jobType, jobId)
        {
        }

        protected override ExplorerQueryV3Dto GetQueryDto(CompleteActionParam jobParam, string pageIndex)
        {
            var queryDto = PickListService.GetLoanQueryDto(pageIndex, PageSize, jobParam.SearchText, jobParam.FilterOptions);
        ...
        protected override async Task ProcessRecordsAsync(BaseRecordDto rec)
        {
            ...
            if (rec.NodeType == (int)RMNodeType.PhyBox)
            {
                ...
                {
                    List<RMRecordLoanAlliance> loanAls = RecordLoanAllianceDao.GetPhyRecordAllianceByIds(children.Select(c=>c.Id).ToList());
                    foreach (var subRecord in children)
                    {
                        var al = loanAls.Where(a => a.RecordsId == subRecord.Id);
                        if (subRecord.LoanPickStatus == (int)PickStatusType.Pendding && al != null && al.Any())
                        {
                            successIds.Add(subRecord.Id);
                            ExplorerDao.UpdateAll(s => s.Id == subRecord.Id, r => { r.LoanPickStatus = (int)PickStatusType.Complete; });
