...
namespace AvePoint.Wrapper.Core.SPRestore.Mapping
{
    ...
    static class BuiltinFieldMappingParameterConverter
    {
        internal static XmlDocument Convert(ColumnMappingDataContract contract)
        {
        ...
        private static void InitMapping(XmlElement element, ColumnMappingValue mappingValue)
        {
            ...
            switch (mappingValue.Type)
            {
                ...
                case ColumnType.ChangeToLookUp:
                    ...
                    break;
                case ColumnType.ChangeToDes:
                    if (mappingValue.DestinationSetting != null)
                    {
                        if (mappingValue.DestinationSetting.IsMigrateString)
                        {
                            setting.SetAttribute("separateChar", mappingValue.DestinationSetting.MigrateBy);
                        }
                        else
                        {
                            setting.SetAttribute("separateChar", string.Empty);
