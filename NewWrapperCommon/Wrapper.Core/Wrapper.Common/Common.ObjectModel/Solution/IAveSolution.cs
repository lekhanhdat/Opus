...
{
    public interface IAveSolution : IAvePersistedObject
    {
        IAveSolutionLanguagePackCollection LanguagePacks { get; }
        Guid SolutionId { get; }
        bool JobExists { get; }
        ...
        Collection<IAveServer> DeployedServers { get; }
        ...
        IAveSolutionLanguagePack GetLanguagePack(uint lcid);
        bool IsOperationResultError(AveSolutionOperationResult result);
        void RetractLocal();
        void RetractLocal(Collection<IAveWebApplication> webApplications);
        void Upgrade(string path, DateTime dt);
