...
namespace AvePoint.ObjectModel.O365
{
    static class ReconnectableHttpWebRequestExtension
    {
        ...
        public static void RefreshDigestInfo(this ReconnectableHttpWebRequest request, ClientContext context)
        {
            if (request.Headers["X-RequestDigest"] != null)
            {
                request.RequestFailed += (r, e) =>
                {
                    try
                    {
                        ...
                        request.Headers["X-RequestDigest"] = Office365.Api.Office365Api.GetFormDigestProvider().GetFormDigest((context)).DigestValue;
                    }
                    catch (Exception ex)
                    {
