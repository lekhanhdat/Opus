...
namespace AvePoint.Media.Storage.Cloud.Common
{
    ...
    class Retry
    {
        ...
        public StorageLogger Logger { get; set; }
        ...
        public Retry(int count, int interval, bool needRetry, bool flushDns)
        {
            ...
            this.Logger = StorageLogger.GetInstance(MethodBase.GetCurrentMethod().DeclaringType);
        }
        ...
        public T CloudRetry<T>(RetryDelegate<T> del, int numberOfRetries, int msPause)
        {
            ...
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        Logger.Info("Error: " + ex.Message + ". Cloud Retry after " + Interval + " ms. Retry count: " + counter);
                        if (msPause > 0)
                        {
