...
namespace AvePoint.Wrapper.Backup
{
    ...
    public class AveSPContentTypeHub : IAveSPContentTypeHub, IDisposable
    {
        ...
        public bool IsMetadataPartition { get; private set; }

        public List<string> IncludeCententTypes { get; private set; }
        ...
        public IAveSite SPSite
        {
            get
            {
                ...
                {
                    Uri url = null;
                    if (IsMetadataPartition) 
                    {
                        url = this.mSPServiceApplication.GetContentTypeSyndicationHubLocal(PartitionId);
                    }
                    else
                    {
                        url = this.mSPServiceApplication.GetContentTypeSyndicationHubLocal();
                    }
                    if (url != null)
                    {
        ...
        public AveSPContentTypeHub(AveObjectModelFactory fac, Guid applicationId, Guid partitionId)
            : this(fac, fac.CreateMetadataServiceApplication(applicationId, partitionId))
        {
            PartitionId = partitionId;
            IsMetadataPartition = true;
            needDispose = true;
        }
        ...
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("Backup.AveSPContentTypeHub.ExportContentTypes"))
            {
                this.IncludeCententTypes = filterContentTypes;
                var contentTypes = AveSPContentTypeCollection.CreateInstance(this);
                contentTypes.Export(output);
            }
        }
    }
}