...
{
    class RepeatingSectionControl : BaseControl
    {
        ...
        public RepeatingSectionControl(AveSPWeb web, IAveList list, string contentTypeId, XmlNode controlNode, XmlNamespaceManager nsManager)
        ...
        public override void ProcessControl(bool isPost)
        {
            try
            {
                ...
                var referencesNode = GetPropertyNode("d2p1:InsertReferences");
                if (referencesNode != null)
                {
                    foreach (XmlNode node in referencesNode.ChildNodes)
                    {
                        var keyNode = node.SelectSingleNode("d4p1:Key", nsManager);
                        var valueNode = node.SelectSingleNode("d4p1:Value", nsManager);
