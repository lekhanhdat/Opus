...
{
    public class IdentifierConnectorColumn : IConnectorColumn
    {
        public Contract.TemplateManagement.ColumnType Type => Contract.TemplateManagement.ColumnType.Identifier;
        ...
        public async Task<(bool, CustomColumn)> TryConvertToCustomColumnAsync(CustomizeConnectorColumnInfo columnInfo, object valueJson)
        {
            ...
            if (!Guid.TryParse(valueJson?.ToString(), out var guidId) || guidId == Guid.Empty)
            {
        ...
        {
            if(columnInfo.IsRequired && valueJson == null)
            {
