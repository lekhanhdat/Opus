...
namespace AvePoint.RA.Api.Web.Public.Controllers
{
    ...
    public class CustomizeConnectorApiV1Controller : RAWebApiBase
    {
        ...
        {
            return CustomizeConnectorApiService.InsertDataV1(connectorDataObj).GetAwaiter().GetResult();
        }

        [HttpGet]
        public object GetDueRecords([FromQuery] string id, [FromQuery] string disposalDueDate, [FromQuery] int itemsPerPage, [FromQuery] string startIndex)
        {
            return CustomizeConnectorApiService.GetDataV1(id, disposalDueDate, itemsPerPage, startIndex).GetAwaiter().GetResult();
        }

        [HttpDelete]
        public object DisposeRecords([FromBody] object disposalInfo)
        {
            return CustomizeConnectorApiService.DisposeRecordsV1(disposalInfo).GetAwaiter().GetResult();
        }
    }
}
