...
using AvePoint.GCommon.Utility.Cryptography.Encryption.Aes;

namespace AvePoint.GCommon.Utility.Cryptography.Encryption
{
    public class AESEncryption : AbstractEncryption
    {
        protected System.Security.Cryptography.Aes CreateAesEncryption()
        {
        ...
        {
            Crypto = CreateAesEncryption();
            this.SetKeyAndIV(null);

        }

        public AESEncryption(SecureString key)
        {
            Crypto = CreateAesEncryption();
