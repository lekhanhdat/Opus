...
namespace AutoInstallation.Common.GUI
{
    internal class GifAnimation : Viewbox
    {
        ...
        public void CreateGifAnimation(MemoryStream memoryStream)
        {
            ...
            var decoder = new GifBitmapDecoder(memoryStream, BitmapCreateOptions.PreservePixelFormat,
            ...
            for (var i = 0; i < frameList.Count; i++)
                Console.WriteLine(frameList[i].disposalMethod + " " + frameList[i].width + " " +
                                  frameList[i].delayTime);
        ...
        public void NextFrame(object sender, EventArgs e)
        {
            ...
            if (frameList[frameCounter].disposalMethod == 2) frameList[frameCounter].Visibility = Visibility.Hidden;
            if (frameList[frameCounter].disposalMethod >= 3) frameList[frameCounter].Visibility = Visibility.Hidden;
            frameCounter++;
            ...
            {
                if (numberOfLoops != 0) currentLoop++;
                if (currentLoop < numberOfLoops || numberOfLoops == 0)
                {
                    for (var f = 0; f < frameList.Count; f++) frameList[f].Visibility = Visibility.Hidden;
                    frameCounter = 0;
                    frameList[frameCounter].Visibility = Visibility.Visible;
                    frameTimer.Interval = new TimeSpan(0, 0, 0, 0, frameList[frameCounter].delayTime * 10);
                    frameTimer.Start();
        ...
        private int ParseGraphicControlExtension(byte[] gifData, int offset)
        {
            ...
            while (gifData[returnOffset] != 0x00) returnOffset = returnOffset + gifData[returnOffset] + 1;
        ...
        private int ParseGraphicBlock(byte[] gifData, int offset)
        {
            ...
            if (currentParseGifFrame.width > logicalWidth) logicalWidth = currentParseGifFrame.width;
            if (currentParseGifFrame.height > logicalHeight) logicalHeight = currentParseGifFrame.height;
        ...
        private int ParseExtensionBlock(byte[] gifData, int offset)
        {
            ...
            if (gifData[offset + 1] == 0xFF && length > 10)
            {
                ...
                if ("NETSCAPE".Equals(netscape))
                {
                    numberOfLoops = BitConverter.ToUInt16(gifData, offset + 16);
                    if (numberOfLoops > 0) numberOfLoops++;
                }
            }
        ...
        private int ParseHeader(byte[] gifData, int offset)
        {
            ...
            if (!"GIF".Equals(str)) throw new Exception("Not a proper GIF file: missing GIF header");
            return 6;
        }
        ...
        private void ParseGifDataStream(byte[] gifData, int offset)
        {
            ...
            while (offset != -1) offset = ParseBlock(gifData, offset);
        }
