...
namespace AvePoint.RA.SharePoint.ExplorerSync.Report
{
    ...
    {
        private static readonly RALogger logger = RALogger.GetInstance(typeof(RMSPDashboardWorker));
        ...
        public RMSPDashboardWorker(JobContext jobContext)
        {
        ...
        public async Task ProcessBoardChangedItemAsync(BoardItem item)
        {
            try
            {
                using (var performance = new PerformanceScope($"RMDashboard.ProcessChange",addToStatistics:true))
                {
        ...
        public void ProcessTermChange()
        {
            try
            {
                foreach (var item in RMExplorerBoardCache.Instance.TermChangedDic)
                {
                    ...
                    var currentTermUsage = RMTermUsageDao.FindWithNewContext(s => s.TermId == termId && s.SourceFlag == (int)SourceFlag.SharePoint);
        ...
        private void DeleteTotals(BoardItem item)
        {
            if (item.OldRecord != null)
            {
                ...
                else if (item.OldRecord.RecordStatus == (int)RMRecordStatus.Destroyed || item.OldRecord.RecordStatus == (int)RMRecordStatus.Moved
                    || item.OldRecord.RecordStatus == (int)RMRecordStatus.MoveOverwrite)
                {
                    RMExplorerBoardCache.Instance.AddTotalChange(BoardRecordStatus.ManagedRecord, -1);
    ...
    public class BoardItem
    {
        ...
        public BoardChangeType ChangeType { get; set; }
        //For Total,DataOfDate
        public BoardRecordStatus InnerType { get; set; }
    }
}
