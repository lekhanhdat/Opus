...
{
    public class AzureFileShareServiceBeforeAuditHandler : IBeforeAuditHandler
    {
        public IAzureFileShareSettingDao AzureFileShareSettingDao => PlatformWindsorManager.GetService<IAzureFileShareSettingDao>();
        ...
        public async Task<RMAuditInfo> CollectAsync(int model, int category, int action, object[] args, object target)
        {
            ...
            {
                #region AuditAction.AzureFileSaveTermSetting
                AzureFileSettingDto dto = (AzureFileSettingDto)args[0];
                var node = dto.SelectedNode;
                info.Object = node.FullPath;
                var dbSetting = AzureFileShareSettingDao.LoadSetting(new Guid(node.Id), new Guid(node.ContainerId));
                if (dbSetting != null)
                {
            ...
            else if (action == (int)AuditAction.AzureFileInheritSetting)
            {
                #region AuditAction.AzureFileInheritSetting
                AzureFileShareTreeNode node = (AzureFileShareTreeNode)args[0];
                info.Object = node.FullPath;
                #endregion
            }
            else if (action == (int)AuditAction.AzureFileDeactiveSetting)
            {
                #region AuditAction.AzureFileDeactiveSetting
                AzureFileSettingDto dto = (AzureFileSettingDto)args[0];
                var node = dto.SelectedNode;
                if (dto.IsActive)
                {
                    info.Action = AuditAction.AzureFileActiveSetting;
                }
                info.Object = node.FullPath;
                #endregion
            }
