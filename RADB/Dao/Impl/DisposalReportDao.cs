...
{
    public class DisposalReportDao : BaseDao<RMProfile>, IDisposalReportDao
    {
        ...
        public async Task<bool> Edit(RMProfile profile)
        {
            using(var context = GetNewContext())
            {
                ...
                if(existResult == null)
                {
                    throw new Exception($"Can't find disposal profile: [{profile.Name}] by: [{profile.Id}]");
                }
        ...
        public async Task<bool> Delete(int id)
        {
            using(var context = GetNewContext())
            {
                ...
                if (existResult == null)
                {
                    throw new Exception($"Can't find disposal profile by: [{id}]");
                }
