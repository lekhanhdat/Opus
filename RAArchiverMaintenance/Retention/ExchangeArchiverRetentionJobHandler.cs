...
{
    public class ExchangeArchiverRetentionJobHandler
    {
        ...
        private IEXOArchiverIndexSubInfoDao ArchiverIndexSubInfoDao => PlatformWindsorManager.GetService<IEXOArchiverIndexSubInfoDao>();
        ...
        public ExchangeArchiverRetentionJobHandler(string jobId, JobType jobType)
        {
            SubJobId = jobId;
            mJobType = jobType;
        ...
        public ExchangeArchiverRetentionJobHandler()
        {
        }
        public void InitExchangeArchiverRetentionForOrphanBlob(string jobId, JobType jobType)
        {
            SubJobId = jobId;
            mJobType = jobType;
        }
            ...
            try
            {
                ...
                foreach (var archiverPruningJob in archiverPruningJobs)
                {
                    ...
                    try
                    {
                        ...
                        {
                            ReportManager.SendJobDetail(GenerateRetentionSkipReport(archiverPruningJob.JobId, archiverPruningJob.RetentionAction, archiverPruningJob.IsSoftDelete, archiverPruningJob.SiteUrl));
        ...
        private async Task<RARetentionJobTelemetry> BuildRARetentionJobTelemetry(ArchiverPruningJob archiverPruningJob)
        {
            ...
            try
            {
                ...
                telemetry.JobType = ((int)JobType.EXOArchiverRetention).ToString();
                ...
                EXOArchiverIndexSubInfo indexSubInfo = await ArchiverIndexSubInfoDao.GetSubInfoBySubsubJobIdAsync(archiverPruningJob?.JobId);
        ...
        private async Task EndAndSendRetentionJobTelelmetry(RARetentionJobTelemetry telemetry)
        {
            try
            {
                EXOArchiverIndexSubInfo indexSubInfo = await ArchiverIndexSubInfoDao.GetSubInfoBySubsubJobIdAsync(telemetry.ArchivedSubJobId);
        ...
        public void RemoveSuccessDeletedOrphanDatasSubjob()
        {
            try
            {
                foreach (string subsubJobid in successDeleteSubsubJobIds)
                {
                    ...
                    {
                        mLog.Warn($"exo there exsit something wrong when delete orphan datas job,so donot delete this subjob record,id:{subsubJobid}");
                    }
                    else
                    {
                        mLog.Info($"exo delete orphan datas job success,delete subjob record,id:{subsubJobid}");
        ...
        private JMArchiverRententionJobDetails GenerateRetentionSkipReport(string subinfojobId,MediaArchiverRetentionAction action,bool isSoft,string siteUrl)
        {
            try
            {
            ...
            if (result != null && result.State == 2)
            {
                if (archiverRetentionInfo.RetentionDataTimeType != KeepDateType.ModifiedTime)
                {
                    if (archiverRetentionInfo.RetainType == RetainType.DeleteOrphanDatas)
                    {
                        ...
                        mLog.Info("exo this delete action is delete orphan job.");
                        successDeleteSubsubJobIds.Add(archiverRetentionInfo.JobId);
                    }
                    else
                    {
        ...
        {
            mLog.Info($"MarkSubJobDataTier : Teams Address:{archiverRetentionInfo.SiteUrl}");
            ArchiverRetentionResult result = null;
        ...
        private ArchiverRetentionResult RealDeleteSubJobData(ArchiverPruningJob archiverRetentionInfo, string backupSubJobId)
        {
            ...
            var retentionInfo = new ExchangeArchiverRetentionInfo(archiverRetentionInfo);
            if (archiverRetentionInfo.RetainType != RetainType.DeleteOrphanDatas)
            {
            ...
            var retentionService = (IRetentionService)PlatformWindsorManager.GetService("AvePoint.Media.Service.ArchiverBackup.ExchangeArchiverBackupRetentionService", typeof(IRetentionService));
