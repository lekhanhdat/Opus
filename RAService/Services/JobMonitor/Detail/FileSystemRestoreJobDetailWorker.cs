...
{
    public class FileSystemRestoreJobDetailWorker : AbstractJobDetailWorker
    {
        ...
        public override IEnumerable<JMJobDetails> GetData(int PageSize, int StartPage, string conditionFilter, BaseJobDto jobInfo)
        {
            ...
            foreach (JMFSRestoreJobDetails jobDetail in result.Cast<JMFSRestoreJobDetails>())
            {
                jobDetail.Size = JobDetailHelper.GetDataSizeToViewForFS(jobDetail.Size.ToInt64());
            }
            return result;
        }
