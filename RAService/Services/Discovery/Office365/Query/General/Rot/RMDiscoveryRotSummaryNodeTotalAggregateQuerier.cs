...
{
    public class RMDiscoveryRotSummaryNodeTotalAggregateQuerier : RMDiscoveryRotDataQuerier<Dictionary<string, object>>
    {
        public RMDiscoveryRotSummaryNodeTotalAggregateQuerier(RMDiscoveryOffice365QueryParameter queryParameter) : base(queryParameter)
        {
        }

        public override async Task<Dictionary<string, object>> QueryAsync()
        {
            var sql = $@"SELECT SUM(FileTotalSize) AS rotFileTotalSize FROM [{_schemaName}].[RMBasicRotData] AS data";
