...
{
    public class OnPremiseSPScanLocalNodeJobDetialWorker : AbstractDetailWorker
    {
        public override List<List<ReportCell>> ConvertReportData(IEnumerable<JMJobDetails> details)
        {
            ...
            {
                if (detail is JMScanLocalNodesJobDetails)
                {
                    List<ReportCell> reports = new List<ReportCell>();
                    var reportnfo = detail as JMScanLocalNodesJobDetails;
                    ...
                    reports.Add(new ReportCell() { Key = "ItemType", Value = reportnfo.ItemType });
        ...
        public override void InitCreateTableSQLString()
        {
            ...
            CREATE_TABLE_SQL = string.Format(JobMonitorConstants.CREATE_TABLE_OnPremiseSP_ScanLocalNode, TABLE_NAME);
            INSERT_DATA_SQL = string.Format(JobMonitorConstants.INSERT_DATA_OnPremiseSP_ScanLocalNode, TABLE_NAME);
        }
    }
}
