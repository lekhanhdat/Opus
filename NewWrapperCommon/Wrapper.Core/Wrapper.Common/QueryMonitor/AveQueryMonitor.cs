...
namespace AvePoint.Wrapper.Common
{
    ...
    internal class AveQueryMonitor
    {
        private static AveLogger mLog = AveLogger.GetInstance(typeof(AveQueryMonitor));
        ...
        {
            defaultTimeOut = AveQueryWorker.DEFAULT_TIMEOUT;
        ...
        public static void RegisterConnection(AveQueryWorker aveSqlconn)
        {
            try
            {
                lock (locker)
            ...
            {
                mLog.Debug("An error occurred when Register Query Monitor; Message: {0}", ex.ToString());
            }
        }
        ...
        private static void CheckConnectionTimeOut()
        {
            ...
            {
                mLog.Log(AveLogLevel.WARN, WrapperQueryServiceResource.TimeOutError, e);
                //make sure this thread won't corrupt whole process
                //log.Error("timeout checker thread is down", e);
            }
        }
