...
public class RecordManager
{
    ...
    private GoogleDriveTreeNodeDto _currentTopNode;

    private Dictionary<Guid, Record> RecordsCache = new();

    private List<Record> NewRecordsCache = [];
    ...
    public RecordManager Init(ReportCenter reportCenter, SourceFlag sourceFlag)
    {
    ...
    public bool IsLoadedCache(GoogleDriveTreeNodeDto currentTopNode, bool useCache = false)
    {
        if (!useCache)
        {
    ...
    public void LoadCache()
    {
        ...
        do
        {
            var result = _explorerDao.QueryByPage(item =>
                item.SourceFlag == _sourceFlagValue && item.ContainerId == _currentTopNode.ContainerId
                                                    && item.CollectTime < _jobStartTime &&
                item.RecordStatus == (int)RMRecordStatus.Active, limit, continuation, false);
    ...
    public void LoadRuleActionCache()
    {
        ...
        do
        {
            var result = _explorerDao.QueryByPage(item =>
                ...
                (IsGoogleContainer(item.NodeType) ||
                 (IsGoogleDrive(item.NodeType) && item.RuleId != Guid.Empty)), limit, continuation, false);
            continuation = result.Item2;
            foreach (var item in result.Item1)
            {
                RecordsCache.Add(item.Id, item);
    ...
    public bool TryGetRecordValue(Guid id, int createDate, out Record record)
    {
        ...
        record = Retry(() => _explorerDao.GetFirstOrDefault(item =>
            item.Id == id && item.SourceFlag == _sourceFlagValue /*&& item.ContainerId == _currentTopNode.ContainerId */ &&
            /*item.CreateDate == createDate &&*/ item.CollectTime < _jobStartTime));
        return record != null;
    }
    ...
    /*public bool TryGetRecordByAncestor(Guid id, int createDate, out Record record)
    {
        ...
        record = Retry(() => _explorerDao.GetFirstOrDefault(item =>
            ...
            (_currentTopNode.Level == RMNodeLevel.Go || item.Ancestors.Contains(new Guid(_currentTopNode.ID))) &&
            item.CollectTime < _jobStartTime &&
            item.RecordStatus == (int)RMRecordStatus.Active));
        return record != null;
    }
    */
    ...
    public bool TryGetRuleActionRecordValue(Guid id, int createDate, out Record record)
    {
        ...
        record = Retry(() => _explorerDao.GetFirstOrDefault(item =>
            ...
            item.CreateDate == createDate && item.CollectTime < _jobStartTime &&
            item.RecordStatus == (int)RMRecordStatus.Active &&
            item.RecordStatus == (int)RMRecordStatus.Active && (IsGoogleContainer(item.NodeType) ||
                                                                (IsGoogleDrive(item.NodeType) &&
                                                                 item.RuleId != Guid.Empty))));
        return record != null;
    }
    ...
    public List<Record> GetRuleActionRecordsByParent(Guid parentId)
    {
        ...
        return Retry(() => _explorerDao.QueryAll(item =>
                item.ParentId == parentId && item.SourceFlag == _sourceFlagValue &&
                item.ContainerId == _currentTopNode.ContainerId &&
                item.CollectTime < _jobStartTime && item.RecordStatus == (int)RMRecordStatus.Active &&
                (IsGoogleContainer(item.NodeType) ||
                 (IsGoogleDrive(item.NodeType) && item.RuleId != Guid.Empty))))
            .ToList();
    }
    ...
    public List<Record> GetRecordsByParentId(Guid parentId)
    {
        ...
        return Retry(() => _explorerDao.QueryAll(record =>
                record.ParentId == parentId && record.SourceFlag == _sourceFlagValue &&
                record.ContainerId == _currentTopNode.ContainerId && record.CollectTime < _jobStartTime &&
                record.RecordStatus == (int)RMRecordStatus.Active))
            .ToList();
    }

    public List<Record> GetRecordsByTermId(Guid termId)
    {
        if (_useCache)
        {
            return RecordsCache.Values.Where(record => record.TermId == termId).ToList();
        }

        return Retry(() => _explorerDao.QueryAll(record =>
                record.TermId == termId &&
                record.CollectTime < _jobStartTime && record.RecordStatus == (int)RMRecordStatus.Active))
                .ToList();
    }

    public List<Record> GetRecordsByTermIds(IEnumerable<Guid> termId)
    {
        if (_useCache)
        {
            return RecordsCache.Values.Where(record => termId.Contains(record.TermId)).ToList();
        }

        return Retry(() => _explorerDao.QueryAll(record =>
                termId.Contains(record.TermId) &&
                record.CollectTime < _jobStartTime && record.RecordStatus == (int)RMRecordStatus.Active))
            .ToList();
    }

    public void UpdateRecordInfo(Guid recordId, Action<Record> operation)
    {
        Retry(() => _explorerDao.UpdateAll(r => r.Id == recordId, operation));
    }

    public List<Guid> UpdateTermInfoInRecord(List<Record> updateRecords)
    {
        return _explorerDao.BatchUpdate(updateRecords, 1000);
    }

    public IEnumerable<List<Record>> GetRecordsByLabelIds(Dictionary<Guid, long> changedTerms, string scopeId)
    {
        var limit = 1000;
        //if (_useCache)
        //{
        //    var records = RecordsCache.Values
        //        .Where(record => changedTerms.Keys.Contains(record.TermId) &&
        //                         record.ManualCollectionTime < changedTerms[record.TermId] &&
        //                         record.ScopeId.ToString().Equals(scopeId))
        //        .ToList();
        ...
        //else
        {
            ...
            do
            {
                var result = Retry(() => _explorerDao.QueryByPage(item =>
                        ...
                        item.CollectTime < _jobStartTime &&
                        item.RecordStatus == (int)RMRecordStatus.Active &&
                        item.ScopeId.ToString().Equals(scopeId) &&
                        changedTerms.Keys.Contains(item.TermId)
                    , limit, continuation, false));
                continuation = result.Item2;
                yield return result.Item1.Where(item =>
                    item.ManualCollectionTime < changedTerms[item.TermId] &&
                    item.CollectTime < changedTerms[item.TermId]).ToList();
            } while (!string.IsNullOrEmpty(continuation));
        }
    }
    ...
    public Tuple<IEnumerable<Record>, string> QueryFileRecordsByParent(Guid folderId, string pageIndex,
        RMRecordStatus recordStatus)
    {
        return Retry(() => _explorerDao.QueryByPage(o =>
            o.SourceFlag == _sourceFlagValue
            && o.RecordStatus != (int)recordStatus
            && o.ParentId == folderId
            && o.NodeType == (int)RMNodeLevel.GoogleFile, LIMIT, pageIndex));
    }

    public Tuple<IEnumerable<Record>, string> QueryFolderRecordsByParent(Guid parentId, string pageIndex)
    {
        return Retry(() => _explorerDao.QueryByPage(o =>
            o.SourceFlag == _sourceFlagValue
            && o.ParentId == parentId
            && o.NodeType == (int)RMNodeLevel.GoogleFolder, LIMIT, pageIndex));
    }

    public Tuple<IEnumerable<Record>, string> QueryRecordsByScopeId(Guid scopeId, string pageIndex, RMRecordStatus recordStatus)
    {
        return Retry(() => _explorerDao.QueryByPage(o =>
            o.SourceFlag == _sourceFlagValue
            && o.RecordStatus == (int)recordStatus
            && o.ScopeId == scopeId
            && (o.NodeType == (int)RMNodeLevel.GoogleFolder || o.NodeType == (int)RMNodeLevel.GoogleFile), LIMIT, pageIndex));
    }

    public Record? QueryExistRecordByScopeId(Guid scopeId, Guid recordId)
    {
        if (scopeId == Guid.Empty || recordId == Guid.Empty)
        {
            _logger.Warn($"Invalid input parameters: scopeId={scopeId}, recordId={recordId}");
            return null;
        }

        return Retry(() =>
            _explorerDao.GetFirstOrDefault(o =>
                o.SourceFlag == _sourceFlagValue &&
                o.ScopeId == scopeId &&
                o.Id == recordId &&
    ...
    public Record? QueryHoldRecordById(Guid id)
    {
        return Retry(() => _explorerDao.GetHoldRecordsByIds([id]).FirstOrDefault());
    }

    public Record GetGoogleRecordById(Guid id)
    {
        return Retry(() => _explorerDao.GetGoogleDriveRecordById(id));
    }

    public void AddNewRecord(Record record)
    {
        _explorerDao.Upsert(record);
        _reportCenter.RecordSuccessful(record.GenerateSyncActionDetail(), record.NodeType);
    }
    ...
    public void Add(Record item)
    {
        if (!_isEnableBulkOperation)
        {
            ...
            _reportCenter.RecordSuccessful(item.GenerateSyncActionDetail(), item.NodeType);
            return;
        }
        ...
        lock (_lockObject)
        {
            ...
            {
                _logger.Info(
                    $"NewRecordsCache Count [{NewRecordsCache.Count}], recordCount [{recordCount}] when Add. Start Commit");
                Commit();
            }
    ...
    public void UpdateRecordStatusAndDestroyedTime(Record item, Rule rule, int updateStatus)
    {
        ...
        {
            Retry(() => _explorerDao.UpdateAll(
                s => s.ContainerId == item.ContainerId && s.CreateDate == item.CreateDate && s.Id == item.Id, r =>
                {
                    r.RecordStatus = updateStatus;
    ...
    public Record CreateRecordByCurrentNode(GoogleDriveTreeNodeDto currentNode, File movedFile, Record record, string currentFullDirPath, string tenantId, string memberEmail)
    {
        GoogleDriveData driveData = ConvertHelper.ConvertDtoNodeTreeToData(currentNode, tenantId);
        var currentGoogleItemData = movedFile.ConvertToDto(driveData, string.Empty, string.Empty, memberEmail);
        currentGoogleItemData.RelativePath = currentFullDirPath;
        InitGoogleDataItemMetadata(currentGoogleItemData, record, driveData);

        currentNode.ContainerId = currentNode.ContainerId ?? currentNode.ParentId;
        return currentGoogleItemData.ConvertToRecord(currentNode, record);
    }
    ...
    public void UpdateRecordInfo(Record existRecord, GoogleItemData item)
    {
    ...
    public void UpdateManualProperties(Record record, bool isChangedRule = false)
    {
        ...
        {
            Retry(() => _explorerDao.UpdateAll(
                s => s.ContainerId == record.ContainerId && s.CreateDate == record.CreateDate && s.Id == record.Id, r =>
                {
                    ...
                    r.GControlApprovalProcessId = record.GControlApprovalProcessId;
                    r.GControlCurrentStageId = record.GControlCurrentStageId;
                    r.GControlPlatformTaskId = record.GControlPlatformTaskId;
                    r.GControlManualApprovedStatus = record.GControlManualApprovedStatus;
                    r.IsGControlRecord = record.IsGControlRecord;
                    r.GControlCurrentApproverId = record.GControlCurrentApproverId;
                    r.GControlManualReviewers = record.GControlManualReviewers;
                    r.GControlManualInternalApprovedStatus = record.GControlManualInternalApprovedStatus;
                    r.RuleId = record.RuleId;
                    r.TermId = record.TermId;
                    r.TermName = record.TermName;
                    ...
                    r.RecordStatus = record.RecordStatus;
                    r.ManualDisposalDueDate = record.ManualDisposalDueDate;
                    r.WebViewLink = record.WebViewLink;
                }));
            return;
        }
    ...
    public void Commit()
    {
        ...
        lock (_lockObject)
        {
            ...
            _logger.Info(
                $"new added records count is {newCount} when Commit.{(newCount == 0 ? "" : " Start generating UniqueId and clear cache")}");
            if (newCount != 0)
            {
    ...
    public void WaitComplete()
    {
        ...
        try
        {
            lock (_lockObject)
            {
                var newCount = NewRecordsCache.Count;
                _logger.Info(
                    $"new added records count is {newCount} when WaitComplete.{(newCount == 0 ? "" : " Start generating UniqueId and clear cache")}");
                if (newCount != 0)
                {
                    var recordUniqueIds = new UniqueIdUtil(TenantLocalValue.LogonGroupId, newCount);
                    foreach (var newRecord in NewRecordsCache)
    ...
    {
        //to do: change to google data sync
        if (_reportCenter.JobType == JobType.GoogleRecordsDisposal)
        {
    ...
    private void FailedProcessRecord(Record item, Exception e)
    {
        ...
        _reportCenter.RecordFailed(item.GenerateSyncActionDetail(e.Message), item.NodeType, item.GenerateFailureItemEntity(_reportCenter.JobId));
    }
    ...
    public void DeleteRecordsByScopeId(Guid scopeId)
    {
        if (scopeId == Guid.Empty)
        {
            _logger.Warn($"Invalid scopeId: {scopeId}");
            return;
        }

        try
        {
            var recordsToDelete = Retry(() => _explorerDao.QueryAll(o =>
                o.SourceFlag == _sourceFlagValue &&
                o.ScopeId == scopeId &&
                o.RecordStatus == (int)RMRecordStatus.Active));
            ...
            {
                _logger.Info($"Found {recordsToDelete.Count()} records for ScopeId {scopeId}. Deleting...");
                foreach (var record in recordsToDelete)
                {
                    _explorerDao.Delete(record.CreateDate, record.Id);
                }
                _logger.Info($"Successfully deleted {recordsToDelete.Count()} records for ScopeId {scopeId}.");
            }
            else
            {
                _logger.Info($"No records found for ScopeId {scopeId}. Nothing to delete.");
            }
        }
        catch (Exception ex)
        {
            _logger.Error($"Error occurred while deleting records for ScopeId {scopeId}. Exception: {ex}");
        }
    }

    private void InitGoogleDataItemMetadata(GoogleItemData currentGoogleItemData, Record record, GoogleDriveData driveData)
    {
        currentGoogleItemData.MetaInfo = JsonConvert.DeserializeObject<GoogleItemMetaInfo>(record.MetaInfo)!;
        currentGoogleItemData.MetaInfo.DriveId = driveData.Id;
        currentGoogleItemData.MetaInfo.DriveName = driveData.Name;
    }
