...
namespace AvePoint.Media.Storage.Cloud.Amazon
{
    ...
    class AmazonUtils : CommonUtility
    {
        private static StorageLogger logger = new StorageLogger(typeof(AmazonUtils));
         ...
         */
        public static void AddAuthorization(HttpWebRequest request, string awsAccessKeyId, string awsSecretAccessKey, int signatureVersion, string region)
        {
            if (signatureVersion == 2)
            {
                logger.Debug("signature version is 2 ");
                AddAuthorization(request, awsAccessKeyId, awsSecretAccessKey);
            }
            else if (signatureVersion == 4)
            {
                logger.Debug("signature version is 4 ");
                AWS4Signature.Sign(request, awsAccessKeyId, awsSecretAccessKey, GetAmazonRegion(region));
            }
        }
        ...
        public static string GetCanonicalizedResource(HttpWebRequest request)
        {
            ...
            if (host.Contains("." + StorageUrl.AmazonHostName))
            {
                string bucketName = host.Substring(0, host.IndexOf("." + StorageUrl.AmazonHostName, StringComparison.OrdinalIgnoreCase)).ToLower(CultureInfo.InvariantCulture);
                resource = "/" + bucketName + resource;
            }
            else if (host.Contains("." + StorageUrl.AmazonSaopauloHostName))
            {
                string bucketName = host.Substring(0, host.IndexOf("." + StorageUrl.AmazonSaopauloHostName, StringComparison.OrdinalIgnoreCase)).ToLower(CultureInfo.InvariantCulture);
                resource = "/" + bucketName + resource;
            }
            else if (host.Contains("." + StorageUrl.AmazonTokyoHostName))
            {
                string bucketName = host.Substring(0, host.IndexOf("." + StorageUrl.AmazonTokyoHostName, StringComparison.OrdinalIgnoreCase)).ToLower(CultureInfo.InvariantCulture);
                resource = "/" + bucketName + resource;
            }
            else if (host.Contains("." + StorageUrl.AmazonSydneyHostName))
            {
                string bucketName = host.Substring(0, host.IndexOf("." + StorageUrl.AmazonSydneyHostName, StringComparison.OrdinalIgnoreCase)).ToLower(CultureInfo.InvariantCulture);
                resource = "/" + bucketName + resource;
            }
            else if (host.Contains("." + StorageUrl.AmazonSingaporeHostName))
            {
                string bucketName = host.Substring(0, host.IndexOf("." + StorageUrl.AmazonSingaporeHostName, StringComparison.OrdinalIgnoreCase)).ToLower(CultureInfo.InvariantCulture);
                resource = "/" + bucketName + resource;
            }
            else if (host.Contains("." + StorageUrl.AmazonOregonHostName))
            {
                string bucketName = host.Substring(0, host.IndexOf("." + StorageUrl.AmazonOregonHostName, StringComparison.OrdinalIgnoreCase)).ToLower(CultureInfo.InvariantCulture);
                resource = "/" + bucketName + resource;
            }
            else if (host.Contains("." + StorageUrl.AmazonIrelandHostName))
            {
                string bucketName = host.Substring(0, host.IndexOf("." + StorageUrl.AmazonIrelandHostName, StringComparison.OrdinalIgnoreCase)).ToLower(CultureInfo.InvariantCulture);
                resource = "/" + bucketName + resource;
            }
            else if (host.Contains("." + StorageUrl.AmazonCaliforniaHostName))
            {
                string bucketName = host.Substring(0, host.IndexOf("." + StorageUrl.AmazonCaliforniaHostName, StringComparison.OrdinalIgnoreCase)).ToLower(CultureInfo.InvariantCulture);
                resource = "/" + bucketName + resource;
            }
        ...
        public static string GetCanonicalizedAmzHeaders(HttpWebRequest request)
        {
            ...
            foreach (string hd in headers)
            {
                ...
                if (name.StartsWith(AmazonConstants.AWS3_REST_HEADER_PREFIX, StringComparison.OrdinalIgnoreCase))
                {
                    //去掉前后空格、多重空格、制表符、换行符
        ...
        public static string GetAmazonRegion(string regionType)
        {
            string regionStr = "";
            switch (regionType)
            {
                case AmazonConstants.US:
                    regionStr = AmazonConstants.REGION_US_EAST;
                    break;
            ...
            if (regionStr.Equals("s3.amazonaws.com", StringComparison.OrdinalIgnoreCase))
            {
                regionStr = "us-east-1";
            }
            if (regionStr.StartsWith("s3-", StringComparison.OrdinalIgnoreCase))
            {
                regionStr = regionStr.Substring(3, regionStr.Length - 3);
            }
            if (regionStr.EndsWith(".amazonaws.com", StringComparison.OrdinalIgnoreCase))
            {
                regionStr = regionStr.Substring(0, regionStr.Length - ".amazonaws.com".Length);
            }
            return regionStr.ToLower(CultureInfo.InvariantCulture);
        }
    }
}
