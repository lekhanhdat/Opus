...
namespace AvePoint.Wrapper.Common
{
    ...
    {
        private static IAveLogger logger = AveLogger.GetInstance(typeof(CabinetCreator));
        ...
        private int CabDeleteFile(string path, out int err, IntPtr pv)
        {
            ...
            {
                logger.Error($"error occured when CabDeleteFile,error:{e}");
            }
            err = 0;
            return 1;
        }
        ...
        public void Create(ICabinetCreateStreamContext streamContext, IList<string>[] foldersAndFiles, long maxCabSize, long maxFolderSize)
        {
            ...
            lock (this.innerLock)
            {
                try
                {
                    ...
                    pccab.setID = (short) SecurityUtils.GetRandomNumber(-32768, 0x8000);
