...
namespace AvePoint.Wrapper.Mapping
{
    ...
    public class AveRestoredFieldMapping : IAveRestoredFieldMapping
    {
        ...
        public void AddSkippedFields(string internalName)
        {
            lock (skippedFields)
            {
                if (!skippedFields.Contains(internalName, StringComparer.OrdinalIgnoreCase))
                {
                    skippedFields.Add(internalName);
                }
            }
        }

        public IEnumerable<string> EnumSkippedFields()
        {
            foreach (var value in skippedFields)
            {
                yield return value;
            }
        }
        ...
        #region FailedFields
        public void AddFailedFields(string internalName)
        {
            lock (failedFields)
            {
                if (!failedFields.Contains(internalName, StringComparer.OrdinalIgnoreCase))
                {
                    failedFields.Add(internalName);
                }
            }
        }

        public IEnumerable<string> EnumFailedFields()
        {
            foreach (var value in failedFields)
            {
                yield return value;
            }
        }
