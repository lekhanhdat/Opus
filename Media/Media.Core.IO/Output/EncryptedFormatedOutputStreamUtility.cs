...
namespace AvePoint.Media.Core.IO.Output
{
    ...
    #endregion
    public static class EncryptedFormatedOutputStreamUtility
    {
        public static IDataFilteringBox EncryptedFormatedOutputStreamInit(DataEncryptionInfo encryptionInfo)
        {
            DataEncryptionInfoWrapper wrapper = DataEncryptionInfoManager.ResolveDynamicKey(encryptionInfo);
            return DataFilteringBoxFactory.GetEncryptionFilteringBox((EncryptionAlgorithm)wrapper.EncryptionInfo.EncryptionType, wrapper.DynamicKey);
        }
    }
}
