...
{
    public class AveSPSearch:IDisposable
    {
        ...
        public void Restore(AveSearchInfo aveSearchInfo)
        {
            try
            {
                if (mSearchLevel == SearchLevel.site)
                {
                    ...
                    {
#endif
                        AveSPSearchScope searchScope = new AveSPSearchScope(mAveSite, SearchServiceAppProxy);
            ...
            catch (AveSecurityTrimingException ex)
            {
                if (mSearchLevel == SearchLevel.web)
                {
                    ...
                    report.AddDetail(new AveWrapperReportDto("WebSearch", "WebSearch", AveReportObjectType.WebSearch, AveStatus.Skipped, "You don't have permission to restore WebSearch" + ex.Message));
                }
                if (mSearchLevel == SearchLevel.site)
                {
                    mLog.Warn("An error occurred while restore Site AveSPSearch. error:{0}", ex.ToString());
                    report.AddDetail(new AveWrapperReportDto("SiteSearch", "SiteSearch", AveReportObjectType.SiteSearch, AveStatus.Skipped, "You don't have permission to restore SiteSearch" + ex.Message));
                }
            }
            catch (Exception e)
            {
    ...
    public class AveSPSearchScope:IDisposable
    {
        private AveSPSite mAveSite = null;
        private AveSPWeb mAveWeb = null;
        private SearchLevel mSearchLevel;
        ...
        public AveSPSearchScope(AveSPSite aveSite, IAveOSearchServiceApplicationProxy searchServiceAppProxy)
        {
            ...
            else
            {
                ...
                {
                    mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url,mAveSite.SPSite.Url,AveReportObjectType.SiteSearchScope,AveStatus.Failed,string.Format("An error occurred while get SearchServiceApplicationProxy. siteUrl:{0}\n error message:{1}", mAveSite.SPSite.Url, e.Message)));
        ...
        public AveSPSearchScope(AveSPWeb aveWeb, IAveOSearchServiceApplicationProxy searchServiceAppProxy)
        {
            ...
            else
            {
                ...
                {
					 mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url,mAveSite.SPSite.Url,AveReportObjectType.WebSearchScope,AveStatus.Failed,string.Format("An error occurred while get SearchServiceApplicationProxy. siteUrl:{0}\n error message:{1}", mAveSite.SPSite.Url, e.Message)));
                    log.Log(AveLogLevel.WARN, string.Format("An error occurred while get SearchServiceApplicationProxy. siteUrl:{0}\n error message:{1}", mAveSite.SPSite.Url, e));
                    //mLog.Warn(e, "An error ocurred when get SearchServiceApplicationProxy.siteUrl:{0}", mAveSite.SPSite.Url);
                }
        ...
        private void RestoreScope(List<AveScopeInfo> aveScopeInfos)
        {
            ...
            foreach (AveScopeInfo scopeInfo in aveScopeInfos)
            {
                ...
                catch(Exception ex)
                {
                    ...
                    {
                        mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url,mAveSite.SPSite.Url,AveReportObjectType.SiteSearchScope,AveStatus.Failed,string.Format("An error occurred while create a new scope. consumerName:{0}\n error message:{1}", consumerName, e.Message)));
                ...
                if (scopeId > 0)
                {
                    ...
                    foreach (AveRuleInfo ruleInfo in scopeInfo.AveRuleInfos)
                    {
                        ...
                        if (!isExist)
                        {
                            ...
                            {
                                mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url,mAveSite.SPSite.Url,AveReportObjectType.SiteSearchScope,AveStatus.Failed,string.Format("An error occurred while create a new scope rule. consumerName:{0}\n error message:{1}", consumerName, e.Message)));
        ...
        private void RestoreDisplayGroup(List<AveDisplayGroupInfo> aveDisplayGroupInfo)
        {
            ...
            foreach (AveDisplayGroupInfo groupInfo in aveDisplayGroupInfo)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url,mAveSite.SPSite.Url,AveReportObjectType.SiteSearchScope,AveStatus.Failed,string.Format("An error occurred while create a new DisplayGroup. consumerName:{0}\n error message:{1}", consumerName, e.Message)));
                ...
                if (displayGroupId > 0)
                {
                    ...
                    foreach (AveDisplayGroupMember member in groupInfo.AveDisplayGroupMembers)
                    {
                        ...
                        {
                            mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url,mAveSite.SPSite.Url,AveReportObjectType.SiteSearchScope,AveStatus.Skipped,string.Format("Cannot get scope. scope name:{0}\n error message:{1}", member.Name, e.Message)));
                    ...
                    {
                        mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url, mAveSite.SPSite.Url, AveReportObjectType.SiteSearchScope, AveStatus.Failed, string.Format("An error occurred while set displayGroupListInfo. consumerName:{0}, displayGroup:{1}\n error message:{2}", consumerName, displayGroupId, e.Message)));
    ...
    public class AveSPSearchKeywords:IDisposable
    {
        private AveSPSite mAveSite = null;
        private IAveOSearchServiceApplicationProxy SearchServiceAppProxy = null;
        private static AveLogger mLog = AveLogger.GetInstance(MethodBase.GetCurrentMethod().DeclaringType);
        ...
        public AveSPSearchKeywords(AveSPSite aveSite, IAveOSearchServiceApplicationProxy searchServiceAppProxy)
        {
            ...
            {
                mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url, mAveSite.SPSite.Url, AveReportObjectType.SiteSearchKeyWords, AveStatus.Failed, string.Format("An error occurred while get SearchServiceApplicationProxy. site url:{0}\n error message:{1}", mAveSite.SPSite.Url, e.Message)));
        ...
        public void Retore(List<AveKeyword> aveKeywords)
        {
            ...
            foreach (AveKeyword keyword in aveKeywords)
            {
                try
                {
                    ...
                    {
                        mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url, mAveSite.SPSite.Url, AveReportObjectType.SiteSearchKeyWords, AveStatus.Failed, string.Format("An error occurred while get or create keyword. keyword team:{0}\n error message:{1}", keyword.Term, e.Message)));
                    ...
                    if (word.BestBets != null)
                    {
                        foreach (AveBestBet aveBet in keyword.BestBets)
                        {
                            ...
                            {
                                mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url, mAveSite.SPSite.Url, AveReportObjectType.SiteSearchKeyWords, AveStatus.Failed, string.Format("An error occurred while restore a bestBets. bestBets title:{0}\n error message:{1}", aveBet.Title, e.Message)));
                    ...
                    if (word.Synonyms != null)
                    {
                        foreach (AveSynonym aveSyn in keyword.Synonyms)
                        {
                            ...
                            {
                                mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url, mAveSite.SPSite.Url, AveReportObjectType.SiteSearchKeyWords, AveStatus.Failed, string.Format("An error occurred while restore a synonym. Synonym term:{0}\n error message:{1}", aveSyn.Term, e.Message)));
                ...
                {
                    mReport.AddDetail(new AveWrapperReportDto(mAveSite.SPSite.Url, mAveSite.SPSite.Url, AveReportObjectType.SiteSearchKeyWords, AveStatus.Failed, string.Format("An error occurred while restore keyword. keyword definition:{0}, keyword contact:{1}\n error message:{2}", keyword.Definition, keyword.Contact, e.Message)));
