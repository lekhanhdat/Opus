...
namespace AvePoint.Media.Core.Index
{
          ...
          where TIndexProcessorParameter : IndexProcessorParameter
    {
        ...
        IIndexDatabaseScriptGenerator scriptGenerator = MediaServiceLocator.Discover<IIndexDatabaseScriptGenerator>();
        IIndexDatabaseUpgradeManager upgradeManager = MediaServiceLocator.Discover<IIndexDatabaseUpgradeManager>();
        ...
        public void Close(Boolean isCheckIntegrity = false)
        {
            try
            {
                logger.Info("Begin to close IndexProcessor.isCheckIntegrity: {0},needCheckIntegrity:{1},currentIndexFullPath:{2}", isCheckIntegrity, needCheckIntegrity, currentIndexFullPath);
                if (!string.IsNullOrEmpty(this.currentIndexFullPath) && isCheckIntegrity && this.needCheckIntegrity)
                {
                    ...
                    {
                        throw new IndexNotIntegratedException(string.Format("The SQLite .db file is not integrated. Details: {0}", checkResult));
                    }
                }
            }
            finally
        ...
        {
            this.dbHelper.ChangePassword(newPassword);
        }

        public string GetCurrentIndexFullPath()
        {
            return this.currentIndexFullPath;
        }

        public String CheckIntegrity()
        {
            logger.Info("Begin to Check Index DB Integrity:{0}", currentIndexFullPath);
            ...
            logger.Info("Index DB Check method:{0}", commandText);
            checkResult = (string)this.dbHelper.ExecuteScalar(commandText);
            logger.Info("Check action time cost {0}.CheckMethod:{1}.The check result for the SQLite .db file is {2}.", DateTime.Now - start, commandText, checkResult);
            return checkResult;
        }
            ...
            Boolean isForceUpgrade)
        {
                ...
                && databaseFullPath.EqualsIgnoreCase(this.currentIndexFullPath)))
            {
                ...
                {
                    this.logger.Info(MediaCoreIndexResource.IndexProcessorInnerOpenUseDBCurrentCatalog, this.currentIndexFullPath);
                    this.dbHelper.Close();
                }
                ...
                if (openMode == IndexDatabaseOpenMode.Create)
                {
                    try
                    {
                        ...
                        if (finfo.Exists)
                        {
                            ...
                            this.logger.Warn("The index file exists {0} Create time{1}", this.currentIndexFullPath, finfo.CreationTimeUtc.ToString());

                            foreach (var file in finfo.Directory.GetFiles())
                            {
                                this.logger.Info(file.FullName);
                            }
                        }
                    }
                    catch (Exception e)
                ...
                && databaseFullPath.EqualsIgnoreCase(this.currentIndexFullPath)))
            {
                ...
                if (openMode == IndexDatabaseOpenMode.Create)
                {
                    try
                    {
                        ...
                        if (finfo.Exists)
                        {
                            this.logger.Info("The index file exists {0}", this.currentIndexFullPath);

                            foreach (var file in finfo.Directory.GetFiles())
                            {
                                this.logger.Info($"{file.FullName} | {file.Length}");
                            }
                        }
                    }
                    catch (Exception e)
