...
{    
    ...
    public class FileTransferService:IFileTransferService,IDisposable
    {
        ...
        public bool CheckStatus(string fileName, string fileDir, Boolean overwriteFile)
        {
            if (string.IsNullOrEmpty(fileDir))
            {
                fileDir = DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.FileTransferServiceTempFolder;//如果Client不赋值，则使用默认的Service Temp目录
                if (string.IsNullOrEmpty(fileDir))
                {
        ...
        public void Dispose()
        {
            if (mFile != null)
            {
                mFile.Flush();
                mFile.Close();
                if (!overwriteFile && File.Exists(mFilePath) && DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.DeleteTransferFileInService)
                {
