...
namespace AvePoint.ObjectModel.Common
{
    class AveNavigationNodeCollection : AveAbstractCommonCollection<IAveNavigationNode>, IAveNavigationNodeCollection
    {
        ...
        {
            var navNodePropertiesList = base.DataCache.GetChildren();
            mListData = new List<IAveNavigationNode>(navNodePropertiesList.Count);
            foreach (var navNodeProperties in navNodePropertiesList)
            {
        ...
        public void Delete(IAveNavigationNode navNode)
        {
            ...
            prop["Id" + AveObjectModelConstant.ObjectPropertySuffix] = (navNode as AveNavigationNode).DataCache.GetPropertyWithoutChange("Id" + AveObjectModelConstant.ObjectPropertySuffix);
            if ((navNode as AveNavigationNode).DataCache.IsPropertyAvailable("ClientContext"))
            {
                prop["ClientContext"] = (navNode as AveNavigationNode).DataCache.GetPropertyWithoutChange("ClientContext");
            }
            mRequest.DeleteNavigationNode(this.mWeb.ServerRelativeUrl, mParent.DataCache.GetPropertyCache(), prop);
