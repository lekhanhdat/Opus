...
namespace AvePoint.RA.Service.Services.Discovery.Office365.Work.Analyzer.V5.General
{
    public class RMDiscoveryOffice365AggregateTotalDataAnalyzer
    {
        ...
        public async Task<bool> RefreshAndSaveAsync()
        {
            ...
            {
                _logger.Error($"An error occurred while refresh tenant [{_o365TenantId}] [{_contentSource}] aggregate total data. Error: {e}");
                return false;
            }
        }
        ...
        {
            return await SaveDeltaAsync(_data);
        }

        public async Task<bool> SaveDeltaAsync(RMDiscoveryOffice365AggregateTotalData delta)
        {
            try
            {
                if (delta == null)
                {
                    _logger.Error($"Cannot save aggregate total data delta for tenant [{_o365TenantId}] [{_contentSource}] because delta is null.");
                    return false;
                }

                var data = await _dataDao.GetAggregateTotalDataAsync(_o365TenantId, _contentSource);
                data.FileSumCount += delta.FileSumCount;
                data.FileTotalSize += delta.FileTotalSize;
                data.TotalVersionSize += delta.TotalVersionSize;
                data.PHLVolume += delta.PHLVolume;
                data.MaxFileAge = Math.Max(data.MaxFileAge, delta.MaxFileAge);
                data.DuplicateFileTotalSize = -1;

                await _dataDao.AddOrUpdateAggregateTotalDataAsync(_o365TenantId, data);
                _logger.Info($"Succeed save delta tenant [{_o365TenantId}] [{_contentSource}] aggregate total data. Delta FileSumCount [{delta.FileSumCount}], Delta FileTotalSize [{delta.FileTotalSize}].");
                return true;
            }
            catch (Exception e)
            {
                _logger.Error($"An error occurred while save delta tenant [{_o365TenantId}] [{_contentSource}] aggregate total data. Error: {e}");
                return false;
            }
        }
