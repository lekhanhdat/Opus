...
function CheckParameter() {
	...
	Write-Host 'Examples:'
	Write-Host '.\UpdateSitePropertyValue.ps1 -WebUrl "https://contoso.sharepoint.com/sites/targetsite" -PropertyName "SiteType" -PropertyValue "OBR"  -ClientID "Azure AD Application Client ID" -Thumbprint "Azure AD Application Certificate Thumbprint" -TenantID "Microsoft 365 Tenant ID"';
}
...
function Startup() {
	try {
		...
		[Logger]::Info("-----------------------Start-----------------------");
		if (!$WebUrl -or !$PropertyName -or !$PropertyValue -or !$ClientID -or !$TenantID -or !$Thumbprint) {
			CheckParameter;
			return;
		}
		Connect-PnPOnline -Url $WebUrl -ClientId $ClientId -Tenant $TenantID -Thumbprint $Thumbprint;

        #Set-PnPSite -Identity $WebUrl -NoScriptSite $true
        $tenantSite = Get-PnPTenantSite -Identity $WebUrl
        $needResetStatus = $false
        if($tenantSite.DenyAddAndCustomizePages -ne [Microsoft.Online.SharePoint.TenantAdministration.DenyAddAndCustomizePagesStatus]::Disabled) {
            [Logger]::Info("Set Site.DenyAddAndCustomizePages to Disabled status");
            Set-PnPSite -Identity $WebUrl -NoScriptSite $false
            $needResetStatus = $true
        }

        [Logger]::Info("Set Site Property $PropertyName to $PropertyValue");
        Set-PnPPropertyBagValue -Key $PropertyName -Value $PropertyValue

        if($needResetStatus) {
            [Logger]::Info("Set Site.DenyAddAndCustomizePages to Enabled status");
            Set-PnPSite -Identity $WebUrl -NoScriptSite $true
        }
