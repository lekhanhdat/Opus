        ...
        public override bool RealRestore()
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveSPViewWebPart.RealRestore"))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    mReport.AddDetail(new AveWrapperWebpartReportDto(webPartBaseInfo.DisplayName, webPartBaseInfo.DisplayName, webPartBaseInfo, assemblyName, webPartType, AveStatus.Failed, AveReportResource.Wrapper_Report_CannotGetViewByID, AveReportResource.Wrapper_Report_RestoreWebPartError, assemblyName + "|" + webPartType, ex.Message));
                }
                return result;
            }
        }
