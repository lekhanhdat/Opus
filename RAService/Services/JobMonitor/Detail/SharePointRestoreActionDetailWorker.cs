...
{
    public class SharePointRestoreActionDetailWorker : AbstractJobDetailWorker
    {
        ...
        public override IEnumerable<JMJobDetails> GetData(int PageSize, int StartPage, string conditionFilter, BaseJobDto jobInfo)
        {
            ...
            string reportFilePath = DownloadReports(jobInfo);
            TABLE_NAME = JobMonitorConstants.JOBDETAIL;
            InitGetDataSQL(reportFilePath, PageSize, StartPage, conditionFilter);
        ...
        public override void InsertData(IEnumerable<JMJobDetails> jobDetails, BaseJobDto jobInfo)
        {
            ...
            {
                if (details.Any(i => i is JMMigrationRestoreActionJobDetailes))
                {
                    if (details.Any(i => i is not JMMigrationRestoreActionJobDetailes))
                    {
                        logger.Warn($"The job details contain mixed types of restore action details. Jobid {jobInfo.Id}");
                    }
                    InitCreateMigrationTableSQLString();
                }
                else
                {
        ...
        public void InitCreateMigrationTableSQLString()
        {
            TABLE_NAME = JobMonitorConstants.JOBDETAIL;
            CREATE_TABLE_SQL = string.Format(JobMonitorConstants.CREATE_TABLE_SharePoint_Migration_Restore_Report, TABLE_NAME);
            INSERT_DATA_SQL = string.Format(JobMonitorConstants.INSERT_DATA_SharePoint_Migration_Restore_Report, TABLE_NAME);
        }

        private void InitGetDataSQL(string reportFilePath, int PageSize, int StartPage, string conditionFilter)
        {
            // support sort job detail for job that have inconsistent finish time
            //var (ExistColumns, _) = SQLCommond.CheckColumnsExistenceInTable(reportFilePath, TABLE_NAME, ["StartTime"]);
            if (SQLCommond.CheckOneColumnExistenceInTable(reportFilePath, TABLE_NAME, JobMonitorConstants.START_TIME_COLUMN))
            {
                logger.Info($"Column {JobMonitorConstants.START_TIME_COLUMN} exist in table {TABLE_NAME}, use it to sort data.");
                string orderByCondition = JobMonitorConstants.START_TIME_COLUMN;
                if (string.IsNullOrEmpty(conditionFilter))
                {
                    SELECT_DATA_SQL = string.Format(JobMonitorConstants.SELECT_DATA_FROM_TABLE_ORDERBY_CONDITONSTR, TABLE_NAME, orderByCondition, PageSize, (StartPage - 1) * PageSize);
                    SELECT_DETAIL_COUNT_SQL = string.Format(JobMonitorConstants.SELECT_DETAIL_COUNT_SQL, TABLE_NAME);
                }
                else
                {
                    SELECT_DATA_SQL = string.Format(JobMonitorConstants.SELECT_DATA_ON_CONDITION_FROM_TABLE_ORDERBY_CONDITONSTR, TABLE_NAME, conditionFilter, orderByCondition, PageSize, (StartPage - 1) * PageSize);
        ...
        {
            TABLE_NAME = JobMonitorConstants.JOBDETAIL;
            CREATE_TABLE_SQL = string.Format(JobMonitorConstants.CREATE_TABLE_SharePoint_Restore_Report, TABLE_NAME);
            INSERT_DATA_SQL = string.Format(JobMonitorConstants.INSERT_DATA_SharePoint_Restore_Report, TABLE_NAME);
        }
