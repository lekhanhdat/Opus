...
namespace AvePoint.Media.Service.ArchiverBackup
{
        ...
        , IMergeIndexService
    {
        ...
        private static readonly ICommonSiteMasterIndexDao CommonSiteMasterIndexDao = PlatformWindsorManager.GetService<ICommonSiteMasterIndexDao>();
        ...
        static readonly string DeleteCommonSiteMasterIndex = "DELETE FROM " + IndexConstants.TableNameCommonSiteMaster + " WHERE COL_JOB_ID like @COL_JOB_ID";
        ...
        static readonly string SelectTableNameCommonSiteMaster = "SELECT * FROM " + IndexConstants.TableNameCommonSiteMaster;
        ...
        public override void MergeIndex()
        {
            ...
            {
                var openParameter = new ArchiverIndexServiceOpenParameter(item, this.indexLogicalDevice, this.CacheManager.CacheSystem, this.mergeIndexInfo.IndexVolume);
                var indexFiles = this.indexLogicalDevice.ListFiles(new StorageInfo(this.mergeIndexInfo.IndexVolume, string.Empty));
                var subJobIndexes = indexFiles.FindAll(index => index.Name.StartsWith(item.JobId, StringComparison.OrdinalIgnoreCase)
            ...
            if (syncArchivedSiteInfo != null)
            {
                ...
                {
                    UpdateArchivedInfo(mergeIndexInfo.SiteUrl, mergeIndexInfo.SiteId);
                }
                else
                {
        ...
        public override void ProcessException(Exception e)
        {
            ...
            this.logger.Error(MediaServiceArchiverBackupResource.MergeIndexServiceProcessExceptionError, e.ToString());
        }
        ...
        public override void UpdateJobStatusAndControlTable(MergeIndexState mergeIndexState, Int32 jobStatus)
        {
            ...
            this.mergeIndexInfo.MergeIndexJobsState.ForEach(item =>
            {
                try
                {
                    ...
                    {
                        ArchiverManagementService.UpdateMergeIndexStateAsync(item.JobId, siteInfo, mergeIndexState, IdentityManager.IdentityContent).Wait();
                    }
                }
                catch (Exception ex)
                {
        ...
        public override void GenerateJobReport()
        {
            ...
            {
                WriteRelativeDataDetailToTempFile(jobDetailList, subJobInfo);
            }
            else
            {
        ...
        private void WriteRelativeDataDetailToTempFile(List<JobDetail> details, SubJobDto subJobInfo)
        {
        ...
        private void OpenMapIndex(MergeIndexSubJobInfo jobInfo)
        {
            ...
            {
                var openParameter = new ArchiverIndexServiceOpenParameter(item, this.indexLogicalDevice, this.CacheManager.CacheSystem, jobInfo.IndexVolume);
                this.InitIndexProcessor(openParameter);
            });
        }
        ...
        private void DeleteLastUnfinishedIndex(MergeIndexJobState jobInfo)
        {
            ...
            //this.IndexMainProcessor.Execute(DeleteCommonSiteMasterIndex, param);
        }
        ...
        private void InsertIntoMainIndex()
        {
            ...
            //this.InsertIntoCommonSiteMasterIndex();
        ...
        private void UpdateArchivedInfo(string siteUrl,string siteId)
        {
            ...
            var subJobId = mergeIndexInfo?.JobDto?.Id;
            ...
            logger.Info($"UpdateArchivedInfo resolved O365TenantId. SiteId:{siteId}, SiteUrl:{siteUrl}, SubJobId:{subJobId}, subJobTenantId:{subJobTenantId}, O365TenantId:{o365TenantId}");
            ArchiveSiteInfoDao.UpdateArchiverInfo(siteUrl, GetFileCount(), GetFileVersionCount(), o365TenantId, siteUrlAndSizeMapping[siteUrl], siteId);
        }
        ...
        {
            List<ArchiverSiteMasterIndex> indexes = this.IndexMapProcessor.ExecuteQuery<ArchiverSiteMasterIndex>(SelectTableNameArchiverSiteMaster, null);
            this.IndexMainProcessor.Insert(indexes);
        }
        private void InsertIntoCommonSiteMasterIndex()
        {
            List<CommonSiteMasterIndex> indexes = this.IndexMapProcessor.ExecuteQuery<CommonSiteMasterIndex>(SelectTableNameCommonSiteMaster, null);
            this.IndexMainProcessor.Insert(indexes);
        }
