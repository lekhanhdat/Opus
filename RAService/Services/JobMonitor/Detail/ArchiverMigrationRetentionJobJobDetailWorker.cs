...
{
    public class ArchiverMigrationRetentionJobJobDetailWorker : AbstractDaoMigrationJobDetailWorker
    {
        ...
        public override IEnumerable<JMJobDetails> GetData(int PageSize, int StartPage, string conditionFilter, BaseJobDto jobInfo)
        {
            ...
            foreach (JMArchiverMigrationRententionJobDetails jobDetail in result.Cast<JMArchiverMigrationRententionJobDetails>())
            {
                jobDetail.SizeStr = JobDetailHelper.GetDataSizeToView(Convert.ToInt64(jobDetail.Size));
                jobDetail.Action = DaoMigrationI18NEntity.Execution(jobDetail.Action, jobDetail.Action);
