...
namespace RAGoogle.Archive.Scan.Base
{
    internal class ScanDataCache : IDisposable
    {
        ...
        private string GetBCSColumnDisplayName(string scopeId)
        {
            string result = string.Empty;
            var info = spSettingDao.GetColumnInfos(new string[] { scopeId }).FirstOrDefault();
            if (info != null)
            {
                result = info.ColumnName;
            }
            return result;
        }
        ...
        private void LoadRules(bool IsOneDrive)
        {
            ...
            logger.Debug($"End to load Rules to cache,rule count:{Rules?.Count}");
        }
