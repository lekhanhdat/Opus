...
namespace AvePoint.Common
{
    ...
    using AvePoint.GCommon.JobManagement.Modules.ControlPanel;
    ...
    public class AgentRegisterUtil
    {
        ...
        public static void RegisterAgent()
        {
            ...
            AgentRegisterService registerService = new AgentRegisterService();
            var agentDto = new ServiceDto();
            agentDto.Schema = AveEnv.AgentSchema;
            //if (string.IsNullOrEmpty(AveEnv.AgentDeploymentId))
            //{
            //    agentDto.Name = AveEnv.AgentRoleId;
            //}
            //else
            //{
            //    agentDto.Name = string.Concat(AveEnv.AgentDeploymentId, '_', AveEnv.AgentRoleId);
            //}
            agentDto.Name = AveEnv.AgentRoleId;
            agentDto.Address = AveEnv.AgentAddress;
            agentDto.Port = AveEnv.AgentPort;
            agentDto.AgentType = AveEnv.AgentType;                        
            agentDto.Version = AveEnv.AgentVersion.ToString();
            ...
            RegisterResult result = registerService.Register(agentDto);
            ...
            AveEnv.CommunicationEncryptionKey = result.CommunicationEncryptionKey;
    ...
    public class PRAgentRegisterProxy
    {
        ...
        public static Tuple<FarmRoles, int> GetFarmRoles(string domainName, string userName, string plainPassword)
        {
            try
            {
                string serviceExe = "AgentCommonPlatformRecoveryChecker.exe";
                string serviceName = "PRCheckService";
                if (AveEnv.IsSharePoint2010)
                {
                    serviceExe = "AgentCommonPlatformRecoveryInstaller.exe";
                    serviceName = "PRInstallService";
                }
                ...
                mLog.Info("Checking Platform Recovery Install Service's status.");
                ...
                while (true)
                {
                    if (DateTime.Now > deadline)
                    {
                        ...
                        mLog.Error("Platform Recovery Install Service does not started before deadline.");
                        break;
                    }
                    if (!File.Exists(Path.Combine(AveEnv.AgentTempFolder, serviceStartedFlagFile)))
                    {
                        mLog.Info("Waiting for Platform Recovery Install Service Started.");
                        Thread.Sleep(5000);
                    }
                    else
                    {
                        mLog.Info("Platform Recovery Install Service is started.");
                ...
                else
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            List<string> spServers = prAgentInstallService.GetAllSPServers();
