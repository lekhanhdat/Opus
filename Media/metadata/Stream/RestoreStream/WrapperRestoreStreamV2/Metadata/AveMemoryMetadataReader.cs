...
/// </summary>
public class AveMemoryMetadataReader : IAveMetadataReader
{
    private static RALogger logger = RALogger.GetInstance(typeof(AveMemoryMetadataReader));
    ...
    protected virtual void InitRootElement()
    {
        if (!IsAllMetadataLoaded)
        {
            ...
            if (mRootElement == null || string.Compare(mRootElement.Name, AveMetadataConstants.ROOT_ELEMENT, StringComparison.OrdinalIgnoreCase) != 0)
            {
                mRootElement = null;
            }
        }
