...
{
    public class GoogleDisposalActionDetailWorker : AbstractJobDetailWorker
    {
        ...
        {
            string sortAction = nameof(JMArchiverActionJobDetails.ActionTab);
            if (string.IsNullOrEmpty(conditionFilter))
            {
                SELECT_DATA_SQL = string.Format(JobMonitorConstants.SELECT_DATA_FROM_TABLE_ORDERBY_CONDITONSTR, TABLE_NAME, sortAction, PageSize, (StartPage - 1) * PageSize);
                SELECT_DETAIL_COUNT_SQL = string.Format(JobMonitorConstants.SELECT_DETAIL_COUNT_SQL, TABLE_NAME);
            }
            else
            {
                SELECT_DATA_SQL = string.Format(JobMonitorConstants.SELECT_DATA_ON_CONDITION_FROM_TABLE_ORDERBY_CONDITONSTR, TABLE_NAME, conditionFilter, sortAction, PageSize, (StartPage - 1) * PageSize);
        ...
        {
            TABLE_NAME = JobMonitorConstants.JOBDETAIL;
            CREATE_TABLE_SQL = string.Format(JobMonitorConstants.CREATE_TABLE_Google_Disposal_Report, TABLE_NAME);
            INSERT_DATA_SQL = string.Format(JobMonitorConstants.INSERT_DATA_Google_Disposal_Report, TABLE_NAME);
        }

        public void InitCreateSummaryTableSQLString()
        {
            SUMMAY_TABLE_NAME = JobMonitorConstants.JOBSUMMAYDETAIL;
            CREATE_SUMMAYTABLE_SQL = string.Format(JobMonitorConstants.CREATE_TABLE_Google_Disposal_SUMMARYReport, SUMMAY_TABLE_NAME);
            INSERT_SUMMAYDATA_SQL = string.Format(JobMonitorConstants.INSERT_DATA_Google_Disposal_SUMMARYReport, SUMMAY_TABLE_NAME);
        }
