...
namespace AvePoint.Media.Core.Index
{
    ...
    using TableAttribute = Service.DomainModel.TableAttribute;
    ...
    public class IndexDatabaseHelper
    {
                            ...
                            if (File.Exists(connStringBuilder.DataSource))
                            {
                                using (FileStream file = new FileStream(connStringBuilder.DataSource, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                                {
                                    ...
                                    while (bRead < 16)
                                    {
                                        ...
                                        if (rd == -1)
                                        {
                                            throw new Exception("file is unusually small");
                                        }
                                        bRead += rd;
                                    }
        ...
        {
            {
                ...
                {
                    logger.Warn($"IndexDatabaseHelper Close Exception:{e}");
                }
            }
        }
        ...
        String GenerateInsertCommandText(Object index)
        {
            ...
            if (!cachedInsertSqlCommandTextDictionary.ContainsKey(indexType))
            {
                ...
                else throw new Exception(String.Format($"IndexDatabaseHelper Generate Insert Cmd Exception Map Table Attribute Not Specified,index.GetType().FullName:{index.GetType().FullName}"));
