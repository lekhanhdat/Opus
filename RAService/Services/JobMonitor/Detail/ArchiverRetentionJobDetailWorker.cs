...
{
    public class ArchiverRetentionJobDetailWorker : AbstractJobDetailWorker
    {
    ...
    public class ArchiverRetentionSimulateSumDetailWorker : ArchiverRetentionDashboardDetailWorker
    {
        //public override string NeedCreateDictionary(BaseJobDto jobInfo)
        //{
        //    string reportFilePath = GetReportFileDownloadPath(jobInfo);
        //    CheckAndCreateDirectory(reportFilePath);
        //    return reportFilePath;
        //}

        public new bool MergeJobDetails(BaseJobDto sourceJobInfo, BaseJobDto targetJobIndo)
        {
            string sourceDBPath = DownloadReports(sourceJobInfo);
            string targetDBPath = DownloadReports(targetJobIndo);
            string targetDBReportPath = NeedCreateDictionary(targetJobIndo);
            if (JobDetailHelper.MergeJobDetails(JobMonitorConstants.JOBDETAIL, sourceDBPath, targetDBPath))
            {
                System.IO.File.Move(targetDBPath, targetDBReportPath, true);
                return true;
            }
            return false;
        }

        public (int, long) GetSummary(BaseJobDto jobInfo)
        {
            string reportFilePath = DownloadReports(jobInfo);
            TABLE_NAME = JobMonitorConstants.JOBDETAIL;

            SELECT_DETAIL_COUNT_SQL = string.Format(JobMonitorConstants.SELECT_DETAIL_COUNT_SQL, TABLE_NAME);
            var SELECT_DETAIL_Size_SQL = string.Format("SELECT SUM(CAST(Size AS INTEGER)) FROM {0}", TABLE_NAME);

            return (base.GetCountForDetail(reportFilePath, base.SELECT_DETAIL_COUNT_SQL, jobInfo), JobDetailDao.GetTotalSizeForDetail(reportFilePath, SELECT_DETAIL_Size_SQL, jobInfo));
        }
    }

    public class ArchiverRetentionDashboardDetailWorker : AbstractJobDetailWorker
    {
        public override IEnumerable<JMJobDetails> GetData(int PageSize, int StartPage, ref int totalCount, string conditionFilter, BaseJobDto jobInfo)
        {
            string reportFilePath = DownloadReports(jobInfo);
