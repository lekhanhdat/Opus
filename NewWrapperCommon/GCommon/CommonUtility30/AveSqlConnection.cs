...
namespace AvePoint.Common
{
    public class AveSqlConnection : IDisposable
    {
        ...
        public const int SQL_ConnectionRefused = 10054;
        ...
        {
            Open(connString, DEFAULT_TIMEOUT, false);
        }

        public void Open(string connString, bool clearPool)
        {
            Open(connString, DEFAULT_TIMEOUT, clearPool);
        }

        public void Open(string connString, int timeout)
        {
            Open(connString, timeout, false);
        }

        /// <summary>
        /// clear the specific connection pool before init the connection
        /// </summary>
        /// <param name="connString"></param>
        /// <param name="timeout"></param>
        /// <param name="clearPool"></param>
        public void Open(string connString, int timeout, bool clearPool)
        {
            mTimeout = timeout;
        ...
        {
            SqlConnection.ClearPool(mConnection);
        }
        ...
        /// <summary>
        /// Clear Parameters and reset the command Type
        /// </summary>
                ...
                catch (SqlException sqlException)
                {
                    switch (sqlException.Number)
                    {
                        case SQL_DEAD_LOCK:
                            ...
                            mLog.Warn(string.Format("Retry Times:{0}\r\nDead lock exception:{1}", retryCount, sqlException.ToString()));
                            Thread.Sleep(1000);
                            break;
                        case SQL_ConnectionRefused:
                            retryCount--;
                            if (retryCount <= 0)
                            {
                                throw;
                            }
                            mLog.Warn(string.Format("Retry Times:{0}\r\nConnection refused exception:{1}", retryCount, sqlException.ToString()));
                            ResetConnection(false);
                            break;
