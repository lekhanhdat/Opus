...
{
    public class DestructionCacheWrapper : IDisposable
    {
        private static readonly AvePoint.RA.Contract.Services.IRALogger logger = RALogger.GetInstance(typeof(DestructionCacheWrapper));
        private readonly string _defaultCollectionName = "rec";
        private string _filename;
        private LiteDatabase _db;
        private static DestructionCacheWrapper fileSystemSqliteWrapper = null;
        private readonly static object mLiteLock = new object();
        public static DestructionCacheWrapper CreateInstance(string dbFilePath)
        {
            if (fileSystemSqliteWrapper == null)
            {
                lock (mLiteLock)
                {
                    if (fileSystemSqliteWrapper == null)
                    {
                        fileSystemSqliteWrapper = new DestructionCacheWrapper(dbFilePath);
                    }
                }
            }
            return fileSystemSqliteWrapper;
        }
        private DestructionCacheWrapper(string dbFilePath)
        {
