...
{
    private static RALogger logger = RALogger.GetInstance(typeof(AveMetadataV2));
    ...
    public void Dispose()
    {
        if (!isDisposed)
        {
            ...
            {
                logger.Error("An error occurred while dispose AveMetadataV2.MetadataPath:{0},Error:{1}", MetadataPath, e);
            }
        }
    }
