...
{
    internal class DoNotDestoryActivityConverter
    {
        public static  Activity Convert(WorkflowNode node)
        {
            var activity = new DoNotDestoryActivity()
            {
                DisplayName = node.Name,
            };

            activity.RequestInfo = new VisualBasicValue<DisposalReviewRequestInfo>(DisposalReviewWorkflowBuilder.ArgRequestInfoName);
            activity.RunningRequestInfo = new VisualBasicValue<DisposalReviewRequestInfo>(DisposalReviewWorkflowBuilder.VarRequestInfoName);
