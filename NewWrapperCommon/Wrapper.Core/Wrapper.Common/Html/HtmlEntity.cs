...
namespace AvePoint.Wrapper.Common
{
    ...
    {
        private static AveLogger Logger = AveLogger.GetInstance(typeof(HtmlEntity));
                            ...
                            case ';':
                                ...
                                else
                                {
                                    if (entity[0] == '#')
                                    {
                                        ...
                                        try
 										{
                                            string codeStr = e.Substring(1).Trim().ToLower(System.Globalization.CultureInfo.InvariantCulture);
											int fromBase;
                                        ...
                                        {
                                            sb.Append("&#" + e + ";");
                                            Logger.Warn("failed to deentitize text due to: {0}", ex.ToString());
                                        }
                                    }
                                    else
                                    {
