...
namespace AvePoint.Wrapper.Common
{
    ...
    public class AveRBSBackup
    {
        ...
        public AveRBSStubInfo BackupRBSStub(byte[] rbs_id)
        {
            ...
            return mQueryService.BackupRBSStub(rbs_id, mBlobStoreId, collectionId);
        }

        public List<AveRBSStubInfo13> BackupRBSStub13(List<AveRBSStubInfo13> stubInfoList)
        {
            List<AveRBSStubInfo13> rbsStubInfoList = new List<AveRBSStubInfo13>();
            int collectionId;
            foreach (var stubInfo in stubInfoList)
            {
                AveRBSStubInfo13 newStubInfo;
                if (stubInfo.RBSId != null)
                {
                    collectionId = stubInfo.RBSId[8] | stubInfo.RBSId[9] << 8 | stubInfo.RBSId[10] << 16 | stubInfo.RBSId[11] << 32;
                    newStubInfo = new AveRBSStubInfo13(mQueryService.BackupRBSStub(stubInfo.RBSId, mBlobStoreId, collectionId));
                }
                else
                {
                    newStubInfo = new AveRBSStubInfo13();
                }
                
                newStubInfo.CopyValue(stubInfo);
                rbsStubInfoList.Add(newStubInfo);
            }
            return rbsStubInfoList;
        }
    }
    ...
    public class AveRBSRestore
    {
        ...
        public byte[] RestoreRBSStub(AveRBSStubInfo stubinfo)
        {
            ...
            return mQueryService.RestoreRBSStub(stubinfo, mPoolsOfDB, mBlobStoreId, collectionId);
        }

        public void RestoreDocsToStreamsAndDocStreams(Guid siteId,Guid docId,int histVersion,byte level,List<AveShredStubInfo> shredStubInfoList,bool ClearDocStreams = false)
        {
            #region clear all records

            mQueryService.ClearDocsToStreamsAndDocStreams(siteId, docId, histVersion, level,ClearDocStreams);

            #endregion

            #region insert DocsToStreams

            foreach (var shredStubInfo in shredStubInfoList)
            {
                mQueryService.InsertDocsToStreams(siteId, docId, shredStubInfo.RBSInfo,(level == 255), level);
            }

            #endregion

            #region insert DocStreams

            foreach (var shredStubInfo in shredStubInfoList)
            {
                mQueryService.InsertDocStreams(siteId, docId, shredStubInfo.RBSInfo);
            }
    ...
    public class AveRBSCommon
    {
        ...
        public const string RBS_PROVIDER_NAME_SP2013 = "SP2013RBSProvider";
        public const string RBS_PROVIDER_NAME_SP2016 = "SP2016RBSProvider";
        public const string RBS_PROVIDER_NAME_SP2019 = "SP2019RBSProvider";
        public const string RBS_PROVIDER_NAME_SPSE = "SPSERBSProvider";
        ...
        public static List<Guid> GetPoolsOfDB(IAveBackupRestoreQueryService queryService)
        {
            ...
            try
            {
                temList = queryService.AveRBSCommon_GetPoolsOfDB();
            }
            catch (Exception ex)
            {
 ...
 */
        ...
        public static Guid GetPoolGuid(byte[] storeBlobPoolIDBinary)
        {
            ...
            {
                throw new AveWrapperBaseException(AveInternalResourceKey.Wrapper_Exception_Common_BlobPoolnotGeneratedByDocAve6Extender);
            }
            byte[] poolIdBinary = new byte[16];
            int position = 4;
            GetBinaryFromBinary(ref storeBlobPoolIDBinary, ref poolIdBinary, ref position);
