...
namespace AvePoint.ObjectModel.ClientOM
{
    public class AveRetryProjectContext : ProjectContext
    {
        ...
        public AveRetryProjectContext(string webFullUrl)
            : base(webFullUrl)
        {
            this.WebRequestExecutorFactory = new AveWebRequestExecutorFactory();
        }
        ...
        public override void ExecuteQuery()
        {
            ...
            for (int count = 0; count < RETRYCOUNT; count++)
            {
                ...
                /*review-qlluo*/catch (ServerException se)
                {
        ...
        private bool ShouldRetry(ServerException se)
        {
            ...
            //    return false;
            //}
            //if (errorCode == AveSPErrorCode.TP_E_USER_DOESNOT_EXIST || errorCode == AveSPErrorCode.COR_E_APPLICATION)
            //{
            //    return false;
            //}
            //if (string.Equals(se.ServerErrorTypeName, "System.IO.FileNotFoundException", StringComparison.OrdinalIgnoreCase)
            //    || string.Equals(se.ServerErrorTypeName, "Microsoft.ProjectServer.PJClientCallableException", StringComparison.OrdinalIgnoreCase))
            //{
            //    return false;
            //}
        ...
        private bool ShouldRetry(Exception ex)
        {
            if (IsConnectonForciblyClosedExceptioin(ex) || IsRetryableServerException(ex)
                || IsUnstableNetworkException(ex as WebException))
            {
                System.Threading.Thread.Sleep(RETRYINTERVAL);
                return true;
            }
            if (IsServerProtocolViolationError(ex))
            {
                System.Threading.Thread.Sleep(RETRYINTERVAL * 2);
                return true;
            }
        ...
        private bool IsServerProtocolViolationError(Exception e)
        {
        ...
        {
            if (e.GetType().FullName.Equals("Microsoft.SharePoint.Client.ClientRequestException", StringComparison.OrdinalIgnoreCase))
            {
