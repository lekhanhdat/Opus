...
namespace AvePoint.GCommon.Transfer.Data
{
    ...
    public class CMDataSender : BaseDataTransferLogic, IDataSender
    {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (!checking)
                    {
                        ...
                        {
                            DataTransferWorkStatus = DataTransferWorkStatus.UnHandlerError;
                            DataTransferErrorMessage += "\r\nThe result code from destination is " + resultCode.ToString();
                            break;
                        }
                        //checking = true;
                    }
        ...
        protected override void TransferThreadTemp()
        {
            ...
            try
            {
                byte[] buffer = new byte[DataTransferConfiguration.SendBufferSize];
            ...
            {
                logger.Debug("Send {0} buffer within {1}.", serialNo, DateTime.Now - currentTime);
        ...
        protected override void TransferThread()
        {
            ...
            try
            {
                ...
                while (threadWrapper.KeepRunning)
                {
                    int needRead = DataTransferConfiguration.SendBufferSize;
                    int totalRead = 0;
                    while (totalRead < DataTransferConfiguration.SendBufferSize)
                    {
                    ...
                    if (totalRead < DataTransferConfiguration.SendBufferSize)
                    {
        ...
        protected override void WaitHandShakeToClose()
        {
            ...
            {
                if (TransferChannel.BufferSessinInUse(true))
                {
