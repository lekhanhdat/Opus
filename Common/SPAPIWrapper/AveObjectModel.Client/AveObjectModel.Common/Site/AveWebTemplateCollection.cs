...
namespace AvePoint.ObjectModel.Common
{
    class AveWebTemplateCollection : AveAbstractCommonCollection<IAveWebTemplate>, IAveWebTemplateCollection
    {
        ...
        {
            var webTemplateList = base.DataCache.GetChildren();
            mListData = new List<IAveWebTemplate>();
            foreach(var webTemplateProperties in webTemplateList )
            {
            ...
            get 
            {
                return mListData.Find(
                    delegate(IAveWebTemplate template) {
                        return template.Name.Equals(name,StringComparison.OrdinalIgnoreCase) || template.Name.Equals(name+"#0",StringComparison.OrdinalIgnoreCase);
                    });
            } 
        }
