...
namespace AvePoint.GCommon.Utility.Cryptography.DataEncryptionManagement
{
    public static class DataEncryptionUtil
    {
        ...
        public static DataEncryptionInfoWrapper CreateDataEncryptionInfoWrapper(DataEncryptionProfile profile)
        {
            ...
            else if (profile.CurrentProtectionAlgorithm.Type == ProtectionAlgorithmType.KeyVault)
            {
                   var  provider = new KeyVaultServiceProvider(profile.CurrentProtectionAlgorithm.AOSProfile);
            ...
            {
                info.EncryptedDynamicKey = CspCommunicationWrapper.WrapKey(result);
            }
