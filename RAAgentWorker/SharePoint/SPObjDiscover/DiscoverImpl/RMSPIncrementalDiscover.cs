...
namespace AvePoint.RA.SharePoint.SPObjDiscover.DiscoverImpl
{
    ...
    {
        private static readonly AveLogger logger = AveLogger.GetInstance(typeof(RMSPIncrementalDiscover));
        ...
        {
            var discoverRootFolder = list.GetChangeRootFolder(null);
            return discoverRootFolder.GetSubFoldersWithoutCache();
        }
        ...
        {
            var rootFolder = list.GetChangeRootFolder(null);
        ...
        public IEnumerable<IAveListItem> GetAllItems(AveDiscoverList list, out long totalCount, List<AveCamlQuery> aveCamlQueries = null)
        {
            ...
            {
                result.CompleteAdding();
                logger.Error($"Error Get i items by query:{list?.RootFolderUrl.LogBase64()}, ERROR:{ex.ToString()}");
            }
            return result.GetConsumingEnumerable();
        }

        private void DiscoverFolders(IAveList list, List<IAveDiscoverFolder> folders, BlockingCollection<IAveListItem> result, ref long total)
        {
