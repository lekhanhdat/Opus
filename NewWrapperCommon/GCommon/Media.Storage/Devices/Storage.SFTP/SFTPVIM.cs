...
namespace AvePoint.Media.Storage.SFTP
{
    ...
    #endregion
    [VIM(VIMName.SFTP, typeof(AvePoint.Media.Storage.SFTP.SFTPVIM))]
    class SFTPVIM : AbstractVIM
    {
        public override IXSystem CreateSystem(string xri, AbstractXSystem parentSystem)
        {
            return new SFTPSystem(xri, parentSystem);
        }

        public override List<string> GetFeatureXML(int type)
        {
            return SFTPFeature.Getstances(type).FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(int type)
        {
            return SFTPFeature.Getstances(type).FeatureObjs;
        }

        public override List<StorageFeature> GetFeatureObj(int type, string culture)
        {
            return SFTPFeature.Getstances(type, culture).FeatureObjs;
        }
    }
}
