...
namespace AvePoint.ObjectModel.Server13
{
    class AveFolderCollection : AveAbstractCommonCollection<IAveFolder>, IAveFolderCollection, IDisposable
    {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveFolderCollection.RestoreSystemFolderRelativeProperty"))
            {
                ...
                if (allDocData.ContainsKey("MetaInfo"))
                {
                    ...
                    try
                    {
                        ...
                        if (!isSystemFolder && MetaInfoDic.ContainsKey("vti_contenttypeorder") && MetaInfoDic["vti_contenttypeorder"] != null)
                        {
                            try
                            {
                                ...
                                if (contentTypeOrder != null && contentTypeOrder.Count == tempContentTypes.Length)
                                {
                                    ...
                                    {
                                        if (type.ID != tempContentTypes[index].ID)
                                        {
                                            flag3 = true;
                                            break;
                                        }
        ...
        private AveRestoreResult RealRestoreFolder(AveFolderInfo info, AveItem aveItem, Dictionary<string, object> allDocData, Dictionary<string, object> allUserData)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveFolderCollection.RealRestoreFolder"))
            {
                ...
                if (allDocData.ContainsKey("MetaInfo"))
                {
                    ...
                    {
                        RestoreDocumentSet(aveItem, info, allDocData);
                    }
                }
        ...
        private static void RestoreDocumentSet(AveItem aveItem, AveFolderInfo folderInfo, Dictionary<string, object> allDocData)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveFolderCollection.RestoreDocumentSet"))
            {
                ...
                if (aveItem.ListItem.ContentType != null && AveSPDocumentSet.IsDocumentSet(aveItem.ListItem.ContentType.ID))
                {
                    try
                    {
                        //var documentSet = DocumentSet.GetDocumentSet(aveItem.mSPListItem.Folder);
                        //documentSet.Provision();
                        string editor = string.Empty;
                        string author = string.Empty;
                        DateTime modified = DateTime.MinValue;
                        DateTime created = DateTime.MinValue;
                        if (folderInfo.FieldsInfo.Fields.ContainsKey("tp_Editor"))
                        {//insert folder version
                            editor = folderInfo.FieldsInfo.Fields["tp_Editor"] is AveFieldValueInfo ? ((AveFieldValueInfo)folderInfo.FieldsInfo.Fields["tp_Editor"]).ColValue.ToString() : ((int)folderInfo.FieldsInfo.Fields["tp_Editor"]).ToString();
                            author = folderInfo.FieldsInfo.Fields["tp_Author"] is AveFieldValueInfo ? ((AveFieldValueInfo)folderInfo.FieldsInfo.Fields["tp_Author"]).ColValue.ToString() : ((int)folderInfo.FieldsInfo.Fields["tp_Author"]).ToString();
                            modified = folderInfo.FieldsInfo.Fields["tp_Modified"] is AveFieldValueInfo ? (DateTime)((AveFieldValueInfo)folderInfo.FieldsInfo.Fields["Modified"]).ColValue : (DateTime)(folderInfo.FieldsInfo.Fields["tp_Modified"]);
                            created = folderInfo.FieldsInfo.Fields["tp_Created"] is AveFieldValueInfo ? (DateTime)((AveFieldValueInfo)folderInfo.FieldsInfo.Fields["tp_Created"]).ColValue : (DateTime)(folderInfo.FieldsInfo.Fields["tp_Created"]);
                        }
                        else
                        {
                            editor = folderInfo.FieldsInfo.Fields["Editor"] is AveFieldValueInfo ? ((AveFieldValueInfo)folderInfo.FieldsInfo.Fields["Editor"]).ColValue.ToString() : ((int)folderInfo.FieldsInfo.Fields["Editor"]).ToString();
                            author = folderInfo.FieldsInfo.Fields["Author"] is AveFieldValueInfo ? ((AveFieldValueInfo)folderInfo.FieldsInfo.Fields["Author"]).ColValue.ToString() : ((int)folderInfo.FieldsInfo.Fields["Author"]).ToString();
                            modified = folderInfo.FieldsInfo.Fields["Modified"] is AveFieldValueInfo ? (DateTime)((AveFieldValueInfo)folderInfo.FieldsInfo.Fields["Modified"]).ColValue : (DateTime)(folderInfo.FieldsInfo.Fields["Modified"]);
                            created = folderInfo.FieldsInfo.Fields["Created"] is AveFieldValueInfo ? (DateTime)((AveFieldValueInfo)folderInfo.FieldsInfo.Fields["Created"]).ColValue : (DateTime)(folderInfo.FieldsInfo.Fields["Created"]);
                        }
                        aveItem.UpdateSpecialPropertyByNative(editor, author, modified, created, folderInfo);

                        if (allDocData.ContainsKey("snapshots"))
                        {
                            aveItem.info.MappingManager.ListMappingManager.DocumentSetGuidMetaInfoMapping.Add(aveItem.mSPListItem.UniqueId, allDocData["snapshots"].ToString());
                        }
                    }
                    catch (Exception e)
                    {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveFolderCollection.CheckAndCreateFolder"))
            {
                ...
                if (aveItem.Folder == null)
                {
                    ...
                    if (aveItem.mList != null)
                    {
                        ...
                        if (info.OriginalVersion > 0 && info.OriginalVersion < 512 && !aveItem.mSPList.EnableMinorVersions)
                        {
                            aveItem.mSPList.EnableMinorVersions = true;
                            aveItem.mSPList.EnableModeration = true;
                            info.SettingInfo.LIST_SETTING_CHANGED = true;
                            aveItem.mSPList.Update();
                        }
                        try
                        {
                            //the new document set item's version will be 513 if moderation enabled and MinorVersions enabled
                            if ((AveSystemContentTypeId.DocumentSet as AveContentTypeId).ContentTypeId.IsParentOf(itemContentTypeId))
                            {
                                if (info.OriginalVersion > 0 && info.OriginalVersion == 512 && aveItem.mSPList.EnableModeration && aveItem.mSPList.EnableMinorVersions)
                                {
                                    //Minorversion disabled.
                                    info.SettingInfo.LIST_SETTING_CHANGED = true;
                                    aveItem.mSPList.EnableMinorVersions = false;
                                    aveItem.mSPList.Update();
                                }
                            }//一些情况下开启Approval功能会导致update异常
                            //else if (aveItem.mSPList.BaseType == SPBaseType.DocumentLibrary && aveItem.mSPList.BaseTemplate != SPListTemplateType.NoCodePublic
                            //&& !aveItem.mSPList.EnableModeration && !aveItem.mSPList.HasExternalDataSource
                            //&& aveItem.mSPList.BaseTemplate != SPListTemplateType.Survey && !(aveItem.mSPList.ParentWeb.WebTemplate == "MPS"
                            //&& (aveItem.mSPList.BaseTemplate == SPListTemplateType.Meetings || aveItem.mSPList.BaseTemplate == SPListTemplateType.HomePageLibrary)))
                            //{
                            //    aveItem.mSPList.EnableModeration = true;
                            //    info.SettingInfo.LIST_SETTING_CHANGED = true;
                            //    aveItem.mSPList.Update();
