...
namespace AvePoint.RA.SharePoint.Archiver
{
    internal class ArchiverMergeIndexJobHandler
    {
        ...
        public ArchiverMergeIndexJobHandler()
        {
            ...
            MergeIndexService = new MergeIndexService();
        }
        ...
        public void PerformMergeIndexSubJob(MergeIndexJobInfo job, string subJobId)
        {
            List<MergeIndexSubJobInfo> mergeInfos = new List<MergeIndexSubJobInfo>();
            ...
            job.CacheLocation.Extension.Path.Add(new PathMap() { DiskInfo = disk });
            job.CacheLocation.LimitFreeSpace = 1024 * 1024 * 1024;//1 GB
            MergeIndexSubJobInfo subJobInfo = new MergeIndexSubJobInfo(job);
            subJobInfo.JobDto.Id = subJobId;
            subJobInfo.MergeIndexJobsState = new List<MergeIndexJobState>() { new MergeIndexJobState(subJobId,false) };
            mergeInfos.Add(subJobInfo);
            //subJobInfo.IgnoreUpdateJobState = true;
            //need further optimization
            MergeIndexService.Merge(mergeInfos);
        }
