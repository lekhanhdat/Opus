...
";
        ...
        public IAveQueryDataReader BlobRawDataAttachmentInfos(Guid siteId, Guid attachmentsFolderId)
        {
            ...
            using (SqlCommand cmd = mQueryWorker.CreateCommand())
            {
                ...
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = @"
select doc.Id,doc.UIVersion,doc.Level,doc.UIVersionString,doc.MetaInfo,doc.ExtensionForFile,doc.DirName,doc.LeafName,doc.TimeCreated,doc.TimeLastModified from AllDocs(nolock) parent
...
on AllDocs.ID= SchedSubscriptions.itemdocid and AllDocs.Level>0 and SchedSubscriptions.Id=@AlertId";
        ...
        public IAveQueryDataReader BlobSP2013DocumentCurrentVersion(Guid siteId, Guid parentId)
        {
            ...
            using (SqlCommand cmd = mQueryWorker.CreateCommand())
            {
                ...
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = @"
  select Id,DocStreams.Partition,DocStreams.BSN,AllDocs.SizeRead as FileRealSize,DocStreams.Size as BSNSize,DocStreams.RbsId,
  ...
  and AllDocs.Type=0 and RbsId is null";
        ...
        public IAveQueryDataReader BlobSP2013DocVersionInfo(Guid siteId, Guid parentId, Guid docId)
        {
            ...
            using (SqlCommand cmd = mQueryWorker.CreateCommand())
            {
                ...
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = @"
select Id,UIVersion,AllDocs.Level,DocStreams.BSN,DocStreams.Partition,AllDocs.SizeRead as FileRealSize,DocStreams.Size as BSNSize,
...
and AllDocs.Type=0 and RbsId is null
union all
select Id,AllUserData.tp_UIVersion,AllUserData.tp_Level,DocStreams.BSN, DocStreams.Partition,AllDocVersions.SizeRead as FileRealSize,DocStreams.Size as BSNSize,
...
select Extension, COUNT(*) ItemCount, SUM(VersionCount) VersionCount, MAX(VersionCount) MaxVersionCount,  SUM(SizeRead) Size
from CurrentVersionDoc with(nolock)
inner join
(
  select Id, COUNT(*) VersionCount,  SUM(SizeRead) SizeRead from
     (
        select doc.Id, SizeRead from AllDocs doc(nolock)
        where doc.SiteId=@SiteId and doc.ParentId=@ParentId and DeleteTransactionId=0x and Type=0 and DoclibRowId is not Null
        union all
        select doc.Id, version.SizeRead from CurrentVersionDoc doc
...
and Occurred between @StartTime and @EndTime order by Occurred desc";
        ...
        public IAveQueryDataReader GetLastAccessTimeOfSite(Guid siteId, DateTimeOffset startTime, DateTimeOffset endTime)
        {
            ...
            using (SqlCommand cmd = mQueryWorker.CreateCommand())
            {
                ...
                cmd.CommandText = AveQueryStringReportCenter16.GetLastAccessTimeOfSite_Select_UserInfo_AuditData;
        ...
        public IAveQueryDataReader GetLastAccessTimeOfWeb(Guid siteId, Guid webId, DateTimeOffset startTime, DateTimeOffset endTime)
        {
            ...
            using (SqlCommand cmd = mQueryWorker.CreateCommand())
            {
                ...
                cmd.CommandText = AveQueryStringReportCenter16.GetLastAccessTimeOfWeb_Select_UserInfo_AuditData;
        ...
        public IAveQueryDataReader GetLastAccessTimeOfList(Guid siteId, Guid webId, DateTimeOffset startTime, DateTimeOffset endTime)
        {
            ...
            using (SqlCommand cmd = mQueryWorker.CreateCommand())
            {
                cmd.CommandText = AveQueryStringReportCenter16.GetLastAccessTimeOfList_Select_UserInfo_AuditData;
    ...
    where 
        ...
        public IAveQueryDataReader GetUserStorageDocInfo(Guid siteId, Guid parentId)
        {
            ...
            using (SqlCommand command = mQueryWorker.CreateCommand())
            {
                command.CommandType = CommandType.Text;
                command.CommandText = @"
select tp_Author,tp_Editor,SizeRead as Size,tp_DocId,type from AllDocs doc(nolock)
...
union all
select tp_Author,tp_Editor,ver.SizeRead as Size,tp_DocId,0 from AllUserData data (nolock)
...
union all
select tp_Author,tp_Editor,version.SizeRead as Size,tp_DocId,0 from AllDocs doc(nolock)
...
where AggregationId=@AggregationId and {0} and IncludeSubSites=1";
        ...
        public long BPRSP2010GetUserProfileCount()
        {
            ...
            using (SqlCommand cmd = mQueryWorker.CreateCommand())
            {
                cmd.CommandText = @"select COUNT(1) from upa.UserProfile_Full (nolock)";
                object objCount = cmd.ExecuteScalar();
                if (objCount != DBNull.Value)
                {
