...
{
    public class RMDiscoveryAOSPContainerInactiveDataAnalyzer
    {
        private readonly RALogger _logger = RALogger.GetInstance(typeof(RMDiscoveryAOSPContainerInactiveDataAnalyzer));
        ...
        private readonly List<RMDiscoveryAOSPContainerInactiveData> _dataList;

        public RMDiscoveryAOSPContainerInactiveDataAnalyzer(
        ...
        public void Increse(List<RMDiscoveryAOSPSiteInactiveData> siteDataList)
        {
            ...
            foreach (var siteData in siteDataList)
            {
                ...
                if (data == null)
                {
                    data = new RMDiscoveryAOSPContainerInactiveData
                    {
        ...
        private async Task<bool> RecalculateAndSaveAsync()
        {
            try
            {
                ...
                await foreach (var data in enumerableDataList)
                {
                    ...
                    {
                        containerInactiveData = new RMDiscoveryAOSPContainerInactiveData
                        {
                            ContainerId = _containerId,
