...
{
    internal class NAAExport : VaultExportBase, IVaultExport
    {
        private static AveLogger mLog = AveLogger.GetInstance(MethodBase.GetCurrentMethod().DeclaringType);
        private bool HasInitObjs = false;
        private NAAColumnContainer columnArray = null;
        ...
        private const string SKIPMESSAGE = "StorageOptimization_NAASkipItemAndAttachment";
        ...
        {
            HasInitObjs = true;
            columnArray = new NAAColumnContainer(aveSite);
        }

        public NAAExport(PhysicalDeviceDto deviceDto, string jobId, string disposalClass, VaultExportFormat format, byte[] NAAConfigFile, byte[] encryptionKey, byte[] encryptionIV)
            : base(deviceDto, jobId, format, encryptionKey, encryptionIV)
        {
            InitClass(NAAConfigFile, disposalClass);
        }

        private void InitClass(byte[] naaConfigFile, string disposalClass)
        {
            NAAData.InitConfig(naaConfigFile);
            mDisposalClass = disposalClass;
        }

        public NAAExport(SharePointLocationDto spoDto, AveBPOSAccountInfo user, string siteUrl, string jobId, string disposalClass, VaultExportFormat format, byte[] NAAConfigFile, byte[] encryptionKey, byte[] encryptionIV)
        ...
        {
            mLog.Info(VaultLogFormat.LOGWITHPATH, "Start Export Site(NAA).", aveSite.SPSite.Url.ToString());
        ...
        {
            mLog.Info(VaultLogFormat.LOGWITHPATH, "Start Export Web(NAA).", aveWeb.SPWeb.Url.ToString());
            if (!HasInitObjs)
            {
        ...
        {
            mLog.Info(VaultLogFormat.LOGWITHPATH, "Start Export List(NAA).", aveList.ParentWeb.SPWeb.Url + aveList.Path);
            if (!HasInitObjs)
            {
        ...
        public ExportStatus ExportFolder(AveSPFolder aveFolder, VaultExportInfo info, bool isRootFolder)
        {
            try
            {
                using (AvePerformanceScope pc = new AvePerformanceScope("NAAExport_ExportFolder"))
                {
                    ExportStatus exportStatus = new ExportStatus() { State = ExportState.Failed };
                    if (aveFolder == null)
                    {
                    ...
                    mLog.Info(VaultLogFormat.LOGWITHPATH, "Start Export folder(NAA).", FullURL.GetItemFullUrl(aveFolder, false));
                    if (isRootFolder)
                    {
        ...
        public ExportStatus ExportDocOrDocVersion(AveSPDoc aveDoc, VaultExportInfo info)
        {
            try
            {
                using (AvePerformanceScope pc = new AvePerformanceScope("NAAExport_ExportDocOrDocVersion"))
                {
                    ExportStatus exportStatus = new ExportStatus() { State = ExportState.Failed };
                    if (aveDoc == null)
                    {
                    ...
                    mLog.Info(VaultLogFormat.LOGWITHPATH, "Start Export Doc Or DocVersion(NAA).", aveDoc.AveSPItem.Id);
                    lock (lockObj)
                    {
                        if (!HasInitObjs)
                        {
                    ...
                    using (Stream docStream = aveDoc.AveSPItem.GetContent())
                    {
                        ...
                        //export content
                        result = RealVaultExport.ExportContent(contentInfo, info, docStream);
                    ...
                    CsvMetaData metaData = new CsvMetaData
                    {
                        CsvMetadataInfo = columnArray.GetCSVListFromColumnValue(aveDoc, this.DisposalClass, filePath, info.ContentFilePath)
                    };
                    ...
                    {
                        mLog.Error("Export Doc Or DocVersion(NAA) failed, because the content size is 0.");
                    }
                    exportStatus.State = ExportState.Succeed;
                    return exportStatus;
                }
            ...
            {
                mLog.Error(VaultLogFormat.LOGWITHEXCEPTIONPATH, "An error occurred while export DocOrDocVersion(NAA).", FullURL.GetItemFullUrl(aveDoc), e2.ToString());
                return new ExportStatus() { State = ExportState.Failed, ErrorMessage = e2.Message.ToString() };
            }
        }
        ...
        public void ExtensionMethod(params object[] parameter)
        {
            //to do rule test..
            //headline
            using (AvePerformanceScope pc = new AvePerformanceScope("NAAExport_ExtensionMethod"))
            {
