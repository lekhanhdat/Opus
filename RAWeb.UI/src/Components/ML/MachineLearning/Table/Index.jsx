...
import CheckedItemsCounter from "./Component/CheckedItemsCounter";
import { modifyItemsCheckStatus } from "./Utility";
...
const MLTable = (
    (
        ...
        const resetItems = () => {
            ...
            let currentPagerItems = _.cloneDeep(items);
            let selectedItems = [];
            onSelectTableItems(false, currentCachedItems, currentPagerItems, selectedItems);
        };

        const selectAllItems = () => {
            let currentCachedItems = modifyItemsCheckStatus(cachedItems, true);
            let currentPagerItems = modifyItemsCheckStatus(_.cloneDeep(items), true);
            let selectedItems = Object.values(currentCachedItems);
            onSelectTableItems(true, currentCachedItems, currentPagerItems, selectedItems);
        };
        ...
        const onSelectItems = () => {
            let currentCachedItems = getCurrentCachedItems(cachedItems, _.cloneDeep(pagerItems));
        ...
        const onSelectTableItems = (isSelectAll, currentCachedItems, currentPagerItems, selectedItems) => {
        ...
        const renderNav = () => {
            if (showSelectedCounts) {
                ...
                return <div className="ra-common-table-item">{actionComponent}</div>;
            }
        }; 
        ...
        const renderTable = () => {
            return (
                <R.Table
                    ...
                    onRowEvent={(args) => { onRowEvent(args);}}
                />
            );
        };
        ...
        const renderFooter = () => {
            if(pagerComponent){
                return <div className="ra-common-table-item">
                    <div>
