...
{
    [VIM(VIMName.ObjectAtmos, typeof(AvePoint.Media.Storage.Cloud.ObjectAtmos.ObjectAtmosVIM))]
    class ObjectAtmosVIM : AbstractVIM
    {
        #region VIM Members

        public override IXSystem CreateSystem(string xri, AbstractXSystem parentSystem)
        {
            return new ObjectAtmosSystem(xri, parentSystem);
        }

        public override List<string> GetFeatureXML(int type)
        {
            return ObjectAtmosFeature.Getstances(type).FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(int type)
        {
            return ObjectAtmosFeature.Getstances(type).FeatureObjs;
        }

        public override List<StorageFeature> GetFeatureObj(int type, string culture)
        {
            return ObjectAtmosFeature.Getstances(type, culture).FeatureObjs;
        }
