...
{
    public class AnalysisOptimizationAOSPDiscoverNodeWorker : DiscoverNodeWorkerBase
    {
        ...
        private DAO2SORuleConverter mDAO2SORuleConverter;
        ...
        private RuleManagement RuleEngineForScope
        {
            get
            {
                ...
                {
                    var rule = mDAO2SORuleConverter.GetScopeDocumentRule();
                ...
                if (mRuleEngineForDocumentTag == null)
                {
                    ...
                    if (mConfig.UseAospArchiverProfile)
                    {
                        ...
                        foreach (var rule in rules)
                        {
                            ...
                            //else
                            //{
                            realRules.Add(key, rule);
                            //}
                            key++;
                            //}
                        }
                        mConfig.RuleCollection = realRules;
                        mConfig.currentRule = new Rule();
                    }
                    Dictionary<int, Rule> Rules = new Dictionary<int, Rule>();
                    int i = 1;
                    foreach (var rule in rules)
        ...
        private RuleManagement RuleEngineForDocumentVersionTag
        {
            get
            {
                ...
                {
                    var rules = mDAO2SORuleConverter.GetDocumentVersionTagRules();
                    if (mConfig.UseAospArchiverProfile)
                    {
                        mLog.Info("current rule is discover archiver rule,will use archiver profile");
                        int key = 1;
                        if (mConfig.RuleCollection == null)
                        {
                            mConfig.RuleCollection = new Dictionary<int, Rule>();
                        }
                        else
                        {
                            key = mConfig.RuleCollection.Count + 1;
                        }
                        foreach (var rule in rules)
                        {
                            mConfig.RuleCollection.Add(key, rule);
                            key++;
                        }
                        mConfig.currentRule = new Rule();
                    }
        ...
        public AnalysisOptimizationAOSPDiscoverNodeWorker(ScanJobSettings jobSettings, ScheduleConfiguration paraConfig, IBackwardDependencyNodeCache<object> dependencyObjs) : base(jobSettings, paraConfig, dependencyObjs)
        {
            ScheduleContainer = new BackupNodeCache(pcContainer);
            mConfig = paraConfig;
            if (mConfig.UseAospArchiverProfile)
            {
                mApprovalReportProxy = new BackwardDependenceNodeCache<ArchiveApproveReport>(new ApprovalReportService(mConfig));
            }
            else
            {
            ...
            mDAO2SORuleConverter = new DAO2SORuleConverter(mConfig.RMDiscoveryAOSPOptimizationSetting, mConfig.UseAospArchiverProfile);
        }
        ...
        public override async Task<ProcessResult> ProcessItemAsync(ArchiverNodeItem item, ArchiverNodeItem parent)
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.AnalysisOptimizationDiscoverNodeWorker.RealProcessItem"))
            {
                ...
                else
                {
                    ...
                    if (mConfig.UseAospArchiverProfile)
                    {
                        if (item.ItemType == ArchiverCommon.ItemType.DOCUMENT)
                        {
                            ...
                            if (resultRule != null)
                            {
                                ...
                                item.RuleId = resultRule.Id;
                                itemApprove.RuleId = resultRule.Id;
                                mApprovalReportProxy.PutIn(itemApprove, item.Cache_NodeType, item.ShouldDoArchive);
                                JobExecutionProcessStatisticExecutor.Instance.CalculateRuleAndScanSummary(result, itemApprove, resultRule);
                            }
                            else
                            {
                                mApprovalReportProxy.PutIn(itemApprove, item.Cache_NodeType, item.ShouldDoArchive);
                        ...
                        else
                        {
                            if (parent.ShouldDoArchive)
                            {
                                item.ShouldDoArchive = true;
                                itemApprove.DoDelete = true;
                                item.RuleId = parent.RuleId;
                                itemApprove.RuleId = parent.RuleId;
                                mApprovalReportProxy.PutIn(itemApprove, item.Cache_NodeType, item.ShouldDoArchive);
                                JobExecutionProcessStatisticExecutor.Instance.CalculateRuleAndScanSummary(result, itemApprove, mConfig.currentRule);
                            }
                            else if (item.ItemType == ArchiverCommon.ItemType.DOCUMENT_VER)
                            {
                    ...
                    else if (mConfig.currentRule.PolicyLevel == GCommon.Contract.CommonFilter.PolicyLevel.Document)
                    {
                        if (item.ItemType == ArchiverCommon.ItemType.DOCUMENT)
                        {
        ...
        private Rule CheckItemRuleForScope(ArchiverNodeItem item)
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.ScanDiscovrerNodeWorker.CheckItemRule"))
            {
                ...
                if (result != null)
                {
                    result.DeleteRecords = mConfig.currentRule == null?false:mConfig.currentRule.DeleteRecords;
