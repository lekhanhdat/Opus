...
using AvePoint.ObjectModel.Server13.List;
...
namespace AvePoint.ObjectModel.Server13
{
    class AveItem : IAveItem, IDisposable
    {
        ...
        private static readonly List<string> suffixNeedRemoveWhenApproval = new List<string>() { ".doc", ".msg", ".ppt", ".xls", ".vsd", ".mht", ".xml" };
        ...
        internal AveReportingService mReportingFile;
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveItem.CreateANewFileOrVersion"))
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            file = web.GetFile(folderServerRelativeUrl + fileName);
                        }
                        else
                        {
                            file = web.GetFile(folderServerRelativeUrl + "/" + fileName);
                ...
                if (file != null && file.Exists)
                {
                    ...
                    if (file.UIVersion > uiVersion)
                    {
                        ...
                        {
                            logger.Warn("Failed to restore only the historical versions  {0}. To restore historical versions in SharePoint 2013, select the desired versions along with the current version, and then perform the restore job.", fileName);
                            throw new AveWrapperSkipException(AveInternalResourceKey.Wrapper_Exception_Server13_FaileRestoreHistoricalVersions);//已经国际化
                ...
                try
                {
                    var method = typeof(SPListItem).GetMethod("UpdateInternal", BindingFlags.NonPublic | BindingFlags.CreateInstance | BindingFlags.Instance | BindingFlags.Public
                        , null, new Type[]{typeof(bool),typeof(bool),typeof(Guid),typeof(bool),typeof(bool),typeof(bool),
                            typeof(bool), typeof(bool),typeof(bool), typeof(string), typeof(bool)}, null);
                    ...
                    method.Invoke(item, new object[] {bSystem, bPrseverItemVersion, Guid.Empty, true,
                            bPublish , bNoVersion, false, false, false, null, bPrseverItemVersion});
                }
                catch (Exception ex)
                {
        ...
        public SPFile LoadCheckOutFile(SPWeb spWeb, SPFolder folder, string fileName)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveItem.LoadCheckOutFile_2"))
            {
                ...
                var file = spWeb.GetFile(fileServerRelativeUrl);
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveItem.AddFileWithStream"))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            file = folder.Files.Add(fileName, new byte[] { }, true);
                        }
                        info.IsNewCreated = true;
                    }
                }
        ...
        private SPFile AddRealCheckoutFile(SPListTemplateType listType, SPFolder folder, string fileName, IAveRestoreStream receiver, Hashtable HTMetaInfo, AveDocumentInfo docInfo)
        {
            ...
            if (folder.ParentWeb.CurrentUser != null && folder.ParentWeb.CurrentUser.ID != info.CheckoutUserId)
            {
                ...
                checkoutFolder = checkoutWeb.GetFolder(folder.ServerRelativeUrl);
            }
            else
            {
                list = mSPList;
            ...
            else
            {
                ...
                {
                    file = checkoutFolder.Files.Add(fileName, new byte[] { }, true);
                }
                info.IsNewCreated = true;
            }
            return file;
        ...
        private SPFile CreateMasterPage(SPFolder folder, string fileName)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveItem.CreateMasterPage"))
            {
                ...
                try
                {
                    ...
                    file = folder.ParentWeb.GetFile(strNewUrl);
                }
                catch (Exception e)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (HTMetaInfo.Count != 0)
                        {
                            ...
                            {
                                file = folder.Files.Add(fileName, stream, HTMetaInfo, true);
                            }
                        }
                        else
                        {
        ...
        private SPFile CreateOrUpdateFileAndItem(Stream stream, string fileName)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveItem.CreateOrUpdateFileAndItem"))
            {
                ...
                Type[] types = new Type[]{typeof(string), typeof(string), typeof(Guid), typeof(object),
                    typeof(int), typeof(int), typeof(string), typeof(string),
                    typeof(int),typeof(int), typeof(int), typeof(object),
                    typeof(object), typeof(object),typeof(string), typeof(int),
                    typeof(string), typeof(uint), typeof(string) };
                ...
                object[] pars = new object[] { webUrl, webRelativeUrl, uniqueId, lockBytes, length, 0, null, null, 0, 0, 1, (object)0, (object)0, null, "", rowId, listId, num, str3 };
                AveAssemblyUtility.InvokeMethod(request, "CreateOrUpdateFileAndItem", pars);
                SPFile file = mWeb.Web.GetFile(webRelativeUrl);
                fileStream.Close();
                return file;
        ...
        private SPFile CreateAGhostedPage(string name, string setupPath, bool needForceCheckout, string path)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveItem.CreateAGhostedPage"))
            {
                ...
                {
                    IAveCommonRequest request = mWeb.Request;
                    object[] args = new object[] { setupPath, (byte)mWeb.Site.CompatibilityLevel, 0, Guid.Empty, Guid.NewGuid(), 0, 0, string.Empty, path, name, false, false, true, new object() };
                    Type[] paramTypes = new Type[] { args[0].GetType(), args[1].GetType(), args[2].GetType(), Type.GetType("System.Guid&"), Type.GetType("System.Guid&"), args[5].GetType(), args[6].GetType(), args[7].GetType(), args[8].GetType(), args[9].GetType(), args[10].GetType(), args[11].GetType(), args[12].GetType(), args[13].GetType() };
                    request.SetGhostedFile(args, paramTypes);
                    Guid docId = (Guid)args[4];
                    file = mWeb.Web.GetFile(docId);
                }
        ...
        private bool MoveListItemToConflictFolder(SPList parentList, SPListItem listItem, SPFolder parentFolder, SPFolder conflictFolder, DateTime lastModified, bool isSourceWin)
        {
            try
            {
                ...
                SPFile file = parentList.ParentWeb.GetFile(listItem.Url);
                ...
                if (file.Level != SPFileLevel.Checkout)
                {
                    if (!this.HasFullControlPermission && isCheckOut)
                    {
                        if (Web.CurrentUser.ID != info.CheckoutUserId)
                        {
                            ...
                            file = checkoutWeb.GetFile(file.ServerRelativeUrl);
                ...
                if (restoreContent && info.HasStream)
                {
                    try
                    {
                        if (HTMetaInfo.Count != 0)
                        {
                            ...
                            {
                                file = checkoutFolder != null ? checkoutFolder.Files.Add(file.Name, stream, HTMetaInfo, true)
                                    : folder.Files.Add(file.Name, stream, HTMetaInfo, true);
                            }
                        }
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    file.SaveBinary(stream, false, true, null, null, null, out etage);
                                }
                            }
                        }
                    }
        ...
        private void CreateFolderVersion(int originalVersion, bool deleteBaseVersion)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveItem.CreateFolderVersion"))
            {
                ...
                int preVersion = -1;
                UpdateListSettings(mSPList, info, originalVersion % 512 == 0, false);
                try
                {
                    //一些情况下开启Approval功能会导致update异常
                    if (mSPList.BaseType == SPBaseType.DocumentLibrary && mSPList.BaseTemplate != SPListTemplateType.NoCodePublic
                    && !mSPList.EnableModeration && !mSPList.HasExternalDataSource
                    && mSPList.BaseTemplate != SPListTemplateType.Survey && !(mSPList.ParentWeb.WebTemplate == "MPS"
                    && (mSPList.BaseTemplate == SPListTemplateType.Meetings || mSPList.BaseTemplate == SPListTemplateType.HomePageLibrary)))
                    {
                ...
                //添加对类似1.0 2.0 3.1这种version的特殊处理
                if (originalVersion - info.Version == 513)
                {
                ...
                int updateTime = 0;
                //mSPListItem.ModerationInformation.Status = SPModerationStatusType.Pending;
                bool firstIn = true;
                bool rename = false;
                string name = string.Empty;
                try
                {
                    name = mSPListItem["Name"].ToString();
                }
                catch (Exception ex)
                {
                    logger.Log(AveLogLevel.WARN, "Can not get folder value of column Name. Error:{0}", ex.ToString());
                }
                //添加对类似1.0 2.0 4.0 这种version的特殊处理
                //remove this condition when folder moderation status is draft(API only) || ((SPModerationStatusType)info.ModerationStatus != SPModerationStatusType.Denied && (SPModerationStatusType)info.ModerationStatus != mSPListItem.ModerationInformation.Status)
                while (originalVersion > info.Version)
                {
                    if (!firstIn)
                    {
                        if (mSPListItem.ModerationInformation.Status == SPModerationStatusType.Pending)
                        {
                            //ADO-113367 在sharepoint2013环境的library中，如果folder的名字以.doc结尾，对该folder执行approve操作时会出现异常，添加rename处理。
                            if (!rename && CheckEndWithSpecifySuffix(name))
                            {
                                mSPListItem["Name"] = name + "RENAME";
                                mSPListItem.SystemUpdate(false);
                                rename = true;
                            }
                ...
                if (rename)
                {
                    mSPListItem["Name"] = name;
                    mSPListItem.SystemUpdate(false);
                }
                //delete middle versions
                DeleteMiddleVersions(versionLabels);
                //update folder level, when moderation status is enabled, update will change status to pending.
                info.Level = (int)mSPListItem.Level;
            }

        }

        private bool CheckEndWithSpecifySuffix(string name)
        {
            foreach (var subffix in suffixNeedRemoveWhenApproval)
            {
                if (name.EndsWith(subffix, StringComparison.OrdinalIgnoreCase))
                {
                    return true;
                }
            }
        ...
        public void CreateItemVersion(int originalVersion, bool deleteBaseVersion)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveItem.CreateItemVersion"))
            {
                ...
                //For replicator realtime to restore large version item
                ...
                if (isDenied)
                {
                    SPFile file = mSPListItem.Web.GetFile(mSPListItem.Url);
                    file.TakeOffline();
                }
                //delete middle versions
                DeleteMiddleVersions(versionLabels);
        ...
        private void ReloadItem()
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    logger.Debug("Failed to reload survey item. Error:{0}", ex);
        ...
        internal void UpdateFolderModerationStatus(AveFolderInfo info)
        {
            try
            {
                ...
                //if (mSPListItem.ModerationInformation != null && mSPListItem.ModerationInformation.Status != (SPModerationStatusType)info.ModerationStatus)
                if (mSPListItem.ModerationInformation != null && (mSPListItem.ModerationInformation.Status != (SPModerationStatusType)info.ModerationStatus || (mSPListItem.ModerationInformation.Status == 0 && mSPListItem.Level != SPFileLevel.Published)))
                {
                    ...
                    //ADO-113367 在sharepoint2013环境的library中，如果folder的名字以.doc结尾，对该folder执行approve操作时会出现异常，添加rename处理。
                    bool rename = false;
                    if (CheckEndWithSpecifySuffix(info.Name)
                        && info.ModerationStatus == 0 && mSPList.BaseType == SPBaseType.DocumentLibrary)
                    {
                        mSPListItem["Name"] = info.Name + "RENAME";
                        mSPListItem.SystemUpdate(false);
                        rename = true;
                    }
                    try
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveItem.GetFolder"))
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        folder = parentFolder.ParentWeb.GetFolder(parentFolder.ServerRelativeUrl.TrimEnd('/') + "/" + info.Name);
