...
{
    public abstract class AbstractDriveItemRestore
    {
        protected static readonly AveLogger mLog = AveLogger.GetInstance(typeof(AbstractDriveItemRestore));
        ...
        protected GDriveRestoreRequest AssembleRestoreMessage(string subjobId, GoogleDriveTreeNodeDto TreeRoot, GDriveRestoreSettingAndTree mRestore)
        {
            using var _ = new PerformanceScope("AbstractDriveItemRestore.AssembleRestoreMessage");
            var message = new GDriveRestoreRequest();
            ArchiverSiteMasterIndexContract siteinfo = new ArchiverSiteMasterIndexContract();
            this.GetIndexInfo(TreeRoot, siteinfo);
            var allSiteMasterIndexes = ArchiverSiteMasterIndexService.GetGoogleDriveWithSubInfos(siteinfo);
            ...
            var request = this.BuildRestoreRequest(allSiteMasterIndexes, TreeRoot, mRestore, subjobId);
            ...
            message = request;
            SOGDriveArchiverJobInfoStatistics.Instance.InitGDriveInstance(subjobId, siteinfo.SiteURL, mJobType, siteinfo.SiteId);
            SOGDriveArchiverJobInfoStatistics.Instance.KeepDataOption = -2;
            SOGDriveArchiverJobInfoStatistics.Instance.IsDeleteOnlyActionOrRestore = true;
            var volumeGenerator = new VolumeGeneratorFactory().GetVolumeGenerator(ProductModule.GDriveArchiverBackup);
        ...
        protected static CacheSettingDto GenerateCacheSettings()
        {
            ...
            {
                Path = Path.Combine(RecordsEnv.AppDomainRootFolder, "ArchiverCache", GDriveRestoreConfig.CACHE_DATA_FOLDER_NAME),
        ...
        private GDriveRestoreRequest BuildRestoreRequest(List<ArchiverSiteMasterIndexContract> indexes, GoogleDriveTreeNodeDto TreeRoot, GDriveRestoreSettingAndTree mRestore, string subjobId)
        {
            ...
            var request = new GDriveRestoreRequest();
            ...
            request.DriveId = index.SiteId;
            request.DriveName = index.SiteURL;
            request.TenantId = index.WebId;
            request.IsSharedDrive = !index.SiteURL.Contains("@");
            ...
            if (mJobType == JobType.SimulateRestore)
            {
            ...
            else
            {
                ...
                if (storage != null)
                {
                    ...
                    DownloadDataInfoDao.CreateZipPasswordInfo(new AvePoint.RA.DB.Model.RMDownloadDataInfo() { Name = encryptPassword, JobId = subjobId, FileDownloadTime = DateTime.UtcNow.Ticks, DownloadType = DownloadContentType.ZipPasswordInfo });
                }
                else
                {
        ...
        private ArchiverSiteMasterIndexContract GetIndexInfo(GoogleDriveTreeNodeDto node, ArchiverSiteMasterIndexContract index)
        {
            ...
            //{
            index.SiteId = node.ObjectId;
            index.SiteURL = node.FullPath;
            //}
            //if (node.Children != null)
            //{
            //    foreach (GoogleDriveTreeNodeDto child in node.Children)
            //    {
