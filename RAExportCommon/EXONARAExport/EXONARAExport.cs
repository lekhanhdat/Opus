...
{
    internal class EXONARAExport : EXOExportBase, IEXOExport
    {
        ...
        private EXONARAColumnContainer columnArray = null;
        ...
        {
            HasInitObjs = true;
            columnArray = new EXONARAColumnContainer();
        }

        public EXONARAExport(PhysicalDeviceDto deviceDto, string jobId, string disposalClass, VaultExportFormat format, byte[] NARAConfigFile, byte[] encryptionKey, byte[] encryptionIV)
        ...
        {
            EXONARAData.InitConfig(NARAConfigFile);
            mDisposalClass = disposalClass;
        }
        
        public EXONARAExport(SharePointLocationDto spoDto,AveBPOSAccountInfo user, string siteUrl, string jobId, string disposalClass, VaultExportFormat format, byte[] NARAConfigFile, byte[] encryptionKey, byte[] encryptionIV)
        ...
        {
            mLog.Info(VaultLogFormat.LOGWITHPATH, "Start Export EXOFolder(NARA).", EXOFolder.Id.ToString());
        ...
        {
            mLog.Info(VaultLogFormat.LOGWITHPATH, "Start Export EXOItem(NARA).", EXOItem.Id.ToString());
            try
            {
                ExportStatus exportStatus = new ExportStatus() { State = ExportState.Failed };
                lock (lockObj)
                ...
                {
                    hashString = GetHashStringFromFileContent(docStream);
                ...
                CsvMetaData metaData = new CsvMetaData
                {
                    CsvMetadataInfo = columnArray.GetCSVListFromColumnValue(EXOItem, this.DisposalClass, exportPath, info.MailFullPath, tempFilePath, info.JobID, info.service,hashString)
                };
                this.csvMetadatas.Add(metaData);
                exportStatus.State = ExportState.Succeed;
                return exportStatus;
            ...
            {
                mLog.Error(VaultLogFormat.LOGWITHEXCEPTIONPATH, "An error occurred while export ExportEXOItem(NARA).", EXOItem.Id.ToString(), ex.ToString());
        ...
        private string GetHashStringFromFileContent(Stream fileStream)
        {
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                fileContentByte = memoryStream.ToArray();
            }
            using SHA256 alg = SHA256.Create();
            byte[] hash = alg.ComputeHash(fileContentByte);
        ...
        private void SaveSignature(Stream csvStream, EXOExportInfo medatainfo)
        {
            ...
            if (setting.EnableExportSignature)
            {
                ...
                EXOExportInfo signatureFile = new EXOExportInfo();
        ...
        {
            mLog.Info(VaultLogFormat.LOGWITHPATH, "Start Export EXOItem(NARA).", EXOItem.ItemId.ToString());
            try
            {
                ExportStatus exportStatus = new ExportStatus() { State = ExportState.Failed };
                lock (lockObj)
            ...
            {
                mLog.Error(VaultLogFormat.LOGWITHEXCEPTIONPATH, "An error occurred while export ExportEXOItem(NARA).", EXOItem.ItemId.ToString(), ex.ToString());
                return new ExportStatus() { State = ExportState.Failed, ErrorMessage = ex.Message.ToString() };
            }
        }
    }
