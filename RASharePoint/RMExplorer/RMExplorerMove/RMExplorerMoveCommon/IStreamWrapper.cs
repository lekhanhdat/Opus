...
namespace AvePoint.RA.SharePoint.RMExplorer
{
    ...
    internal class FileReceiverWrapper : IInputStreamWrapper
    {
        private IFileReceiver receiver;
        public FileReceiverWrapper(IFileReceiver receiver)
        {
        ...
        public string GetFileTail()
        {
            ...
            while (this.receiver.ReadBytes(temp, temp.Length) != 0) ;
            return this.receiver.GetFileTail();
        }

        public string GetNextFileHead()
        {
            return this.receiver.GetNextFileHead();
        }

        public int ReadContent(byte[] buffer, int offset, int len)
        {
            return this.receiver.ReadBytes(buffer, offset, len);
        }
