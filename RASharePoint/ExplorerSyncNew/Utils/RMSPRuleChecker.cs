...
namespace AvePoint.RA.SharePoint.ExplorerSync.Utils
{
    ...
    {
        protected static readonly AvePoint.RA.Contract.Services.IRALogger logger = RALogger.GetInstance(typeof(RMSPRuleChecker));
        ...
        public SyncItemRuleInfo CheckDisposalRule(IAveListItem aveItem, SyncItemRuleInfo parentItemRule = null)
        {
            ...
            if (NeedCheckRule(aveItem?.Url))
            {
                ...
                {
                    using (new RA.Common.PerformanceScope(string.Format("CollectionData.Scan.Item.CheckItemCretia:{0}", aveItem?.ID/*discoverItem.FullUrl*/)))
                    {
                    ...
                    {
                        logger.Info($"Get due disposal info for item. UniqueId:{aveItem?.UniqueId}");
                        string disposalAction = string.Empty;
