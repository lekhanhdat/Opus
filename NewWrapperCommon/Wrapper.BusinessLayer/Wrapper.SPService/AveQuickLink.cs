...
namespace AvePoint.Wrapper.SPService
{
    public class AveQuickLink
    {
        ...
        public void CreateQuickLinks(List<AveQuickLinkInfo> linkInfos)
        {
            ...
            if (WrapperConfiguration.DeleteDestinationUserProfileLink)
            {
                try
                {
                    foreach (IAveOQuickLink link in mUserProfile.UserProfile.QuickLinks.GetItems())
                    {
                        try
                        {
                            mLog.Info("Link title is {0},mUserProfile.ExistSkip is {1},link url is {2},link privacyLevel is {3},link group is {4}.", link.Title, mUserProfile.ExistSkip.ToString(), link.Url, link.PrivacyLevel.ToString(), link.Group);
                            link.Delete();
                        }
                        catch (Exception ex)
                        {
                            mLog.Warn("An error occured when deleining link,ex: {0}", ex);
                        }
                    }
                }
                catch (Exception ex2)
                {
                    mLog.Warn("An error occured when deleining all link,ex: {0}",  ex2);
                }
            }
        ...
        public void CreateQuickLink(AveQuickLinkInfo linkInfo)
        {
            ...
            try
            {
                ...
                foreach (IAveOQuickLink link in mUserProfile.UserProfile.QuickLinks.GetItems())
                {
                    if (link.Title == linkInfo.Title && link.Url == linkInfo.Url && link.Group == linkInfo.Group && link.GroupType == linkInfo.GroupType)// xe.Attributes["NameValue"].Value)
                    {
                ...
                {
                    mLog.Log(AveLogLevel.ERROR, "WP10RTMySite0531 {0}", e);
                }
                quickLink.Commit();
            }
            catch (Exception e)
            {
                mLog.Log(AveLogLevel.ERROR, "WP10RTMySite0530 , error: {0}", e);
            }
        }
    }
}
