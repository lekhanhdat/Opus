...
namespace AvePoint.ObjectModel.Common.Office
{
    class AveOSocialTagManager : AveAbstractCommonCollection<IAveOSocialTag>, IAveOSocialTagManager
    {
        ...
        {
            mRequest = (context as AveServiceContext).Request;
        }
        ...
        public IAveOSocialTag[] GetTags(IAveOUserProfile user)
        {
            ...
            List<Dictionary<string, object>> socialTagList = SocialTagProp[AveObjectModelConstant.ChildrenProperties] as List<Dictionary<string, object>>;
            IAveOSocialTag[] socialTags = new IAveOSocialTag[socialTagList.Count];
            int i = 0;
            foreach (Dictionary<string, object> socialTagProp in socialTagList)
            {
                AveOSocialTag socialTag = new AveOSocialTag(this.mRequest, profile, socialTagProp);
        ...
        {
            mRequest.AddTag(url.ToString(), term.ID, tagTitle, isPrivate);
        }
        ...
        {
            mRequest.DeleteTag(uri.ToString(), term.ID);
        }
    }
}
