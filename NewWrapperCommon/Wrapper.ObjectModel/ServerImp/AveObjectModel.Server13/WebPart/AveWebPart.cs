...
namespace AvePoint.ObjectModel.Server13
{
    ...
    public class AveWebPart : IAveWebPart
    {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveWebPart.RealRestore"))
            {
                ...
                try
                {
                        ...
                        && WrapperConfiguration.SkipInWssWebPartLists.Exists(type => string.Equals(type, webPartType.Trim(), StringComparison.OrdinalIgnoreCase)))
                    {
                        ...
                        mReport.AddDetail(new AveWrapperWebpartReportDto(webPartBaseInfo.DisplayName, webPartBaseInfo.DisplayName, webPartBaseInfo, assemblyName, webPartType, AveStatus.Skipped, AveReportResource.Wrapper_Report_CannotFindWebPartAssembly, webPartBaseInfo.WebPartTypeId));
                        return false;
                    }
        ...
        protected bool RealRestoreCore()
        {
            ...
            if (!internalAdd && this.Manager.HasFullControlPermission)
            {
                ...
                {
                    UpdatePropertiesByNative(webPartId);
                }
            }
            return result;
        }
        ...
        protected byte[] ReplaceViewFields(byte[] view, IAveFieldMapping fieldMapping, SPList list, ref bool needPostRestore)
        {
            ...
            if (!string.IsNullOrEmpty(viewString))
            {
                ...
                {
                    logger.Log(AveLogLevel.WARN, "Failed to load view string. Error:{0}", ex.ToString());
                }
        ...
        private bool IsSandBoxSolutionWebPart()
        {
            ...
            {
                // add for app part
                if (this.webPartBaseInfo.Assembly.Equals("Microsoft.SharePoint, Version=15.0.0.0, Culture=neutral, PublicKeyToken=71e9bce111e9429c", StringComparison.OrdinalIgnoreCase) &&
                    this.webPartBaseInfo.Class.Equals("Microsoft.SharePoint.WebPartPages.ClientWebPart", StringComparison.OrdinalIgnoreCase))
                {
                    flag = false;
                }
            }
        ...
        protected void EnsureAssemblyInfo()
        {
            ...
            {//Sandboxed Solutions Webpart.
                assemblyName = "Microsoft.SharePoint, Version=15.0.0.0, Culture=neutral, PublicKeyToken=71e9bce111e9429c";
        ...
        protected virtual void UpdateView()
        {
            ...
            //ADO-171441 此Case 中webpart 的contenttypeid 也需要替换，才能link 到discussion 的subject 上
        ...
        protected void RealRestorePersonalization(System.Web.UI.WebControls.WebParts.WebPart webPart, AvePersonalizationInfo personalInfo, int userId)
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveWebPart.RealRestorePersonalization"))
            {
                ...
                if (result != 0 && personalInfo.PerUserProperties != null)
                {
                    ...
                    {
                        var webpartId = this.manager.GetStorageKey(webPart, true);
                        UpdatePersonalPropertiesByNative(webpartId, personalInfo.PerUserProperties);
                    }
                }
        ...
        private bool RestoreWebPartProperties(Dictionary<string, object> properties)
        {
            ...
            //Disable the search box setting if the backup data is SP10. Search box setting does not exist in SP10 Mode.
            if (!hasInplaceSearchEnabled && xsltListViewWebpart != null && manager.Web.Site.CompatibilityLevel > 14)
            {
    ...
    public class SpecialWebPartUpdater
    {
        ...
        public static SpecialWebPartUpdater GetWebPartUpdater(System.Web.UI.WebControls.WebParts.WebPart webPart, AveWebPart aveWebPart)
        {
            try
            {
                ...
                var updater = AveCustomWebPartUpdaterUtility.GetWebPartUpdater(webPart, aveWebPart);
                if (updater == null)
                {
                    Type subType = Type.GetType(string.Format("AvePoint.ObjectModel.Server13.Ave{0}Updater", webPart.GetType().Name), false, true);
                    if (subType != null)
                    {
                        updater = subType.GetConstructor(new Type[] { typeof(System.Web.UI.WebControls.WebParts.WebPart), typeof(AveWebPart) }).Invoke(new object[] { webPart, aveWebPart }) as SpecialWebPartUpdater;
                    }
                }
                return updater;
            }
    ...
    {
        public IListWebPartUpdater(System.Web.UI.WebControls.WebParts.WebPart webPart, AveWebPart aveWebPart)
    ...
    {
        public AveXsltListViewWebPartUpdater(System.Web.UI.WebControls.WebParts.WebPart webPart, AveWebPart aveWebPart)
            : base(webPart, aveWebPart) { }
    ...
    class AveKPIListWebPartUpdater : SpecialWebPartUpdater
    {
        ...
        public override bool DoUpateAfterAdd(AveWebPartBaseInfo webPartInfo)
        {
            ...
            {
                logger.Log(AveLogLevel.WARN, ServerAPIResource.KPIWebPartUpdateFailed, e);
                return false;
            }
        }
    }
