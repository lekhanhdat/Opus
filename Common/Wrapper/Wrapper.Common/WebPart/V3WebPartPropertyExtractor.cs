    ...
    public class V3WebPartPropertyExtractor : WebPartPropertyExtractorBase
    {
        ...
        public override bool RemoveAndAddNewProperty(bool properties, string propertyName, object value)
        {
            if (properties)
            {
                ...
                var propertyNode = base.webpartDefinition.SelectSingleNode(string.Format("d:data/d:properties/d:property[@name='{0}']", propertyName), v3NameSpaceManager);
            ...
            else
            {
                ...
                var propertyNode = base.webpartDefinition.OwnerDocument.SelectSingleNode($"/webParts/{propertyName}", v3NameSpaceManager);
                if (propertyNode != null)
                {
                    webPartsNode.RemoveChild(propertyNode);
                }
            }
