...
namespace AvePoint.Common.FilterEngine
{
    internal class TreeNodeFilterEngine : FilterEngineBase
    {
        ...
        public TreeNodeFilterEngine(List<FilterPolicy> policyLists, Dictionary<PolicyLevel, string> filterConditionExpressionLists, FilterEngine engine)
        ...
        protected override bool IsQualified(ObjectInfoBase objectInfo, FilterPolicy policy)
        {
            ...
            {
                return StringConditionChecker.IsQualified(policy.Condition, treeNodeInfo.Name, policy.Value);
            }
            else if (policy.Rule is UrlRule)
            {
                return StringConditionChecker.IsQualified(policy.Condition, treeNodeInfo.Url, policy.Value);
            }
            else
            {
