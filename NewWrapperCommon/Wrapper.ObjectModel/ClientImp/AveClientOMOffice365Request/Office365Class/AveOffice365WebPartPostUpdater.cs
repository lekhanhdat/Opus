...
{
    internal class AveOffice365WebPartPostUpdater
    {
        ...
        public static AveOffice365WebPartPostUpdater CreateInstance(Guid webPartId, AveWebPartBaseInfo webPartInfo, IAveWeb parentWeb, string fileUrl, ITokenProvider tokenProvider)
        {
            switch (AveClientWebPartUrlHandlerFactory.GetWebPartTypeId(webPartInfo.WebPartTypeId))
            {
                case AveWebPartType.BrowserFormWebPart:
                    return new Office365BrowserFormWebPartPostUpdater(webPartId, webPartInfo, parentWeb, fileUrl, tokenProvider);
                default:
                    return new AveOffice365WebPartPostUpdater(webPartId, webPartInfo, parentWeb, fileUrl, tokenProvider);
            }
        }

        public AveOffice365WebPartPostUpdater(Guid webPartId, AveWebPartBaseInfo webPartInfo, IAveWeb parentWeb, string fileUrl, ITokenProvider tokenProvider)
        {
            WebPartId = webPartId;
    ...
    internal class Office365BrowserFormWebPartPostUpdater : AveOffice365WebPartPostUpdater
    {
        public Office365BrowserFormWebPartPostUpdater(Guid webPartId, AveWebPartBaseInfo webPartInfo, IAveWeb parentWeb, string fileUrl, ITokenProvider tokenProvider)
            : base(webPartId, webPartInfo, parentWeb, fileUrl, tokenProvider)
        {
        }
        ...
        public override void PostUpdate()
        {
            ...
            try
            {
                ...
                {
                    using (AveWebServiceNetWork mNetWork = new AveWebServiceNetWork(null, webUrl, null,tokenProvider))
                    {
                        ...
                        string result = AveHttpWebRequestUtility.HttpReturn(url, null, "application/x-vermeer-urlencoded", body, headerInformation, "MSFrontPage/15.0", tokenProvider,false);
                    }
                }
            }
            catch (Exception e)
