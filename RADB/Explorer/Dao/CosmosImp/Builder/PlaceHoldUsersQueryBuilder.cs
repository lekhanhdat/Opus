...
{
    public class PlaceHoldUsersQueryBuilder : IFilterBuilder, IAdvancedQueryBuilder
    {
        ...
        private ExplorerFilterOptionV2 Convert2SearchOptionV2(ExplorerQueryColumn column, string objJson, ExplorerSearchColumnOperationLogic columnOperationLogic, ExplorerSearchKeyOperationLogic keyOperationLogic)
        {
            ...
            {
                HoldBy = JsonConvert.DeserializeObject<List<AOSUserDto>>(objJson)
            };
        }
            ...
            if (CanFilter(filterOption))
            {
                ...
                foreach (var value in values)
                {
                    if (!isFirst)
                    {
                        ...
                        isFirst = false;
                    string searchValue = $"%\"HoldBy\":\"{value.UserPrincipalName}\"%";
                    //string searchValue = $"%{value.UserPrincipalName}%";
                    query.WhereLike(GetFilterColumnName().FormatColumnName(), searchValue);
                }
            }
            return query;
        }
        ...
        {
            return filterOption != null && filterOption.HoldBy != null;
        }

        private string GetColumnId()
        {
            return Contract.TemplateManagement.QueryCloumnIds.HoldByUsersId;
        }

        private string GetFilterColumnName()
        {
            return CosmosConst.C_HoldByUsers;
        }

        private List<AOSUserDto> GetFilterValue(ExplorerFilterOptionV2 filterOption)
        {
            return filterOption.HoldBy;
        }
    }
}
