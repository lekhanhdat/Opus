    ...
    class AveFieldUrlValue : AveClientObject, IAveFieldUrlValue
    {
        ...
        public AveFieldUrlValue(string fieldValue)
        {
            if (!string.IsNullOrEmpty(fieldValue))
            {
                ...
                {
                    base.DataCache.PropertiesCache["Url"] = fieldValue.Substring(0, length);
                    base.DataCache.PropertiesCache["Url"] = this.Url.Replace(",,", ",");
                    length += 2;
                    if (length < fieldValue.Length)
                    {
                        base.DataCache.PropertiesCache["Description"] = fieldValue.Substring(length, fieldValue.Length - length);
                    }
                }
                else
                {
                    base.DataCache.PropertiesCache["Url"] = fieldValue.Replace(",,", ",");
                    base.DataCache.PropertiesCache["Description"] = fieldValue.Replace(",,", ",");
                }
            }
