...
namespace AvePoint.ObjectModel.Common
{
    class AveOSocialCommentManager : AveAbstractCommonCollection<IAveOSocialComment>, IAveOSocialCommentManager
    {
        ...
        public IAveOSocialComment[] GetComments(IAveOUserProfile user)
        {
            ...
            List<Dictionary<string, object>> socialCommentList = socialCommentProps[AveObjectModelConstant.ChildrenProperties] as List<Dictionary<string, object>>;
            IAveOSocialComment[] socialComments = new IAveOSocialComment[socialCommentList.Count];
            int i = 0;
            foreach (Dictionary<string, object> socialCommentProp in socialCommentList)
            {
                AveOSocialComment socialComment = new AveOSocialComment(this.mRequest, profile, socialCommentProp);
        ...
        {
            mRequest.AddComment(url.ToString(), comment, isHighPriority, title);
        }
