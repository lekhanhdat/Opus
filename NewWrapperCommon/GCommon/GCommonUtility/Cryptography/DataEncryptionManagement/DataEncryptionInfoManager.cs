...
namespace AvePoint.GCommon.Utility.Cryptography.DataEncryptionManagement
{
    public static class DataEncryptionInfoManager
    {
        ...
        //这里缓存动态key时暂时使用了Dictionary，正在查找比较好的替代方案，需要均衡key 缓存的数量,查找插入相关性能，会尽快进行修改.
        static DynamicKeyInfo dynamicKeyInfo = new DynamicKeyInfo();
        static IDynamicKeyDecrypter decrypter;
        ...
        private static bool isSOModule = false;
        ...
        /// <summary>
        /// Get all Security profiles from Control service. and put profiles into encryptionInfoTable.
        /// this method is used to StorageManger , we suggest other modules don't use this method.
        /// </summary>
        static public void InitForStorageManager()
        {
            if (securityService != null)
            {
                //Get all Security profile from control ervice
                var profiles = securityService.GetAllDataEncryptionAndHistorey();
                if (profiles != null && profiles.Count > 0)
                {
                    lock (tableLock)
                    {
                        foreach (var encryptionProfile in profiles)
                        {
                            DataEncryptionInfoWrapper wrapper = null;
                            if (encryptionProfile.EncryptionInfo != null
                                && encryptionProfile.EncryptionInfo != null
                                && !string.IsNullOrEmpty(encryptionProfile.EncryptionInfo.ProtectionGuid))
                            {
                                //add security profile infomation into cache.
                                if (!encryptionInfoTable.TryGetValue(encryptionProfile.EncryptionInfo.ProtectionGuid, out wrapper))
                                {
                                    wrapper = ConvertWrapper(encryptionProfile);
                                    encryptionInfoTable.Add(encryptionProfile.EncryptionInfo.ProtectionGuid, wrapper);
                                }
                            }
                        }
                        isSOModule = true;
        ...
        static public DataEncryptionInfoWrapper PutEncryptionInfo(DataEncryptionProfile profile)
        {

            DataEncryptionInfoWrapper wrapper = DataEncryptionUtil.CreateDataEncryptionInfoWrapper(profile);
            lock (tableLock)
            {
            if (!encryptionInfoTable.ContainsKey(wrapper.EncryptionInfo.ProtectionGuid))
            {
        ...
        static private DataEncryptionInfoWrapper GetDataEncryptionWrapperFromControl(DataEncryptionInfo info, bool isById)
        {
            ...
            if (securityService != null)
            {
                ...
                {
                    //In So module will check the cache secrity profile and try to find in cache.
                    if (isSOModule)
                    {
                        DataEncryptionInfoWrapper wrapper = null;
                        if (encryptionInfoTable.TryGetValue(info.ProtectionGuid, out wrapper))
                        {
                            profile = UnWrapperInfoFromCache(info, wrapper);
                        }
                    }
                    if (profile == null)
                    {
        ...
        static private AvePoint.GCommon.Contract.Server.ControlPanel.Cryptography.Wrapper.DataEncryptionInfoWrapper UnWrapperInfoFromCache(DataEncryptionInfo info, DataEncryptionInfoWrapper wrapper)
        {
            AvePoint.GCommon.Contract.Server.ControlPanel.Cryptography.Wrapper.DataEncryptionInfoWrapper profile = null;
            if (wrapper.EncryptionInfo != null && wrapper.DynamicKey != null)
            {
                try
                {
                    var securityProfilekey = CspCommunicationWrapper.UnWrapKey(wrapper.DynamicKey);
                    IEncryption encryption = EncryptionFactory.GetEncryption((EncryptionAlgorithm)wrapper.EncryptionInfo.EncryptionType, securityProfilekey, HashAlgorithmFactory.CreateHashAlgorithm(HashAlgorithm.SHA1).ComputeHash(securityProfilekey));
                    profile = new Contract.Server.ControlPanel.Cryptography.Wrapper.DataEncryptionInfoWrapper();
                    profile.EncryptionInfo = info;
                    profile.DynamicKey = CspCommunicationWrapper.WrapKeyToBase64String(encryption.DecryptBinary(info.EncryptedDynamicKey));
