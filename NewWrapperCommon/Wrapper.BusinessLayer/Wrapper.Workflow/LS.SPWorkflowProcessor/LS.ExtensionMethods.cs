...
namespace LS.SPWorkflowProcessor
{
    ...
    public static class ExtensionMethods
    {
        ...
        internal static string ToUpperEx(this string source, int startIndex, int len)
        {
            ...
            builder.Append(source.Substring(startIndex, len).ToUpper(CultureInfo.CurrentCulture));
            if((len+startIndex)<source.Length)
        ...
        internal static bool IsContainsBinaryBitValue(this int source, int position)
        {
            int var_0 = source % (position*2);
            return (var_0 / position) == 1;
        }
    }

    public static class StringExtension
    {
        public static bool Contains(this string source, string chekString, StringComparison comparison)
        {
            return source.IndexOf(chekString, comparison) >= 0;
        }
    }
}
