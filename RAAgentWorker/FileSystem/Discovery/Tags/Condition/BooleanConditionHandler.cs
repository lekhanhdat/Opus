...
{
    internal class BooleanConditionHandler : ConditionHandler
    {
        public override ConditionCategory Category => ConditionCategory.BooleanLogic;

        public override bool Handle(ConditionInfo info, object dataObject)
        {
            var logic = (BooleanConditionLogicType)info.Logic;
            var conditionValue = bool.Parse(info.Value);
            ...
            switch (logic)
            {
                case BooleanConditionLogicType.IsEmpty:
                    return conditionValue ? string.IsNullOrWhiteSpace(dataValue) : !string.IsNullOrWhiteSpace(dataValue);
                default:
