    ...
    public class AveClientTaskRetryHelper : AveTaskRetryHelper
    {
        ...
        public override void ExecuteWithRetryMechanism(Action taskToRetry)
        {
            ...
            while (attemptCount++ < MAX_RETRY_ATTEMPT_COUNT)
            {
                ...
                /*review-qlluo*/catch (ServerException e)
                {
                    DateTime end = DateTime.Now;
                    if (attemptCount == MAX_RETRY_ATTEMPT_COUNT)
                    {
                        logger.Error("Failed to retry from an exception ( {0} time(s) retried ), error message : {1}, ServerErrorCode: {2},TimeCost:{3}", MAX_RETRY_ATTEMPT_COUNT, e.ToString(), e.ServerErrorCode,end-start);
                        throw;
                    }
                    if (NeedRetryServerException(e))
                    {
                        logger.Warn("Retry from an exception ( {0} time(s) retried ) , error message : {1}, ServerErrorCode: {2},TimeCost:{3}", attemptCount + 1, e.Message, e.ServerErrorCode, end - start);
                        System.Threading.Thread.Sleep(WrapperConfiguration.WrapperConfigurationForBPOS.RetryInterval);
                        continue;
                    }
                    logger.Warn("Retry mechanism encountered unexcepted exception, error message : {0}, ServerErrorCode: {1},TimeCost:{2}", e.ToString(), e.ServerErrorCode,end - start);
                    throw;
                }
                /*review-qlluo*/catch (Exception e)
                {
                    ...
                    if (attemptCount == MAX_RETRY_ATTEMPT_COUNT)
                    {
                        logger.Error("Failed to retry from an exception ( {0} time(s) retried ), error message : {1},TimeCost:{2}", MAX_RETRY_ATTEMPT_COUNT, e.ToString(), end - start);
                        throw;
                    }
                    //里面checkCommonException又sleep了一遍，不过总共也就多了外面RetryInterval的5s，先不动
                    if (NeedRetry(e))
                    {
                        logger.Warn("Retry from an exception ( {0} time(s) retried ) , error message : {1},TimeCost:{2}", attemptCount + 1, e.Message, end - start);
                        System.Threading.Thread.Sleep(WrapperConfiguration.WrapperConfigurationForBPOS.RetryInterval);
                        continue;
                    }
                    logger.Warn("Retry mechanism encountered unexcepted exception, error message : {0},TimeCost:{1}", e.ToString(), end - start);
                    throw;
                }
            }
        }
