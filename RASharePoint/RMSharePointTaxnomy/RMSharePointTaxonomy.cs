...
namespace AvePoint.RA.SharePoint.RMSharePointTaxnomy
{
    ...
    public class RMSharePointTaxonomy : IDisposable
    {
        ...
        public void InitClientContext(RemoteSiteCollection site)
        {
            ...
            IsAppAuth = (site.AuthType == GCommon.Contract.CentralAdmin.Object.BposConnectionType.AppToken || 
                site.AuthType == GCommon.Contract.CentralAdmin.Object.BposConnectionType.Modern);
            mTaxonomySession = TaxonomySession.GetTaxonomySession(mClientcontext);
            mClientcontext.Load(mTaxonomySession);
            mClientcontext.ExecuteQuery();
        }
        ...
        private void LoadBasicInfoForTermStore(TermStore termStore)
        {
            ...
            {
                mLcid = termStore.WorkingLanguage;
            }
        ...
        private async System.Threading.Tasks.Task CreateTermSetAsync(TermGroup defaultGroup, RMTermSet rmTermSet)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    AddJobDetailForFailedTerm(rmTermSet.Name, "RM_TS_Action_Update", JobDetailsStatus.Failed, "RM_TS_TermSetRepeatOrDenyOrMaxLimitReached", rmTermSet.subTerms);
                }
                return;
            }
        ...
        private void UpdateTerm(Term term, RMTerm rmTerm, bool needChange)
        {
            ...
            var legalRmTermName = GetLegalName(rmTerm.Name);
            bool isChanged = CheckIsChanged(termName, rmTerm.Name, termDes, rmTerm.Description);

            logger.Info(
                "UpdateTerm compare. TermId:[{0}], SharePointName:[{1}], OpusName:[{2}], OpusLegalName:[{3}], SharePointDescription:[{4}], OpusDescription:[{5}], CheckIsChanged:[{6}], NeedChange:[{7}]",
                rmTerm.UniqueId,
                termName ?? string.Empty,
                rmTerm.Name ?? string.Empty,
                legalRmTermName ?? string.Empty,
                termDes ?? string.Empty,
                rmTerm.Description ?? string.Empty,
                isChanged,
                needChange);

            //判断这个term和之前同步的相比是否有变化
            if (isChanged || needChange)
            {
                try
                {
