...
{
    class AveContentReplacer
    {
        private static AveLogger logger = AveLogger.GetInstance(typeof(AveContentReplacer));

        private IAveSite mSite;
        private IAveFile mFile;
        private int mVersion;
        AveDocumentInfo mDocumentInfo;
        Stream mStream;

        public AveContentReplacer(IAveSite site, IAveFile file, AveDocumentInfo info)
        {
            mSite = site;
            mFile = file;
        ...
        public AveContentReplacer(IAveSite site, Stream stream, AveDocumentInfo info)
        {
            mSite = site;
            mVersion = info.OriginalVersion;
            mDocumentInfo = info;
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveSPDocContentReplacer.ReplaceWebPartContent"))
            {
                ...
                try
                {
                    ...
                    if (!mDocumentInfo.Name.EndsWith(".aspx", StringComparison.OrdinalIgnoreCase)
                        ...
                        && !mDocumentInfo.Name.EndsWith(".html", StringComparison.OrdinalIgnoreCase)
                        )
                    {
                        return mStream;
                    }
                    if(mDocumentInfo.IsLinkFile)
                    {
                        return mStream;
                    }
                    byte[] content = null;
                    ...
                    if (mFile != null)
                    {
                        ...
                        {
                            content = mFile.OpenBinary(AveOpenBinaryOptions.SkipVirusScan);
                        }
                        else
                        {
                            IAveFileVersion fileVersion = mFile.Versions.GetVersionFromID(mVersion);
                    ...
                    //For CI-34202 The page content has value  href="javascript:%20{}"  , the %20 is our encoding result,we need revert it.
                    else if (mDocumentInfo.Name.EndsWith(".html", StringComparison.OrdinalIgnoreCase))
                    {
                        contentStr = System.Web.HttpUtility.UrlDecode(contentStr);
                        strChanged = true;
                    }
                    else
                    {
        ...
        private string GetInnerText(string content, int index, string tag)
        {
        ...
        private Dictionary<string, List<string>> GetBindingAndDataSourceDic(string contentStr, int index)
        {
