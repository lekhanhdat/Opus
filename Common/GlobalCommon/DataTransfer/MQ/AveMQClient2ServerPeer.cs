    ...
    internal class AveMQClient2ServerPeer
    {
        ...
        public void SendMessage(AveMessage message, int msgTimeout = 30 * 1000)
        {
            ...
            while (true)
            {
                ...
                //retryCount++;
                Thread.Sleep(AveMQConfigure.NoReconnectTimeOut * 1000);//MQ重连成功结束之后一定时间内不会重连，所以避免失误，这里需要等待相应的时间，确保下次判断的时候可以重连
            }
            //}
        }
        /// <summary>
        ...
        private bool Init(bool requireCallback)
        {
            ...
            {
                if (AveMQConfigure.IsOneWayConnection)
                {
                    messageChannel = MessageChannelFactory.GetMessageChannel(host, port, relatedBaseUri, jobId, null);
                }
                else
                {
                    messageChannel = MessageChannelFactory.GetMessageChannel(host, port, relatedBaseUri, jobId, new AveMQClientCallback(client));
                }
        ...
        private void ReopenConnection()
        {
            ...
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        Thread.Sleep(AveMQConfigure.ReconnectionTime * 1000);
                    }
                }
        ...
        private void ReopenConnectionWithLock()
        {
            lock (syncObj)
            {
                if (lastReconnectionSuccessTime.AddSeconds(AveMQConfigure.NoReconnectTimeOut) > DateTime.Now)
                {
                    Thread.Sleep(AveMQConfigure.NoReconnectTimeOut * 1000);
                }
