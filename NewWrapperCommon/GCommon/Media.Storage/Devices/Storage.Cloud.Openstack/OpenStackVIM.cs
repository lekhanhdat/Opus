...
namespace AvePoint.Media.Storage.Cloud.OpenStack
{
    ...
    #endregion
    [VIM(VIMName.OpenStack, typeof(AvePoint.Media.Storage.Cloud.OpenStack.OpenStackVIM))]
    class OpenStackVIM : AbstractVIM
    {
        public override IXSystem CreateSystem(string xri, AbstractXSystem parentSystem)
        {
            return new OpenStackSystem(xri, parentSystem);
        }

        public override List<string> GetFeatureXML(int type)
        {
            return OpenStackFeature.Getstances(type).FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(int type)
        {
            return OpenStackFeature.Getstances(type).FeatureObjs;
        }

        public override List<StorageFeature> GetFeatureObj(int type, string culture)
        {
            return OpenStackFeature.Getstances(type, culture).FeatureObjs;
        }
    }
}
