...
namespace AvePoint.Wrapper.Restore
{
    public class AvePublishing
    {
        ...
        public static AveObjectModelFactory Factory
        {
            get
            {
                if (mFactory == null)
        ...
        public static void SetWelcomePage(IAveWeb web, string welcomePageUrl)
        {
            try
            {
                ...
                {
                    using (IAvePublishingWeb currentPublishingWeb = GetPublishingWeb(web))
                    {
        ...
        {
            if (AveEnv.IsPublishing || Factory.ContextKind == AveContextKind.ClientObjectModel)
            {
        ...
        public static void RemoveHold(IAveListItem item)
        {
            IAveOHold hold = Factory.CreateHold();
            List<IAveListItem> holds = hold.GetHolds(item);
            if (holds != null && holds.Count > 0)
            {
                foreach (IAveListItem holdItem in holds)
                {
                    hold.RemoveHold(item, holdItem, "");
                }
            }
        }
    }
            ...
            get
            {
                if (mFactory == null)
                {
                    lock (mLock)
                    {
                        if (mFactory == null)
                        {
                            mFactory = AveObjectModelFactory.CreateObjectModelFactory("", null, AveContextKind.Auto);
                        }
                    }
                }
                return mFactory;
