...
namespace RAExportCommon
{
    ...
    internal class VaultExportBase : IDisposable
    {
        protected ExportMode CurrentExportMode;
        protected IAveVaultExport RealVaultExport = null;
        protected Dictionary<string, IAveVaultExport> MultileVaultExport = null;
        ...
        private void Init(PhysicalDeviceDto deviceDto, string jobId, VaultExportFormat format, byte[] encryptionKey, byte[] encryptionIV)
        {
            ...
            RealVaultExport = new AveVaultExport(deviceDto, jobId, exportFormat, encryptionKey, encryptionIV);
        }
        ...
        private void Init(SharePointLocationDto spoDto,AveBPOSAccountInfo user, string siteUrl, string jobId, VaultExportFormat format, byte[] encryptionKey, byte[] encryptionIV)
        {
            ...
            RealVaultExport = new AveVaultExport(spoDto, user, siteUrl, jobId, exportFormat, encryptionKey, encryptionIV);
        }
        ...
        private void Init(List<PhysicalDeviceDto> deviceDtos, string jobId, VaultExportFormat format, byte[] encryptionKey, byte[] encryptionIV)
        {
            ...
            MultileVaultExport = new Dictionary<string, IAveVaultExport>();
            foreach (PhysicalDeviceDto Dev in deviceDtos)
            {
                ...
                IAveVaultExport VaultExport = new AveVaultExport(Dev, jobId, exportFormt, encryptionKey, encryptionIV);
                MultileVaultExport.Add(Dev.Id, VaultExport);
            }
        }

        public VaultExportBase(PhysicalDeviceDto deviceDto, string jobId, VaultExportFormat format, byte[] encryptionKey, byte[] encryptionIV)
        {
            Init(deviceDto, jobId, format, encryptionKey, encryptionIV);
        }
        
        public VaultExportBase(SharePointLocationDto spoDto,AveBPOSAccountInfo user, string siteUrl, string jobId, VaultExportFormat format, byte[] encryptionKey, byte[] encryptionIV)
        {
            Init(spoDto,user, siteUrl, jobId, format, encryptionKey, encryptionIV);
        }

        public VaultExportBase(List<PhysicalDeviceDto> deviceDtoWithRule, string jobId, VaultExportFormat format, byte[] encryptionKey, byte[] encryptionIV)
        {
