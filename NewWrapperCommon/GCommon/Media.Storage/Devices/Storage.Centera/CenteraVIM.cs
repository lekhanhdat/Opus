...
namespace AvePoint.Media.Storage.Centera
{
    ...
    [VIM(VIMName.Centera, typeof(AvePoint.Media.Storage.Centera.CenteraVIM))]
    class CenteraVIM : AbstractVIM
    {
        #region VIM Members

        public override IXSystem CreateSystem(String xri, AbstractXSystem parentSystem)
        {
            return new CenteraSystem(xri, parentSystem);
        }

        public override List<string> GetFeatureXML(Int32 type)
        {
            return CenteraFeature.Getstances(type).FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(Int32 type)
        {
            return CenteraFeature.Getstances(type).FeatureObjs;
        }

        public override List<StorageFeature> GetFeatureObj(Int32 type, String culture)
        {
            return CenteraFeature.Getstances(type, culture).FeatureObjs;
        }
