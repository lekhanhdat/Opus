...
{
    public class ItemRestoreConfig : RestoreConfig
    {
        ...
        internal ContainerReportTracker ContainerReportTracker { get; set; }

        public ItemRestoreConfig() { }
        ...
        public void LoadMigrationPreferences(IRMKeyValueDao dao, IRMGlobalKeyValueDao globalDao = null)
        {
            if (dao == null)
            {
                return;
            ...
            MigrationImportJobTimeOutMinutes = dao.GetMigrationImportJobTimeOutMinutes();
            MigrationImportJobCount = dao.GetMigrationImportJobCount();
            MigrationImportJobPackageCountCapacity = dao.GetMigrationImportJobPackageCountCapacity();
            MigrationImportJobPackageSizeCapacity = dao.GetMigrationImportJobPackageSizeCapacity();
            // If migration import job tenant-level setting is not set, fallback to DC-level setting for backward compatibility.
            if (TeamsRestoreState.IsEnableMigrationImportJob.HasValue)
            {
                EnableMigrationImportJob = TeamsRestoreState.IsEnableMigrationImportJob.Value;
            }
            else
            {
                var migrationJobSetting = dao.GetMigrationImportJobSetting();
                if (migrationJobSetting is not null)
                {
                    EnableMigrationImportJob = bool.TryParse(migrationJobSetting.Value, out var result) && result;
                }
                else if (globalDao is not null)
                {
                    EnableMigrationImportJob = globalDao.IsMigrationImportJobEnabled();
                }
            }
            ResetMajorVersionApprovalStatus = dao.ShouldResetMajorVersionApprovalStatus();
            log.Info($"LoadMigrationPreferences.MigrationImportJobTimeOutMinutes:{MigrationImportJobTimeOutMinutes}." +
                $"MigrationImportJobCount:{MigrationImportJobCount}." +
                $"MigrationImportJobPackageCountCapacity:{MigrationImportJobPackageCountCapacity}." +
                $"MigrationImportJobPackageSizeCapacity:{MigrationImportJobPackageSizeCapacity}." +
                $"EnableMigrationImportJob:{EnableMigrationImportJob}." +
                $"ResetMajorVersionApprovalStatus:{ResetMajorVersionApprovalStatus}.");
        }
        ...
        #region Migration Import Preferences

        public int MigrationImportJobTimeOutMinutes { get; private set; }

        public int MigrationImportJobCount { get; private set; }

        public int MigrationImportJobPackageCountCapacity { get; private set; }

        public int MigrationImportJobPackageSizeCapacity { get; private set; }

        public bool EnableMigrationImportJob { get; private set; }

        public bool ResetMajorVersionApprovalStatus { get; private set; }

        public bool IsUsingMigrationImportJob { get; set; }
        ...
        public ArchiverRestoreRequest ArchiverConfigForMedia { get; set; }
        ...
        public override string ToString()
        {
            ...
            config.AppendFormat("IsSpecifyUser: {0}\r\n", IsSpecifyUser);
            config.AppendFormat("SpecifyUser: {0}\r\n", SpecifyUser);
    ...
    public class BposSiteCollectionsConfig : IDisposable, ISingleton
    {
        ...
        private async System.Threading.Tasks.Task InitBPOSAccountInfosAsync(IAveTreeNodeDto rootNode)
        {
            if (rootNode.Level == NodeLevel.SiteCollection)
            {
                ...
                var mapping = WrapperConfiguration.WrapperConfigurationForBPOS.IsRestoreToSPOLibOrFolder ? null : RMRestoreSiteMappingDao.GetMappingBySourceSiteUrl(siteUrl);
                ...
                if (rootNode.NodeExtension != null)
                {
                    ...
                    sw.Stop();
                    log.Info($"linkRestoreReport InitBPOSAccountInfosAsync query db cost time :{sw.ElapsedMilliseconds}");
                    if (remoteSiteCollection == null)
                    {
                        var profiles = RMAosApiClient.GetHasADPermissionProfiles(TenantLocalValue.LogonGroupId).OrderByDescending(p => p.DomainName.Length).ToList();
                        ...
                        if (!string.IsNullOrEmpty(tenantId))
                        {
                            ...
                            CustomerAppForSensitivityLabel = PoolUserUtil.GetCustomAppProfilesForSensitivityLabel(tenantId);
                        }
                    }
                    else
                    {
                        ...
                        sw2.Start();
                        AveBPOSAccountInfo aveBPOSAccountInfo = await PoolUserUtil.GetBPOSInfo2Async(remoteSiteCollection);
                        ...
                        sw2.Stop();
                        log.Info($"linkRestoreReport InitBPOSAccountInfosAsync call aos api get app cost time:{sw2.ElapsedMilliseconds}");
                    }
        ...
        private async System.Threading.Tasks.Task InitAOSPBPOSAccountInfosAsync(IAveTreeNodeDto rootNode, string appProfileId, string siteAdminUrl)
        {
            if (rootNode.Level == NodeLevel.SiteCollection)
            {
                ...
                if (rootNode.NodeExtension != null)
                {
                    ...
                    BPOSAccountInfos[siteUrl] = aveBPOSAccountInfo;
                    log.Info($"InitAOSPBPOSAccountInfosAsync siteAccount profiles is:ClientId:{aveBPOSAccountInfo?.ClientId},AuthenticationProfileId:{aveBPOSAccountInfo?.AuthenticationProfileId},tenantId:{aveBPOSAccountInfo?.TenantId}.siteAdminUrl:{siteAdminUrl}.");
        ...
        public async System.Threading.Tasks.Task InitBPOSAccountInfoAsync(string siteUrl)
        {
            if (BPOSAccountInfos.ContainsKey(siteUrl))
            {
                log.Info($"BPOSAccountInfos already contains siteUrl:{siteUrl}, no need to init again.");
                return;
            }
            Stopwatch sw2 = new Stopwatch();
            sw2.Start();
            RemoteSiteCollection remoteSiteCollection = RABrowserClient.GetRemoteSiteCollectionByUrl(siteUrl);
            AveBPOSAccountInfo aveBPOSAccountInfo;
            //if (!string.IsNullOrWhiteSpace(appProfileId) && !string.IsNullOrWhiteSpace(siteAdminUrl))
            //{
            //    aveBPOSAccountInfo = await PoolUserUtil.GetAOSPBPOSInfoAsync(appProfileId, siteAdminUrl);
            //}
            //else
            //{
                aveBPOSAccountInfo = await PoolUserUtil.GetBPOSInfo2Async(remoteSiteCollection);
            //}
            BPOSAccountInfos[siteUrl] = aveBPOSAccountInfo;
            log.Info($"GetBPOSInfoAsync siteAccount profiles is:ClientId:{aveBPOSAccountInfo?.ClientId},AuthenticationProfileId:{aveBPOSAccountInfo?.AuthenticationProfileId},tenantId:{aveBPOSAccountInfo?.TenantId}");
            ServicesAccounts = PoolUserUtil.GetSAInfoFromAOS(remoteSiteCollection.TenantId);
