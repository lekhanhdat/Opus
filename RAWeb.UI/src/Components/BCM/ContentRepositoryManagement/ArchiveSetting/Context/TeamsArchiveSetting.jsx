...
export default {
    getContext() {
        return {
            configurations: {
                createRuleComponentType: CRComponentType.TeamsSetting,
                isRequire: true,
                showRuleLevel: true,
    ...
    saveArchiveSetting(that, callback) {
        ...
        currNode.SupportLockedSite = stateData.isSupportLockedSite;
        currNode.SupportArchivedTeams = stateData.isIncludeArchivedTeamsChecked;

        $$.loading(true);
        let sendUrl = "/api/TeamsSettingApi/SaveArchiverNodeSetting";
        let option = {
            url: sendUrl,
            method: "Post",
            data: currNode
