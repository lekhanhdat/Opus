...
{
    public class MigrationGenerator : TelemetryGenerator
    {
        public override TelemetryModule Module => TelemetryModule.Magration;
        ...
        protected override Dictionary<TelemetryEventType, Func<IList<object>, CloudRecordsCommonRecord>> EventTypeGenerateMapping =>
            ...
            {
                { TelemetryEventType.MigrationJob, GetMigrationJob },
            };

        public CloudRecordsCommonRecord GetMigrationJob(IList<object> args)
        {
            var record = new CloudRecordsReportJobExecutionRecord();
            if (!string.IsNullOrEmpty(args[0].ToString()))
            {
                _ = long.TryParse(args[0].ToString(), out var result);
                record.EndTime = new DateTime(result);
            }
            return record;
        }
    }
