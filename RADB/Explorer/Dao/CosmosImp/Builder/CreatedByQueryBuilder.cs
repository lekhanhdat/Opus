...
{
    public class CreatedByQueryBuilder : BaseArrayFilterBuilder
    {
        //private static List<string> builtInOrDefaultColumnIds = new List<string> { DefaultColumnIDs.CreatedBy, BuildInColumnIDs.CreatedBy};

        protected override string GetFilterColumnName()
        {
            return CosmosConst.C_CreatedByLower;
        }

        protected override bool CanFilter(ExplorerFilterOptionV2 filterOption)
        {
            return filterOption != null && filterOption.CreatedBy != null && filterOption.CreatedBy.Count > 0;

        }

        protected override object GetFilterValue(ExplorerFilterOptionV2 filterOption)
        {
            return filterOption.CreatedBy.Select(o => o.DisplayName.ToLower()).ToList();
        }
        ...
        protected override ExplorerFilterOptionV2 Convert2SearchOptionV2(ExplorerQueryColumn column, string objJson, ExplorerSearchColumnOperationLogic columnOperationLogic, ExplorerSearchKeyOperationLogic keyOperationLogic)
        {
            ...
            {
                CreatedBy = JsonConvert.DeserializeObject<List<AOSUserDto>>(objJson)
            };
        }

        protected override string GetColumnId()
        {
            return QueryCloumnIds.CreatedBy;
        }
    }
}
