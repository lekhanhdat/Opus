...
namespace AvePoint.ObjectModel.Common
{
    internal class AveListUserResource : AveUserResource
    {
        ...
        protected override void RetrieveValuesForAllLanguage()
        {
            if (mWeb.SupportedUICultures.Count() == 1)
            {
                switch (mResourceName)
                {
                    case AveUserResourceConstants.TITLE_RESOUCE:
                        cultureAndValueMappings[mWeb.LanguageCulture.Name] = mList.Title;
                        break;
                    case AveUserResourceConstants.DESCRIPTION_RESOUCE:
                        cultureAndValueMappings[mWeb.LanguageCulture.Name] = mList.Description;
                        break;
                    default:
            ...
            {
                var mappings = mRequest.GetListUserResource(mWeb.ServerRelativeUrl, mList.ID, mResourceName,
