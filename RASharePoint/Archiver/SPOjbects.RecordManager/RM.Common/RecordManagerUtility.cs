...
{
    public class RecordManagerUtility
    {
        ...
        public string ConvertToXML(List<FieldDataInfo> itemFieldsInfo, string srcUrl)
        {
            try
            {
                ...
                foreach (FieldDataInfo fieldInfo in itemFieldsInfo)
                {
                    ...
                    {
                        mLog.Warn(string.Format("Get Field failed ,field name : {0} , field value : {1}, Reason : {2}", fieldInfo.DisplayName, fieldInfo.Value, ex.ToString()));
                    }
                }
