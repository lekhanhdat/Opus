...
namespace AvePoint.Wrapper.Common
{
    internal class LiveIdentityManager
    {
        ...
        private readonly AzureNativeMethods AzureNativeMethods;
        private const int ResultCode = 0;
        private Guid serviceGuid;

        // Methods
        public LiveIdentityManager() : this(new AzureNativeMethods())
        {
        }

        internal LiveIdentityManager(AzureNativeMethods nativeLibrary)
        {
            this.identityPtr = IntPtr.Zero;
            this.serviceGuid = Guid.NewGuid();
            this.AzureNativeMethods = nativeLibrary;
        }

        internal void CloseIdentity()
        {
            if ((IntPtr.Zero != this.identityPtr) && (this.AzureNativeMethods.CloseIdentityHandle(this.identityPtr) == 0))
            {
                this.identityPtr = IntPtr.Zero;
            }
        }
        ...
        {
            if ((IntPtr.Zero != resource) && (this.AzureNativeMethods.PassportFreeMemory(resource) == 0))
            {
        ...
        internal void GetAuthenticationStatus(string siteName)
        {
            ...
            try
            {
                errorCodeParameter = this.AzureNativeMethods.GetAuthenticationStatus(this.identityPtr, siteName, 1, out zero);
                ...
                AzureNativeMethods.IdcrlStatusCurrent structure = new AzureNativeMethods.IdcrlStatusCurrent();
        ...
        {
            IntPtr ptr;
            string str = WindowsIdentity.GetCurrent().Name.ToLower(CultureInfo.InvariantCulture);
            string message = "Enumerating identities to get upn of the user {0}";
            Logger.WriteTracingInfo("ConnectMsolService", "GetLoggedOnUser", message, new object[] { str });
            string[] strArray = str.Split(new char[] { '\\' });
            if ((strArray.Length == 1) && str.Contains("@"))
            {
                return str;
            }
            int num = this.AzureNativeMethods.EnumIdentitiesWithCachedCredentials(null, out ptr);
            message = "EnumIdentitiesWithCachedCredentials returned {0}";
            Logger.WriteTracingInfo("ConnectMsolService", "GetLoggedOnUser", message, new object[] { num });
            string wszMemberName = null;
            string str4 = null;
            if (num == 0)
            {
                while (this.AzureNativeMethods.NextIdentity(ptr, ref wszMemberName) == 0)
                {
                    message = "Next identity returned to get upn of the user {0}";
                    Logger.WriteTracingInfo("ConnectMsolService", "GetLoggedOnUser", message, new object[] { wszMemberName });
            ...
            this.AzureNativeMethods.CloseEnumIdentitiesHandle(ptr);
            if ((num != 0) || (str4 == null))
            {
                throw new WindowsLiveException(num, Messages.FailAuthIdentityToService);
            }
            return str4;
        }
        ...
        {
            this.Uninitialize();
            AzureNativeMethods.IdcrlOption[] pOptions = null;
            uint dwOptions = 0;
            ...
            try
            {
                ...
                {
                    pOptions = new AzureNativeMethods.IdcrlOption[1];
                    dwOptions = 1;
                ...
                int errorCodeParameter = 0;
                errorCodeParameter = this.AzureNativeMethods.InitializeEx(ref this.serviceGuid, 1, 0, pOptions, dwOptions);
                if (errorCodeParameter < 0)
                {
                    ...
                    WindowsLiveException exception = new WindowsLiveException(errorCodeParameter, message);
                    throw exception;
                }
        ...
        {
            AzureNativeMethods.RstParams[] pcRSTParams = new AzureNativeMethods.RstParams[1];
            ...
            int errorCodeParameter = this.AzureNativeMethods.LogonIdentityEx(this.identityPtr, policy, 0, pcRSTParams, (uint) pcRSTParams.Length);
        ...
        internal virtual string LogonService(string siteName, string policy)
        {
            ...
            int errorCodeParameter = 0;
            errorCodeParameter = this.AzureNativeMethods.AuthIdentityToService(this.identityPtr, siteName, policy, 0x20000, out zero, out pdwResultFlags, out ppbSessionKey, out pcbSessionKeyLength);
            if (errorCodeParameter < 0)
            {
        ...
        internal void OpenIdentity(string federationProviderId, string userName, string password)
        {
            ...
            {
                int errorCodeParameter = 0;
                errorCodeParameter = this.AzureNativeMethods.CreateIdentityHandle2(federationProviderId, userName, 0xff, out this.identityPtr);
                if (errorCodeParameter < 0)
                {
                ...
                {
                    errorCodeParameter = this.AzureNativeMethods.SetCredential(this.identityPtr, "ps:password", password);
                    if (errorCodeParameter < 0)
                    {
                        throw new WindowsLiveException(errorCodeParameter, Messages.FailSetCredential);
                    }
                }
            }
            catch
        ...
        internal void Uninitialize()
        {
            ...
            {
                int errorCodeParameter = 0;
                errorCodeParameter = this.AzureNativeMethods.Uninitialize();
                if (errorCodeParameter < 0)
                {
