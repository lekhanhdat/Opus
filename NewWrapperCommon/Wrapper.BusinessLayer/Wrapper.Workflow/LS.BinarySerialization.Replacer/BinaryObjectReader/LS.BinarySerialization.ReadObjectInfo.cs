    ...
    internal sealed class ReadObjectInfo
    {
        ...
        internal MemberInfo GetMemberInfo(string name)
        {
            if (this.cache != null)
            {
                if (this.isSi)
                {
                    throw new SerializationException(string.Format(CultureInfo.CurrentCulture, LSEnvironment.GetResourceString("Serialization_MemberInfo"), new object[] { this.objectType + " " + name }));
                }
                if (this.cache.memberInfos == null)
                {
                    throw new SerializationException(string.Format(CultureInfo.CurrentCulture, LSEnvironment.GetResourceString("Serialization_NoMemberInfo"), new object[] { this.objectType + " " + name }));
                }
        ...
        {
            if (objMember is System.Reflection.FieldInfo)
            {
                return ((System.Reflection.FieldInfo)objMember).FieldType;
            }
            if (!(objMember is PropertyInfo))
            {
                throw new SerializationException(string.Format(CultureInfo.CurrentCulture, LSEnvironment.GetResourceString("Serialization_SerMemberInfo"), new object[] { objMember.GetType() }));
            }
        ...
        {
            ReadObjectInfo info = new ReadObjectInfo();
            info.objectInfoId = readObjectInfoCounter++;
            return info;
        }
        ...
        internal Type GetType(string name)
        {
            ...
            {
                type = (Type)this.memberTypesList[index];
            }
            if (type == null)
            {
                throw new SerializationException(string.Format(CultureInfo.CurrentCulture, LSEnvironment.GetResourceString("Serialization_ISerializableTypes"), new object[] { this.objectType + " " + name }));
            }
            return type;
        }
