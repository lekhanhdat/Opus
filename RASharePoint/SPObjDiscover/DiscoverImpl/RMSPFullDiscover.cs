...
namespace AvePoint.RA.SharePoint.SPObjDiscover.DiscoverImpl
{
    ...
    {
        private static readonly AvePoint.RA.Contract.Services.IRALogger logger = RALogger.GetInstance(typeof(RMSPFullDiscover));
        ...
        public IEnumerable<AveDiscoverItem> GetItems(IAveList list, AveDiscoverFolder folder, ref string pagerInfo)
        {
            ...
            try
            {
                result = folder.GetItems(ref pagerInfo);
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<IAveListItem> GetAllItems(AveDiscoverList list, out long totalCount, List<AveCamlQuery> aveCamlQueries = null)
        {
                ...
                else
                {
                    ThreadPool.QueueUserWorkItem(obj =>
                    {
                        try
                        {
                            ...
                            using (PerformanceScope scope = new PerformanceScope("RMSPFullDiscover.GetAllFolders"))
                            {
                            ...
                            foreach (var discoverFolder in discoverFolders)
                            {
                                foreach (var camlQuery in aveCamlQueries)
                                {
                                    ...
                                    logger.Info("query xml {0}", camlQuery.ViewXml);
            ...
            {
                result.CompleteAdding();
                logger.Error($"Error Get f items by query:{list?.RootFolderUrl}, ERROR:{ex.ToString()}");
            }
