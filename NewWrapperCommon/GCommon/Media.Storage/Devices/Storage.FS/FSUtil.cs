...
namespace AvePoint.Media.Storage.FS
{
    ...
    class AuthUtil
    {
        ...
        private static extern int WNetAddConnection2([MarshalAs(UnmanagedType.LPArray)] NETRESOURCEW[] lpNetResource, [MarshalAs(UnmanagedType.LPWStr)] string lpPassword, [MarshalAs(UnmanagedType.LPWStr)] string UserName, int dwFlags);

        [DllImport("mpr.dll")]
        private static extern int WNetCancelConnection2(string lpName, int dwFlags, bool fForce);

        private static AveLogger logger = AveLogger.GetInstance(typeof(AuthUtil));
        public static void NetUse(string uncPath, string password, string username, bool deleteExists)
        {
            NETRESOURCEW[] n = new NETRESOURCEW[1];
            n[0] = new NETRESOURCEW();
            n[0].dwType = 1;
            n[0].lpLocalName = null;
            n[0].lpRemoteName = uncPath;
            n[0].lpProvider = null;
            int errorCode = WNetAddConnection2(n, password, username, 1);
            if (errorCode != 0)
            {
                if (logger.IsDebugEnabled)
                {
                    logger.Error("NetUse Failed : Error Code {0}, Detail : {1}", errorCode, SystemErrorCodes.ToString(errorCode));
                }
                if (errorCode == SystemErrorCodes.ERROR_SESSION_CREDENTIAL_CONFLICT && deleteExists)
                {
                ...
                errorCode = WNetAddConnection2(n, password, username, 1);
                if (errorCode != 0 && logger.IsDebugEnabled)
                {
                    logger.Error("NetUse Failed After Deleted Old Session : Error Code {0}, Detail : {1}", errorCode, SystemErrorCodes.ToString(errorCode));
                }
            }
            //if (erroCode == SystemErrorCodes.
        }
    }
    ...
    class AccountUtil
    {
        ...
        public static string LookupSidByAccountName(string serverName, string domain, string userName)
        {
            string accountName = userName;
            byte[] Sid = null;
            uint cbSid = 0;
            StringBuilder referencedDomainName = new StringBuilder(domain);
            uint cchReferencedDomainName = (uint)referencedDomainName.Capacity;
            SID_NAME_USE sidUse;

            int err = NO_ERROR;
            if (!LookupAccountName(serverName, accountName, Sid, ref cbSid, referencedDomainName, ref cchReferencedDomainName, out sidUse))
            {
        ...
        public static string GetAcountNameBySid(string serverName, string sid)
        {
            ...
            //new byte[] { 1, 2, 0, 0, 0, 0, 0, 5, 32, 0, 0, 0, 32, 2 };
            ...
            int err = NO_ERROR;
            if (!LookupAccountSid(serverName, SID, name, ref cchName, referencedDomainName, ref cchReferencedDomainName, out sidUse))
            {
            ...
            if (err == 0)
            {
                return string.Format("{0}\\{1}", new object[] { referencedDomainName, name });
            }
            return "";
        }
    ...
    class SystemErrorCodes
    {
        ...
        public const Int32 ERROR_INVALID_SERVICE_ACCOUNT = 1057;    //The account name is invalid or does not exist, or the password is invalid for the account name specified.
        public const Int32 ERROR_INVALID_PASSWORDNAME = 1216;    //The format of the specified password is invalid.
        public const Int32 ERROR_ACCOUNT_RESTRICTION = 1327;//    Logon failure: user account restriction. Possible reasons are blank passwords not allowed, logon hour restrictions, or a policy restriction has been enforced.
        public const Int32 ERROR_PASSWORD_EXPIRED = 1330;//    Logon failure: the specified account password has expired.
        public const Int32 ERROR_NEED_CHANGE_PASSWORD = 1907;
        ...
        public static string ToString(Int32 ERR)
        {
            switch (ERR)
            {
                ...
                case 1057: return "The account name is invalid or does not exist, or the password is invalid for the account name specified.";
                ...
                case 1216: return "The format of the specified password is invalid.";
                ...
                case 1327: return "Logon failure: user account restriction. Possible reasons are blank passwords not allowed, logon hour restrictions, or a policy restriction has been enforced.";
                case 133: return "A JOIN or SUBST command cannot be used for a drive that contains previously joined drives.";
                case 1330: return "Logon failure: the specified account password has expired.";
                ...
                case 151: return "The number of specified semaphore events for Dos" + "M" + "UX".ToLower(CultureInfo.InvariantCulture) + "S" + "EM".ToLower(CultureInfo.InvariantCulture) + "Wait is not correct.";
                case 152: return "Dos" + "M" + "UX".ToLower(CultureInfo.InvariantCulture) + "S" + "EM".ToLower(CultureInfo.InvariantCulture) + "Wait did not execute; too many semaphores are already set.";
                case 153: return "The Dos" + "M" + "UX".ToLower(CultureInfo.InvariantCulture) + "S" + "EM".ToLower(CultureInfo.InvariantCulture) + "Wait list is not correct.";
                ...
                case 1907: return "The user's password must be changed before logging on the first time. ";
                ...
                case 300: return "The " + "OPLOCK".ToLower(CultureInfo.InvariantCulture) + " request is denied.";
                case 301: return "An invalid " + "OPLOCK".ToLower(CultureInfo.InvariantCulture) + " acknowledgment was received by the system.";
    ...
    interface IIdentity : IDisposable
    {
        void ExecuteNetUse(bool doDeleteOld);
        IIdentity Impersonate();
        int GetLastError();
    }

    class FSIdentity : IIdentity
    {
        public AveImpersonator Impersonator { get; set; }
        FSSystem sys;

        public FSIdentity(FSSystem sys)
        {
            this.sys = sys;
        ...
        private long authInterval = 30 * 60 * 1000 * 10000L;

        public void ExecuteNetUse(bool doDeleteOld)
        {
            if (!string.IsNullOrEmpty(sys.SystemLocation) && !string.IsNullOrEmpty(sys.SystemDomain) && !string.IsNullOrEmpty(sys.SystemUserName) && !string.IsNullOrEmpty(sys.SystemPassword))
            {
                if (sys.SystemLocation.StartsWith(FSSystemConst.UNC_FLAG, StringComparison.CurrentCultureIgnoreCase))
                {
                    if (System.DateTime.Now.Ticks - lastAuthTime > authInterval)
                    {
                        AuthUtil.NetUse(sys.SystemLocation, sys.SystemPassword, sys.SystemDomain + "\\" + sys.SystemUserName, doDeleteOld);
                        lastAuthTime = System.DateTime.Now.Ticks;
                    }
                }
            }
        }

        public IIdentity Impersonate()
        {
            FSIdentity identity = new FSIdentity(sys);
            //if (sys.SystemLocation.StartsWith(FSSystemConst.UNC_FLAG, StringComparison.OrdinalIgnoreCase))
            switch (sys.AuthMethod)
            {
                case AuthMethod.NetUse_DeleteOld:
                    ExecuteNetUse(true);
                    break;
                case AuthMethod.NetUse:
                    ExecuteNetUse(false);
                    break;
                case AuthMethod.LogonUser:
                default:
                    if (!string.IsNullOrEmpty(sys.SystemLocation) && !string.IsNullOrEmpty(sys.SystemDomain) && !string.IsNullOrEmpty(sys.SystemUserName) && !string.IsNullOrEmpty(sys.SystemPassword))
                    {
                        if (sys.SystemLocation.StartsWith(FSSystemConst.UNC_FLAG, StringComparison.CurrentCultureIgnoreCase))
                        {
                            if ("nfs_vim".Equals(sys.XriObject.VIM, StringComparison.OrdinalIgnoreCase) ||
                                (sys.XriObject.Params.ContainsKey("NetAppType".ToLower(CultureInfo.InvariantCulture)) && sys.XriObject.Params["NetAppType".ToLower(CultureInfo.InvariantCulture)].Equals("NFS", StringComparison.OrdinalIgnoreCase)))
                            {
                                identity.Impersonator = new AveImpersonator(sys.SystemDomain, sys.SystemUserName,
                                    sys.SystemPassword, false);
                            }
                            else
                            {
                                identity.Impersonator = new AveImpersonator(sys.SystemDomain, sys.SystemUserName, sys.SystemPassword, true);
                            }
                        }
                        //else if (!sys.IsValidate)
                        //{
                        //    identity.Impersonator = new AveImpersonator(sys.SystemDomain, sys.SystemUserName, sys.SystemPassword, false);
                        //}
                        else
                        {
                            identity.Impersonator = new AveImpersonator(sys.SystemDomain, sys.SystemUserName, sys.SystemPassword, false);
        ...
        {
            return AveImpersonator.GetLastErrCode();
        }
    ...
    {
        [DllImport("kernel32.dll")]
        static extern Boolean CreateSymbolicLink(String symlinkPath, String targetPath, Int32 dwFlags);

        public static Boolean ConvertLongPathToSymlink(String symlinkPath, String targetPath)
        {
            return CreateSymbolicLink(symlinkPath, targetPath, 1);
        }
    ...
    class UNCObject : IDisposable
    {
        ...
        public string ShareName { get { return shareName; } set { this.shareName = value; } }
        public string OtherPath { get { return otherPath; } set { this.otherPath = value; } }

        public static UNCObject ValueOf(string uncLocation)
        {
            try
            {
                string pat = @"\\\\([^\\]*)[\\/]+([^\\]*)[\\/]*(.*)";
                Regex reg = new Regex(pat, RegexOptions.IgnoreCase);
                Match m = reg.Match(uncLocation);
                if (m.Success)
                {
                    UNCObject obj = new UNCObject();
                    obj.host = m.Groups[1].Value;
                    if (obj.host.Contains(":"))
                    {
                        obj.host = obj.host.Replace(':', '-').Replace('%', 's') + ".ipv6-literal.net";
                    }
                    obj.shareName = m.Groups[2].Value;
                    obj.otherPath = m.Groups[3].Value;
                    return obj;
                }
        ...
        public string ToLocation()
        {
            ...
            {
                return FSSystemConst.UNC_FLAG + host + FSSystemConst.SEPARATER + shareName;
            }
            return FSSystemConst.UNC_FLAG + host + FSSystemConst.SEPARATER + shareName + FSSystemConst.SEPARATER + otherPath;
        }
    ...
    class FSSystemConst : XSystemConst
    {
        //unc flag
        public const string UNC_FLAG = @"\\";
        ...
        public const int ERROR_INVALID_LEVEL = 124; // unimplemented level for info
        public const int ERROR_INVALID_PARAMETER = 87;
        public const int ERROR_MORE_DATA = 234;
        public const int ERROR_NOT_ENOUGH_MEMORY = 8;
        public const int NERR_BufTooSmall = 2123; // The API return buffer is too small.
        public const int NERR_NetNameNotFound = 2310; // This shared resource does not exist.
        public const int NERR_Success = 0;

        //for securely delete
        public const int SECURELY_DELETE_WRITTEN_COUNT = 7;
    }

    class SharePathUtil
    {
        private static StorageLogger logger = new StorageLogger(typeof(SharePathUtil));
        public static bool IsSameDrive(IXSystem sourceSystem, IXSystem destSystem)
        {
            bool result = false;
            try
            {
                if (!(sourceSystem is FSSystem) || !(destSystem is FSSystem))
                {
                    return false;
                }
                string source = (sourceSystem as FSSystem).GetFormatLocation();
                string dest = (destSystem as FSSystem).GetFormatLocation();
                logger.Info("Validate if same drive, source location: {0}, dest:{1}", source, dest);
            ...
            {
                logger.Warn("Validate if same drive error:{0}", e);
                result = false;
            }
            return result;
        }
    ...
    #region Only for netshare speed

    public class SpeedUtil
    {
        internal static SpeedResults DeserializeXml(String xml)
        {
            XmlSerializer xmlSearializer = new XmlSerializer(typeof(SpeedResults));
            SpeedResults info = (SpeedResults)xmlSearializer.Deserialize(new MemoryStream(Encoding.UTF8.GetBytes(xml)));
            return info;
        }
    }
    ...
    public class Target
    {
        ...
        [XmlElement("WriteBytes")]
        public long WriteBytes { get; set; }
        [XmlElement("WriteCount")]
    ...
    public class Bucket
    {
        ...
        [XmlAttribute("SampleMillisecond")]
        public int SampleMillisecond { get; set; }
    }
    #endregion
}
