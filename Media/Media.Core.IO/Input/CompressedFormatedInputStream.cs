...
namespace AvePoint.Media.Core.IO.Input
{
    ...
    public class CompressedFormatedInputStream : FilterGeneralInputStream
    {
        ...
        private Boolean IsNeedMediaDecompress(bool forItemContent)
        {
            var dateMode = GetDataMode(forItemContent);
            var isMediaCompressedData = (dateMode & GConstants.TransferFlag.MEDIA_COMPRESSED) == GConstants.TransferFlag.MEDIA_COMPRESSED;
            var isAgentCompressedDataButRestoreToFS = (dateMode & GConstants.TransferFlag.AGENT_COMPRESSED) == GConstants.TransferFlag.AGENT_COMPRESSED && CurrentItemIndex.IsRestoreToFS;
        ...
        {
            if (IsNeedMediaDecompress(false))
            {
                ...
                {
                    throw new System.NotSupportedException(string.Format(MediaCoreIOResource.CompressedFormatedInputStreamReadMetaDataPart1Exception, readingState));
                }
            }
            else
            {
