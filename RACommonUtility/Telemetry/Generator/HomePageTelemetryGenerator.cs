...
{
    public class HomePageTelemetryGenerator : TelemetryGenerator
    {

        private readonly IAccountDao AccountDao = PlatformWindsorManager.GetService<IAccountDao>();

        public override TelemetryModule Module => TelemetryModule.HomePage;
        ...
        protected override Dictionary<TelemetryEventType, Func<IList<object>, CloudRecordsCommonRecord>> EventTypeGenerateMapping =>
            ...
            {
                { TelemetryEventType.HomepageLoaded, HomePageLoaded }
            };

        public CloudRecordsCommonRecord HomePageLoaded(IList<object> args)
        {
            var record = new CloudRecordsHomePageRecord();
