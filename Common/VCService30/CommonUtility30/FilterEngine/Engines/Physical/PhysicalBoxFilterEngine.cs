...
namespace AvePoint.Common.FilterEngine
{
    ...
    internal class PhysicalBoxFilterEngine : FilterEngineBase
    {
        ...
        public PhysicalBoxFilterEngine(List<FilterPolicy> policyLists, Dictionary<PolicyLevel, string> filterConditionExpressionLists, FilterEngine engine, bool skipCheckDateTimeMinValue)
        ...
        protected override bool IsQualified(ObjectInfoBase objectInfo, FilterPolicy policy)
        {
            ...
            {
                return StringConditionChecker.IsQualified(policy.Condition, physicalBoxInfo.Name, policy.Value);
            }
            ...
            else if (policy.Rule is CreatedByRule)
            {
                if (policy.Condition == PolicyCondition.DoesNotContains)
                {
                    return StringConditionChecker.IsQualified(policy.Condition, physicalBoxInfo.CreatedByLogonName, policy.Value) && StringConditionChecker.IsQualified(policy.Condition, physicalBoxInfo.CreatedByTitle, policy.Value) && StringConditionChecker.IsQualified(policy.Condition, physicalBoxInfo.CreateByEmail, policy.Value);
                }
                else
                {
                    return StringConditionChecker.IsQualified(policy.Condition, physicalBoxInfo.CreatedByLogonName, policy.Value) || StringConditionChecker.IsQualified(policy.Condition, physicalBoxInfo.CreatedByTitle, policy.Value) || StringConditionChecker.IsQualified(policy.Condition, physicalBoxInfo.CreateByEmail, policy.Value);
            ...
            else if(policy.Rule is LastestFolderDisposalDueDateRule)
            {
                return DateTimeConditionChecker.IsQualified(policy.Condition, physicalBoxInfo.LastestFolderDisposalDueDate, policy.Value, _skipCheckDateTimeMinValue);
            }
            else
            {
