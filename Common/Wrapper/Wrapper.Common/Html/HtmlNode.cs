...
namespace AvePoint.Wrapper.Common
{
    ...
    public class HtmlNode : IXPathNavigable
    {
        ...
        public HtmlNode FirstElementChild
        {
            get
            {
                HtmlNode firstChild = this.FirstChild;
                while (firstChild!= null && firstChild.NodeType != HtmlNodeType.Element)
                {
        ...
        public HtmlNode NextElementSibling
        {
            get
            {
                HtmlNode sibling = this.NextSibling;
        ...
        public int GetAttributeValue(string name, int def)
        {
            ...
            {
                Logger.Warn("convert failed, att.value: {0}, exception: {1}", att.Value, e.ToString());
                return def;
            }
        }
        ...
        public void WriteTo(TextWriter outText)
        {
            ...
            switch (_nodetype)
            {
                ...
                case HtmlNodeType.Element:
                    ...
                    if (!HasChildNodes)
                    {
                        if (_endnode != null && _ownerdocument.OptionKeepOriginalSelfCloseTagFormat && !_ownerdocument.OptionOutputAsXml && !_ownerdocument.OptionWriteEmptyNodes)
                        {
                            if ((closeFormat & TagCloseFormat.Space) == TagCloseFormat.Space)
                            {
                                outText.Write(" ");
                            }
                            if ((closeFormat & TagCloseFormat.New) == TagCloseFormat.New)
                            {
                                outText.Write("></" + name);
                            }
                            else
                            {
                                if ((closeFormat & TagCloseFormat.QuestionMark) == TagCloseFormat.QuestionMark)
                                {
                                    outText.Write("?");
                                }
                                else if ((closeFormat & TagCloseFormat.Slash) == TagCloseFormat.Slash)
                                {
                                    outText.Write("/");
                                }
                            }
                    ...
                    {
                        if(_ownerdocument.OptionKeepOriginalSelfCloseTagFormat)
                        {
                            if ((closeFormat & TagCloseFormat.Space) == TagCloseFormat.Space)
                            {
                                outText.Write(" ");
        ...
        internal void CloseNode(HtmlNode endnode)
        {
            ...
            {
                _endnode = endnode;
                closeFormat = TagCloseFormat.None;
                if (_ownerdocument.OptionKeepOriginalSelfCloseTagFormat)
                {
                    if (_endnode != this)
                    {
                        closeFormat = TagCloseFormat.New;
                        var spaceIndex = this._outerstartindex + this._outerlength - 2;
                        if (spaceIndex > 0 && _ownerdocument._text[spaceIndex] == ' ')
                        {
                            closeFormat |= TagCloseFormat.Space;
                        }
                    }
                    else
                    {
                        var slashIndex = endnode._outerstartindex + endnode._outerlength - 2;
                        if (slashIndex > 0)
                        {
                            var tmpValue = _ownerdocument._text[slashIndex];
                            if (tmpValue == '/')
                            {
                                closeFormat |= TagCloseFormat.Slash;
                                if (slashIndex - 1 > 0 && _ownerdocument._text[slashIndex - 1] == ' ')
                                {
                                    closeFormat |= TagCloseFormat.Space;
                                }
                            }
                            ...
                            {
                                closeFormat |= TagCloseFormat.QuestionMark;
                                if (slashIndex - 1 > 0 && _ownerdocument._text[slashIndex - 1] == ' ')
                                {
                                    closeFormat |= TagCloseFormat.Space;
                                }
