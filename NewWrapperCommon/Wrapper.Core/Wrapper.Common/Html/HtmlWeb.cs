...
namespace AvePoint.Wrapper.Common
{
    ...
    {
        private static AveLogger Logger = AveLogger.GetInstance(typeof(HtmlWeb));
        #region Delegates
        ...
        public static string GetContentTypeForExtension(string extension, string def)
        {
            ...
            if (!SecurityManager.IsGranted(new DnsPermission(PermissionState.Unrestricted)))
            {
                ...
                {
                    contentType = def;
                    Logger.Warn("failed to open sub key:{0}", e.ToString());
                }
            }
            return contentType;
        }
        ...
        [SuppressMessage("FxCopCustomRules", "C100007:SpellCheckStringValues", Justification = "The words is key.")]
        public static string GetExtensionForContentType(string contentType, string def)
        {
            ...
            if (SecurityManager.IsGranted(new RegistryPermission(PermissionState.Unrestricted)))
            {
                ...
                {
                    ext = def;
                    Logger.Warn("failed to open subkey: {0}", e.ToString());
                }
            }
            return ext;
        }
        ...
        [SuppressMessage("FxCopCustomRules", "C100007:SpellCheckStringValues", Justification = "The words is exception messages.")]
        ...
        {
            return contentType.ToLower(System.Globalization.CultureInfo.InvariantCulture).StartsWith("text/html",StringComparison.OrdinalIgnoreCase);
        }
