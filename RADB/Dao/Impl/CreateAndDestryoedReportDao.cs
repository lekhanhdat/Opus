...
{
    public class CreateAndDestryoedReportDao : BaseDao<RMProfile>, ICreateAndDestryoedReportDao
    {
        ...
        public async Task<bool> Edit(RMProfile profile)
        {
            using (var context = GetNewContext())
            {
                ...
                if (existResult == null)
                {
                    throw new Exception($"Can't find create and destryed profile: [{profile.Name}] by: [{profile.Id}]");
                }
        ...
        public async Task<bool> Delete(int id)
        {
            using (var context = GetNewContext())
            {
                ...
                if (existResult == null)
                {
                    throw new Exception($"Can't find create and destryed profile by: [{id}]");
                }
