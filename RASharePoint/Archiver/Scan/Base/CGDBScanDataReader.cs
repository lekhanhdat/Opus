...
{
    internal class CGDBScanDataReader : IScanDataReader
    {
        private ApprovalReportService mScanDB = null;
        private ScheduleConfiguration mConfig;
        public CGDBScanDataReader(ScheduleConfiguration config)
        {
            mConfig = config;
        ...
        public List<string> GetAllRuleIds()
        {
            ...
            foreach (var rule in mConfig.RuleCollection.Values)
            {
                result.Add(rule.Id);
            }
            return result;
