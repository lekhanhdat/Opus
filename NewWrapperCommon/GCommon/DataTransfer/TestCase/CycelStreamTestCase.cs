...
namespace AvePoint.GCommon.Transfer.TestCase
{
    public class CycleStreamTestCase
    {
        ...
        /// <summary>
        /// set fileReadPath for read data first
        /// </summary>
        static void Read()
        {
            string fileReadPath = string.Empty;
            using (FileStream fs = new FileStream(fileReadPath, FileMode.Create, FileAccess.Write))
            {
                Random r = new Random();
                while (true)
                {
        ...
        /// <summary>
        /// set fileWritePath first for write data to
        /// </summary>
        static void Write()
        {
            string fileWritePath = string.Empty;
            using (FileStream fs = new FileStream(fileWritePath, FileMode.Open, FileAccess.Read))
            {
                Random r = new Random();
                while (true)
                {
