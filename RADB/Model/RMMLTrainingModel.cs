/********************************************************************
 *
 *  PROPRIETARY and CONFIDENTIAL
 *
 *  This file is licensed from, and is a trade secret of:
 *
 *                   AvePoint, Inc.
 *                   525 Washington Blvd, Suite 1400
 *                   Jersey City, NJ 07310
 *                   United States of America
 *                   Telephone: +1-201-793-1111
 *                   WWW: www.avepoint.com
 *
 *  Refer to your License Agreement for restrictions on use,
 *  duplication, or disclosure.
 *
 *  RESTRICTED RIGHTS LEGEND
 *
 *  Use, duplication, or disclosure by the Government is
 *  subject to restrictions as set forth in subdivision
 *  (c)(1)(ii) of the Rights in Technical Data and Computer
 *  Software clause at DFARS 252.227-7013 (Oct. 1988) and
 *  FAR 52.227-19 (C) (June 1987).
 *
 *  Copyright © 2017-2026 AvePoint® Inc. All Rights Reserved. 
 *
 *  Unpublished - All rights reserved under the copyright laws of the United States.
 */
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace AvePoint.RA.DB.Model
{
    public class RMMLTrainingModel : BaseModel
    {
        [Key]
        [Column(TypeName = "uniqueidentifier", Order = 1)]
        public Guid Id { set; get; }

        [Column(TypeName = "float")]
        public double Accuracy { get; set; }

        [Column(TypeName = "int"), DefaultValue(0)]
        public int TrainStatus { get; set; }

        [Column(TypeName = "float"), DefaultValue(0)]
        public double Throttling { get; set; }

        [Column(TypeName = "bigint")]
        public long LastTrainedTime { get; set; }

        [Column(TypeName = "int"), DefaultValue(0)]
        public int PublishStatus { get; set; }
        
        [Column(TypeName = "bigint")]
        public long ModifedTime { get; set; }

        [Column(TypeName = "bigint")]
        public long CreateTime { get; set; }

        [Column(TypeName = "nvarchar")]
        public string CurrentTrainingJobId { get; set; }

        [Column(TypeName = "bit"), DefaultValue(0)]
        public bool ExpiredResourcesDeleted { get; set; }

        [Column(TypeName = "bit")]
        public bool? IsModeUsing { get; set; }

        [Column(TypeName = "int"), DefaultValue(0)]
        public TrainingMode Mode { get; set; }

        [Column(TypeName = "int")]
        public TrainingScopeOption TrainingScopeOption { get; set; }

        [Column(TypeName = "varchar")]
        public string Extension { get; set; }
    }

    public enum TrainingMode
    {
        MLTraining = 0,
        ZeroShot = 1
    }

    public enum TrainingScopeOption
    {
        Auto500Laster,
        FromLocation,
        Manual
    }
}
