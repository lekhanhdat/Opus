...
namespace AvePoint.Wrapper.Common
{
    ...
    {
        private static AveLogger Logger = AveLogger.GetInstance(typeof(HtmlDocument));
        ...
        /// <summary>
        /// Defines the server code header under attribute value
        /// abc="title="<SharePoint:EncodedLiteral runat='server' text='<%$Resources:wss,master_turnonaccessibility%>' EncodeMethod='HtmlEncode' __designer:Preview="Turn on more accessible mode" ..."
        /// </summary>
        public Regex OptionServerCodeHeaderUnderAttributeValue;

        /// <summary>
        /// Defines the original format, if the source tag is self closed, we need to keep it.
        /// </summary>
        public bool OptionKeepOriginalSelfCloseTagFormat;
        ...
        public void Load(TextReader reader)
        {
            ...
            if (sr != null)
            {
                ...
                {
                    Logger.Warn("failed to load html due to: {0}", e.ToString());
                    // void on purpose
                }
                _streamencoding = sr.CurrentEncoding;
            }
            ...
            if ((_index - 1) <= (_text.Length - 2))
            {
                if (_text[_index] == '!')
                {
                    ...
                    if (_index < (_text.Length - 2))
                    {
                        ...
                        else if (_index < (_text.Length - 8))
                        {
                            string temp = _text.Substring(_index + 1, 7);
                            if (temp.Equals("[CDATA[", StringComparison.OrdinalIgnoreCase))
                            {
                                commentType = CommentType.CData;
                            }
                            else
                            {
                                commentType = CommentType.SimpleComment;
                            }
                        }
                        else
                        {
        ...
        {
            int lastquote = 0;
            var getHeaderForAttributeValue = false;
            ...
            while (_index < _text.Length)
            {
                ...
                switch (_state)
                {
                    ...
                    case ParseState.QuotedAttributeValue:
                        if (_c == lastquote)
                        {
                            getHeaderForAttributeValue = false;
                            PushAttributeValueEnd(_index - 1);
                            _state = ParseState.BetweenAttributes;
                            continue;
                        }
                        ...
                        if ((!getHeaderForAttributeValue) && OptionServerCodeHeaderUnderAttributeValue != null && IsWhiteSpace(_c))
                        {
                            if (_currentattribute != null)
                            {
                                getHeaderForAttributeValue = true;
                                var match = OptionServerCodeHeaderUnderAttributeValue.Match(_text, _currentattribute._valuestartindex, _index - 1 - _currentattribute._valuestartindex);
                                if (match.Success)
                                {
                                    _oldstate = _state;
                                    _state = ParseState.CustomServerSideCode;
                                    continue;
                                }
                            }
                        }
                    ...
                    case ParseState.Comment:
                        if (_c == '>')
                        {
                            if (commentType != CommentType.SimpleComment)
                            {
                                char specialSymbol = commentType == CommentType.CData ? ']' : '-';
                                if (commentType != CommentType.Unknown)
                                {
                                    if ((_text[_index - 2] != specialSymbol) ||
                                        (_text[_index - 3] != specialSymbol))
                                    {
                                        continue;
                                    }
                                }
                    ...
                    case ParseState.ServerSideCode:
                        ...
                        break;
                    case ParseState.CustomServerSideCode:
                        if (_c == '/')
                        {
                            if (_index < _text.Length)
                            {
