...
namespace AvePoint.GCommon.Transfer.TestCase
{
    public class FileTransferTestCase
    {
        ...
        {
            using (ServiceHost service = WCFServiceHostFactory.CreateServiceHost(typeof(FileTransferService), typeof(IFileTransferService), DataTransferGlobalConfig.DataTransferConfiguration.FileTransferServiceUriSchema,
                                                                          "localhost", 9999, "ReplicatorFileTransfer", "jobId", WCFServiceHostType.FileTransfer.ToString(), true))
            {
                service.Open();
        ...
        /// <summary>
        /// set dataFileDir location and fileForTransfer first
        /// </summary>
        static void SenderThread()
        {
            ...
            //setting.DataFileDir = @"D:\Temp\ftp";
            ...
            try
            {
                fileName = AveAgentFileSender.SendFileToAgent(fileForTransfer, setting);
            }
            catch (Exception e)
            {
        ...
        static void ReceiverThread()
        {
            ...
            if (AveAgentFileSender.ReceiveFileFromAgent(fileName, receiveFileLocation, setting))
            {
