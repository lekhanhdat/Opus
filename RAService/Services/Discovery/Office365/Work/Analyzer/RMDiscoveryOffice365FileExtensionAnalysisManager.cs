...
{
    public class RMDiscoveryOffice365FileExtensionAnalysisManager
    {
        private readonly RALogger _logger = RALogger.GetInstance(typeof(RMDiscoveryOffice365FileExtensionAnalysisManager));

        private readonly Guid _o365TenantId;

        private readonly Dictionary<string, int> _fileTypes = new();

        private readonly IRMDiscoveryOffice365FileExtensionDao _fileTypeDao;

        public RMDiscoveryOffice365FileExtensionAnalysisManager(Guid o365TenantId)
        {
            _o365TenantId = o365TenantId;
            _fileTypeDao = new RMDiscoveryOffice365FileExtensionDao();
        }
        ...
        public async Task AddOrUpdateAsync(params string[] fileTypes)
        {
            ...
            {
                using var performance = new PerformanceScope("AddOrUpdateFileTypesAsync");
                _logger.Info($"The file types that does not exist in memory are [{string.Join(", ", notExistentInMemoryFileTypes)}].");
                var needAddFileTypes = notExistentInMemoryFileTypes.ConvertAll(item => new RMDiscoveryOffice365FileExtension
                {
                    Name = item,
                });
