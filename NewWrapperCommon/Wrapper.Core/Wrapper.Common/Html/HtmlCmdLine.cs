    ...
    internal class HtmlCmdLine
    {
        ...
        private static void GetBoolArg(string Arg, string Name, ref bool ArgValue)
        {
            ...
            if (('/' != Arg[0]) && ('-' != Arg[0])) // not a param
                return;
            if (Arg.Substring(1, Name.Length).ToLower(System.Globalization.CultureInfo.InvariantCulture) == Name.ToLower(System.Globalization.CultureInfo.InvariantCulture))
                ArgValue = true;
        }
        [SuppressMessage("FxCopCustomRules", "C100007:SpellCheckStringValues", Justification = "The word 'arg' is a value name.")]
