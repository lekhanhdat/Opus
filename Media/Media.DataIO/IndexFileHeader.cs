...
namespace MediaDataIO
{
    ...
    public class IndexFileHeader
    {
        ...
        public string IndexMD5
        {
            get { return this.indexMD5; }
            set
            {
                this.indexMD5 = value;
            }
        }
        private long indexCRC;
        public long IndexCRC
        {
            get { return this.indexCRC; }
            set
            {
                this.indexCRC = value;
            }
        ...
        public IndexFileHeader(Stream stream)
        {
            var buffer = new byte[HEADER_LENGTH];
            stream.ReadEx(buffer, 0, buffer.Length);
            Init(buffer);
        }
        ...
        private void Init(byte[] indexInfo)
        {
            ...
            char hasMD5 = Convert.ToChar(indexInfo[28]);
            if (hasMD5 == '1')
            {
                //this.indexMD5 = Encoding.UTF8.GetString(indexInfo, 32, 32);
                this.indexCRC = AveConverter.ToBigLong(indexInfo, 32);
                this.IndexLength = AveConverter.ToBigLong(indexInfo, 64);
                AssemblyExtension(indexInfo, FIXED_HEADER_LENGTH, extensionLength);
            }
            else
            {
                this.IndexLength = AveConverter.ToBigLong(indexInfo, 32);
        ...
        {
            var xml = Encoding.UTF8.GetString(indexInfo, offset, count);
        ...
        public byte[] ToBytes()
        {
            ...
            // 28: has md5 or not
            header[28] = Convert.ToByte('1');
            // 29-31: RESERVED
            //32-35 md5
            //Buffer.BlockCopy(Encoding.UTF8.GetBytes(this.IndexMD5), 0, header, 32, 32);
            AveConverter.ToBigBytes(this.indexCRC, header, 32);
            // 64-71: originalLength
            AveConverter.ToBigBytes(this.IndexLength, header, 64);
            // 128-(4k-1): extension
