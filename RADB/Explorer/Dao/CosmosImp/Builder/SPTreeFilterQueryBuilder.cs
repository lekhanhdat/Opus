...
{
    public class SPTreeFilterQueryBuilder : IFilterBuilder, IAdvancedQueryBuilder
    {
        ...
        public Query Filter(Query query, ExplorerFilterOptionV2 filterOption)
        {
            ...
            var isQuerySiteId = IsQuerySiteId(filterOption);
            if (isQueryContainerId && isQuerySiteId)
            {
        ...
        private bool IsQuerySiteId(ExplorerFilterOptionV2 filterOption)
        {
        ...
        {
            return Contract.TemplateManagement.QueryCloumnIds.SPOLocation;
        }
