...
namespace AvePoint.Common
{
    ...
    public sealed class DbHelper : IDisposable, ISingleton
    {
        ...
        {
            //HACK: need to add error reprot or exception logic
            var dbConnectionStringSection = ConfigurationManager.AppSettings["JobReportDb"];
            if (!String.IsNullOrEmpty(dbConnectionStringSection))
                this.ProviderName = ConfigurationManager.ConnectionStrings[dbConnectionStringSection].ProviderName;
        }
