...
{
    public class CustomRetentionSettingsAfterAuditHandler : IAfterAuditHandler
    {
        ...
        public async Task<RMAuditInfo> CollectAsync(RMAuditInfo info, int model, int category, int action, object[] args, object target, object returnValue)
        {
            ...
            {
                var auditItem = info.ModifyContent[0];
                auditItem.NewValue = await _keyValueDao.GetValueByKeyAsync(KeyNameCollection.UploadedCustomRetentionSettingsFileName);
