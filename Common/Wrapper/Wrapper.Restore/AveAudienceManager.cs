...
namespace AvePoint.Wrapper.Restore
{
    public class AveAudienceManager
    {
        ...
        public AveAudienceManager(AveSPSite aveSite)
        {
            ...
            {
                mLog.Warn("An error occurred while get user profile serviceContext. error", e);
            }
        ...
        public void GenerateIDMapping(Dictionary<string, string> sourceMapping)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Restore.AveAudienceManager.GenerateIDMapping"))
            {
                ...
                {
                    mAveParentSite.MappingManager.SiteMappingManager.AudienceIDMapping = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
                }
                ...
                if (destMapping != null)
                {
                    foreach (string key in sourceMapping.Keys)
                    {
                        ...
                        {
                            mAveParentSite.MappingManager.SiteMappingManager.AudienceIDMapping[sourceMapping[key]] = destMapping[key];
                        }
                    }
                }
