    ...
    public class TenantHelper
    {
        ...
        public static async Task<string> GetTenantBySiteUrlAsync(string siteUrl)
        {
            try
            {
                ...
                var aosTenantId = await GetTenantByDomainNameAsync(domain, cidInRequest: string.Empty);
                logger.Info($"Get AOS tenant id by site url {siteUrl}, domain: {domain}, aosTenantId: {aosTenantId}, for CORS check.");
                return aosTenantId;
            }
            catch (Exception e)
            {
