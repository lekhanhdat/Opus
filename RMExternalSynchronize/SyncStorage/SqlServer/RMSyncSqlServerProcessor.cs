...
{
    public class RMSyncSqlServerProcessor
    {

        public async Task<bool> ChangeNameAsync(RMSyncNodeChangeInfo changeInfo)
        {
            var changeRes = await new RMSyncSqlServerSettingsProcessor().ChangeNameAsync(changeInfo);
            return changeRes;
        }

        public async Task<bool> DeleteAsync(RMSyncNodeChangeInfo changeInfo)
        {
            var changeRes = await new RMSyncSqlServerSettingsProcessor().DeleteAsync(changeInfo);
            return changeRes;
        }

        public async Task<bool> MoveContainerAsync(RMSyncNodeChangeInfo changeInfo)
        {
            var changeRes = await new RMSyncSqlServerSettingsProcessor().MoveContainerAsync(changeInfo);
            return changeRes;
        }
    }
}
