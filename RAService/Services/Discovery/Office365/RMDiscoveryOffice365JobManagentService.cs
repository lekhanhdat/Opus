...
{
    public class RMDiscoveryOffice365JobManagentService : IRMDiscoveryOffice365JobManagentService
    {
        private readonly RALogger _logger = RALogger.GetInstance(typeof(RMDiscoveryOffice365JobManagentService));
        ...
        public async Task<RMDiscoveryLatestJobInfo> GetLatestAsync()
        {
            try
            {
                if (!await _tenantInfoDao.IsInitTenantDiscoveryDBInfoAsync() || !await RMDiscoveryDBManager.CheckOffice365TablesExistsAsync())
                {
                    return new()
                    {
                        Version = RMDiscoveryJobVersion.V5,
