...
{
    public class RMDiscoveryFSBasicInfoQueryService : IRMDiscoveryFSBasicInfoQueryService
    {
        private readonly RALogger _logger = RALogger.GetInstance(typeof(RMDiscoveryFSBasicInfoQueryService));

        private readonly IRMDiscoveryFSDataQueryDao _dataQueryDao = new RMDiscoveryFSDataQueryDao();

        private readonly IRMDiscoveryFSFileExtensionDao _fileExtensionDao = new RMDiscoveryFSFileExtensionDao();
        ...
        public async Task<List<RMDiscoveryFileExtensionDataInfo>> GetFileExtensionsAsync()
        {
            try
            {
                ...
                if (limit.Enabled)
                {
                    ...
                    var sql = $@"SELECT fileType.Id AS Id, fileType.Name AS Name, SUM(data.FileTotalSize) AS FileTotalSize 
                                FROM [{schemaName}].[RMFSBasicInactiveData] AS data 
                                JOIN [{schemaName}].[RMFSFileExtensions] AS fileType 
                ...
                return (await _fileExtensionDao.GetAllAsync()).ConvertAll(item => new RMDiscoveryFileExtensionDataInfo
                {
                    Id = item.Id,
        ...
        public async Task<RMDiscoverySummaryStatisticalDataInfo> GetSummaryStaticalDataInfoAsync()
        {
            try
            {
                var res = await _dataQueryDao.GetAggregateTotalDataListAsync();
                return new()
                {
