...
{
    [DataContract]
    public class AveUserInfo : ICloneable
    {
        [DataMember]
        ...
        public object Clone()
        {
            return new AveUserInfo
            {
                ...
                ExternalTokenLastUpdated = this.ExternalTokenLastUpdated,
                Locale = this.Locale,
                CalendarType = this.CalendarType,
                AdjustHijriDays = this.AdjustHijriDays,
                TimeZone = this.TimeZone,
                Time24 = this.Time24,
                AltCalendarType = this.AltCalendarType,
                CalendarViewOptions = this.CalendarViewOptions,
                WorkDays = this.WorkDays,
                WorkDayStartHour = this.WorkDayStartHour,
                WorkDayEndHour = this.WorkDayEndHour,
                Mobile = this.Mobile,
                Flags = this.Flags,
                HasPermission = this.HasPermission
                // Roles = this.Roles// Roles 属性没有用到, 没有copy
            };
        }
    }
