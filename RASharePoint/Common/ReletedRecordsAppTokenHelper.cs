...
{
    public static class ReletedRecordsAppTokenHelper
    {
        private static RALogger logger = RALogger.GetInstance(typeof(ReletedRecordsAppTokenHelper));
        ...
        public static string GetAccessToken(Dictionary<string, string> parm)
        {
            return GetAccessToken(parm[SPAppConstants.ParamRelateToken], parm[SPAppConstants.ParamAppHost], parm[SPAppConstants.ParamHostUrl]);
        }

        public static string GetAccessToken(string contextTokenString, string appHost, string targetHost)
        {
            SharePointContextToken contextToken = ReadAndValidateContextToken(contextTokenString, appHost);
            var spHostUri = new Uri(targetHost);
            return GetAccessToken(contextToken, spHostUri.Authority).AccessToken;
        }
        ...
        public static SharePointContextToken ReadAndValidateContextToken(string contextTokenString, string appHostName = null)
        {
            ...
            logger.Info($"App context token is null {string.IsNullOrEmpty(contextTokenString)} security  keys count{securityKeys.Count}");
            ...
            SharePointContextToken token = null;
            try
            {
                tokenHandler.ValidateToken(contextTokenString, parameters, out Microsoft.IdentityModel.Tokens.SecurityToken securityToken);
                token = SharePointContextToken.Create(securityToken as JwtSecurityToken);
            }
            catch (Exception e)
            {
                logger.Warn($"App web Validate token by context e{e} ");
                //var jwtToken = tokenHandler.ReadJwtToken(contextTokenString);
                //token = SharePointContextToken.Create(jwtToken);
            }
            ...
            if (!StringComparer.OrdinalIgnoreCase.Equals(token?.Issuer, GetFormattedPrincipal(AcsPrincipalName, "", realm)))
            {
        ...
        //}
        //public static string GetS2SAccessTokenWithSid(Uri targetApplicationUri, string sid)
        //{
        //    string realm = string.IsNullOrEmpty(Realm) ? GetRealmFromTargetUrl(targetApplicationUri) : Realm;
        //    Microsoft.IdentityModel.S2S.Tokens.JsonWebTokenClaim[] claims = GetClaimsWithWindowsIdentity(sid);
        //    return GetS2SAccessTokenWithClaims(targetApplicationUri.Authority, realm, claims);
        //}
        /// <summary>
        ...
        //
        private static readonly string ClientId = string.IsNullOrEmpty(RMGlobalConfiguration.AppConfig[AvePoint.RA.Contract.Configurations.RMAppSettingKey.CLIENT_ID_IN_RELATED_RECORDS_APP]) ? ConfigurationManager.AppSettings.Get("HostedAppName") : RMGlobalConfiguration.AppConfig[AvePoint.RA.Contract.Configurations.RMAppSettingKey.CLIENT_ID_IN_RELATED_RECORDS_APP];

        private static readonly string HostedAppHostNameOverride = ConfigurationManager.AppSettings.Get("HostedAppHostNameOverride");
        private static readonly string HostedAppHostName = ConfigurationManager.AppSettings.Get("HostedAppHostName");
        private static readonly string ClientSecret = string.IsNullOrEmpty(RMGlobalConfiguration.EncryptConfig[AvePoint.RA.Contract.Configurations.RMCommonSettingKey.RELATED_RECORDS_APP_CLIENT_SECRET]) ? ConfigurationManager.AppSettings.Get("HostedAppSigningKey") : RMGlobalConfiguration.EncryptConfig[AvePoint.RA.Contract.Configurations.RMCommonSettingKey.RELATED_RECORDS_APP_CLIENT_SECRET];
        private static readonly string SecondaryClientSecret = string.IsNullOrEmpty(RMGlobalConfiguration.EncryptConfig[AvePoint.RA.Contract.Configurations.RMCommonSettingKey.RELATED_RECORDS_APP_CLIENT_SECONDARY_SECRET]) ? ConfigurationManager.AppSettings.Get("HostedAppSigningKey") : RMGlobalConfiguration.EncryptConfig[AvePoint.RA.Contract.Configurations.RMCommonSettingKey.RELATED_RECORDS_APP_CLIENT_SECONDARY_SECRET];
        private static readonly string Realm = ConfigurationManager.AppSettings.Get("Realm");
        ...
        public static class AcsMetadataParser
        {
            ...
            private static JsonMetadataDocument GetMetadataDocument(string realm)
            {
                ...
                using (WebClient webClient = new WebClient())
                {
                    ...
                    acsMetadata = webClient.DownloadData(AvePoint.GCommon.Utility.SecurityUtils.SanitizeRequestUrl(acsMetadataEndpointUrlWithRealm));
                }
                string jsonResponseString = Encoding.UTF8.GetString(acsMetadata);

                JsonMetadataDocument document = SerializerHelper.DeserializeByJsonSerializer<JsonMetadataDocument>(jsonResponseString);
    ...
    public class OAuthTokenResponse
    {
        ...
        {
            JToken tokenkeyValuePairs = new JObject();
            this.Data = SerializerHelper.DeserializeByJsonSerializer<object>(Encoding.UTF8.GetString(response)) as Dictionary<string, object>;
            if (this.Data != null)
            {
            ...
            {
                tokenkeyValuePairs = SerializerHelper.DeserializeByJsonSerializer<object>(Encoding.UTF8.GetString(response)) as JObject;
                this.Data = tokenkeyValuePairs.ToObject<Dictionary<string, object>>();
                this.AccessToken = this.GetValue("access_token");
                this.TokenType = this.GetValue("token_type");
                this.Resource = this.GetValue("resource");
                this.UserType = this.GetValue("user_type");
    ...
    public class OAuthClient
    {
            ...
            string ClientSecret, string refreshToken, string resource)
        {
            using (WebClient client = new WebClient())
            {
                ...
                byte[] response = client.UploadValues(AvePoint.GCommon.Utility.SecurityUtils.SanitizeRequestUrl(uri), "POST", values);
