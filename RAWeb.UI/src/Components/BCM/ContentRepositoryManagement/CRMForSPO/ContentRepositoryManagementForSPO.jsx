...
import SharePointGeneralManagement from "../GeneralManagement/Context/SharePointOnlineGeneralManagement";
import SharePointOnlineDocumentTerm from "../DocumentTermSetting/Context/SharePointOnlineDocumentTerm";
...
import SPOManualApprovalSetting from "../ManualApprovalSetting/Context/SPOManualApprovalSetting";
import SPOScheduleSetting from "../ScheduleSetting/Context/SPOScheduleSetting";
import UniqueIdSettingCommon from "../UniqueIdSetting/UniqueIdSettingCommon";
import SPColumnSetting from "../ColumnSetting/Context/SPColumnSetting";
...
import SPOUniqueIdSetting from "../UniqueIdSetting/Context/SPOUniqueIdSetting";
...
import SPContainerSetting from "../ContainerTermSetting/Context/SPContainerSetting";
...
export default class ContentRepositoryManagement4SPO extends R.Component {
    idAttr = true;
    constructor(props) {
        super(props);
    ...
    applySettingDoAction = (runApplySettingMethod, hasRunningJob) => {
        $$.messagedialog(false);
        $$.loading(true);
        let url = runApplySettingMethod == RunApplySettingMethod.SelectedScope ? "/api/SPSettingApi/ApplySettingsOnSelectedNode" : "/api/SPSettingApi/ApplySettings";
        ...
        fetchUtility(option).then((result) => {
            ...
            if (resultData.MessageType == RAMessageType.Successful) {
                ...
                if (hasRunningJob) {
                    showToast.withTitle({
                        id: `toast-source-${SourceFlags.SP}-apply-setting-is-running`,
        ...
        addTelemetryRecord(TelemetryModule.ContentRepositoryManagement, TelemetryEventType.ApplySettings, [SourceFlags.SP, runApplySettingMethod]);
    }

    getRunningJobConflictMessage = async (defaultResult) => {
        const option = {
            url: "/api/SPSettingApi/CheckRunningSharePointSettingJob",
            method: "GET",
        }
        ...
        return {
            ...
            messageContent: (
                <div>
                    ...
                    <div>{RMResx.RM_JS_BCM_NSPS_EnsureApply}</div>
                </div>
            ),
        };
    }
    ...
    getApplySettingMessageBoxContent = async () => {
        const defaultResult = {
            ...
            messageContent: RMResx.RM_JS_BCM_NSPS_EnsureApply,
        };
    ...
    inheritParentDoAction() {
        $$.messagedialog(false);
        let option = {
            url: "/api/SPSettingApi/InheritParentSettings",
            method: "Post",
            data: this.settingNode
        };
        fetchUtility(option).then((result) => {
    ...
    runDataSyncDoAction() {
        ...
        $$.loading(true);
        let option = {
            url: "/api/SPSettingApi/RunCollectionJob",
            method: "Post",
            data: this.settingNode
        };
        fetchUtility(option).then((result) => {
    ...
    runDataSyncMessageBox() {
        let args = {
            ...
            hideActions: false,
            title: RMResx.RM_JS_Common_Confirmation,
            content: RMResx.RM_JS_BCM_NSPS_EnsureSync,
            buttons: [
                {
                    text: RMResx.RM_JS_Common_Cancel, onClick: () => {
                        $$.messagedialog(false);
    ...
    runRuleActionDoAction() {
        ...
        addTelemetryRecord(TelemetryModule.ContentRepositoryManagement, TelemetryEventType.RunEnforceRuleActions, [SourceFlags.SP]);
    }
    ...
    loadNodeSettings(reload) {
        ...
        $$.loading(true);
        let option = {
            url: "/api/SPSettingApi/LoadSampleNodeSettings",
            method: "Post",
            data: nodeItem
        };
        fetchUtility(option).then((result) => {
    ...
    renderSPOTree() {
        const shouldRenderSiteCollectionTree = this.state.shouldRenderSiteCollectionTree;

        if (!shouldRenderSiteCollectionTree) {
            return <SPCRMTree
    ...
    onExportSPSetting = () => {
        const option = {
            url: "/api/BCMAdminSettingApi/ExportSPSetting",
            method: "POST",
    ...
    renderImportSettingPanel() {
        ...
        >
            <ImportSettingPanel
                ...
                downloadTemplateUrl="/api/BCMAdminSettingApi/DownloadTemplate"
                saveSettingUrl="/api/BCMAdminSettingApi/ImportSPSetting"
            ></ImportSettingPanel>
            <>
        ...
        return (
            ...
            >
                ...
                <>
                    ...
                    <R.Button slot="buttons" primary classify="theme" text={RMResx.RM_JS_SP_ExportSetting_ExportBtn} onClick={this.onExportSPSetting} />
                </>
            </R.Dialog>
        );
    }
            ...
            <section className="crm-content">
                        ...
                        <div className="ra-splitter-right">
                            ...
                            {this.settingNode && this.state.showRightSetting && <div>
                                <GeneralManagementComponent
                                    context={SharePointGeneralManagement.getContext()}
                            ...
                            {this.state.showRightSetting && this.state.enableClassification == EnableRecordManagementSetting.Enable && <div>
                                <ColumnSettingComponent
                                    context={SPColumnSetting.getContext()}
                                ...
                                {((!this.state.isUsingExistColumnName) || (this.state.isUsingExistColumnName && this.state.setDocLevelTermForExistColumn)) &&
                                    <DocumentTermSettingComponent
                                        context={SharePointOnlineDocumentTerm.getContext()}
                ...
                {this.state.initUniqueIdPanel &&
                    <UniqueIdSettingCommon
                        ...
                        context={SPOUniqueIdSetting.getContext()}
                        sourceFlag={SourceFlags.SP}
                    />}
