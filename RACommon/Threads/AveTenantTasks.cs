...
namespace AvePoint.RA.Common.Threads
{
    public class AveTenantTasks
    {
        ...
        private static RALogger logger = RALogger.GetInstance(typeof(AveTenantTasks));
        ...
        public static void RunParallel<TSource>(IEnumerable<TSource> items, int itemsPerTask, CancellationTokenSource cts, Action<TSource> action)
        {
            ...
            RunParallel(partioner, setting, items, cts, action);
        }
        /// <summary>
        ...
        public static void RunParallelBatch<TSource>(IEnumerable<TSource> items, int itemsPerTask, CancellationTokenSource cts, Action<IEnumerable<TSource>> action)
        {
            ...
            var partioner = Partitioner.Create(0, items.Count(), itemsPerTask);
            RunParallelBatch(partioner, setting, items, cts, action);
        }
        ...
        private static void RunParallel<TSource>(Partitioner<Tuple<int,int>> partioner, ThreadSetting setting, IEnumerable<TSource> items, CancellationTokenSource cts, Action<TSource> action)
        {
            try
            {
                System.Threading.Tasks.Parallel.ForEach(partioner, (range, loopState) =>
                {
                    try
                    {
                        ...
                        var startPos = range.Item1;
                        var endPos = range.Item2;
                        logger.Info($"enter new parallel task. startPos: {startPos}, endPos : {endPos} thread id:{Thread.CurrentThread.ManagedThreadId}");
                        for (var j = startPos; j < endPos; j++)
                        {
                            action(items.ElementAt(j));
                        }
                        logger.Info($"end task. startPos: {startPos}, endPos : {endPos} thread id:{Thread.CurrentThread.ManagedThreadId}");
                    }
                    catch (JobStopException ex)
                    {
                        logger.Warn("Job is stopped.");
            ...
            {
                logger.Warn($"An error occurred while run parallel tasks. error : {ex}");
                DealWithAggregateException(ex);
            }
            catch (Exception e)
            {
        ...
        private static void RunParallelBatch<TSource>(Partitioner<Tuple<int, int>> partioner, ThreadSetting setting, IEnumerable<TSource> items, CancellationTokenSource cts, Action<IEnumerable<TSource>> action)
        {
            try
            {
                System.Threading.Tasks.Parallel.ForEach(partioner, (range, loopState) =>
        ...
        /*private static int RunAndWaitResult<TSource>(Partitioner<Tuple<int, int>> partioner, ThreadSetting setting, IEnumerable<TSource> items, CancellationTokenSource cts, Func<TSource, int> func)
        {
            #region
            var total = 0;
        ...
        public static T ExecuteActionHaveTimeOut<T>(Func<T> func, int minutes = 10)
        {
            using (var cancellationTokenSource = new CancellationTokenSource(TimeSpan.FromMinutes(minutes)))
            {
                var setting = ThreadSetting.GetSetting();
                var task = Task.Run(() =>
                {
                ...
                {
                    throw new Exception($"Task not finished.");
                }
                return task.Result;
            }
        }
        public static Task ExecuteAction(Func<Task> func)
        {
            var setting = ThreadSetting.GetSetting();
        ...
        public static async Task RunParallelAsync<TSource>(IEnumerable<TSource> items, int itemsPerTask, CancellationTokenSource cts, Func<TSource, Task> action)
        {
            var setting = ThreadSetting.GetSetting();
            try
            {
                await Parallel.ForEachAsync(items, new ParallelOptions() { MaxDegreeOfParallelism = itemsPerTask, CancellationToken = cts.Token }, async (item, _) =>
                {
                    try
                    {
                        ThreadSetting.SetSetting(setting);
                    ...
                    {
                        logger.Warn($"An error occurred while executing the parallel task. error : {e}");
                    }
                });
            }
            catch (AggregateException ex)
            ...
            {
                logger.Warn($"An error occurred while run parallel tasks. error : {e}");
            }
        }
    }
}
