            ...
            where T : class, new()
        {
            ...
            if (dataElement != null)
            {
                ...
                Array.ForEach<PropertyInfo>(typeof(T).GetProperties(), info =>
                {
                    ...
                    {
                        logger.Warn("Reflect ConvertXmlElementToObject failed, exception:{0}", ex.ToString());
                        //logHelper.LogToEvent("ReflectUtility001", AvePoint.Common.MetaData.LogLevel.Warn, e.ToString());
                    }
                });
            }
            return result;
