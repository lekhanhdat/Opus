...
{
    public class RMSPRuleChecker
    {
        protected static readonly AveLogger logger = AveLogger.GetInstance(typeof(RMSPRuleChecker));
        private readonly RuleCollection _rules;
        private RuleManagement ruleManagement = null;

        public RMSPRuleChecker(RuleCollection rules)
        {
        ...
        public SyncItemRuleInfo CheckDisposalRule(AveDiscoverWeb web, SyncItemRuleInfo parentItemRule = null)
        {
            ...
            if (NeedCheckRule(aveWeb.Url))
            {
                ...
                {
                    logger.Info("fit Parent rule now {0}:{1}", parentItemRule.Rule.Name.LogBase64(), parentItemRule.DisposalAction.LogBase64());
                ...
                else
                {
                    if (aveWeb.IsRootWeb)
                    {
                        logger.Info("Root web no need check web rule {0}", aveWeb.Title.LogBase64());
                        return itemRule;
                    }
        ...
        public SyncItemRuleInfo CheckDisposalRule(IAveListItem aveItem, SyncItemRuleInfo parentItemRule = null)
        {
            ...
            if (NeedCheckRule(aveItem?.Url))
            {
                ...
                {
                    //using (new RA.Common.AgentPerformanceScope(string.Format("CollectionData.Scan.Item.CheckItemCretia:{0}", aveItem.Url/*discoverItem.FullUrl*/)))
                    using (var performance = new AgentPerformanceScope("CollectionData.Scan.Item.CheckItemCretia", string.Format("CollectionData.Scan.Item.CheckItemCretia:{0}", aveItem.Url/*discoverItem.FullUrl*/), true))
                    {
                        itemRule.Rule = ruleManagement.CheckItemCriteria(aveItem.UniqueId, aveItem);
                    }
                    if (itemRule.Rule != null)
                    {
                        itemRule.DisposalAction = "RDM_RecordsExporer_Status_NextJob";
                    ...
                    if (itemRule.Rule != null)
                    {
                        ...
                        {
                            if (!itemRule.Rule.DeleteRecords && !aveItem.IsBlockDeleteOnlyRecord() && !RuleHelper.CheckMoveRule(itemRule.Rule))
                            {
                                itemRule.Rule = null;
        ...
        private bool NeedCheckRule(string url)
        {
            ...
            if (!haveRule)
            {
                logger.Info("No rules realted {0}", url.LogBase64());
            }
            return haveRule;
        }
