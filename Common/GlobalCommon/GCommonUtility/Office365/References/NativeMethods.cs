...
namespace AvePoint.Common.Office365
{
    ...
    internal class NativeMethods
    {
        ...
        {
            String crlDllPath;
            var registryKey = Registry.LocalMachine.OpenSubKey(IdentityCrlRegistrySubKey);
            if (registryKey != null)
            {
                var str = registryKey.GetValue(IdentityCrlInstallPathRegKeyName).ToString();
                crlDllPath = Path.Combine(str, IdentityCrlDllToLoadName);
            }
            else
            {
                crlDllPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "bin", "MSOIDCLIL.DLL");
                if (!File.Exists(crlDllPath))
                {
                    crlDllPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "MSOIDCLIL.DLL");
                }
            }
            return crlDllPath;
        }
        ...
        private delegate int _InitializeEx([In] ref Guid guid, [In] int lPPCRLVersion, [In] uint dwFlags, [In, MarshalAs(UnmanagedType.LPArray)] NativeMethods.IdcrlOption[] pOptions, [In] uint dwOptions);

        private delegate int _LogonIdentityEx([In] IntPtr Identity, [In, Optional, MarshalAs(UnmanagedType.LPWStr)] string authPolicy, [In] uint dwAuthFlags, [In, MarshalAs(UnmanagedType.LPArray)] NativeMethods.RstParams[] pcRSTParams, [In] uint dwpcRSTParamsCount);

        private delegate int _LogonIdentityExSSO([In] IntPtr Identity, [In, Optional, MarshalAs(UnmanagedType.LPWStr)] string authPolicy, [In] uint dwAuthFlags, [In] uint dwSsoFlags, [In, Out, Optional] NativeMethods.PCUIParam2 pcUIParam2, [In, MarshalAs(UnmanagedType.LPArray)] NativeMethods.RstParams[] pcRSTParams, [In] uint dwpcRSTParamsCount);
