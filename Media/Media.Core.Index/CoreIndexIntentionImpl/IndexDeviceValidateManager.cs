...
namespace AvePoint.Media.Core.Index
{
    ...
    public class IndexDeviceValidateManager : IIndexDeviceValidateManager
    {
        ...
        public void Validate(IndexDeviceValidateParameter parameter)
        {
            ...
            {
                this.logger.Debug(MediaCoreIndexResource.IndexDeviceValidateManagerValidateBegin);
                ...
                this.logger.Debug(MediaCoreIndexResource.IndexDeviceValidateManagerValidateSucceed, parameter.IndexWorkingSystem.SystemLocation);
            }
        }
            ...
            foreach (var subSystem in xLibary.SubSystems)
            {
                try
                {
                    ...
                    if (validateResult.SystemHealth == XSystemHealth.AvailableAndNotFull)
                    {
                        ...
                        var freeSpace = spaceInfo.AvailableSpace;
                        logger.Debug(MediaCoreIndexResource.IndexDeviceValidateManagerGetMaxFreeSizeSystemSize, subSystem.SystemLocation, freeSpace);
        ...
        private IXSystem GetMaxFreeSizeSystemByIndexSize(IndexDeviceValidateParameter parameter, XLibrary xLibary, StorageInfo indexInfo, long indexFileSize, out IXSystem indexSystem)
        {
            ...
            foreach (var subSystem in xLibary.SubSystems)
            {
                try
                {
                    ...
                    var validateResult = subSystem.Validate();
                    {
                        if (subSystem is not SpaceInfo spaceInfo)
                        {
                            continue;
                        }
                        if (subSystem.FileExists(indexInfo))
                        {
                            ...
                            freeSpaces.Add(subSystem, (ulong)indexFileSize + (subSystem as SpaceInfo)?.TotalFreeSpace??0);
                        }
                        else
                        {
                            if (validateResult.SystemHealth == XSystemHealth.AvailableAndNotFull)
                                freeSpaces.Add(subSystem, (subSystem as SpaceInfo)?.TotalFreeSpace ?? 0);
                        }
                    }
                }
                finally
        ...
        private void MoveTo(IXSystem sourceDevice, IXSystem destinationDevice, StorageInfo indexInfo)
        {
            ...
            {
                logger.Error(MediaCoreIndexResource.IndexDeviceValidateManagerMoveToError, e.ToString());
                throw;
            }
            finally
            {
