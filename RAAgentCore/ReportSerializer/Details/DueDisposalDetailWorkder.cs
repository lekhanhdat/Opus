...
{
    public class DueDisposalDetailWorkder : AbstractDetailWorker
    {
        public override List<List<ReportCell>> ConvertReportData(IEnumerable<JMJobDetails> details)
        {
            ...
            {
                if (detail is JMFSAgentDueDisposalReportJobDetails)
                {
                    List<ReportCell> reports = new List<ReportCell>();
                    JMFSAgentDueDisposalReportJobDetails reportnfo = detail as JMFSAgentDueDisposalReportJobDetails;
                    reports.Add(new ReportCell() { Key = "TitleOrName", Value = reportnfo.TitleOrName });
        ...
        public override void InitCreateTableSQLString()
        {
            ...
            CREATE_TABLE_SQL = string.Format(JobMonitorConstants.CREATE_TABLE_ITEMS_DUE_DISPOSAL, TABLE_NAME);
            INSERT_DATA_SQL = string.Format(JobMonitorConstants.INSERT_DATA_ITEMS_DUE_DISPOSAL, TABLE_NAME);
        }
    }
}
