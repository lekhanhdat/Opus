...
namespace RAManualApprovalCommon
{
    public class HistoryAddAction
    {
        ...
        public async System.Threading.Tasks.Task Add(Record item)
        {
            var entity = Convert(item);
            await Add(entity);
        }

        public async System.Threading.Tasks.Task AddRange(IEnumerable<Record> items)
        {
            var entities = items.ToList().ConvertAll(Convert);
            await AddRange(entities);
        }

        public async System.Threading.Tasks.Task Add(RMManualApproveHistoryTableEntity entity)
        {
        ...
        public async System.Threading.Tasks.Task AddRange(List<RMManualApproveHistoryTableEntity> entities)
        {
            var processId = Environment.ProcessId.ToString();
            var threadId = Environment.CurrentManagedThreadId.ToString();
        ...
        public RMManualApproveHistoryTableEntity Convert(Record item, SOApproveDBStatus approvedStatus, int approvedBy, long actionTimeTicks)
        {
            ...
            if (approvedStatus == SOApproveDBStatus.Approved)
            {
