...
namespace AvePoint.Wrapper.Backup
{
    ...
    public abstract class AveColumnCache
    {
        ...
        private object GetMetaDataValueInCache(CachedField fieldInfo, object value)
        {
            ...
            string metaDisplayName = fieldInfo.Title + "_0";
            foreach (AveSPField field in list.Fields.FieldMap.Values)
            {
                if (field.FieldTypeAsString.ToUpper(CultureInfo.InvariantCulture).Equals("NOTE", StringComparison.OrdinalIgnoreCase) && field.DisplayName.Equals(metaDisplayName))
                {
    ...
    public class AveDisPlayNameColumnCache : AveColumnCache
    {
        ...
        {
            return needGetFields.Distinct(new AveFieldEqualityByDisplayNameComparer()).ToDictionary(kv => kv.Value.Title,
                kv =>
                {
        ...
        private class AveFieldEqualityByDisplayNameComparer : IEqualityComparer<KeyValuePair<string, CachedField>>
        {
            ...
            {
                return string.Equals(x.Value.Title, y.Value.Title);
            }

            public int GetHashCode(KeyValuePair<string, CachedField> obj)
            {
                return obj.Value.Title.GetHashCode();
            }
        }
    }
