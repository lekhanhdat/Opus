...
namespace AvePoint.Media.Storage.OneDrive
{
    ...
    [VIM(VIMName.SkyDrive, typeof(AvePoint.Media.Storage.OneDrive.OneDriveVIM))]
    class OneDriveVIM : AbstractVIM
    {
        public override IXSystem CreateSystem(string xri, AbstractXSystem parentSystem)
        {
            return new OneDriveSystem(xri, parentSystem);
        }

        public override List<string> GetFeatureXML(int type)
        {
            return OneDriveFeature.Getstances(type).FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(int type)
        {
            return OneDriveFeature.Getstances(type).FeatureObjs;
        }

        public override List<StorageFeature> GetFeatureObj(int type, string culture)
        {
            return OneDriveFeature.Getstances(type, culture).FeatureObjs;
        }
    }
}
