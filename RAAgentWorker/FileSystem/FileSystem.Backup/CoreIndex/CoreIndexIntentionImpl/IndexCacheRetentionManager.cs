...
namespace AvePoint.Media.Core.Index
{
    ...
    public class IndexCacheRetentionManager : IIndexCacheRetentionManager
    {
        ...
        public void MakeSureCacheHaveEnoughSpaceBeforeDownload(IndexCacheManagerParameter param)
        {
            ...
            if (indexInfo == null)
            {
                throw new FileNotFoundException(string.Format($"IndexCache RetentionManager MakeSure Cache HaveEnoughSpace Before Download Exception:{info.ToString()}"));
            }
            ...
            {
                ulong totalFreeSpace = subSystem.TotalFreeSpace;
                try
                {
                    param.CacheSystem.FindCondition = new Predicate<IXSystem>(xSystem => { return xSystem.TotalFreeSpace > minimumFreeSpace + (ulong)fileSize; });
                    logger.Info($"IndexCacheManager MakeSureCache HaveEnough Space Before Download Succeed:{param.CacheSystem.SystemLocation.LogBase64()}");
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        logger.Warn($"IndexCacheManager MakeSureCache HaveEnoughSpace Before Download Warn,param.CacheSystem.SystemLocation:{param.CacheSystem.SystemLocation.LogBase64()},ex:{ex.ToString()}");
                    }
                }
            }
            if (!findAvailableCache)
                throw new Exception("IndexCacheManager MakeSureCache HaveEnoughSpace Before Download NoEnoughSpace Exception");
        }
        ...
        public void MakeSureCacheHaveEnoughSpaceBeforeBackup(IXSystem cacheSystem, CacheSettingDto cacheSetting)
        {
            ...
            foreach (var subSystem in (cacheSystem as XLibrary).SubSystems)
            {
                ...
                try
                {
                    cacheSystem.FindCondition = new Predicate<IXSystem>(xSystem => { return xSystem.TotalFreeSpace > minimumFreeSpace; });
                    logger.Info($"IndexCacheManager MakeSureCacheHaveEnoughSpace Before Download Succeed:{cacheSystem.SystemLocation.LogBase64()}");
                    findAvailableCache = true;
                    break;
                }
                catch (Exception ex)
                {
                    ...
                    {
                        logger.Warn($"IndexCacheManager MakeSureCacheHaveEnoughSpace Before Download Warn,cacheSystem.SystemLocation:{cacheSystem.SystemLocation.LogBase64()},ex:{ex.ToString()}");
                    }
                }
            }
            if (!findAvailableCache)
                throw new Exception("IndexCacheManager MakeSureCacheHaveEnoughSpace BeforeDownload NoEnoughSpaceException");
        }
        ...
        public void MakeSureCacheHaveEnoughSpaceAfterUpload(IXSystem cacheSystem, CacheSettingDto cacheSetting)
        {
            ulong minimumFreeSpace = cacheSetting.LimitFreeSpace;
            Dictionary<StorageInfo, long> lastAccessTimeDictionary = null;
            ulong totalFreeSpace = cacheSystem.TotalFreeSpace;
            if (totalFreeSpace < minimumFreeSpace)
            {
                lastAccessTimeDictionary = GetLastAccessTimeDictionary(cacheSystem);
                if (!DoIndexRetentionByLastAccessTime(lastAccessTimeDictionary, cacheSystem, totalFreeSpace, minimumFreeSpace))
                {
                    logger.Warn($"IndexCacheManager MakeSureCacheHaveEnoughSpace BeforeDownloadWarn,cacheSystem.SystemLocation{cacheSystem.SystemLocation.LogBase64()}");
                }
            }
        }
        ...
        private UInt64 DeleteCachedIndex(string indexVolume, string indexFileName, IXSystem cacheSystem)
        {
            ...
            try
            {
                ...
                if (fileExist)
                {
                    ...
                    logger.Info($"IndexCacheManager DeleteCachedIndexResult,indexFileName:{indexFileName.LogBase64()},indexVolume:{indexVolume.LogBase64()}");
            ...
            {
                logger.Warn($"IndexCacheRetentionManager DeleteCachedIndexWarn:{Path.Combine(indexVolume, indexFileName).LogBase64()},e:{e}");
            }
