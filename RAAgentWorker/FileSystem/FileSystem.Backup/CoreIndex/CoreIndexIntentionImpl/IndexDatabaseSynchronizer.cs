...
namespace AvePoint.Media.Core.Index
{
    ...
    public class IndexDatabaseSynchronizer : IIndexDatabaseSynchronizer
    {
        ...
        public IIndexCacheManager IndexCacheManager = new IndexCacheManager();
        ...
        public IndexDatabaseDownLoadResult Download(IndexDatabaseInfo dbInfo)
        {
            ...
            logger.Info($"IndexDatabaseSynchronizerIndexDatabaseDownLoadResultBegin,dbInfo.DbFileName:{dbInfo.DbFileName.LogBase64()}");
        ...
        {
            IndexDatabaseDownLoadResult indexDownLoadInfo = default(IndexDatabaseDownLoadResult);
            logger.Info($"IndexDatabaseSynchronizerIndexDatabaseDownLoadResultBegin:{Path.Combine(dbInfo.IndexVolume, dbInfo.DbFileName).LogBase64()}");
            var param = new IndexCacheManagerParameter()
            {
                StorageInfo = dbInfo.StorageInfo,
                IndexName = dbInfo.DbFileName,
        ...
        public IndexDatabaseUpLoadResult Upload(IndexDatabaseInfo dbInfo)
        {
            ...
            if (storageSystem != null)
            {
                ...
                //{
                    logger.Info($"IndexDataBaseSyncListenerUploadIndexBegin,dbInfo.DbFileName:{dbInfo.DbFileName.LogBase64()},storageSystem.SystemLocation:{storageSystem.SystemLocation.LogBase64()},indexVolume:{indexVolume.LogBase64()}");
                    var param = new IndexCacheManagerParameter()
                    {
                        IndexName = dbInfo.DbFileName,
                        IndexVolume = this.indexVolume,
                    ...
                    uploadResult.StorageInfo = storageResult.StorageInfo;
                //}
            }
            return uploadResult;
        }
        ...
        public IndexDatabaseUpLoadResult UploadOtherIndex(IndexDatabaseInfo dbInfo)
        {
            ...
            if (storageSystem != null)
            {
                //if (!storageSystem.IsDirectSystem || MediaConfigInfo.CommonConfigInfo.ForceUseCache)
                //{
                    logger.Info($"IndexDataBaseSyncListenerUploadIndexBegin,dbInfo.DbFileName:{dbInfo.DbFileName.LogBase64()},storageSystem.SystemLocation:{storageSystem.SystemLocation.LogBase64()},dbInfo.IndexVolume:{dbInfo.IndexVolume.LogBase64()}");
                    var param = new IndexCacheManagerParameter()
                    {
                        IndexName = dbInfo.DbFileName,
                        IndexVolume = dbInfo.IndexVolume,
