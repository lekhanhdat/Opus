...
{
    public class SharePointOnPremiseDashboardCollector : DashboardCollector
    {
        ...
        protected override async Task<List<RMDashboardDataUsage>> CollectDataUsageAsync()
        {
            ...
            var createdSql = $"SELECT c.aveSiteId, COUNT(1) AS siteUsageCount FROM items c where c.sourceFlag = {(int)Flag} and  c.recordStatus = {(int)RMRecordStatus.Active} and (c.nodeType = {(int)NodeLevel.Item} or c.nodeType = {(int)NodeLevel.Document}) GROUP BY c.aveSiteId";
            var createdDataUsageDic = ExplorerDao.QuerySiteCollectionUsageCount(createdSql);

            var destoryedSql = $"SELECT c.aveSiteId, COUNT(1) AS siteUsageCount FROM items c where  c.sourceFlag = {(int)Flag} and c.recordStatus = {(int)RMRecordStatus.Destroyed} and (c.nodeType = {(int)NodeLevel.Item} or c.nodeType = {(int)NodeLevel.Document}) GROUP BY c.aveSiteId";
            ...
            var sites = await SharePointOnPremClient.GetLocalSiteCollectionsByIdListAsync(siteIds);
...
and c.termId != {Guid.Empty} and (c.nodeType = {(int)NodeLevel.Document} or c.nodeType = {(int)NodeLevel.Item}) GROUP BY c.termId";
...
AND ARRAY_CONTAINS([{(int)NodeLevel.Document}, {(int)NodeLevel.Item}], c.nodeType)
