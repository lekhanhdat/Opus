...
namespace AvePoint.GCommon.Security.AccessControl
{
    ...
    {
        private static AveLogger logger = AveLogger.GetInstance(typeof(SecurityIdentity), false);
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj is SecurityIdentity)
            {
                ...
                return (String.Compare(this.sid, sd.sid, StringComparison.OrdinalIgnoreCase) == 0);
            }
            else return false;
        }
        ...
        public static SecurityIdentity SecurityIdentityFromString(string sid, bool throwException)
        {
            ...
            try
            {
                ...
                {
                    throw new ExternalException(String.Format("Error Converting SID String to SID Structure: {0}", Util.GetErrorMessage(Util.GetLastError())));
                }
                ...
                try
                {
                    ...
                    if (nameLen == 0)
                    {
                        throw new ExternalException(string.Format("Unable to Find SID:{0}", sid));
                    }
                    ...
                    try
                    {
                        ...
                        {
                            throw new ExternalException(string.Format("Unable to Find SID:{0} when length:{1}", sid, nameLen));
                        }
            ...
            {
                logger.Warn("Cannot get security identity from sid:{0}, exception:{1}", sid, ex);
                if(throwException)
                {
                    throw;
                }
        ...
        public static SecurityIdentity SecurityIdentityFromWellKnownSid(WELL_KNOWN_SID_TYPE sidType)
        {
            ...
            try
            {
                ...
                //The well-known SID, SID of the built-in account such as Administrator even though other products are included, SID Such are based on a SID of the computer.  Therefore, if you want to get the SID of these accounts, you must specify the SID of the computer to the second argument. 
                //CreateWellKnownSid(WinAccountAdministratorSid, pSidComputer, pSidAdministrator, &dwSidSize);
                //http://eternalwindows.jp/security/account/account12.html
                //Note!!! Has Issue
                if (!SecurityIdentity.CreateWellKnownSid(sidType, IntPtr.Zero, sidStruct, ref size))
                {
                    //System.Security.Principal.SecurityIdentifier test = new System.Security.Principal.SecurityIdentifier(System.Security.Principal.WellKnownSidType)
                    //SecurityIdentity.LookupAccountSid()
                    throw new ExternalException(string.Format("Unable to Get Well Known SID with type:{0} and error code:{1}", sidType, Marshal.GetLastWin32Error()));
                }
        ...
        private static extern bool CreateWellKnownSid(WELL_KNOWN_SID_TYPE WellKnownSidType, IntPtr DomainSid, [Out]IntPtr pSid, ref uint cbSid);
