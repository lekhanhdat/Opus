...
namespace AvePoint.ObjectModel.ClientOM
{
    public class AveProjectManager
    {
        ...
        private void RefreshProjectContext()
        {
            ...
            this.mContext = CreateProjectContext();
        }

        private bool ProcessJob(QueueJob job, string jobName)
        {
            var jobState = mContext.WaitForQueue(job, WrapperConfiguration.BPOS_S.HttpWebRequestTimeout);
            mLogger.Info("[{0}] job state:{1}", jobName, jobState.ToString());
