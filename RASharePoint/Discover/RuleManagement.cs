...
using System.Text;
using LOGRESOURCE = Merged18NResources.Common;
...
namespace AvePoint.RA.SharePoint.Discover
{
    public class RuleManagement
    {
        ...
        public readonly KeepDataOption[] KeepLatestVersionsOptions =
            [
                KeepDataOption.KeepLatestVersionAndArhiveOthers,
                KeepDataOption.KeepLatestVersion,
            ];
        #endregion property
                ...
                foreach (var rule in mRuleCollection.Rules.Select(rulet => rulet.Value))
                {
                    ...
                    if (!HasSiteCollectionCondition)
                    {
                        HasSiteCollectionCondition |= (rule.Filters.FirstOrDefault(tmp => tmp.Level == PolicyLevel.SiteCollection || tmp.Level == PolicyLevel.Teams) != null)
                            || rule.PolicyLevel == PolicyLevel.SiteCollection;
                    }
        ...
        /// </summary>
        public Rule CheckItemCriteria(Guid docId, object oItem, bool idDeclareJob = false)
        {
            IAveListItem item = null;
            AveDiscoverItem discoverItem = null;
            ...
            {
                discoverItem = (AveDiscoverItem)oItem;
            ...
            using (PerformanceScope sc = new PerformanceScope("RuleManagement.getbaseinfo", "getbaseinfo", true))
            {
                ...
                {
                    if (idDeclareJob && HasDateOfM2CColumn(item))
                    {
                        ReplaceCreatedRuleWithCustomDateRule(FilterPolicyCollection);
                    }
                    ...
                    if (accessTimeRuleCount > 0)
                    {
                        ...
                        if (discoverItem != null && discoverItem.HasGetLAT)
                        {
                            if (WrapperConfiguration.WrapperConfigurationForBPOS.LATMgtApiEnableTime == DateTime.MinValue && WrapperConfiguration.WrapperConfigurationForBPOS.SiteMgtApiEnable == false)
                            {
                                mLog.Info("Current object use last access time rule but current site does not collect last access time.");
                                return null;
                            }
                            else
                            {
                                documentInfo.StubLastAccessTime = GetItemAccessTimeV2(item, discoverItem.LastAccessTime);
                            }
                        }
                        else
                        {
                    ...
                    if (activeTimeRuleCount > 0)
                    {
                        ...
                        if (discoverItem != null && discoverItem.HasGetLAT)
                        {
                            ...
                            {
                                documentInfo.LastAccessCompatibleModifiedTime = GetItemAccessTimeV2(item, discoverItem.LastAccessTime, true);
                            }
                        }
                        else
                        {
            ...
            if (rs == null)
            {
                mLog.Info(string.Format("ItemInfo:{0} NotFitRuleItemInfoStart", docId) + SerializerHelper.SerializeToBase64StringUseJson(baseInfo)+ "NotFitRuleItemInfoEnd");
            }
            else if((rs.KeepDataOption & (int)KeepDataOption.DeleteOnly) == (int)KeepDataOption.DeleteOnly)
            {
                ...
                {
                    mLog.Info(string.Format("DeleteOnlyActionItemInfo:{0} DeleteOnlyActionStart", docId) + SerializerHelper.SerializeToBase64StringUseJson(baseInfo)+ "DeleteOnlyActionEnd");
                }
                else
                {
                    mLog.Info("This DeleteOnly action include keep latest version,no need log fileInfo");
                }
            }
            return rs;
        }
        ...
        public Rule CheckListCriteria(object oList)
        {
            ...
            if (rs == null)
            {
                mLog.LogToXml("ListInfo:", baseinfo);
            }
            return rs;
        }
        ...
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("Archiver.RuleManagement.CheckListType"))
            {
                ...
                {
                    mLog.Warn(LOGRESOURCE.SCURuleManagementCheckListType + ex.ToString());
                    return true;
                }
            }
        }
        ...
        public Rule CheckFolderCriteria(object oFolder, bool IsMicroFeedList = false)
        {
            ...
            {
                mLog.Info("Folder rule doesn't process MicroFeedList. Folder id:{0}.", folder.ID);
                rule = null;
            }
            if (rule == null)
            {
                mLog.LogToXml(string.Format("FolderInfo : {0}", folder.ServerRelativeUrl), baseInfo);
            }
            return rule;
        }
        ...
        public Rule CheckSiteCriteria(object oWeb)
        {
            ...
            if (rs == null)
            {
                mLog.LogToXml("SiteInfo:", baseInfo);
            }
            return rs;
        }
        ...
        public Rule CheckSiteCollectionCriteria(object oSitecollection)
        {
            ...
            if (rs == null)
            {
                mLog.LogToXml("SiteCollectionInfo:", baseInfo);
            }
            return rs;
        }
        ...
        public string GetDueDisposalTime(IAveListItem aveItem, Rule rule, object oItem)
        {
            AveDiscoverItem discoverItem = null;
            if (oItem != null && oItem is AveDiscoverItem)
            {
                discoverItem = (AveDiscoverItem)oItem;
            }
            DateTime resutlTime = DateTime.MinValue;
            ...
            if(rule.PolicyLevel != policyLevel || rule.Filters.All(fi => fi.Condition != PolicyCondition.OlderThan))
            {
                return dueDisposalTime;
            }
            ...
            if (rule.AndOrExpression.FirstOrDefault().Value.Contains("Or") || rule.Filters.Count == 1)
            {
                foreach (var filter in rule.Filters)
                {
                    var timeValue = GetCustomDueDate(filter, aveItem, discoverItem);
                    if (timeValue != DateTime.MinValue)
                    {
                        if (disposalTime == DateTime.MinValue || disposalTime > timeValue)// or关系取due date最小值
                        {
        ...
        private void ReplaceCreatedRuleWithCustomDateRule(List<FilterPolicy> FilterPolicyCollection)
        {
            var createdRules = FilterPolicyCollection.AsQueryable().Where(t => t.Rule.GetType().Name.Equals("CreatedRule", StringComparison.OrdinalIgnoreCase)).ToList();
            foreach (var tempCreatedRule in createdRules)
            {
                tempCreatedRule.Rule = new ColumnDateTimeRule() { Value1 = $"[{CSDFieldName.ReclassDateOfModified2Creation}]" };
                mLog.Info($"FilterPolicyAfterReplace:[{tempCreatedRule.ToString()}]");
            }
        }

        public Rule CheckItemDueDateCriteria(Guid docId, object oItem, Rule rule)
        {
            DateTime BuildQualifiedOlderThanDate(FilterPolicy currentPolicy)
            {
                return DateTime.SpecifyKind(DateTime.MinValue.AddTicks(1), DateTimeKind.Utc);
            }
            ...
            if (baseInfo is DocumentInfo)
            {
                ...
                foreach (var policy in rule.Filters.Where(f => f.Condition == PolicyCondition.OlderThan))
                {
                    var qualifiedDate = BuildQualifiedOlderThanDate(policy);
                    if (policy.Rule is CreatedRule)
                    {
                        docInfo.Created = qualifiedDate;
                    }
                    if (policy.Rule is ModifiedRule)
                    {
                        docInfo.Modified = qualifiedDate;
                    }
                    if (policy.Rule is StubLastAccessTimeRule)
                    {
                        docInfo.StubLastAccessTime = qualifiedDate;
                    }
                    if (policy.Rule is StubLastActiveTimeRule)
                    {
                        docInfo.LastAccessCompatibleModifiedTime = qualifiedDate;
                    }
                    #region custom column date
                    try
                    {
                        if (policy.Rule is ColumnDateTimeRule)
                        {
                            ...
                            if (columnName.StartsWith("[", StringComparison.OrdinalIgnoreCase) && columnName.EndsWith("]", StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                {
                                    docInfo.ColumnInfos[internalName] = qualifiedDate;
                                }
                            ...
                            else
                            {
                                ...
                                {
                                    docInfo.ColumnInfos[columnName] = qualifiedDate;
                                }
                            }
        ...
        private DateTime GetDueDate(FilterPolicy filter, IAveListItem aveItem)
        {
            ...
            if (filter.Rule is CreatedRule || filter.Rule is ModifiedRule
                    || filter.Rule is ColumnDateTimeRule || filter.Rule is StubLastAccessTimeRule || filter.Rule is StubLastActiveTimeRule || filter.Rule is ParentSiteCollectionDateTimeRule || filter.Rule is ParentLibraryDateTimeRule
                    || filter.Rule is PropertyBagDateTimeRule)
            {
                ...
                else if (filter.Rule is ColumnDateTimeRule)
                {
                    try
                    {
                        if(filter.Rule.Value1.Equals("Retention label Applied", StringComparison.OrdinalIgnoreCase))
                        {
                            timeValue = DateTime.Parse(aveItem["_ComplianceTagWrittenTime"].ToString()).ToUniversalTime();
                        }
                        else
                        {
                    ...
                    {
                        mLog.Info("no such column {0}:{1}:{2}", aveItem.ID, filter.Rule.Value1, e.ToString());
                        return timeValue;
                    }
                }
                else if(filter.Rule is ParentLibraryDateTimeRule)
                {
                    try
                    {
                        var parentListProperty = aveItem.ParentList.RootFolder.Properties;
                        if (parentListProperty.Contains(filter.Rule.Value1))
                        {
                            timeValue = (DateTime)parentListProperty[filter.Rule.Value1];
                            timeValue = ToUniversalTimeWithTimeZone(timeValue, aveItem.ParentList.ParentWeb);
                        }
                        else
                        {
                            mLog.Info("no such column {0}:{1}", aveItem.ID, filter.Rule.Value1);
                ...
                else if(filter.Rule is ParentSiteCollectionDateTimeRule)
                {
                    try
                    {
                        var parentSiteCollection = FilterAnalyser.FillSiteColumns(aveItem.Web.Site.RootWeb);
                        if (parentSiteCollection.Contains(filter.Rule.Value1))
                        {
                            timeValue = (DateTime)parentSiteCollection[filter.Rule.Value1];
                            timeValue = ToUniversalTimeWithTimeZone(timeValue, aveItem.ParentList.ParentWeb);
                        }
                        else
                        {
                ...
                else if(filter.Rule is PropertyBagDateTimeRule)
                {
                    try
                    {
                        var parentSite = aveItem.ParentList.ParentWeb.AllProperties;
                        if (parentSite.Contains(filter.Rule.Value1))
                        {
                            timeValue = (DateTime)parentSite[filter.Rule.Value1];
                            timeValue = ToUniversalTimeWithTimeZone(timeValue, aveItem.ParentList.ParentWeb);
                        }
                        else
                        {
                ...
                if (filter.Condition == PolicyCondition.OlderThan)
                {
                    ...
                    {
                        mLog.Warn($"The filter policy no.{filter.SequenceNo} of rule name: [{filter.Rule}] has time value less than min datetime. Force using min datetime");
                        timeValue = DateTime.MinValue; // avoid exception from converting to negative time zone
                    }
                    catch (Exception e)
                    {
                        mLog.Warn($"Error occured while calculate time, error: {e}");
                        throw;
                    }
        ...
        private DateTime GetCustomDueDate(FilterPolicy filter, IAveListItem aveItem, AveDiscoverItem discoverItem)
        {
            DateTime timeValue = DateTime.MinValue;
            if (filter.Condition != PolicyCondition.OlderThan)
            {
                    ...
                    || filter.Rule is ColumnDateTimeRule || filter.Rule is StubLastAccessTimeRule || filter.Rule is StubLastActiveTimeRule)
            {
                ...
                else if (filter.Rule is StubLastAccessTimeRule)
                {
                    if (discoverItem != null && discoverItem.HasGetLAT)
                    {
                        timeValue = GetItemAccessTimeV2(aveItem, discoverItem.LastAccessTime);
                    }
                    else
                    {
                        timeValue = RuleManagement.GetItemAccessTime(aveItem);
                ...
                else if (filter.Rule is StubLastActiveTimeRule)
                {
                    if (discoverItem != null && discoverItem.HasGetLAT)
                    {
                        timeValue = GetItemAccessTimeV2(aveItem, discoverItem.LastAccessTime, true);
                    }
                    else
                    {
                        timeValue = RuleManagement.GetItemAccessTime(aveItem, isCompatibleByModifiedTime: true);
                ...
                else if (filter.Rule is ColumnDateTimeRule)
                {
                    try
                    {
                        ...
                        if (columnName.StartsWith("[", StringComparison.OrdinalIgnoreCase) && columnName.EndsWith("]", StringComparison.OrdinalIgnoreCase))
                        {
                            var internalName = columnName.Trim(new char[] { '[', ']' });
                            if(aveItem.FieldValues.TryGetValue(internalName, out var value))
                            {
                        ...
                        {
                            if (aveItem.FieldValues.TryGetValue(filter.Rule.Value1, out var value))
                            {
                                timeValue = (DateTime)value;
                            }
                            else
                            {
                                try
                                {
                                    var aveColumnValue = aveItem[columnName];
                                    timeValue = (DateTime)aveColumnValue;
                                }
                                catch
                                {
                                    throw;
                ...
                if (filter.Condition == PolicyCondition.OlderThan)
                {
                    ...
                    {
                        mLog.Warn($"The filter policy no.{filter.SequenceNo} of rule name: [{filter.Rule}] has time value less than min datetime. Force using min datetime");
                        timeValue = DateTime.MinValue.AddDays(1); // avoid exception from converting to negative time zone
                    }
                    catch (Exception e)
                    {
                        mLog.Warn($"Error occured while calculate time, error: {e}");
        ...
        private DateTime GetDueDate(FilterPolicy filter, IAveFolder aveFolder)
        {
                        ...
                        || filter.Rule is ColumnDateTimeRule) // || filter.Rule is AccessTimeRule)
            {
                ...
                else if (filter.Rule is ColumnDateTimeRule)
                {
                    ...
                    {
                        mLog.Info("no such column {0}:{1}:{2}", aveFolder.ID, filter.Rule.Value1, e.ToString());
                        return timeValue;
                    }
                }
                #endregion
        ...
        private DateTime GetDueDate(FilterPolicy filter, IAveList aveList)
        {
                   ...
                   || filter.Rule is ColumnDateTimeRule) // || filter.Rule is AccessTimeRule)
            {
                ...
                else if (filter.Rule is CustomPropertyDateTimeRule)
                {
                    ...
                    {
                        mLog.Info("no such property {0}:{1}:{2}", aveList.RootFolder.ServerRelativeUrl, filter.Rule.Value1, e.ToString());
                        return timeValue;
                    }
                }
        ...
        private DateTime GetDueDate(FilterPolicy filter, IAveWeb aveWeb)
        {
                    ...
                    || filter.Rule is ColumnDateTimeRule) // || filter.Rule is AccessTimeRule)
            {
                ...
                else if (filter.Rule is CustomPropertyDateTimeRule)
                {
                    ...
                    {
                        mLog.Info("no such property {0}:{1}:{2}", aveWeb.Url, filter.Rule.Value1, e.ToString());
                        return timeValue;
                    }
                }
        ...
        private DateTime GetDueDate(FilterPolicy filter, IAveSite aveSite)
        {
                        ...
                        || filter.Rule is ColumnDateTimeRule) // || filter.Rule is AccessTimeRule)
            {
                ...
                else if (filter.Rule is CustomPropertyDateTimeRule)
                {
                    ...
                    {
                        mLog.Info("no such property {0}:{1}:{2}", aveSite.RootWeb.Url, filter.Rule.Value1, e.ToString());
                        return timeValue;
                    }
                }
            ...
            foreach (FilterPolicy policy in policies)
            {
                ...
                switch (ruleName)
                {
                    ...
                    case "ParentFolderNameHeirarchicallyRule":
                        ...
                        {
                            mLog.Info($"use file object to init name path property {e}");
                    ...
                    case "SensitivityLabelRule":
                        ...
                        if (item?.FieldValues != null)
                        {
                            ...
                            if (item.FieldValues.ContainsKey(SPColumnConstants.Sensitive_Label_Display_Name))
                            {
                                ...
                                {
                                    mLog.Info($"Get sensitive label display name have some issue {ex}");
                    ...
                    case "SensitivityLabelFullNameRule":
                        ...
                        if (item?.FieldValues != null)
                        {
                            if (item.FieldValues.ContainsKey(SPColumnConstants.Sensitive_Label_Display_Name))
                            {
                                ...
                                {
                                    mLog.Info($"Get sensitive label full name have some issue {ex}");
                                    result.SensitivityLabelFullName = labelDispalyName;
                                }
                            }
                        }
        ...
        private static Hashtable GetItemColumns(IAveListItem item)
        {
            ...
            mLog.Info($"Process item {item.ID} {item.FieldValues.Count}");
            if (item != null)
            {
                ...
                foreach (var field in item.Fields)
                {
                    try
                    {
                        ...
                        switch (field.Type)
                        {
                            ...
                            case AveFieldType.Lookup:
                                ...
                                if (field.InternalName == "_ComplianceTagWrittenTime")
                                {
                                    mLog.Info($"column is _ComplianceTagWrittenTime,will convert to time,title:{field.Title}");
                                    var dateTime = DateTime.Parse(item["_ComplianceTagWrittenTime"].ToString()).ToUniversalTime();
                                    columnCollection[field.Title.ToLower()] = dateTime;
                                }
                                else
                                {
                    ...
                    {
                        mLog.Warn($"Get the metadata of item error.Field id:{field.Title} type {field.Type}.Exception:{ex.Message}");
                    }
                }
                if (!string.IsNullOrEmpty(indexOutRangeColumns) || !string.IsNullOrEmpty(keyNotFoundColumns))
                {
                    mLog.Info($"Data column exception Index Out Range Columns {indexOutRangeColumns} Data exception column Get the metadata of item error.Field Name:{keyNotFoundColumns}");
                }
            }
        ...
        private static List<Hashtable> GetItemInternalColumns(IAveListItem item)
        {
            ...
            if (item != null)
            {
                ...
                foreach (var field in item.Fields)
                {
                    ...
                    {
                        mLog.Info($"Get the metadata of item error.Field id:{field.Title} type {field.Type}.Exception:{ex.Message}");
                    }
                }
                if (!string.IsNullOrEmpty(indexOutRangeColumns) || !string.IsNullOrEmpty(keyNotFoundColumns))
                {
                    mLog.Info($"Internal Data column exception Index Out Range Columns {indexOutRangeColumns} Data exception column Get the metadata of item error.Field Name:{keyNotFoundColumns}");
                }
            }
        ...
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.LATPerformance.GetItemAccessTime"))
            {
                ...
                mLog.Info("Fetch last access time {0} on item {1}", result, item.UniqueId);
                return result;
            }
            //var file = item.File;
        ...
        public static DateTime GetItemAccessTimeV2(IAveListItem item, DateTime itemLastAccessTime, bool isCompatibleByModifiedTime = false)
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.LATPerformance.GetItemAccessTimeV2"))
            {
                var fileModfied = item["Modified"];//item == null ? file.TimeLastModified : item["Modified"];
                DateTime modifiedTime = ToUniversalTimeWithTimeZone((DateTime)fileModfied, item.ParentList.ParentWeb);
                DateTime latMgtApiEnableTime = WrapperConfiguration.WrapperConfigurationForBPOS.LATMgtApiEnableTime;
                if (itemLastAccessTime != DateTime.MinValue)
                {
                    mLog.Info($"GetItemAccessTimeV2 [QueryLastAccessTime]Use itemLastAccessTime, itemid: {item.ID}.ItemUniqueID:{item.UniqueId}.LATTime:{itemLastAccessTime}.");
                    return itemLastAccessTime;
                }

                if (isCompatibleByModifiedTime)
                {
                    mLog.Info($"GetItemAccessTimeV2 [QueryLastAccessTime]Because compatibility, directly use modifiedTime, itemid: {item.ID}.ItemUniqueID:{item.UniqueId}.modifiedTime:{modifiedTime}.");
                    return modifiedTime;
                }
                ...
                {
                    mLog.Info($"GetItemAccessTimeV2 [QueryLastAccessTime]Use modifiedTime, itemid: {item.ID}.ItemUniqueID:{item.UniqueId}.modifiedTime:{modifiedTime}.");
                }
                else
                {
                    mLog.Info($"GetItemAccessTimeV2 [QueryLastAccessTime]Use latMgtApiEnableTime, itemid: {item.ID}.ItemUniqueID:{item.UniqueId}.latMgtApiEnableTime:{latMgtApiEnableTime}.");
                }
                return itemLastAccessTime;
            }
        }
        ...
        private Rule CheckCurrentCriteria(ObjectInfoBase info, Rule rulet, int thresholdRuleOrder = -1)
        {
            ...
            if (-1 == thresholdRuleOrder || rulet.Order < thresholdRuleOrder)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        mLog.Error("A property was not assigned while checking a SharePoint rule. Exception:{0}", ex.ToString());
                    }
