...
using RAManualApprovalCommon.ReportRelateSettingManagers;
using System;
...
namespace RAManualApprovalCommon
{
    public class ManualApprovalWorkflowManager
    {
        ...
        private static readonly HashSet<GControlWorkflowDto> _cacheGControlWorkflowAndStageId = new();
        
        private static readonly SemaphoreSlim _locker = new(1);
        
        private static readonly IGControlPlatformApprovalProcessService GControlPlatformApprovalProcessService = PlatformWindsorManager.GetService<IGControlPlatformApprovalProcessService>();
        private static readonly IRMWorkflowInformationOwnersDao WorkflowInformationOwnersDao = PlatformWindsorManager.GetService<IRMWorkflowInformationOwnersDao>();
        ...
        public static WorkflowDefinitionDto GetGControlWorkflow(string workflowReferenceId)
        {
            if (!Workflows.TryGetValue(workflowReferenceId, out var workflow))
            {
                workflow = GControlPlatformApprovalProcessService.GetPlatformApprovalProcess(new Guid(workflowReferenceId)).Result;
                if (!Workflows.TryAdd(workflowReferenceId, workflow))
                {
                    Logger.Warn($"Add workflow: [{workflowReferenceId}] to cache failed");
                }
            }
            return workflow;
        }
        ...
        public static async System.Threading.Tasks.Task SyncSiteOwnerAsync(string workflowRefernceId, Record reportInfo, Guid siteId)
        {
            ...
            var aadAccounts = GetSiteOwners(recordSiteCollection);
        ...
        public static async System.Threading.Tasks.Task SyncInformationOwnerToWorkflowInfomationOwnersAsync(List<RMAccount> accounts, Guid connectionId, string workflowReferenceId)
        {
            var workflowId = Get(workflowReferenceId).Id;
            var existInformationOwners = await WorkflowInformationOwnersDao.FindListAsync(item => item.DefinitionId == workflowId.ToString() && item.ConnectionId == connectionId);
            var informationOwners = accounts.ConvertAll(item => new RMWorkflowInformationOwner
            {
                Id = Guid.NewGuid().ToString(),
                DefinitionId = workflowId.ToString(),
                ConnectionId = connectionId,
                OwnerId = item.UserId,
                OwnerType = item.ObjectType == RMActiveDirectoryObjectType.Group ? RMActiveDirectoryObjectType.Group : RMActiveDirectoryObjectType.User
            });
            var needAdded = informationOwners.Except(existInformationOwners, new WorkflowInformationOwnerComparer());
            var needDeleted = existInformationOwners.Except(informationOwners, new WorkflowInformationOwnerComparer());
            Logger.Info($"Sync workflow information owners, workflowId: [{workflowId}], connectionId: [{connectionId}], needAdded count: [{needAdded.Count()}], needDeleted count: [{needDeleted.Count()}]");
            WorkflowInformationOwnersDao.BatchAdd(needAdded.ToList());
            WorkflowInformationOwnersDao.BatchDelete(needDeleted.ToList());
        }
        public static async Task SyncSharePointGroupAsync(string workflowId, Record reportInfo, Guid siteId, AvePoint.RA.RACommonUtility.Workflow.RMWorkflowInstance workflowInstance, bool isRetentionProcess = false)
        {
            if (isRetentionProcess)
            {
                throw new Exception("RM_MA_DoesNotSupportSPGroupForRetention");
            }
            ...
            var sPGroupsInWorkflow = workflowInstance.GetAllSharePointGroupNameAndIsAssignSiteOwners();
        ...
        private static List<AADAccount> GetSiteOwners(RemoteSiteCollection recordSiteCollection)
        {
        ...
        {
            var workflowId = Get(workflowReferenceId).Id;
            var existSiteOwners = await WorkflowSiteOwnersDao.FindListAsync(item => item.DefinitionId == workflowId.ToString() && item.SiteId == siteId && item.IsSPGroup && groupName.Equals(item.GroupName, StringComparison.OrdinalIgnoreCase));
        ...
        public static async System.Threading.Tasks.Task SyncOwnersToWorkflowSiteOwnersAsync(List<AADAccount> accounts, Guid siteId, string workflowRefernceId)
        {
        ...
        public static async Task CacheGControlWorkflowIdAndStageIdMapping(GControlWorkflowDto needToSaveGControlWorkflowIdAndStageId)
        {
            if (!_cacheGControlWorkflowAndStageId.Contains(needToSaveGControlWorkflowIdAndStageId))
            {
                await _locker.WaitAsync();
                try
                {
                    if (_cacheGControlWorkflowAndStageId.Add(needToSaveGControlWorkflowIdAndStageId))
                    {
                        Logger.Info("Cache GControl WorkflowId and StageId mapping, WorkflowId: " + needToSaveGControlWorkflowIdAndStageId.WorkflowId + ", StageId: " + needToSaveGControlWorkflowIdAndStageId.StageId);
                    }
                }
                finally
                {
        ...
        public static List<GControlWorkflowDto> GetCachedGControlWorkflowForUserTaskMappings()
        {
            return _cacheGControlWorkflowAndStageId.ToList();
        }

        public static List<string> GetNewUserTaskMapping(List<GControlWorkflowDto> datas)
        {
            return datas.Select(item => item.ApproverId).Distinct().ToList();
        }
    }
}
