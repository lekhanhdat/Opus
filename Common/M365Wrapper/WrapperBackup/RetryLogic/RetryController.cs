...
namespace ExchangeUtility.Graph
{
        ...
        public override async Task<T> Retry<T>(Func<Task<T>> tryBlockAction, RequestInfo requestInfo)
        {
            ...
            do
            {
                ...
                catch (ServiceResponseException srspEx)
                {
                    ...
                    if (srspEx.ErrorCode == ServiceError.ErrorMailboxConfiguration) throw new Exception("ErrorMailboxConfiguration");
                    if (srspEx.Message.Contains("The mailbox operation failed")) throw;
                    if (requestInfo.WebServiceMethod.Equals("GetFolder", StringComparison.OrdinalIgnoreCase) && srspEx.ErrorCode == ServiceError.ErrorAccessDenied)
                    {
                ...
                catch (ServiceRequestException sreqEx)
                {
                    ...
                    if (specificRetryTime >= maxSepcificRetryTime) throw;
                    if (sreqEx.Message.Contains("Server Unavailable") || sreqEx.Message.Contains("Service Unavailable")
                        ...
                        || (!requestInfo.WebServiceMethod.Equals("SyncFolderItems", StringComparison.OrdinalIgnoreCase) && sreqEx.Message.Contains("The request was canceled due to the configured HttpClient.Timeout"))) //throttlling error
                    {
                        System.Threading.Thread.Sleep(5 * 60 * 1000);
                    }
                    else if ((!requestInfo.WebServiceMethod.Equals("SyncFolderItems", StringComparison.OrdinalIgnoreCase) && sreqEx.Message.Contains("The request failed"))
                        || sreqEx.Message.Contains("Error while copying content to a stream", StringComparison.OrdinalIgnoreCase)
                        || sreqEx.Message.Contains("An error occurred while sending the request", StringComparison.OrdinalIgnoreCase))
                ...
                catch (Exception ex) when (ex is not XmlException && retryTime < maxRetryTime)
                {
    ...
    {
        public AADTokenRefresher(IEWSAuthObject authObj, ExchangeService service, IRetryable innerRetryable = null)
            : base(innerRetryable)
        {
        ...
        internal IEWSAuthObject AuthObj { get; private set; }
