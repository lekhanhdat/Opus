...
import { GoogleDriveFileExtension, GoogleDriveSizeRange, GoogleDriveWithoutModifiedDate } from "../Components";
...
const InactiveOptimizationProfilePanel = ({ organizationId, reRenderProfilesFunc }, ref) => {
    ...
    const onSizeRangeChange = (queryParameter) => {
        const clonedProfileInfo = _.cloneDeep(profileInfo);
        clonedProfileInfo.sizeRange = queryParameter.sizeRangeQueryParameter.sizeRange;
        clonedProfileInfo.sizeRangeQueryMode = queryParameter.sizeRangeQueryParameter.queryMode;
    ...
    const onSave = useStableCallback(async () => {
        ...
        if(actionType == DiscoveryActionType.Create) {
            const res = await GoogleDriveProfileRequester.addInactiveProfileInfo(clonedProfileInfo);
            ...
            const res = await GoogleDriveProfileRequester.updateInactiveProfileInfo(clonedProfileInfo);
            if(res.MessageType == 1) {
                showToast.error(res.ErrorMessage);
                return false;
            }
...
export default forwardRef(InactiveOptimizationProfilePanel);