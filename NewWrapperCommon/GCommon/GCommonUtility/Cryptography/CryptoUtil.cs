...
namespace AvePoint.GCommon.Utility.Cryptography
{
    public static class CryptoUtil
    {
        ...
        /// <summary>
        /// 清除内存中使用过的，且从SecureString转换而来的字符串。
        /// </summary>
        /// <param name="fromSecureStringToBSTR">安全字符串起始地址（Marshal.SecureStringToBSTR（））</param>
        /// <param name="fromPtrToStringBSTR">字符串（Marshal.PtrToStringBSTR(addr)）</param>
        public static void ClearMemory(IntPtr fromSecureStringToBSTR, ref string fromPtrToStringBSTR)
        {
            //清理掉非托管代码中对应的内存
            Marshal.ZeroFreeBSTR(fromSecureStringToBSTR);
            //清理托管代码对应的内存的值（采用重写的方法）
            int id = MemoryCleaner.GetProcessID();
            byte[] writeBytes = GetBrokenDownBytes(fromPtrToStringBSTR.Length * 2 - 1);
            IntPtr intPtr = MemoryCleaner.Open(id);

            unsafe
            {
                fixed (char* c = fromPtrToStringBSTR)
                {
                    MemoryCleaner.WriteMemory((IntPtr)c, writeBytes, (IntPtr)writeBytes.Length);
                }
            }
