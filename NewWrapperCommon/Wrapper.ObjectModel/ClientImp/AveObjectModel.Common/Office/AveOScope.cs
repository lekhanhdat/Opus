...
namespace AvePoint.ObjectModel.Common.Office
{
    class AveOScope : AveClientObject, IAveOScope
    {
        ...
        public AveOScope(Dictionary<string, object> scopeProp, IAveRequest m_Request)
        {
        ...
        public AveScopeCompilationState CompilationState
        {
            get
            {
                string state = base.DataCache.PropertiesCache["CompilationState"].ToString();
                if (!state.Equals("Ready") && !state.Equals("準備完了"))
                {
