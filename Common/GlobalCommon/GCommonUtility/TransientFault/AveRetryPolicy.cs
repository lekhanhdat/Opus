...
namespace AvePoint.GCommon.Utility.TransientFault
{
    ...
    public class AveRetryPolicy
    {
        ...
        public virtual TResult ExecuteAction<TResult>(Func<TResult> func)
        {
            ...
            while (true)
            {
                ...
                catch (Exception ex3)
                {
                    ...
                    {
                        logger.Debug("Retry policy terminated. Current retry count {0}", num);
                        throw;
                    }
                }
        ...
        public Task<TResult> ExecuteAsync<TParam, TResult>(Func<TParam, TResult> action, TParam param) 
        {
            Guard.ArgumentNotNull(action, "action");
            return this.ExecuteAsync<TResult>(() => { return new TaskFactory().StartNew<TResult>(() => { return action(param); }); });
            //return this.ExecuteAsync<TResult>(delegate 
            //{
            //   return action(param);
            //});
        }
        ...
        public Task ExecuteAsync(Func<Task> taskAction, CancellationToken cancellationToken)
        {
            ...
            return new AveAsyncExecution(taskAction, this.AveRetryStrategy.GetShouldRetry(), new Func<Exception, bool>(this.ErrorDetectionStrategy.IsTransient), new Action<int, Exception, TimeSpan>(this.OnRetrying), this.AveRetryStrategy.FastFirstRetry, cancellationToken).ExecuteAsync();
        }
        /// <summary>
        /// 异步方式执行某操作，配以相应重试策略
        /// </summary>
        ...
        public Task<TResult> ExecuteAsync<TResult>(Func<Task<TResult>> taskFunc, CancellationToken cancellationToken)
        {
            ...
            return new AveAsyncExecution<TResult>(taskFunc, this.AveRetryStrategy.GetShouldRetry(), new Func<Exception, bool>(this.ErrorDetectionStrategy.IsTransient), new Action<int, Exception, TimeSpan>(this.OnRetrying), this.AveRetryStrategy.FastFirstRetry, cancellationToken).ExecuteAsync();
        }
    ...
    /// 统一重试策略，当执行操作可能遇到临时性故障或异常需要重试时使用
    /// </summary>
    public class AveRetryPolicy<T> : AveRetryPolicy where T : ITransientErrorDetectionStrategy, new()
    {
