...
import DiscoveryGoogleDriveScope from '../Constants/DiscoveryGoogleDriveScope'
import ExclusionsComponent from "../Components/ExclusionsComponent";
import {
    GoogleDriveAnalysisConfigurationScopeComponent,
    GoogleDriveAnalysisConfigurationInactiveComponent,
    GoogleDriveAnalysisConfigurationRotComponent
} from './Components'
...
const DEFAULT_DISCOVERY_INFO = {
    scopeInfo: {
        scopeType: DiscoveryGoogleDriveScope.All,
        specifyContainerIds: [],
    },
    sizeRangeInfoes: [],
    dateRangeInfoes: [],
...
const AnalysisConfigurationEditPage = ({ history }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const configurationInfos = fetchUtility({
                url: "/api/RMDiscoveryGoogleConfigurationApi/GetConfigurationInfo",
                method: "Get"
            });
            ...
            const ggDrivecontainers = fetchUtility({
                url: "/api/RMDiscoveryGoogleConfigurationApi/GetNewlyAvaliableOpusContainers",
                method: "Get"
            });
            const values = await Promise.all([configurationInfos, jobStatusInfo, ggDrivecontainers]);
            setDriveContainers(Array.isArray(values[2]) ? values[2] : []);
            $$.loading(false);

            setJobStatusInfo(values[1]);
            stepsRef.current = steps;

            const clonedConfigurationInfo = {
                ...values[0],
                sizeRangeInfoes: values[0].sizeRangeInfoes || [],
                dateRangeInfoes: values[0].dateRangeInfoes || [],
                inactiveDefinition: {
                    ...DEFAULT_DISCOVERY_INFO.inactiveDefinition,
                },
                rotDefinition: {
                    ...values[0].rotDefinition,
                    activeTab: 0,
                },
            }
            setSteps(steps);
    ...
    const onChange = (field, value) => {
        let clonedConfigurationInfo = Object.assign({}, configurationInfo);
    ...
    const runDiscoveryDoAction = async () => {
        ...
        if (clonedConfigurationInfo.scopeInfo.specifyContainerIds.length === driveContainers.length) {
            clonedConfigurationInfo.scopeInfo.scopeType = DiscoveryGoogleDriveScope.All;
        } else {
            clonedConfigurationInfo.scopeInfo.scopeType = DiscoveryGoogleDriveScope.Specify;
        }
        const result = await fetchUtility({
            url: "/api/RMDiscoveryGoogleConfigurationApi/AddOrUpdateNewlyConfigurationInfo",
                ...
                <div className="reco-fac-panel">
                    ...
                    <div className="reco-fac-form-panel">
                        <div className="reco-fac-form-content">
                            ...
                            >
                                <GoogleDriveAnalysisConfigurationScopeComponent
                                    allDriveContainer={driveContainers}
                            ...
                            >
                                <GoogleDriveAnalysisConfigurationInactiveComponent
                                    info={configurationInfo.inactiveDefinition}
                                    hasJob={jobStatusInfo.hasLatestJob}
                                    onChange={(value) => onChange("inactiveDefinition", value)}
                                    ref={steps[2].ref}
                                />
                            </div>
                            <div
                            ...
                            >
                                <GoogleDriveAnalysisConfigurationRotComponent
                                    ...
                                    hasJob={jobStatusInfo.hasLatestJob}
                                    onChange={(value) => onChange("rotDefinition", value)}
                                    ref={steps[3].ref}
                                />
                            </div>
                        </div>
                        <div className="reco-fac-form-bottom">
                            ...
                            {steps[3].status === ConfiguratorStepStatus.Processing ? (
                                    <div>
                                        <R.Button
                                            primary={true}
                                            classify={"theme"}
