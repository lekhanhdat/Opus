...
{
    /// <summary>
    /// 生产者消费者模型，用来控制缓存大小，进而控制内存
    /// 使用一次PCContainer 后一定要进行Dispose ，或者主动调用EndProduce +EndConsume + StartProduce， 否则直接进行第二轮使用，可能会导致第一次consume时候，发现mProducerEnd = true，就返回了null，直接退出discover
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public class PCContainer<T> : IDisposable where T : class
    {
        private readonly long mMaxCount = 1000;
        ...
        public PCContainer(long maxCount)
            : this()
        {
            mMaxCount = maxCount;
        }
        ...
        public void Produce(T obj)
        {
            lock (mLocker)
            {
                do
                {
                    ...
                    if (mProducerEnd)
                    {
                        throw new ObjectDisposedException("Produce end.");
                    }
                ...
                if (mLists.Count > mMaxCount)
                {
                    throw new SynchronizationLockException("Count is over the max count of container.");
                }
        ...
        public T Consume()
        {
            lock (mLocker)
            {
                do
                {
                    if (mConsumerEnd)
                    {
                        throw new ObjectDisposedException("Consume end.");
                    }
                    if (0 == mLists.Count)
                    {
                        if (mProducerEnd)
