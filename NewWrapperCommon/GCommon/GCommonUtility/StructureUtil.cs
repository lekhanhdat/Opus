        ...
        public static void AddVirtualNode(SPTreeNodeDto node)
        {
            if (node != null)
            {
                ...
                if (node.Level == NodeLevel.Site && node.ChildrenLoaded)
                {
                    ...
                    if (node.SPVersion >= (Int32)AveSPVersion.SharePoint2013 )
                    {
                        ParseNodeExtraOptions(appsNode, node.ExtraOptions);
                        //当包含apps属性是ExtraOptions个数为27个
                        if (node.IncludeNew == IncludeNewState.Checked && node.ExtraOptions.Count<27)
                        {
                            appsNode.Expanded = false;
                            appsNode.IncludeNew = IncludeNewState.Checked;
                            appsNode.ChildrenLoaded = false;
                            appsNode.CheckNumber = 1;
                            appsNode.SelectAll = SelectAllState.Checked;
                            appsNode.CurrentPage = 0;
                            appsNode.ChildrenCount = appsNode.Children.Count;
                            appsNode.StartIndex = 0;
                        }
                    }
                    if (node.SPVersion >= (Int32)AveSPVersion.SharePoint2013)
                    {
        ...
        public static void AddVirtualNodeForAdminSearch(SPTreeNodeDto root)
        {
            if (root != null)
            {
                ...
                if (root.Level == NodeLevel.Site || root.Level == NodeLevel.RootFolder || root.Level == NodeLevel.Folder)
                {
                    ...
                    SPTreeNodeDto itemsNode = new SPTreeNodeDto() { ID = Guid.NewGuid().ToString(), Name = GConstants.SPNodeName.Items, Level = NodeLevel.Items, CanChildrenBeLoaded = false, FarmID = root.FarmID, SPType = root.SPType, PageNodeType = PageNodeType.PreNext, SPVersion = root.SPVersion };
