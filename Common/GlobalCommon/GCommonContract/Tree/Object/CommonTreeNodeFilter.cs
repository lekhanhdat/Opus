...
namespace AvePoint.GCommon.Contract.Tree.Object
{
    ...
    public class CommonTreeNodeFilter : IFilter<IAveTreeNodeDto>
    {
        ...
        /// 判断节点是否应该被过滤掉。
        /// </summary>
        /// <param name="node">需要验证的节点</param>
        /// <returns>true表示过滤掉</returns>
        public bool IsMatch(IAveTreeNodeDto node, List<NodeFilterPolicy> filters)
        {
            if (filters == null)
            {
                return true;
            ...
            foreach (NodeFilterPolicy nodePolicy in filters)
            {
                ...
                if (_intFilterPolicy.IsMatch(GetLevelValue(nodePolicy.Level), (int)node.Level) &&
                    _intFilterPolicy.IsMatch(nodePolicy.Type, (int)node.Type) &&
                    _stringFilterPolicy.IsMatch(nodePolicy.DisplayName, node.Name) &&
                        (
                        !(node is SPTreeNodeDto) ||
                        _boolFilterPolicy.IsMatch(nodePolicy.Hidden, ((SPTreeNodeDto)node).Hidden) &&
                        _intFilterPolicy.IsMatch(nodePolicy.Template, ((SPTreeNodeDto)node).Template)
                        )
                    )
                {
