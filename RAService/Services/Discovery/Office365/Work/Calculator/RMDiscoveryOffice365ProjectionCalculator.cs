...
{
    public class RMDiscoveryOffice365ProjectionCalculator
    {
        private readonly RALogger _logger = RALogger.GetInstance(typeof(RMDiscoveryOffice365ProjectionCalculator));

        private readonly IRMDiscoveryOffice365DataDao _dataDao;

        private readonly IRMDiscoveryOffice365TenantConfigurationDao _configurationDao;

        private readonly IRMDiscoveryOffice365TenantDao _tenantDao;

        public RMDiscoveryOffice365ProjectionCalculator()
        {
            _dataDao = new RMDiscoveryOffice365DataDao();
            _configurationDao = new RMDiscoveryOffice365TenantConfigurationDao();
            _tenantDao = new RMDiscoveryOffice365TenantDao();
        }
        ...
        public async Task<bool> CalculateAsync()
        {
            try
            {
                ...
                var discoveredO365Tenants = await _tenantDao.GetAllAsync();
        ...
        private async Task<Dictionary<Guid, (long latestDateTime, long latestStorageSize, long oldestDateTime, long oldestStroageSize, long montylyGrowthStorageSize)>> CalculateO365TenantStorageMonthlyGrowthValueAsync(SourceFlag contentSource, List<RMDiscoveryOffice365TenantInfo> discoveredO365Tenants)
        {
            ...
            foreach (var discoveredO365Tenant in discoveredO365Tenants)
            {
                ...
                var manager = new RMGraphUsageReportManager(o365TenantId.ToString());
        ...
        private async Task<Dictionary<Guid, long>> CalculateO365TenantArchiveSpeedAsync(List<RMDiscoveryOffice365TenantInfo> discoveredO365Tenants)
        {
