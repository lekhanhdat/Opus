...
namespace AvePoint.GCommon.Utility.OS.Windows.Wmi
{
    ...
    internal class WindowsOperatingSystem : WindowsOperatingSystemBase, IOperatingSystem
    {
        ...
        public WindowsOperatingSystem()
        {
            ...
            {
                var win32OperatingSystemCollection = AvePoint.GCommon.Utility.OperatingSystem.GetInstances(new String[] { "Caption", "CSDVersion", "SerialNumber", "TotalVisibleMemorySize" });
                foreach (AvePoint.GCommon.Utility.OperatingSystem operatingSystemItem in win32OperatingSystemCollection)
                {
                ...
                var win32ProcessorCollection = Processor.GetInstances(new String[] { "CurrentClockSpeed", "Name" });
        ...
        Int64 GetFreePhysicalMemory()
        {
            ...
            var win32OperatingSystemCollection = AvePoint.GCommon.Utility.OperatingSystem.GetInstances(new String[] { "FreePhysicalMemory" });
            foreach (AvePoint.GCommon.Utility.OperatingSystem operatingSystemItem in win32OperatingSystemCollection)
            {
                using (operatingSystemItem)
                {
