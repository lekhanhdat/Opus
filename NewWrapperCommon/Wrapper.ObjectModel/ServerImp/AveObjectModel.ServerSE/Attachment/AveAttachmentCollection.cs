...
namespace AvePoint.ObjectModel.ServerSE
{
    class AveAttachmentCollection : AveAbstractCommonCollection<IAveAttachment>, IAveAttachmentCollection
    {
            ...
            : base(AveAssemblyUtility.GetFieldValue(attachments, "m_Attachments") as IList)
        {
            ...
            mDirty = false;
            var path = AveAssemblyUtility.GetFieldValue(attachments, "m_strServerRelativeUrlPrefix");
            mServerRelativeUrlPrefix = path == null ? string.Empty : path.ToString();
        }
        ...
        {
            //ADO-207038 ADO-228248 参考ADO-207038改法 将阈值提高到2G，走else 如果list 开启version的话 AddNow 和 SaveBinaryExtension 两个方法都会导致涨version
            //为保持功能稳定 将阈值提高到2G，如果客户真的存在特别大的attachment 导致内存问题 考虑使用开关list version 的方式 使用else 方法 避免内存问题
            ...
            else
            {
                ...
                var file = aveWeb.Web.GetFile(SPResourcePath.FromDecodedUrl(fileServerRelativeUrl));
