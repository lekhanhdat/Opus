...
namespace AvePoint.Wrapper.Mapping
{
    ...
    public class FMExcelOpenXml : IDisposable
    {
        ...
        protected Dictionary<string, string> mFirstLine;
        /// <summary>
        ...
        public void CreateExcel(string excelPath)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Wrapper.Mapping.FMExcelOpenXml.CreateExcel"))
            {
                ...
                {
                    throw new AveWrapperBaseException(AveInternalResourceKey.Wrapper_Exception_Mapping_PathTooLongException ,excelPath);
                }
                ...
                catch (System.UnauthorizedAccessException e)
                {
                    ...
                    throw new Exception("Unauthorized access exception.");
                }
                catch (Exception e)
                {
                    mIsCreateFailed = true;
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Wrapper.Mapping.FMExcelOpenXml.Open"))
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        mCurrentWorksheetPart = (WorksheetPart)mCurrentWorkbooPart.GetPartById(tempRelid);
        ...
        /// <summary>
        /// 此方法返回的Dic  key 为excel 第一行的值，value 为第二行开始的值
        /// </summary>
        /// <returns></returns>
        public Dictionary<string, string> ReadLine()
        {
            using (new AvePerformanceScope("Wrapper.Mapping.FMExcelOpenXml.ReadLine"))
            {
                ...
                try
                {
                    //取第一行的column name 集合。 key 为excel 的标签名。如：A1。 value 为sp中的column name。
                    Regex reg = new Regex(@"\d");
                    FillFirstLineDic(reg);

                    Dictionary<string, string> linekeyValues = new Dictionary<string, string>();
                    if (mExcelCellsDic.Count >= mCurrentRowIndexRead && mExcelCellsDic.ContainsKey(mCurrentRowIndexRead.ToString()))
                    {
                        ...
                        {
                            var tempStr = GetValue(cell, mSharedStringTablePart);
                            var tempCol = reg.Replace(cell.CellReference.ToString(), "");
                            linekeyValues.Add(tempCol, tempStr.Trim());
                        }
                    }

                    //遍历第一行
                    string tempVal;
                    foreach (KeyValuePair<string, string> k in mFirstLine)
                    {
                        if (linekeyValues.ContainsKey(k.Key))//如果对应的位置有值 那么加入
                        {
                            if (linekeyValues.TryGetValue(k.Key, out tempVal))
                            {
                                dic[k.Value] = tempVal;
                            }
                            else
                            {
                                log.Warn("The Excel format error. key was not in collection. key:{0},", k.Key);
                            }
                        }
                ...
                {
                    log.Error(" Read Current Sheet Error.Exception:{0}" , e);
                    mCurrentRowIndexRead++;
                }
                return dic;
            }
        ...
        private void FillFirstLineDic(Regex reg)
        {
            if (mFirstLine == null)
            {
                mFirstLine = new Dictionary<string, string>();
                ...
                if (mExcelCellsDic.ContainsKey("1"))
                {
                    foreach (Cell cell in mExcelCellsDic["1"])
                    {
                        ...
                        {
                            if (!mInsensitiveTableHeader.ContainsKey(tempStr.ToLower(CultureInfo.InvariantCulture)))
                            {
                                mInsensitiveTableHeader.Add(tempStr.ToLower(CultureInfo.InvariantCulture), tempStr);
                            }
                            var tempCol = reg.Replace(cell.CellReference.ToString(), "");
                            mFirstLine.Add(tempCol, tempStr.Trim());
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// 走入此方法，说明在遍历一个新的sheet
        /// </summary>
        private void InitExcelRows()
        {
            mFirstLine = null;
            ...
            var readLineRows = mCurrentWorksheet.Descendants<Row>();
            foreach (Row w in readLineRows)
            {
        ...
        public void WriteLine(Dictionary<string, string> values)
        {
            ...
            if (mIsCreateFailed)
            {
                ...
                throw new AveWrapperBaseException(AveInternalResourceKey.Wrapper_Exception_Mapping_FailedToCreateExcelFile);
            }
            ...
            if (!sheetId.Equals(mTempSheetId, StringComparison.OrdinalIgnoreCase))
            {
                if (mCurrentWorksheetPart != null)
                {
                    mCurrentWorksheetPart.Worksheet.Save();
                }
                mCurrentWorksheetPart = (WorksheetPart)mDoc.WorkbookPart.GetPartById(sheetId);
                mTempSheetId = sheetId;
                mCurrentRow = 1;
                needToAddHead = true;
            }
        ...
        [SuppressMessage("FxCopCustomRules", "C100007:SpellCheckStringValues", Justification = "Forheader is the part of method name. ")]
