    ...
    class AveNavigation : AveClientObject, IAveNavigation
    {
        ...
        public IAveNavigationNode Home
        {
            get
            {
                if (base.DataCache.IsPropertyNotLoaded("Home"))
                {
                    ...
                    base.DataCache.PropertiesCache["Home"] = homeNode;
                }
        ...
        public IAveNavigationNodeCollection QuickLaunch
        {
            get
            {
                if (base.DataCache.IsPropertyNotLoaded("QuickLaunch"))
                {
                    ...
                    base.DataCache.PropertiesCache["QuickLaunch"] = aveNavigationNodCol;
        ...
        public IAveNavigationNodeCollection TopNavigationBar
        {
            get
            {
                if (base.DataCache.IsPropertyNotLoaded("TopNavigationBar"))
                {
                    ...
                    base.DataCache.PropertiesCache["TopNavigationBar"] = aveNavigationNodCol;
                    return aveNavigationNodCol;
                }
        ...
        public bool UseShared
        {
            ...
            set
            {
                ...
                base.DataCache.PropertiesCache["UseShared"] = value;
            }
        }
        ...
        private void BuildNavNodesTree(AveNavigationInfo parentNode, AveNavigationNodeCollection NodeCollection, AveNavigationScope scope)
        {
            foreach (AveNavigationNode node in NodeCollection)
            {
                ...
                {
                    mLogger.Debug(AveObjectModel_CommonResource.BuildNavNodesTreeError, node.Title, this.mWeb.Url, e.ToString());
                }
            }
        }
        #endregion
        ...
        public bool RestoreNavigation(AveNavigationInfoList navigationList, NavigationRestoreSetting setting)
        {
            ...
            if (navigationList.BackupFromInheritedWeb)
            {
                webAllProp["BackupFromInheritedWeb"] = true;
            }
            webAllProp["IsOnlineSite"] = this.mWeb.Site.SPVersion.StartsWith("16", StringComparison.OrdinalIgnoreCase) ? true : false;
            bool compoundSupport = mRequest.RestoreNavigation(this.mWeb.ServerRelativeUrl, nodes, webAllProp);
        ...
        public bool NeedRestore(AveNavigationScope aveNavigationScope, AveNavigationInfoList value)
        {
            ...
            {
                if (mWeb.IsRootWeb && value.ShareQuickLaunch)
                {
        ...
        private bool IsUrlAvailable(string nodeUrl, object obj)
        {
            ...
            return AveHttpWebRequestUtility.IsUrlAvailable(absoluteUrl, obj);
        }
        ...
        private string GetNodes(AveNavigationInfoList navigationList)
        {
            ...
            object credential = (this.mWeb.Site as AveSite).Request.Credentials;

            foreach (AveNavigationInfo navInfo in navigationList.NavNodes.Where(navInfo => NeedRestore(navInfo.Scope, navigationList)))
            {
                if (i == 0 && navInfo.NodeType == -1 && (navInfo.Url.Equals(this.mWeb.Url) || navInfo.Url.Equals(this.mWeb.ServerRelativeUrl)) && navInfo.Scope != AveNavigationScope.QuickLaunch)
                {
                    i++;
                    continue;
                }
                ...
                if (!navInfo.IsExternal && !IsUrlAvailable(navInfo.Url, credential))
                {
                    continue;
                }
                ...
                string nodeProp = "<" + id + ">" + name + ">" + url + ">" + description + ">" + nodeType + ">" + visible + ">" + target + ">" + audience + ">" + created + ">" + modified;
                ...
                if (navInfo.Children.Count > 0)
                {
                    ...
                    GetSubNavigationNodes(children, navInfo.Children, navigationList.SharedTopLink, navigationList.ShareQuickLaunch, navigationList.BackupFromInheritedWeb);
        ...
        private void GetSubNavigationNodes(Dictionary<string, object> parent, List<AveNavigationInfo> childrenNavigationNodes, bool isSharedTopLink, bool isShareQuickLunch, bool backupFromInheritedWeb)
        {
            int i = 0;
            object credential = (this.mWeb.Site as AveSite).Request.Credentials;
            foreach (AveNavigationInfo navInfo in childrenNavigationNodes)
            {
                ...
                if (navInfo.Scope == AveNavigationScope.QuickLaunch && mWeb.Properties != null && mWeb.Properties["__InheritCurrentNavigation"] == "True" && !isShareQuickLunch && backupFromInheritedWeb)
                {
                    continue;
                }
                ...
                if (nodeType.Equals(AveNodeTypes.Area.ToString()))
                {
                    IAveWeb web = (this.mWeb.Site as AveSite).OpenWeb(url);
                ...
                {
                    Dictionary<string, object> children = new Dictionary<string, object>();
                    GetSubNavigationNodes(children, navInfo.Children, isSharedTopLink, isShareQuickLunch, backupFromInheritedWeb);
                    parent["<\"" + id] = children;
                }
            }
        }
