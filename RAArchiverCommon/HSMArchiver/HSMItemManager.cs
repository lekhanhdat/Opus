...
namespace RAArchiverCommon.HSMArchiver
{
    public class HSMItemManager : SqliteDBBase, IDisposable
    {
        private static AvePoint.RA.Contract.Services.IRALogger logger = RALogger.GetInstance(typeof(HSMItemManager));
        ...
        private const string DB_NAME_PREFIX = "HSMArchiverTemp";

        private const string DB_SUFFIX = ".rpt";

        private const string TABLE_NAME = "Items";

        private string QEURY_SQL_BASE = $"SELECT [Id],[FullUrl],[ModifiedTime],[FileSize],[ItemId],[ListId],[WebId],[ItemUniqueId] FROM {SecurityUtils.SanitizeSQLSchemaName(TABLE_NAME)} ";

        private string GET_COUNT_SQL_BASE = $"SELECT Count(*) FROM {SecurityUtils.SanitizeSQLSchemaName(TABLE_NAME)} ";
        ...
        public static HSMItemManager GetInstance()
        {
            return new HSMItemManager(CACHE_FODER_PATH, DB_NAME_PREFIX + DateTime.Now.Ticks + DB_SUFFIX);
        }
        ...
        private HSMItemManager(string dirPath, string name) : base(dirPath, name)
        {
            _dbName = name;
        ...
        public override void CreateSchemaIfNotExists(IDbCommand command)
        {
            string query = string.Format($"CREATE TABLE IF NOT EXISTS {SecurityUtils.SanitizeSQLSchemaName(TABLE_NAME)}(" +
                ...
                "[ItemId] [int] NOT NULL," +
                "[ListId] [nvarchar](500) NOT NULL," +
                "[WebId] [nvarchar](500) NOT NULL," +
                ...
                using (var command = conn.CreateCommand())
                {
                    foreach (var dataInfo in dataInfos)
                    {
                        ...
                        SQLiteParameter[] parameters = {
                            ...
                            new SQLiteParameter{ ParameterName = "@ListId" , Value = dataInfo.ListId},
                            new SQLiteParameter{ ParameterName = "@WebId" , Value = dataInfo.WebId},
        ...
        private List<RMDiscoveryFileDataInfo> ParseResultFromReader(IDataReader reader)
        {
            ...
            while (reader.Read())
            {
                ...
                info.ListId = reader.GetString(5);
                info.WebId = reader.GetString(6);
