...
namespace AvePoint.ObjectModel.Common
{
    class AveXmlDocumentCollection : AveAbstractCommonCollection<string>, IAveXmlDocumentCollection
    {
        ...
        public void Add(System.Xml.XmlDocument document)
        {
            ...
            {
                AddXmlDocument(document);
            }
            else
            {
                var temp = mDict[document.DocumentElement.NamespaceURI].ToString();
                if (!string.Equals(temp, document.DocumentElement.OuterXml, StringComparison.OrdinalIgnoreCase))
                {
                    AddXmlDocument(document);
                }
            }
        }
        private void AddXmlDocument(System.Xml.XmlDocument document)
        {
