...
namespace AvePoint.RA.SharePoint.Extension
{
    public static class AveListItemExtension
    {
        ...
        public static bool GetSingleTaxonomyFieldValue(this IAveListItem item, string fieldName, out Guid termId, out string termName)
        {
            ...
            {
                mLog.Warn("Get single taxonomy field value failed! Item url: {0}, fieldName: {1}, error message: {2}.", item.Url.LogBase64() ,fieldName.LogBase64(), ex.ToString());
                result = false;
            }
            return result;
        }
        ...
        public static string GetSingleUserFieldValue(this IAveListItem item, string fieldName)
        {
            ...
            {
                mLog.Warn("Get single user field value failed! Item url: {0}, fieldName: {1}, error message: {2}.", item.Url.LogBase64(), fieldName.LogBase64(), ex.ToString());
            }
            return userName;
        }
        ...
        public static DateTime GetUTCDateWithTimeZone(this IAveListItem item, string fieldName)
        {
            ...
            if (item.FieldValues.ContainsKey(fieldName))
            {
                ...
                {
                    mLog.Error("An error occurred while getting utc time for field:{0} error:{1}", fieldName.LogBase64(), e.ToString());
        ...
        public static DateTime GetDateTimeFieldValue(this IAveListItem item, IAveTimeZone SPWebTimeZone, string fieldName)
        {
            ...
            try
            {
                TimeZoneInfo cstZone = TimeZoneInfo.FindSystemTimeZoneById(AveTimeZoneUtility.ToTimeZoneInfoId(SPWebTimeZone.ID));
            ...
            {
                mLog.Warn("Get datetime field value failed! Item url: {0}, fieldName: {1}, error message: {2}.", item.Url.LogBase64(), fieldName.LogBase64(), ex.ToString());
                try
                {
                    return SPWebTimeZone.UTCToLocalTime(dt);
                }
                ...
                {
                    mLog.Warn("Get datetime field value failed! Item url: {0}, fieldName: {1}, error message: {2}.", item.Url.LogBase64(), fieldName.LogBase64(), e1.ToString());
                }
            }
            return new DateTime();
        }
        ...
        public static string GetItemFieldValue(this IAveListItem item, string fieldName)
        {
            ...
            try
            {
                ...
                {
                    mLog.Debug("The item doesn't have this field. Item id: {0}", item?.ID);
                }
            ...
            {
                mLog.Warn("Get field value failed! Item url: {0}, error message: {1}.", item?.ID, ex.ToString());
            }
            return fieldValue;
        }
        ...
        public static bool IsHoldOnly(this IAveListItem item)
        {
            var status = GetHoldAndRecordStatus(item);
