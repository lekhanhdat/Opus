...
namespace AvePoint.ObjectModel.Common
{
    class AveUser : AvePrincipal, IAveUser
    {
        ...
        public AveUser(IAveRequest request, AveWeb parentWeb, string source, Dictionary<string, object> properties)
        {
            mRequest = request;
        ...
        public bool IsSiteAdmin 
        {
            ...
            set
            {
                List<Dictionary<string, object>> newAdmins = new List<Dictionary<string, object>>();
                StringBuilder oldAdmins = new StringBuilder();
                foreach (IAveUser user in this.mParentWeb.SiteAdministrators)
                {
                    oldAdmins.Append(user.LoginName.ToLowerInvariant());
                    oldAdmins.Append(",");
        ...
        { 
            ...
            set
            {
                base.DataCache.PropertiesCache["RegionalSettings"] = value;
        ...
        public string NoPrefixLoginName
        {
            get
            {
                if (!string.IsNullOrEmpty(base.LoginName) && base.LoginName.LastIndexOf('|') > 0)
                {
                    return base.LoginName.Substring(base.LoginName.LastIndexOf('|') + 1);
                }
        ...
        public IAveUserIdInfo UserId
        {
            get
            {
                if (base.DataCache.IsPropertyNotLoaded("UserId"))
                {
                    if (base.DataCache.IsPropertyAvailable("UserId" + AveObjectModelConstant.ObjectPropertySuffix))
                    {
                        var userIdInfo = new AveUserIdInfo(base.DataCache.GetProperty<Dictionary<string, object>>("UserId" + AveObjectModelConstant.ObjectPropertySuffix));
                        base.DataCache.PropertiesCache["UserId"] = userIdInfo;
                        return userIdInfo;
                    }
                    else
                    {
                        var properties  = this.mRequest.GetEnsureUser(this.ParentWeb.ServerRelativeUrl, LoginName);
                        if (properties.ContainsKey("UserId" + AveObjectModelConstant.ObjectPropertySuffix))
                        {
                            var userIdInfo = new AveUserIdInfo((properties["UserId" + AveObjectModelConstant.ObjectPropertySuffix]) as Dictionary<string, object>);
                            base.DataCache.PropertiesCache["UserId"] = userIdInfo;
                            return userIdInfo;
                        }
                    }
                }
                return base.DataCache.GetProperty<IAveUserIdInfo>("UserId");
            }
        }
    }
}
