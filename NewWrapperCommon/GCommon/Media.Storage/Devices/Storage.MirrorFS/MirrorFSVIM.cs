...
namespace AvePoint.Media.Storage.MirrorFS
{
    ...
    [VIM(VIMName.MirrorFS, typeof(AvePoint.Media.Storage.MirrorFS.MirrorFSVIM))]
    class MirrorFSVIM : AbstractVIM
    {
        public override IXSystem CreateSystem(string xri, AbstractXSystem parentSystem)
        {
            return new MirrorFSSystem(xri, parentSystem);
        }

        public override List<string> GetFeatureXML(int type)
        {
            return null;// MirrorFSFeature.Instances[type].FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(int type)
        {
            return null;// MirrorFSFeature.Instances[type].FeatureObjs;
        }
