...
namespace AvePoint.ObjectModel.Common
{
    class AveChangeCollection : AveAbstractCommonCollection<IAveChange>, IAveChangeCollection
    {
        ...
        {
            List<Dictionary<string, object>> ChangePropertiesList = base.DataCache.GetChildren();
            mListData = new List<IAveChange>(ChangePropertiesList.Count);
            foreach (Dictionary<string, object> dic in ChangePropertiesList)
            {
                IAveChange tempChange = GenerateChangeObject(dic);
                mListData.Add(tempChange);
            }
        }
        private IAveChange GenerateChangeObject(Dictionary<string, object> changeProperties)
        {
        ...
        public IAveChangeToken LastChangeToken
        {
            get
            {
                ...
                return (this[Count - 1] as AveChange).ChangeToken;
            }
        }
