...
{
    public class RMDiscoveryOffice365SQLiteDBManager
    {
        ...
        private const string SQLITE_DB_NAME = "discovery.db";
        ...
        public static RMDiscoveryOffice365SQLiteDBContext GetContext()
        {
        ...
        {
            var dbPath = GetDBPath();
            using var context = new RMDiscoveryOffice365SQLiteDBContext(dbPath);
        ...
        public static async Task InitRotTablesAsync(Guid o365TenantId)
        {
            ...
            var tableSets = new List<RMDiscoveryDBTableSet>
            {
                new RMDiscoveryDBTableSet(typeof(RMDiscoveryOffice365SiteRuleLevelRotData), schemaName),
                new RMDiscoveryDBTableSet(typeof(RMDiscoveryOffice365SiteCategoryLevelRotData), schemaName),
                new RMDiscoveryDBTableSet(typeof(RMDiscoveryOffice365SiteRootLevelRotData), schemaName),
            };
        ...
        public static async Task DeleteSiteDataBySiteIdAsync(Guid o365TenantId, int siteIdInt)
        {
            var dbPath = GetDBPath();
            if (!File.Exists(dbPath))
            {
            ...
            foreach (var tableSet in tableSets)
            {
                var deleteSql = tableSet.GetQueryDeleteRowsBySiteId(siteIdInt);
                await context.ExecuteNonQueryAsync(deleteSql);
            }
        }
    }
}
