    ...
    public class PermissionProvisionManager
    {
        ...
        public static void EnsureDocAveLocalGroupAndPermissions(bool forcePermission)
        {
            try
            {
                ...
                if (forcePermission)
                {
                    EnsureDirectoryFullControl(AveEnv.AgentRootFolder, DocAveGroupName);
                    EnsureRegistryFullControl(@"SOFTWARE\AvePoint\DocAve6", DocAveGroupName);
                    EnsureRegistryFullControl(@"SOFTWARE\Network Appliance\SnapManager for SharePoint 7", DocAveGroupName);
                    /*
                     * 添加这个注册表是因为Agent Account不是Local Admin时，无法创建Performance Counter
                     */
                    EnsureRegistryFullControl(@"SYSTEM\CurrentControlSet\Services", DocAveGroupName);
        ...
        {
            String groupPath = String.Format("WinNT://{0}/{1},group", Dns.GetHostName(), groupName);
        ...
        static void AddDomainUserToLocalGroup(string domain, string userName, string groupName = DocAveGroupName)
        {
            ...
            {
                userPath = String.Format("WinNT://{0}/{1},user", Dns.GetHostName(), userName);
            }
        ...
        {
            var ad = new DirectoryEntry("WinNT://" + Dns.GetHostName() + ",computer");
            try
            {
        ...
        static void EnsureDirectoryFullControl(string directoryName, string accountName)
        {
            try
            {
                logger.Info("Assigning file system full control permission for [{0}] on {1}", accountName, directoryName);
                DirectorySecurity dSecurity = Directory.GetAccessControl(directoryName);
                dSecurity.AddAccessRule(new FileSystemAccessRule(accountName, FileSystemRights.FullControl, InheritanceFlags.ContainerInherit | InheritanceFlags.ObjectInherit, PropagationFlags.None, AccessControlType.Allow));
                Directory.SetAccessControl(directoryName, dSecurity);
                logger.Info("Assigning file system full control permission for [{0}] on {1} successfully", accountName, directoryName);
            }
            catch (Exception ex)
            {
                logger.Error("An error occurred while assigning file system full control permission for [{0}] on {1}. Exception: {2}", accountName, directoryName, ex.ToString());
            }
        }
        ...
        static void EnsureRegistryFullControl(string registryPath, string accountName)
        {
            try
            {
                logger.Info("Assigning registry full control permission for [{0}] on {1}", accountName, registryPath);
                RegistryKey rkey = Registry.LocalMachine.OpenSubKey(registryPath, true);
                if (rkey == null)
                {
                    logger.Debug("Cannot find registryr [{0}] ", registryPath);
                    return;
                }
