...
namespace AvePoint.GCommon.Transfer.Common
{
    ...
    internal class BufferStorage
    {
        ...
        public static BufferSessionManagement GetSessionManagement(string sessionId)
        {
            lock (mOpLock)
            {
    ...
    {
        private static readonly AveLogger logger = AveLogger.GetInstance(typeof(BufferWrapperSession));
        ...
        public int InputTimeout = DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.DefaultReconnectTimeout + DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.MinReconnectTimeout;//ADO-109631 receiver should wait longer than sender for receiver init session and start read first
        public int OutputTimeout = DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.DefaultReconnectTimeout;
        ...
        private readonly DataBufferPerformanceCounter performanceCounter;
        ...
        public BufferWrapperSession(string session, string identify)
        {
            ...
            this.performanceCounter.Init(DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.EnablePerformanceCounter, string.Format("{0}-{1}", session, identify));
            Reset();
        }
        ...
        public void Reset()
        {
            ...
            this.InputTimeout = DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.DefaultReconnectTimeout;
            this.OutputTimeout = DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.DefaultReconnectTimeout;
            this.Buffers.Clear();
        }
        ...
        public BufferStatus PutBuffer(long serialNo, byte[] buffer)
        {
            ...
            if (Buffers.Count >= DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.MaxCacheBuffer)
            {
                if (IsTimeout(false))
                {
        ...
        public void SetTimeout(bool isInput, int timeout)
        {
            if (timeout <= 0)
            {
                timeout = DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.DefaultReconnectTimeout;
            }
            if (isInput)
            {
