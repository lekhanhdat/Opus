...
{
    public class CommonSiteMasterIndexService
        : ArchiverTableIndexServiceBase
        , ICommonSiteMasterIndexService
    {
        AveLogger logger = new AveLogger(MethodBase.GetCurrentMethod().DeclaringType);

        public IMArchiverSiteMasterIndexService SiteMasterIndexService { get; set; }
        public ICommonSiteMasterIndexDao CommonSiteMasterIndexDao => PlatformWindsorManager.GetService<ICommonSiteMasterIndexDao>();
        ...
        public void InsertSiteMaster(CommonSiteMasterIndex siteMasterIndex)
        {
            this.IndexProcessor.Insert<CommonSiteMasterIndex>(siteMasterIndex);
        }
        ...
        public Int32 GetSPVersionBySiteCollection(String siteCollection)
        {
            ...
            var sql = "select distinct COL_SP_VERSION from "
                + IndexConstants.TableNameCommonSiteMaster
                + " where COL_SITE_URL = @SiteUrl "
                + "order by COL_BACKUP_TIME desc";
            var index = this.IndexProcessor.ExecuteQuery<CommonSiteMasterIndex>(sql, parameterDictionary);
            return index.Count > 0 ? index[0].SPVersion : 0;
        }

        public List<CommonSiteMasterIndex> GetAllSiteMasterIndex()
        {
            var parameterDictionary = new Dictionary<String, Object>();
            var sql = "select * from " + IndexConstants.TableNameCommonSiteMaster;
            return this.IndexProcessor.ExecuteQuery<CommonSiteMasterIndex>(sql, parameterDictionary);
        }

        public List<CommonSiteMasterIndex> GetAllSiteMasterIndex(int dataType)
        {
            var parameterDictionary = new Dictionary<String, Object>();
            var sql = @$"SELECT * FROM {IndexConstants.TableNameCommonSiteMaster} WHERE COL_DATATYPE = @COL_DATATYPE)";
            parameterDictionary.Add("@COL_DATATYPE", dataType);
            return this.IndexProcessor.ExecuteQuery<CommonSiteMasterIndex>(sql, parameterDictionary);
        }
        ...
        public Int64 GetRetentionTimeSpanByJobId(String jobId)
        {
            ...
            var selectSql = "select COL_MARK5 from " + IndexConstants.TableNameCommonSiteMaster + " where COL_JOB_ID = @JobId";
            var updateSql = "update " + IndexConstants.TableNameCommonSiteMaster + " set COL_MARK5 = @RetentionTime where COL_JOB_ID = @JobId";
            try
            {
        ...
        {
            ArchiverSiteMasterIndexContract index = CommonSiteMasterIndexDao.GetTeamsInfo(site);

            if (index != null)
            {
                List<ArchiverIndexSubInfoContract> subinfos = ArchiverIndexSubInfoDao.GetSubInfoesBySubJobId(index.JobId);
                {
                    if (subinfos != null)
                    {
        ...
        public ArchiverSiteMasterIndexContract GetSiteCollectionStorageInfo(ArchiverSiteMasterIndexContract site)
        {
            lock (MLock)
            {
                ...
                infos = CommonSiteMasterIndexDao.GetSiteCollectionStorageInfo(site);
        ...
        public List<ArchiverSiteMasterIndexContract> LoadSiteMasterIndexByJobIdOrTeamsGroup(string teamsGroupAddress, string jobId)
        {
            var result = new List<ArchiverSiteMasterIndexContract>();
            var siteCollections = ArchiverSiteMasterIndexDao.LoadSiteMasterIndexByJobIdOrTeamsGroup(jobId, teamsGroupAddress);
            if (!siteCollections.IsNullOrEmpty())
            {
                foreach(var siteCollection in siteCollections)
                {
                    ...
                    {
                        siteCollection.IsSoftDeleted = subInfos.Any(s => s.IsSoftDelete);
        ...
        public List<ArchiverSiteMasterIndexContract> GetSiteCollectionWithSubInfos(ArchiverSiteMasterIndexContract index)
        {
            ...
            List<ArchiverSiteMasterIndexContract> siteCollections = CommonSiteMasterIndexDao.GetSiteCollectionStorageInfo(index);
            if (!siteCollections.IsNullOrEmpty())
            {
        ...
        {
            return CommonSiteMasterIndexDao.CreateByBulkCopyAsync(items);
        }
        ...
        {
            return CommonSiteMasterIndexDao.DeleteMigratedSiteMasterIndexesAsync();
        }
        ...
        public async Task<string> GetFailedJobsDataAsync(JMPager pager)
        {
            ...
            ParameterExpression param = Expression.Parameter(typeof(AvePoint.RA.DB.Model.RMSubJob), "c");
            responseResult.Result = new List<JMItemInfo>();
            ...
            try
            {
                ...
                foreach (var f in pager.Filters)
                {
                    if (timeFrameColumns.Contains(f.ColumnName) && f.ColumnValues != null)
                    {
                        ...
                        var exp1 = Expression4DynamicQuery.GetGreaterThanOrEqualExpression(typeof(AvePoint.RA.DB.Model.RMSubJob), param, f.ColumnName, startTimeTicks);
                        var exp2 = Expression4DynamicQuery.GetLessThanOrEqualExpression(typeof(AvePoint.RA.DB.Model.RMSubJob), param, f.ColumnName, endTimeTicks);
                    ...
                    var exps = f.ColumnValues.Select(c => Expression4DynamicQuery.GetEqualExpression(typeof(AvePoint.RA.DB.Model.RMSubJob), param, f.ColumnName, c));
                ...
                if (!string.IsNullOrEmpty(pager.SearchValue))
                {
                    try
                    {
                        var exps = pager.SearcheKeys.Select(searchKey => Expression4DynamicQuery.GetContainsExpression(typeof(AvePoint.RA.DB.Model.RMSubJob), param, searchKey, pager.SearchValue));
                ...
                List<AvePoint.RA.DB.Model.RMSubJob> failedSubjobs = null;
                if (allExpressionList.Count > 0)
                {
                    logger.Info("this query has filter");
                    queryExpr = allExpressionList.Aggregate(Expression.AndAlso);
                    var lambda = Expression.Lambda<Func<AvePoint.RA.DB.Model.RMSubJob, bool>>(queryExpr, param);
                ...
                {
                    var job = r;
                    var masterIndex = CommonSiteMasterIndexDao.GetIndexByJobId(job.Id);
                    ...
                    responseResult.Result.Add(new JMItemInfo()
                    {
                        ...
                        Status = (AvePoint.RA.Contract.RMWeb.JobMonitor.JobStatus)job.Status,
                        ...
                        Joblocation = masterIndex.FirstOrDefault()?.SiteURL,
                    });
                }
                return JsonConvert.SerializeObject(responseResult);
            }
