...
{
    public class CreatedConnectorBuildInColumn : IConnectorBuildInColumn
    {
        private readonly IGeneralSettingService GeneralSettingService = PlatformWindsorManager.GetService<IGeneralSettingService>();

        public Guid ColumnId => CustomizeConnectorBuildColumnIds.Created;

        public void ApplyRecordValue(Record record, Dictionary<string, CustomColumn> customColumnDic)
        {
            var date = customColumnDic[ColumnId.ToString()].Date;
            record.TimeCreated = date.Ticks;
            if(record.CreateDate == 0)
            {
                record.CreateDate = Convert.ToInt32(date.ToString("yyyyMMdd"));
            }
        ...
        public async Task<CustomizeConnectorNameValue<string>> ConvertToNameValueAsync(CustomizeConnectorColumnInfo columnInfo, Record record, bool forDisplay = true)
        {
            ...
            var dateTimeFormat = forDisplay ? GeneralSettingService.ConvertTiksToDateTime(gls, record.TimeCreated, true).SimplifyFormatTime :
                GeneralSettingService.ConvertTiksToUTCDateTime(gls, record.TimeCreated).SimplifyFormatTime;
