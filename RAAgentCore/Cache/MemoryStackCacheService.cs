    ...
    public class MemoryListCacheService<T> : ICacheService<T>
    {
        ...
        public void Add(T item)
        {
            ...
            while (MaxCount > 0 && _items.Count > MaxCount)
            {
                ...
                {
                    logger.Warn($"Waiting more than 10 minutes, add item to cache. Current count:{_items.Count}  Max Count:{MaxCount}");
                    break;
                }
                Thread.Sleep(2000);
            }
        ...
        public void AddBatch(IEnumerable<T> items)
        {
            ...
            while (MaxCount > 0 && _items.Count > MaxCount)
            {
                if (addTime.AddMinutes(10) < DateTime.UtcNow)
                {
                    logger.Warn($"Waiting more than 10 minutes,add items to cache, count:{items?.Count()} Current count:{_items.Count}  Max Count:{MaxCount}");
                    break;
                }
                Thread.Sleep(2000);
            }
