...
import ResetManualReviewStatusAction from "../Actions/ResetManualReviewStatusAction";
import { ApprovalStatus, ApprovalStatusI18Ns, OrderOptions, ManualReviewAction, ManualReviewActionI18Ns, RelatedRecordsActionI18Ns,Source, FilterOptions } from "../Constants/index";
import WaitingDisposalDetails from "../Details/WaitingDisposalDetails";
...
class WaitDisposalTableRow extends R.TableRow {
                ...
                {rowData.internalApprovedStatus === ApprovalStatus.WorkflowComplete && 
                    <>
                        ...
                        <R.Button
                            ...
                            onClick={() => this.onAction(ManualReviewAction.ResetManualWorkflow)}
                        />
                    </>}
                {rowData.internalApprovedStatus !== ApprovalStatus.WorkflowComplete && 
                <>
    ...
    render(Row, Cell) {
        ...
        let approvalStatus = ApprovalStatusI18Ns.get(rowData.internalApprovedStatus);
        if(rowData.internalApprovedStatus === ApprovalStatus.WorkflowComplete) {
            approvalStatus += ` (${ApprovalStatusI18Ns.get(rowData.approvedStatus)})`;
        }
...
export default class WaitDisposalTable extends R.Component {
    componentCreate() {
    ...
    onExecuteAction = (item, actionType) => {
        switch (actionType) {
            case ManualReviewAction.Approve:
                ApprovalAction.onApprove(new Set([item.id]), "" , ManualTab.WaitDisposal, "",this.props.onReload);
                break;
            case ManualReviewAction.Reject:
                ApprovalAction.onReject(new Set([item.id]), "" , ManualTab.WaitDisposal, "",this.props.onReload);
                break;
            case ManualReviewAction.ResetManualWorkflow:
                ResetManualReviewStatusAction.onResetManualWorkflow(new Set([item.id]), this.props.onReload);
                break;
        }
    };
    ...
    onColumnResize = (col, width) => {
        const columnId = `wait-disposal-${col.id}`;
    ...
    render() {
        return (
            <>
                <R.Table
                    id={"reco-manual-wait-disposal-table"}
                    rowKeyField={"rowIndex"}
                    rowTemplate={WaitDisposalTableRow}
                    rootData={{
                ...
                <ExtendAction ref={this.extendRef} onReload={this.props.onReload} />
                <WaitingDisposalDetails ref={this.viewDetailRef} onReload={this.props.onReload}/>
