...
{
    public class RecordOwnerQueryBuilder : BaseArrayMultiValueFilterBuilder
    {
        protected override bool CanFilter(ExplorerFilterOptionV2 filterOption)
        {
            return filterOption != null && filterOption.Owners != null && filterOption.Owners.Count > 0;
        }
        ...
        protected override ExplorerFilterOptionV2 Convert2SearchOptionV2(ExplorerQueryColumn column, string objJson, ExplorerSearchColumnOperationLogic columnOperationLogic, ExplorerSearchKeyOperationLogic keyOperationLogic)
        {
            ...
            {
                Owners = JsonConvert.DeserializeObject<List<AOSUserDto>>(objJson)
            };
        }

        protected override string GetColumnId()
        {
            return Contract.TemplateManagement.QueryCloumnIds.RecordOwner;
        }

        protected override string GetFilterColumnName()
        {
            //return CosmosConst.C_RecordOwnerArray;
            return CosmosConst.C_ManualReviewArray;
        }

        protected override object GetFilterValue(ExplorerFilterOptionV2 filterOption)
        {
            //return filterOption.Owners.Select(o => $"|{o}|").ToArray();
            //return filterOption.Owners.Select(o => o.RMUserId.ToString()).ToArray();
            return filterOption.Owners.Select(o => o.RMUserId).ToArray();
