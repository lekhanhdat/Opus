...
{
    public sealed class WrapperRestoreStreamV2: WrapperRestoreStreamBaseV2
    {
        public WrapperRestoreStreamV2(IInputStreamWrapper stream)
            : base(stream)
        {
            log.Info("Init WrapperRestoreStreamV2");
        }
        ...
        protected override void InitMetadataReader()
        {
            long threshold = 1024 * 1024 * 256;
            var streamReader = new StreamReader(internalStream, Encoding.UTF8);
            //larger than 256M,use File Metadata Reader
            if (MetadataLength > threshold)
            {
                log.Info($"Init AveFileMetadataReader, MetadataLength {MetadataLength}");
                MetadataReader = new AveFileMetadataReader(streamReader);
            }
            else
            {
                log.Info($"Init AveMemoryMetadataReader, MetadataLength {MetadataLength}");
                MetadataReader = new AveMemoryMetadataReader(streamReader);
            }
        }
