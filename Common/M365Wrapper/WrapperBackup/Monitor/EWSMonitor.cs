...
namespace ExchangeUtility.Graph
{
    ...
    public class EWSMonitor : ISingleton
    {
        ...
        public bool IsNeedSingleThread
        {
            get
            {
                if (this.requestNumber <= 0) return false;
                if (this.errorResponseNumber / (double)this.requestNumber <= 0.015) return false;
                logger.Info($"Need to use single thread. ErrorResponseNumber: {this.errorResponseNumber}. TotalRequestNumber: {this.requestNumber}. ");
                return true;
            }
        }

        public int RequestNumber { get { return this.requestNumber; } }

        public int ErrorResponseNumber { get { return this.errorResponseNumber; } }

        public int ThrottledResponseNumber { get { return this.eMonitor.GetThrottledResponseNumber(); } }
    ...
    internal class EWSGroupMonitor
    {
        ...
        public int GetThrottledResponseNumber()
        {
            var totalCount = 0;
            var tempCache = new Dictionary<string, int>(internalCache);
            foreach (var temp in tempCache)
            {
                if (ServiceErrorExtension.IsWaitAndTryServiceError(temp.Key)) totalCount = totalCount + temp.Value;
            }
            return totalCount;
        }
    }
