...
{
    //public class RMManualApprovalEmailScheduleUpgradeDao : IDbUpgradeDao
    //{

    //    private static readonly RALogger Logger = RALogger.GetInstance(typeof(RMManualApprovalEmailScheduleUpgradeDao));

    //    public void Upgrade(RMDbContext context)
    //    {
    //        try
    //        {
    //            if (context.Schedule.Count(item => item.JobCategory == (int)ScheduleType.ManualApprovalEmailSchedule) > 0)
    //            {
    //                Logger.Info($"The tenant: [{TenantLocalValue.LogonGroupId}] has manual approval email schedule job schedule.");
    //                return;
    //            }
    ...
    //                JobCategory = (int)ScheduleType.ManualApprovalEmailSchedule,
    ...
    //        catch (Exception e)
    //        {
    //            Logger.Error($"An error occurred while manual approval email schedule upgrade logic. Error: {e}");
    //        }
    //    }
