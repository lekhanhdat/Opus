...
{
    public class GDriveArchiverIndexProcessorParameter : IndexProcessorParameter
    {
        ...
        public GDriveArchiverIndexProcessorParameter(string tenantGroupId) : base()
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();

            base.DBPassWord = SettingProfileService.GetDBSEEMasterKey();
            this.logger.Info($"Get DB see master key by {tenantGroupId}");
            sw.Stop();
            this.logger.Info($"GetDBSEEMasterKey cost time:{sw.Elapsed.TotalMilliseconds}");
        }

        public GDriveArchiverIndexProcessorParameter() : base()
        {
        }
    }
}
