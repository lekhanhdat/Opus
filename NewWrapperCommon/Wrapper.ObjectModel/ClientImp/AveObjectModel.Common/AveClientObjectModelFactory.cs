...
{
    class AveClientObjectModelFactory : AveObjectModelFactory, IDisposable
    {
        private static AveLogger log = AveLogger.GetInstance(typeof(AveClientObjectModelFactory));
        ...
        {
            return new X509Certificate2(System.IO.File.ReadAllBytes(WrapperConfiguration.CertificateFilepath), WrapperConfiguration.CertificateFilePassword,
        ...
        {
            mSiteUrl = siteUrl;
            if (WrapperConfiguration.UseAppToken)
            {
                log.Debug("Use apptoken to connect O365.");
                mAccountInfo = new AveBPOSAccountInfo
                {
                    AppCert = GetCertificate(),
                    TenantId = WrapperConfiguration.TenantId,
                    ClientId = WrapperConfiguration.ClientId,
                    AzureRegion = (AzureRegions)accountInfo.AzureRegion,
        ...
        {
            return new AveTenant(url, mAccountInfo,isOnline, needLoadProperties);
        ...
        {
            return new AveServiceContext(proxyGroup as AveServiceApplicationProxyGroup, identifier as AveSiteSubscriptionIdentifier);
        }
        ...
        {
            //throw new NotSupportedException("This constructor is not supported in BPOS mode.");
            log.Debug("This constructor is not supported in BPOS mode. Method: {0}", "CreateSocialRatingManager(IAveServiceContext context)");
            return null;
        }
        ...
        public override IAveMetadataServiceApplication CreateMetadataServiceApplication(string name, Guid defaultPartitionId)
        {
            throw new NotSupportedException("This constructor is not supported in BPOS mode.");
        }
        public override IAveMetadataServiceApplication CreateMetadataServiceApplication(IAveSite site)
        {
            return new AveMetadataServiceApplication(site);
        }
        public override IAveDiscoveryQuery CreateDiscoveryQuery(string siteUrl, object conn, AveBPOSAccountInfo bposAccount)
        {
            AveDiscoveryQuery query = new AveDiscoveryQuery(siteUrl, bposAccount);
        ...
        {
            AveDiscoveryQuery query = new AveDiscoveryQuery(siteUrl, bposAccount, startTime, endTime);
            this.mRequest = query.Request;
            return query;
        }
        ...
        {
            AveDiscoveryQuery query = new AveDiscoveryQuery(site);
            this.mRequest = query.Request;
            return query;
        }
        ...
        {
            AveDiscoveryQuery query = new AveDiscoveryQuery(site, startTime, endTime);
        ...
        public override IAveMetadataServiceApplication CreateMetadataServiceApplication(Guid applicationId, Guid defaultParititionId)
        {
            throw new NotSupportedException("This constructor is not supported in BPOS mode.");
        }
        ...
        {
            return new AveOSocialFeedManager((mAveSite as AveSite).Request);
        }
        ...
        {
            return new AveServiceContextScope();
        }
        ...
        {
            return new AveUserProfileSerializer(site as AveSite, login, needInit, sourceSiteInfo, userMapping);
        }
        ...
        {
            return new AveAzurePowerShellRequest(accountInfo);
        }

        public override string GetAdminUrl(AveBPOSAccountInfo accountInfo)
        {
            using (AveAzurePowerShellRequest azurePSRequest = new AveAzurePowerShellRequest(accountInfo))
            {
                return azurePSRequest.GetOffice365AdminSiteCollectionUrl();
            }
        }

        public override IAveProfileLoader CreateProfileLoader(string adminUrl)
        {
            return new AveProfileLoader(adminUrl, mAccountInfo);
        }
        ...
        {
            return new SharePointDataProcessorPostAction(site, mapping, sourceSiteInfo, GetUserFromMapping);
        }
        ...
        public override IAveClientRequest CreateClientRequest(string url, AveBPOSAccountInfo userAccountInfo, AuthenticationModeOption[] AveAuthenticationModeOptions)
        {
            return new AveClientRequest(url, userAccountInfo, AveAuthenticationModeOptions);
        }
