...
{
    public class AveTerm : AvePoint.Wrapper.Backup.IAveSPTerm
    {
        private AveTermSet aveTermSet;
        private AveTerm parentTerm;
        private IAveSPTerm term;
        private Boolean isMetadataPartition;
        ...
        {
            this.Term = termId;
            output.WriteMetadata(AveMetadataType.MetadataTerm, this.ServiceApplication.GetTerm(this.aveTermSet.TermSet, termId));
        }

        public void Export(IAveBackupStream output, Guid termId, Guid partitionId)
        {
            this.Term = termId;
            output.WriteMetadata(AveMetadataType.MetadataTerm, this.ServiceApplication.GetTerm(this.aveTermSet.TermSet, termId, partitionId));
        }
