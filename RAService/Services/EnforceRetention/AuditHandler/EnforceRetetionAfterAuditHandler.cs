...
{
    public class EnforceRetetionAfterAuditHandler : IAfterAuditHandler
    {
        public async Task<RMAuditInfo> CollectAsync(Contract.RMWeb.Audit.RMAuditInfo info, int model, int category, int action, object[] args, object target, object returnValue)
        {
            ...
            if (action == (int)AuditAction.RunEnforceRetentionJob)
            {
                ...
                info.Status = !string.IsNullOrEmpty(result) ? (int)AuditStatus.Successful : (int)AuditStatus.Failed;
