...
{
    public class AveSPUserProfile : RestoreableObject
    {
        ...
        internal AveSPUserProfile(AveSPSite _aveSite, string loginName, bool needInit)
        {
            ...
            userProfileSerializer = mSite.ObjectModelFactory.CreateUserProfileSerializer(mSite.SPSite, loginName, needInit, mSite.SourceSiteInfo);
        }
        ...
        //public void RestoreUserProfile(string loginName, Dictionary<string, string> userProfile)
        //{
        ...
        //    try
        //    {
        //        mUserProfile = FindOrCreatePersonalSite(loginName);
        //        this.Restore(userProfile);
        //    }
        //    catch (Exception ex)
        //    {
        //        mLog.Log(AveLogSeverity.Warn, ex, string.Format("An error occurred while retoring user profile. user login name : {0}", loginName));
        //    }
        ...
        public void RestoreUserProfile(List<Dictionary<string, Dictionary<string, string>>> userProfiles)
        {
            ...
            foreach (KeyValuePair<string, Dictionary<string, string>> pair in collection)
            {
                ...
                {
                    log.Log(AveLogLevel.WARN, "WP10RTUserPro110 {0}  {1}", ex, pair.Key);
                }
            }
        }
            ...
            using (AvePerformanceScope pcs = new AvePerformanceScope("Restore.AveSPUserProfile.UpdateDetails"))
            {
                ...
                using (IAveSite site = mUserProfile.PersonalSite)
                {
                    try
                    {
                        ...
                        if (propertyName.ToLower() == "sps-proxyaddresses" || propertyName.ToLower() == "sps-masteraccountname" || propertyName.ToLower() == "adguid" || propertyName.ToLower() == "quicklinks" || propertyName.ToLower() == "sps-peers" || propertyName.ToLower() == "sps-resourceaccountname" || propertyName.ToLower() == "sps-resourcesid" || propertyName == "UserProfile_GUID" || propertyName == "SID" || propertyName == "AccountName" || propertyName == "FirstName" || propertyName == "PreferredName" || propertyName == "WorkEmail" || propertyName == "UserName" || propertyName == "PersonalSpace")
                            return;
        ...
        public void UpdateMemberships(string xml)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Restore.AveSPUserProfile.UpdateMemberships"))
            {
                ...
                {
                    log.Log(AveLogLevel.ERROR, "WP10RTMySite0329 {0}", e.ToString());
                }
#if PerformanceLog
            }
#endif
        ...
        {
#if PerformanceLog
            using (AvePerformanceScope pc = new AvePerformanceScope("Restore.AveSPUserProfile.UpdateColleages"))
            {
#endif
                XmlDocument xDoc = new XmlDocument();
                xDoc.LoadXml(xml);
        ...
        {
#if PerformanceLog
            using (AvePerformanceScope pc = new AvePerformanceScope("Restore.AveSPUserProfile.UpdateUserProfileColleages"))
            {
#endif
                XmlDocument xDoc = new XmlDocument();
                xDoc.LoadXml(xml);
        ...
        public AveSPUserProfile(AveSPSite _aveSite, bool needInit)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Restore.AveSPUserProfile.Constructor"))
            {
                ...
                mAveProfile = new AveUserProfile(_aveSite.ServiceContext, login, needInit, _aveSite.SourceSiteInfo, _aveSite.ServerRelativeUrl);
                mSite = _aveSite;
                userProfileSerializer = mSite.ObjectModelFactory.CreateUserProfileSerializer(mSite.SPSite, login, needInit, mSite.SourceSiteInfo);
#if PerformanceLog
            }
#endif
        }
