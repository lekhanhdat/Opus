...
[module: SuppressMessage("FxCopCustomRules", "C100007:SpellCheckStringValues", Scope = "member", Target = "AvePoint.Media.Storage.Cloud.Azure.AzureClient.#GetCdnEnabledString(System.String)", MessageId = "vo")]
[module: SuppressMessage("FxCopCustomRules", "C100007:SpellCheckStringValues", Scope = "member", Target = "AvePoint.Media.Storage.Cloud.Azure.AzureClient.#GetCdnEnabledString(System.String)", MessageId = "msecnd")]
namespace AvePoint.Media.Storage.Cloud.Azure
{
    ...
    [AveCodeReview(
       ...
       "dapeng.zhang@avepoint.com",
        ...
        true)]
    #endregion
    class AzureClient : AbstractRESTOprationExecutor, ICloudOprationExecutor
    {
        ...
        public override bool CheckContainer(string xSetName)
        {
            ...
            {
                Logger.Error(GetCheckCtnErrorMsg(xSetName) + "Details : " + e);
                throw new AuthenticationFailedException(e.Message, e);
            }
        ...
        public override bool CreateContainer(string xSetName)
        {
            ...
            {
                Logger.Error(GetCreateCtnErrorMsg(xSetName) + "Details : " + e);
                throw;
            }
        ...
        public override bool DeleteContainer(string xSetName)
        {
            ...
            {
                Logger.Error(GetDeleteCtnErrorMsg(xSetName) + "Details : " + e);
                throw;
            }
        ...
        protected override List<string> ListXstream(string xSetName, string prefix, int limit, string marker, bool isGetName)
        {
            ...
            try
            {
                ...
                using (HttpWebResponse resp = DoExecute(request))
                {
                    if (resp != null && resp.StatusCode == HttpStatusCode.OK)
                    {
                        if (isGetName)
                        {
                            result = AnalyzeXML(resp.GetResponseStream(), "EnumerationResults/Blobs/Blob/Name");
                        }
                        else
                        {
                            result = AnalyzeXML(resp.GetResponseStream(), "EnumerationResults/Blobs/Blob/Properties/Content-Length");
                        }
                    }
                }
            }
            ...
            {
                Logger.Error(GetListObjErrorMsg(xSetName) + "Details : " + e);
                throw;
            }
        ...
        public override bool CheckObject(string xSetName, string xStreamName)
        {
            ...
            {
                Logger.Debug("File can not be found. details : {0}.", e);
                result = false;
            }
            catch (Exception e)
            {
                Logger.Error("Check object failed, object : {0}, container : {1}, details : {2}. ", xStreamName, xSetName, e);
                throw;
            }
            return result;
        }
        ...
        {
            string cdnUrl = string.Format(StorageUrl.AzureCnd, openParams.CdnGuid);
            Uri uri = new Uri(openParams.AccessPoint);
            fullURL = fullURL.Replace(String.Format(StorageUrl.AzureCndFormat, uri.Scheme, openParams.UserName), cdnUrl);
            return fullURL;
        }
        ...
        public override HttpWebRequest GetUploadRequest(string xSetName, string xStreamName, string mimeType, HttpWebRequest webRequest, int blockNumber, long dataLength)
        {
            ...
            {
                Logger.Error("An error occurred while handling request, details : {0}.", ex);
                throw;
            }
        }
        ...
        public override bool CreateObject(string xSetName, string xStreamName, HttpWebRequest request, long dataLength)
        {
            ...
            {
                Logger.Error("An error occurred while creating object, details : {0}.", ex);
                throw;
            }
            finally
            {
        ...
        public override Stream OpenObject(string xSetName, string xStreamName, int rangFrom, int rangeTo)
        {
            ...
            {
                Logger.Error(GetOpenObjErrorMsg(xStreamName, xSetName) + "Details : " + e);
                throw;
            }
        }
        ...
        private bool DeleteObject(string xSetName, string xStreamName)
        {
            ...
            {
                Logger.Error(GetDeleteObjErrorMsg(xStreamName, xSetName) + "Details : " + e);
                throw;
            }
        ...
        protected MSAzureRequest GetAzureRequest(string uri)
        {
            ...
            string datatime = MSAzureUtils.convertDateTimeToHttpString(DateTime.UtcNow);
            Dictionary<string, string> headers = new Dictionary<string, string>();
            headers.Add(MSAzureConstants.DateHeader, datatime);
            headers.Add(MSAzureConstants.VersionHeader, MSAzureConstants.ApiVersion);
            result.URI = uri;
        ...
        protected bool PutBlob(string xSetName, string xStreamName, string mimeType, Stream stream)
        {
            ...
            {
                Logger.Error("An error occurred while putting blob, details : {0}.", e);
                throw;
            }
        ...
        protected void PutBlock(string xSetName, string xStreamName, string mimeType, Stream stream, Dictionary<string, string> prams)
        {
            ...
            {
                Logger.Error("An error occurred while putting block, details : {0}.", e);
                throw;
            }
        }
        ...
        protected bool PutBlockList(string xSetName, string xStreamName, string mimeType, List<string> blockIds)
        {
            ...
            {
                Logger.Error("An error occurred while putting block list, details : {0}.", e);
                throw;
            }
            return result;
        }
        ...
        protected virtual string BuildURL(string xSetName)
        {
            ...
            if (openParams.AccessPoint.Equals(StorageUrl.Azure, StringComparison.CurrentCultureIgnoreCase) || openParams.AccessPoint.Equals("https://blob.core.windows.net", StringComparison.CurrentCultureIgnoreCase))
            {
                Uri uri = new Uri(openParams.AccessPoint);
                if (Data_Version == Data_Version.DocAve5)
                {
                    xSetName = GetMD5(xSetName);
                    url = String.Format(StorageUrl.AzureFormat, uri.Scheme, openParams.UserName, GetDataType(tempStr) + xSetName);
                    //Data_Version = 0;
                }
                else
                {
                    url = String.Format(StorageUrl.AzureFormat, uri.Scheme, openParams.UserName, this.openParams.SystemLocation);
                }
            }
            else
            {
                ...
                {
                    xSetName = GetMD5(xSetName);
                    url = openParams.AccessPoint.TrimEnd('/') + "/" + openParams.UserName + "/" + GetDataType(tempStr) + xSetName;
                    Data_Version = 0;
                }
                else
                {
                    url = openParams.AccessPoint.TrimEnd('/') + "/" + openParams.SystemLocation;
                }
            }
        ...
        {
            string baseUrl = openParams.AccessPoint;
            if (prams == null)
            {
                return baseUrl;
            }
            return baseUrl + ConvertQueryList2String(prams);
        }

        protected string BuildURL(string xSetName, string xStreamName, Dictionary<string, string> parameters)
        {
            string baseUrl = BuildURL(xSetName, xStreamName);
            ...
            return baseUrl + ConvertQueryList2String(parameters);
        }

        protected string BuildURL(string xSetName, Dictionary<string, string> prams)
        {
            string baseUrl = BuildURL(xSetName);
        ...
        public override Dictionary<string, string> Headers
        {
            get
            {
                ...
                {
                    {MSAzureConstants.VersionHeader, MSAzureConstants.ApiVersion},
        ...
        internal string GetCopyResource(StorageInfo copyFrom, string storageKey)
        {
            ...
            var version = MSAzureConstants.ApiVersion;
        ...
        internal bool CopyFile(string copyResouce, string copyTo)
        {
            ...
            try
            {
                ...
                headers.Add(MSAzureConstants.VersionHeader, MSAzureConstants.ApiVersion);
                string datetime = MSAzureUtils.convertDateTimeToHttpString(DateTime.UtcNow);
                headers.Add(MSAzureConstants.DateHeader, datetime);
        ...
        {
            get { return new Dictionary<string, string>() { { "RESTYPE".ToLower(CultureInfo.InvariantCulture), "container" }, { "comp", "list" }, { "maxResults".ToLower(CultureInfo.InvariantCulture), "1000" } }; }
        }
        ...
        private Dictionary<string, string> GetContainerQueryParams
        {
            get { return new Dictionary<string, string>() { { "RESTYPE".ToLower(CultureInfo.InvariantCulture), "container" }}; }
        }
        ...
        public bool SetBlobMetadata(string fullURL, Dictionary<string, string> metadataList)
        {
            ...
            return RetryRequset.CloudRetry<bool>(delegate()
            {
                ...
                // Based on current tests, "set blob metadata" does not work with version "2019-02-02"
                var blobMetadataRequestHeaders = this.Headers;
                blobMetadataRequestHeaders["x-ms-version"] = "2014-02-14";
                using (HttpWebResponse response = DoExecute(webRequest, this.Headers))
                {
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
