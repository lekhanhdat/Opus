...
namespace AvePoint.GCommon.Utility.I18N
{
    ...
    public class EventSourcesUtil
    {
        ...
        private const string DocAveAudit = "Audit";
        ...
        public static void CreateEventSources()
        {
            ...
            eventSources.Add(ToEventSourceString(EventSources.DocAveAudit));
            ...
            foreach (string eventSource in eventSources)
            {
                if (EventLog.SourceExists(eventSource))
                {
                    ...
                    {
                        Tuple<string, int> categoryInfos = GetCategoryResourceFileAndCount(eventLogName, eventSource);
                        if (string.Compare(categoryInfos.Item1, categoryDll, StringComparison.OrdinalIgnoreCase) == 0
                            && categoryInfos.Item2 == categoryCount)
                        {
                            continue;
                        }
                    }
        ...
        internal static string ToEventSourceString(EventSources eventSource)
        {
            ...
            switch (eventSource)
            {
                ...
                case EventSources.DocAveAudit:
                    return prefix + DocAveAudit;
                default:
                    return string.Empty;
            }
        }

        private static void EnsureEventLogName(string eventLogName)
        {
            //Log files will be stored under C:\Windows\System32\winevt\Logs\AvePoint.evtx 
            RegistryKey keyEventLog = Registry.LocalMachine.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\EventLog", true);
            RegistryKey keyLogName = keyEventLog.OpenSubKey(eventLogName, true);
            if (keyLogName == null)
            {
                keyLogName = keyEventLog.CreateSubKey(eventLogName);
            }
            keyLogName.SetValue("Retention", 0);
            keyLogName.Close();
            keyEventLog.Close();
            EventLog eventLog = new EventLog(eventLogName);
            if (eventLog.MaximumKilobytes < 4096)
            {
                eventLog.MaximumKilobytes = 4096;
            }
            eventLog.Close();
        }

        private static bool IsEventSourceUnderLogName(string eventLogName, string eventSourceName)
        {
            bool exist = false;
            RegistryKey keyEventLog = Registry.LocalMachine.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\EventLog", false);
            RegistryKey keyLogName = keyEventLog.OpenSubKey(eventLogName, false);
            RegistryKey keySourceName = keyLogName.OpenSubKey(eventSourceName, false);
            if (keySourceName != null)
            {
                exist = true;
                keySourceName.Close();
            }
        ...
        private static Tuple<string, int> GetCategoryResourceFileAndCount(string eventLogName, string eventSourceName)
        {
            RegistryKey keyEventLog = Registry.LocalMachine.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\EventLog", false);
            RegistryKey keyLogName = keyEventLog.OpenSubKey(eventLogName, false);
            RegistryKey keySourceName = keyLogName.OpenSubKey(eventSourceName, false);
            object categoryMessageFileObject = keySourceName.GetValue("CategoryMessageFile");
            object categoryCountObject = keySourceName.GetValue("CategoryCount");
            keySourceName.Close();
            keyLogName.Close();
            keyEventLog.Close();

            string categoryMessageFile = categoryMessageFileObject == null ? string.Empty : categoryMessageFileObject.ToString();
            int categoryCount = categoryCountObject == null ? 0 : int.Parse(categoryCountObject.ToString());
            return new Tuple<string, int>(categoryMessageFile, categoryCount);
        }

        internal static bool IsSMSP()
        {
            bool isSMSP = false;
