...
namespace AvePoint.ObjectModel.ServerSE
{
    class AveFolderCollection : AveAbstractCommonCollection<IAveFolder>, IAveFolderCollection, IDisposable
    {
        ...
        {
            return new AveFolder(mWeb, mFolders.Add(SPResourcePath.FromDecodedUrl(strUrl), new SPFolderCreationInformation()));
        }
        ...
        public IAveFolder this[string name]
        {
            get
            {
                return new AveFolder(mWeb, mFolders[SPResourcePath.FromDecodedUrl(name)]);
            }
        }
        ...
        private IAveDocumentSet CreateDocumentSet(AveFolderInfo info, IAveContentTypeId contentTypeId, IAveList list)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveFolderCollection.CreateDocumentSet"))
            {
                ...
                var folder = (AveFolder)list.ParentWeb.GetFolder(mFolders.Folder.Url);
                ...
                try
                {
                    documentSet = DocumentSet.Create(folder.Folder, SPResourcePath.FromDecodedUrl(info.Name), new SPContentTypeId(contentTypeId.ToString()), hashtable);
                }
                catch (Exception e)
                {
        ...
        public IAveDocumentSet CreateDocumentSet(string name, IAveContentTypeId contentTypeId, Hashtable properties)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveFolderCollection.CreateDocumentSet_1"))
            {
                ...
                var documentSet = DocumentSet.Create(mFolders.Folder, SPResourcePath.FromDecodedUrl(name), new SPContentTypeId(contentTypeId.ToString()), null);
                return new AveDocumentSet(new AveFolder(AveFolder.ParentWeb as AveWeb, documentSet.Folder));
        ...
        public IAveDocumentSet CreateDocumentSet(string name, Hashtable properties)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveFolderCollection.CreateDocumentSet"))
            {
                ...
                {
                    var documentSet = DocumentSet.Create(mFolders.Folder, SPResourcePath.FromDecodedUrl(name), new SPContentTypeId(listContentType.ID.ToString()), properties);
                    return new AveDocumentSet(new AveFolder(AveFolder.ParentWeb as AveWeb, documentSet.Folder));
                }
                return null;
        ...
        private AveRestoreResult RealRestoreFolder(AveFolderInfo info, AveItem aveItem, Dictionary<string, object> allDocData, Dictionary<string, object> allUserData)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveFolderCollection.RealRestoreFolder"))
            {
                ...
                else if(aveItem.mSPListItem!= null)
                {
                    aveItem.InitBySPListItem(aveItem.mSPListItem);
                }
                    ...
                    if (info.OriginalRowId > 0 && aveItem.mSPList != null)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                //请不要随便添加reload操作
                                var docset = CreateDocumentSet(info, new AveContentTypeId(itemContentTypeId), aveItem.mList);
