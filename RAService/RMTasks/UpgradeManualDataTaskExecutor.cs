...
{
    public class UpgradeManualDataTaskExecutor : ITaskExecutor
    {
        public System.Threading.Tasks.Task ExecutorAsync(TaskBase task)
        {
            ...
            foreach (var tenant in tenants)
            {
                TenantUtil.RunUnderTenant(tenant.TenantId, tenant.RegisterEmail, () => {
                    var manualApprovalService = PlatformWindsorManager.GetService<IManualApprovalService>();
                    manualApprovalService.UpgradeManualApprovalDataJob();
                });
            }
