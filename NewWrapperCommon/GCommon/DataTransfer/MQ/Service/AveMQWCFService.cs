...
namespace AvePoint.GCommon.Transfer.MQ.Service
{
   [ServiceBehavior(IncludeExceptionDetailInFaults = true, ConcurrencyMode = ConcurrencyMode.Multiple)]
    internal class AveMQWCFService : IMQWCFService
    {
        private static AveMQServer mServer = AveMQServer.GetInstance();

        #region IMQService Members

        public bool RegisterMQClient(string sessionId, string identifier)
        {
            IMQClientCallback clientCallback = OperationContext.Current.GetCallbackChannel<IMQClientCallback>();
            mServer.AddOrUpdateMQClientPeer(sessionId, identifier, clientCallback);
