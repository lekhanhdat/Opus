    ...
    public class AveOrdinaryFileRestore : BaseDocumentRestore
    {
        ...
        protected override ClientFile AddFile(bool overwriteIfExists)
        {
            ...
            if (SpecialFileList.Contains(fileType, StringComparer.OrdinalIgnoreCase) || FileStream.Length < WrapperConfiguration.BPOS_S.UploadLimit)
            {
                ...
                newFile = ParentWeb.GetFolderByServerRelativeUrl(DocInfo.ParentFolderRelativeUrl).Files.Add(fileCreationInfo);
            }
                        ...
                        while ((bytesRead = br.Read(buffer, 0, buffer.Length)) > 0)
                        {
                            ...
                            if (first)
                            {
                                using (MemoryStream contentStream = new MemoryStream())
                                {
                                    ...
                                    fileInfo.Url = FileServerRelativeUrl;
                                    fileInfo.Overwrite = true;
                                    newFile = ParentWeb.GetFolderByServerRelativeUrl(DocInfo.ParentFolderRelativeUrl).Files.Add(fileInfo);
