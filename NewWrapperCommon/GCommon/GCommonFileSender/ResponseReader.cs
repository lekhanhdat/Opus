...
namespace AvePoint.GCommon.FileTransfer
{
    ...
    internal class ResponseReader
    {
        ...
        readonly BlockSender dataBlockSender;
        readonly IFileSenderResponseWorker readerWorker;
        Thread readerThread;
        ...
        public void Start(string jobId = "")
        {
            ...
            readerThread = new Thread(Run) { Name = currentThreadId + "_Reader", IsBackground = true };
        ...
        private void Run(object o = null)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if (message.Equals("Sync", StringComparison.OrdinalIgnoreCase))
                    {
                        logger.Info("ResponseReader received Sync message.");
                        dataBlockSender.SendMessage("Sync");
                        logger.Info("ResponseReader send Sync message.");
                        continue;
                    }
                    try
                    {
