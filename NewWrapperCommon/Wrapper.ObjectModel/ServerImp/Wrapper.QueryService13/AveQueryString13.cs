...
{
    public class AveQueryString13
    {
        #region Extender SharePoint13

        public const string Sp13AllAttachmentsForExternder =
            @"SELECT doc.Id,doc.DirName,doc.LeafName,doc.ParentId,doc.DocFlags,doc.Level,doc.Size
        ...
        public const string Sp13SingleAttachmentForExternder =
            @"SELECT doc.Id,doc.DirName,doc.LeafName,doc.ParentId,doc.DocFlags,doc.Level,doc.Size
            FROM AllDocs doc WITH(NOLOCK) INNER JOIN AllDocs o WITH(NOLOCK)
            ON doc.SiteId=o.SiteId
            AND doc.DeleteTransactionId=0x AND doc.ParentId=o.Id
        ...
        public const string Sp13ContentOrStub =
        ...
        public const string Sp13AllAttachmentsForExternderWithRecycleBin =
            @"SELECT doc.Id,doc.DirName,doc.LeafName,doc.ParentId,doc.DocFlags,doc.Level,doc.DeleteTransactionId,doc.Size
            FROM AllDocs doc WITH(NOLOCK) INNER JOIN AllDocs o WITH(NOLOCK)
        ...
        public const string Sp13AllItemsInDocsForExtender =
   @"SELECT " + CommonColumn.AllDocsLogicalColumns + @",doc.IsCurrentVersion,doc.UIVersion as UIVersion,doc.DocFlags,doc.HasStream,doc.Level,2 AS QueryType,NULL AS Content,doc.Size,doc.DeleteTransactionId,doc.id as tp_docid,null
            FROM AllDocs AS doc WITH(NOLOCK)
            @WHERE
ORDER BY LeafName,UIVersion DESC
";

        public const string Sp13AllDocValueForEventCache_Extender = "SELECT " + CommonColumn.AllDocsCommonColumns_BigIntSize + ",doc.Level,doc.CheckoutUserId,doc.DocFlags,doc.IsCurrentVersion,doc.ParentId,doc.ListId,doc.HasStream,2 AS QueryType,NULL AS Content,stream.RbsId" + @"
            FROM AllDocs AS doc WITH(NOLOCK)
...
";
        ...
        public const string Sp13AllVersionsForExtenderWithRecycleBin =
   @"SELECT " + CommonColumn.AllDocsLogicalColumns + @",CONVERT(bit,0),docver.UIVersion as UIVersion,docver.DocFlags, CONVERT(int,docver.HasStream) AS HasStream,docver.Level,3 AS QueryType,NULL AS Content,docver.Size,docver.DeleteTransactionId,doc.id as tp_docid
...
";
        ...
        public const string Sp13AllVersionsForExtender =
   @"SELECT " + CommonColumn.AllDocsLogicalColumns + @",CONVERT(bit,0),docver.UIVersion as UIVersion,docver.DocFlags,CONVERT(int,docver.HasStream) AS HasStream,docver.Level,3 AS QueryType,NULL AS Content,docver.Size,docver.DeleteTransactionId,doc.id as tp_docid,stream.RbsId
            FROM AllDocVersions AS docver WITH(NOLOCK) 
...
";
        ...
        public const string Sp13AllItemsAndVersionsForExtender =
           @"SELECT " + CommonColumn.AllDocsLogicalColumns + @",doc.IsCurrentVersion,doc.UIVersion as UIVersion,doc.DocFlags,doc.HasStream,doc.Level,2 AS QueryType,NULL AS Content,doc.Size,doc.DeleteTransactionId,doc.id as tp_docid,null
            FROM AllDocs AS doc WITH(NOLOCK)           
            @WHERE
            UNION
            SELECT " + CommonColumn.AllDocsLogicalColumns + @",CONVERT(bit,0),docver.UIVersion as UIVersion,docver.DocFlags,docver.HasStream,docver.Level,3 AS QueryType,NULL AS Content,docver.Size,docver.DeleteTransactionId,doc.id as tp_docid,null
            FROM AllDocVersions AS docver WITH(NOLOCK) 
            INNER JOIN AllDocs AS doc WITH(NOLOCK) ON docver.SiteId=doc.SiteId AND docver.Id=doc.Id AND docver.DeleteTransactionId=0x AND doc.IsCurrentVersion=1
            @WHERE
            ORDER BY LeafName,UIVersion DESC";

        public const string Sp13AllItemsAndVersionsForExtenderWithRecycleBin =
           @"SELECT " + CommonColumn.AllDocsLogicalColumns + @",doc.IsCurrentVersion,doc.UIVersion as UIVersion,doc.DocFlags,doc.HasStream,doc.Level,2 AS QueryType,NULL AS Content,doc.Size,doc.DeleteTransactionId,doc.id as tp_docid,null
            FROM AllDocs AS doc WITH(NOLOCK)            
            @WHERE
            UNION
        ...
        public const string Sp13StubAllItemAndVersions =
                @"SELECT" + CommonColumn.AllDocsLogicalColumns + @",doc.IsCurrentVersion,doc.UIVersion as UIVersion ,
                doc.DocFlags,doc.HasStream,doc.Level,2 AS QueryType,stream.Content,doc.Size,stream.RbsId,doc.TimeLastModified as TimeLastModified
                FROM AllDocs AS doc with(nolock)
                left outer join DocsToStreams As DTStream WITH(NOLOCK) on doc.Id=DTStream.DocId and doc.SiteId=DTStream.SiteId and DTStream.HistVersion=0 and DTStream.Level=doc.Level
                LEFT OUTER JOIN DocStreams AS stream WITH(NOLOCK) ON DTStream.SiteId = stream.SiteId AND DTStream.DocId = stream.DocId and DTStream.BSN=stream.BSN and DTStream.Partition=stream.Partition
                @WHEREAllDocs
                ...
                docver.DocFlags,docver.HasStream,docver.Level,3 AS QueryType,stream.Content,doc.Size,stream.RbsId,doc.TimeLastModified as TimeLastModified
                FROM AllDocVersions AS docver with(nolock) 
                INNER JOIN AllDocs AS doc with(nolock) ON docver.SiteId=doc.SiteId AND docver.Id=doc.Id AND docver.DeleteTransactionId=0x AND doc.IsCurrentVersion=1 
        ...
        public const string Sp13AllItemAndVersionsStubInfo =
        ...
        public const string Sp13ItemStubsByIdsCammandLine =
        ...
        public const string Sp13ListWithRootFolderById =
                @"
        ...
        public const string Sp13ItemStubsByIds = @"WHERE doc.SiteId=@SiteId AND doc.Id in ({0}) AND doc.DeleteTransactionId=0x  AND doc.Level<=255 AND doc.IsCurrentVersion=1";
        public const string Sp13ItemStubsByIdsWithRecycleBin = @"WHERE doc.SiteId=@SiteId AND doc.Id in ({0}) AND (doc.DeleteTransactionId=0x OR doc.DeleteTransactionId<>0x)  AND doc.Level<=255 AND doc.IsCurrentVersion=1";
        
        public const string Sp13StubAllItemAndVersionsWithRecycleBin =
        ...
        public const string Sp13StubFilesInFolderCountWithRecycle = @" 
                WITH DocsBlob(Id,InternalVersion)
                AS
                (
                --Get All Stubs in AllDocs table
        ...
        public const string Sp13AllAttachmentsStubInfoForExtender =
...
Int1";
    ...
    public class AveDiscoverQueryString13
    {
        /// <summary>
        /// all webs under the site
        /// </summary>
        public const string DiscoverAllWebs = @"SELECT Id,FullUrl,Title,ParentWebId,AppInstanceId FROM AllWebs WITH(NOLOCK) WHERE SiteId = @SiteId AND DeleteTransactionId = 0x ORDER BY FullUrl";
...
";
    ...
    public class AveQueryStringReportCenter13
    {
