...
namespace AvePoint.Media.Service.ArchiverBackup
{
        ...
        , IInputDataListener
    {
        ...
        public XStream OpenDataBlock(DataBlockOpenParam param, out DataBlockOpenOutParam outParam)
        {
            ...
            if(param.FileType == FileType.MetaData && this.converter.IsFirstMetaDataBlock() && outParam.FileSize == this.converter.GetCurrentItemMetaDataStartOffsetFromItemIndex()
                || param.FileType == FileType.Content && this.converter.IsFirstContentDataBlock() && outParam.FileSize == this.converter.GetCurrentItemContentDataStartOffsetFromItemIndex())
            {
                param.FileNumber++;
                outParam.FileName = fileNameGenerator.Generate(new FileNameParameter(param));
                outParam.FileSize = this.DataBlockGetSize(param.FileType, outParam.FileName, dataVolume);
            ...
            sw.Start();
            var result = dataDevice.OpenStream(info, FileMode.Open);
            sw.Stop();
            logger.Info($"linkRestoreReport OpenDataBlock openStream cost :{sw.ElapsedMilliseconds}");
            return result;
        }
        ...
        private long DataBlockGetSize(FileType fileType, string fileName, string dataVolume)
        {
            ...
            else
            {
                ...
                sw1.Stop();
                logger.Info($"linkRestoreReport DataBlockGetSize cost time:{sw1.ElapsedMilliseconds}");
            }
            return blockSize;
        }
