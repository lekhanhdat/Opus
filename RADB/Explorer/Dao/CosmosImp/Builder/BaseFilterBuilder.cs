...
{
    public abstract class BaseFilterBuilder: IFilterBuilder, IAdvancedQueryBuilder
    {
        public Query Filter(Query query, ExplorerFilterOptionV2 filterOption)
        {
            if (CanFilter(filterOption))
            {
                return query.Where(GetFilterColumnName().FormatColumnName(), GetFilterValue(filterOption));
            }
