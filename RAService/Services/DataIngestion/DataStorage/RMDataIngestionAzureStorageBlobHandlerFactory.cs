...
{
    public static class RMDataIngestionAzureStorageBlobHandlerFactory
    {
        private static readonly Lazy<IDictionary<RMDataIngestionType, Type>> ClientMap = new(CreateClientMap);

        public static RMDataIngestionAzureStorageBlobHandler Create(RMDataIngestionType type)
        {
            if (!ClientMap.Value.TryGetValue(type, out var clientType))
            {
                throw new NotSupportedException($"No blob client registered for ingestion type {type}.");
            }

            return (RMDataIngestionAzureStorageBlobHandler)Activator.CreateInstance(clientType);
        }

        private static IDictionary<RMDataIngestionType, Type> CreateClientMap()
        {
            var baseType = typeof(RMDataIngestionAzureStorageBlobHandler);

            return typeof(RMDataIngestionAzureStorageBlobHandlerFactory).Assembly
                .GetTypes()
        ...
        {
            var instance = (RMDataIngestionAzureStorageBlobHandler)Activator.CreateInstance(implementation);
