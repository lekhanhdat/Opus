...
public class RMSalesforceDiscoveryConfigurationAfterAuditHandler : IAsyncAuditAfterHandler
{
        ...
        object returnValue)
    {
        ...
        {
            var newConfiguration = args[0] as RMDiscoverySalesforceConfigurationInfo;

            var newValue = string.Join(";\n ",
                newConfiguration!.ScopeInfo.Organizations.Select(organization => organization.Name));
            auditInfo.ModifyContent.First(content => content.TargetSetting.Equals(S_SFTenant)).NewValue = newValue;
            var responseMessage = returnValue as RAReturnMessage;
            auditInfo.Status = responseMessage.MessageType == RAMessageType.Failed ? 1 : auditInfo.Status;
        }
