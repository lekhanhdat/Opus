...
[Obsolete("Use AvePoint.Common.AveEnv instead, this class may remove later.")]
public class AveSPEnv
{
    ...
    public static bool IsMoss
    {
        get
        {
            ...
            //}
            //return mIsMoss;
            return AvePoint.Common.AveEnv.IsMoss;
        }
    }
    ...
    public static bool IsPublishing
    {
        get
        {
            ...
            //}
            //return mIsMoss;
            return AvePoint.Common.AveEnv.IsPublishing;
        }
    }
    ...
    private static void LoadSPVersion()
    {
        try
        {
            string isApiDir = System.Environment.GetEnvironmentVariable("CommonProgramFiles") + AVE_SP14_DLL_PATH;
            string vssFile = isApiDir + "\\Microsoft.SharePoint.dll";
            ...
            if (!mIsMoss)
            {
                ...
                {
                    string file = Path.Combine(isApiDir, mossFile);
                    if (!File.Exists(file))
                    {
                        isAllInclude = false;
                        break;
            ...
            if (mIsMoss)
            {
                string mossFile = isApiDir + "\\Microsoft.SharePoint.Server.dll";
