...
{
    public class ArchiverDedupReportJobDetailWorker : AbstractJobDetailWorker
    {
        ...
        {
            var details = jobDetails.Where(item => item is JMArchiverDedupReportDetails);
            ...
            var summaryDetails = jobDetails.Where(item => item is JMArchiverDedupReportSummaryDetails);
        ...
        {
            TABLE_NAME = JobMonitorConstants.JOBDETAIL;
            CREATE_TABLE_SQL = string.Format(JobMonitorConstants.CREATE_TABLE_Archiver_Dedup_ReportJob_Report, TABLE_NAME);
            INSERT_DATA_SQL = string.Format(JobMonitorConstants.INSERT_DATA_Archiver_Dedup_ReportJob_Report, TABLE_NAME);
        }
        ...
        public override JMJobDetails GetDataForJobSummaryDetails(string conditionFilter, BaseJobDto jobInfo)
        {
            ...
            result = JobDetailDao.GetDataForArchiverDedupReportSummaryDetails(reportFilePath, "select * from JobSummaryDetail", jobInfo);
