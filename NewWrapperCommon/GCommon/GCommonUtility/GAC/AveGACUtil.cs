    ...
    public class AveGACUtil
    {
        ...
        public static void RemoveAssembly(string AssemblyPath)
        {
            ...
            while (true)
            {
                ...
                string ShortName = GetAssemblyFullName(AssembNameLoc, out pt, out vt, out ct,out pa);
                if (ShortAssemblyName.Equals(ShortName, StringComparison.OrdinalIgnoreCase)) //same name
                {
                    if (CheckTheSameAssembly(AssemblyPath, pt, vt, ct,pa)) // same public taken,version,culture
                    {
        ...
        private static string GetAssemblyFullName(string FullName, out string PublicToken, out string vertion, out string culture, out string processorArchitecture)
        {
            PublicToken = null;
            vertion = null;
            culture = null;
            ...
            foreach (string name in Strings)
            {
                ...
                int index3 = name.IndexOf("processorArchitecture",StringComparison.OrdinalIgnoreCase);
                if (index3 != -1)
                {
                    index3 = name.IndexOf("=", StringComparison.OrdinalIgnoreCase);
                    if (index3 != -1)
                    {
                        processorArchitecture = name.Substring(index3 + 1);
                        processorArchitecture = processorArchitecture.Trim();
                        continue;
                    }
                }
                int index1 = name.IndexOf("Version", StringComparison.OrdinalIgnoreCase);
        ...
        private static bool CheckTheSameAssembly(string assemblypath, string pt, string vt, string ct)
        {
            ...
            if (sourespt.ToLowerInvariant() != pt.ToLowerInvariant() ||
                souresvt.ToLowerInvariant() != vt.ToLowerInvariant() ||
                sourect.ToLowerInvariant() != ct.ToLowerInvariant())
            {
                return false;
            }
            return true;
        ...
        private static bool CheckTheSameAssembly(string assemblypath, string pt, string vt, string ct,string pa)
        {
            Assembly soures = Assembly.LoadFile(assemblypath);
            string sourespt = null;
            string souresvt = null;
            string sourect = null;
            string sourcepa = soures.GetName().ProcessorArchitecture.ToString();
            string[] Strings = soures.FullName.Split(',');
            foreach (string name in Strings)
            {
                int index = name.IndexOf("PublicKeyToken", StringComparison.OrdinalIgnoreCase);
            ...
            if (sourespt.ToLowerInvariant() != pt.ToLowerInvariant() ||
                souresvt.ToLowerInvariant() != vt.ToLowerInvariant() ||
                sourect.ToLowerInvariant() != ct.ToLowerInvariant() || sourcepa.ToLowerInvariant() != pa.ToLowerInvariant())
            {
                return false;
            }
            return true;
