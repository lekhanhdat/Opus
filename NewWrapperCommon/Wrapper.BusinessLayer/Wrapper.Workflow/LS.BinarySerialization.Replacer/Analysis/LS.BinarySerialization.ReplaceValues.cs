...
namespace LS.BinarySerialization
{
    ...
    internal class LSReplaceValues:IDisposable
    {
        ...
        private HashSet<string> mListIdPropNames;
        private HashSet<string> mUserAndEmailPropNames;
        private HashSet<string> mItemUrlRelatedPropNames;
        private HashSet<string> mUrlRelatedPropNames;
        ...
        [SuppressMessage("FxCopCustomRules", "C100007:SpellCheckStringValues", Justification = "ignore case key.")]
        internal LSReplaceValues(byte[] data, Dictionary<string, object> dictionary,LSObjectNodeAnalyze analyzeProc)
        {
            ...
            //mListIdPropNames相关替换处理功能已扩展webid和siteid, folderid相关id替换
            mListIdPropNames = new HashSet<string>();
            mListIdPropNames.Add("listid");
            mListIdPropNames.Add("externallistid");
            mListIdPropNames.Add("tolistid");
            mListIdPropNames.Add("siteid");
            mListIdPropNames.Add("webid");
            mListIdPropNames.Add("camlquery");
            mListIdPropNames.Add("targetsiteid");
            mListIdPropNames.Add("targetwebid");
            mListIdPropNames.Add("targetfolderid");

            mUserAndEmailPropNames = new HashSet<string>();
            ...
            mItemUrlRelatedPropNames = new HashSet<string>();
            mItemUrlRelatedPropNames.Add("m_emailBody");
            mItemUrlRelatedPropNames.Add("m_defaultTaskTitle");
            mItemUrlRelatedPropNames.Add("m_itemUrl");
            mItemUrlRelatedPropNames.Add("m_historyDescStart");
            mItemUrlRelatedPropNames.Add("m_historyDescEnd");

            mUrlRelatedPropNames = new HashSet<string>();
            mUrlRelatedPropNames.Add("ParentWebUrl");
            mUrlRelatedPropNames.Add("Url");
            mUrlRelatedPropNames.Add("HiddenUrl");
            mUrlRelatedPropNames.Add("SiteUrl");
            mUrlRelatedPropNames.Add("WebApplicationUrl");
            mUrlRelatedPropNames.Add("Input");
        }
                    ...
                    if (xn is XmlElement)
                    {
                        ...
                        { 
                            ...
                            case "m_histListId":
                                ...
                                case "m_itemId":
                                    {
                                        originalItemId = fld.InnerXml;
                                        newItemId = repDictionary[name].ToString();
                                        break;
                                    }
                            ...
                            case "m_taskListUrl":/*/sites/WorkflowTestSiteDst/Lists/Tasks*/
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(oldUrl))
                                    {
                                        ...
                                        string newUrl =replaceProcessor.UrlReplace(oldUrl);
                                        fld.InnerXml = newUrl;
                                        repDictionary[name] = new LS.BinarySerialization.Replacer.LSMemberDataInfo(oldUrl, newUrl, name);
                                    }
                                    break;
                                }
                            case "m_itemUrl":/*Shared Documents/EBSPerformanceResult.txt*/
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(oldUrl))
                                    {
                                        if (!oldUrl.StartsWith("/", StringComparison.OrdinalIgnoreCase) && !oldUrl.StartsWith("http", StringComparison.OrdinalIgnoreCase))
                                        {
                                            if (repDictionary.ContainsKey("m_webUrl"))
                                            {
                                                LS.BinarySerialization.Replacer.LSMemberDataInfo dataInfo = (LS.BinarySerialization.Replacer.LSMemberDataInfo)repDictionary["m_webUrl"];
                                                oldUrl = dataInfo.OldValue.ToString() + "/" + oldUrl;
                                                AveWorkflowReplaceProcessor replaceProcessor = new AveWorkflowReplaceProcessor(SPWorkflowProcessorRuntime.MappingManager.SiteMappingManager.SiteManagedMappings, new ReplaceOption(true, true, true), SPWorkflowProcessorRuntime.MappingManager.SiteMappingManager.SourceSiteInfo, SPWorkflowProcessorRuntime.MappingManager.SiteMappingManager.DestSiteInfo.ServerRelativeUrl);
                                                string newUrl = replaceProcessor.UrlReplace(oldUrl);
                                                if (oldUrl.EndsWith("?ID=" + originalItemId, StringComparison.OrdinalIgnoreCase))
                                                {
                                                    newUrl = newUrl.Replace("?ID=" + originalItemId, "?ID=" + newItemId);
                                                }
                                                if (newUrl.StartsWith(dataInfo.NewValue.ToString(), StringComparison.OrdinalIgnoreCase))
                                                {
                                                    fld.InnerXml = newUrl.Substring(dataInfo.NewValue.ToString().Length + 1);
                                                }
                                                repDictionary[name] = new LS.BinarySerialization.Replacer.LSMemberDataInfo(oldUrl, newUrl, name);
                                            }
                                        }
                ...
                try
                {
                    ...
                    if (inData.Length > 500)
                    {
                        ...
                        foreach (XmlNode xe in list)
                        {
                            if (xe is XmlElement)
                            {
                                string value = xe.InnerText.ToLower(CultureInfo.CurrentCulture);
                                if (!string.IsNullOrEmpty(value))
                                {
                                    ...
                                    if (value.Contains("\\"))
                                    {
                                        lenGroup = value.IndexOf('\\');
                                        lenUser = value.Substring(lenGroup + 1).Length;
                                        string pattern = @"([^\/\[\]:;\|=,\+\*\?<>@]){" + lenGroup + @"}\\{1}([^\/\[\]:;\|=,\+\*\?<>@]){" + lenUser + "}";
                                        System.Text.RegularExpressions.Regex regex = new System.Text.RegularExpressions.Regex(pattern);
                                        if (regex.IsMatch(value) || value.StartsWith("i:0#.w|", StringComparison.OrdinalIgnoreCase))
                                        {
        ...
        {
            if (!repDictionary.ContainsKey(node.GuidInternalValue.ToString().ToUpper(CultureInfo.CurrentCulture)))
                return;

            LSObjectNodeValueExtensionEx valueInfoEx = new LSObjectNodeValueExtensionEx();
            valueInfoEx.valueInfo = node.ValueTypes["0"];
            valueInfoEx.value = ((Guid)repDictionary[node.GuidInternalValue.ToString().ToUpper(CultureInfo.CurrentCulture)]).ToByteArray();
            valueInfoEx.objectName = node.Name;
            valueInfoEx.memberName = "0";
            valueInfos.Add(valueInfoEx);
        }
        ...
        private void ReplaceCompiledAssembly(LSObjectNode node)
        {
            if (node.Members["AssemblyName"] is InternalMemberValueE)
                return;
            string key = ((string)node.Members["AssemblyName"]).ToLower(CultureInfo.CurrentCulture);
            if (repDictionary.ContainsKey(key))
            {
                LSObjectNodeValueExtensionEx valueInfoEx = new LSObjectNodeValueExtensionEx();
        ...
        private void ReplacePrimitiveKeyValuePair(object key, object value, LSObjectNodeValueExtensionEx infoExt)
        {
            ...
            if (mListIdPropNames.Contains(key.ToString().ToLower(CultureInfo.InvariantCulture)))
            {
            ...
            else if (mUrlRelatedPropNames.Contains(key.ToString()))
            {
                string oldUrl = value as string;
                if (!string.IsNullOrEmpty(oldUrl))
                {
                    oldUrl = oldUrl.TrimStart('\t');
                }
                if (!string.IsNullOrEmpty(oldUrl) && (oldUrl.StartsWith("http", StringComparison.OrdinalIgnoreCase) || oldUrl.StartsWith("/", StringComparison.OrdinalIgnoreCase)))
                {
                    AveWorkflowReplaceProcessor replaceProcessor = new AveWorkflowReplaceProcessor(SPWorkflowProcessorRuntime.MappingManager.SiteMappingManager.SiteManagedMappings, new ReplaceOption(true, true, true), SPWorkflowProcessorRuntime.MappingManager.SiteMappingManager.SourceSiteInfo, SPWorkflowProcessorRuntime.MappingManager.SiteMappingManager.DestSiteInfo.ServerRelativeUrl);               
                    string newUrl = replaceProcessor.UrlReplace(oldUrl);
                    if (!newUrl.Equals(oldUrl))
                    {
                        infoExt.value = newUrl;
                        infoExt.memberName = key.ToString();
                        valueInfos.Add(infoExt);
                    }
                }
            }
            else if (mUserAndEmailPropNames.Contains(key.ToString().ToLower(CultureInfo.InvariantCulture)) || key.ToString().ToLower(CultureInfo.InvariantCulture).Equals("value"))
            {
                if (value is string)
                {
                    bool needReplace = true;
                    if (key.ToString().ToLower(CultureInfo.InvariantCulture).Equals("value"))
                    {
                        string originalStr = string.Empty;
                        string pattern = string.Empty;
                        int lenGroup = -1;
                        int lenUser = -1;
                        System.Text.RegularExpressions.Regex regex = new System.Text.RegularExpressions.Regex(string.Empty);
                        originalStr = value.ToString();
                        if (originalStr.Contains("\\"))
                        {
                            ...
                            if (!regex.IsMatch(originalStr) && !originalStr.StartsWith("i:0#.w|", StringComparison.OrdinalIgnoreCase))
                            {
                                needReplace = false;
                            }
                        }
                        ...
                        {
                            if (repDictionary.ContainsKey("m_itemUrl"))
                            {
                                Replacer.LSMemberDataInfo dataInfo = (Replacer.LSMemberDataInfo)repDictionary["m_itemUrl"];
                                string oldUrl = dataInfo.OldValue.ToString();
                                string newUrl = dataInfo.NewValue.ToString();
                                if (originalStr.IndexOf(oldUrl, StringComparison.OrdinalIgnoreCase) >= 0)
                                {
                                    newValue = value.ToString().Replace(oldUrl, newUrl);
                                    infoExt.value = newValue;
                                    infoExt.memberName = key.ToString();
                                    valueInfos.Add(infoExt);
                                }
                                else if (originalStr.IndexOf(System.Web.HttpUtility.UrlPathEncode(oldUrl), StringComparison.OrdinalIgnoreCase) >= 0)
                                {
                                    newValue = value.ToString().Replace(System.Web.HttpUtility.UrlPathEncode(oldUrl), System.Web.HttpUtility.UrlPathEncode(newUrl));
                                    infoExt.value = newValue;
                                    infoExt.memberName = key.ToString();
                                    valueInfos.Add(infoExt);
                                }
            ...
            else if (mMemberDataInfoEx.DependencyPropertyNames.Contains(key.ToString().ToLower(CultureInfo.InvariantCulture)))
            {
                #region Data is not a Reference
            ...
            else if (mItemUrlRelatedPropNames.Contains(key.ToString()))
            {
                if (repDictionary.ContainsKey("m_itemUrl"))
                {
                    Replacer.LSMemberDataInfo dataInfo = (Replacer.LSMemberDataInfo)repDictionary["m_itemUrl"];
                    string oldUrl = dataInfo.OldValue.ToString();
                    string newUrl = dataInfo.NewValue.ToString();
                    if (value.ToString().IndexOf(oldUrl, StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        newValue = value.ToString().Replace(oldUrl, newUrl);
                        infoExt.value = newValue;
                        infoExt.memberName = key.ToString();
                    ...
                    else if (value.ToString().IndexOf(System.Web.HttpUtility.UrlPathEncode(oldUrl), StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        newValue = value.ToString().Replace(System.Web.HttpUtility.UrlPathEncode(oldUrl), System.Web.HttpUtility.UrlPathEncode(newUrl));
                        infoExt.value = newValue;
                        infoExt.memberName = key.ToString();
        ...
        private void GetMemberDataEx()
        {
            ...
            foreach (KeyValuePair<string, object> pair in repDictionary)
            {
                if (pair.Key.StartsWith("LS",StringComparison.Ordinal))
                {
                    ...
                    string propName = temp.DependencyPropertyName.ToLower(CultureInfo.InvariantCulture);
