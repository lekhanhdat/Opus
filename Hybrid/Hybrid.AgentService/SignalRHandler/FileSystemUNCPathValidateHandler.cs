...
{
    public class FileSystemUNCPathValidateHandler : EndpointHandlerBase<FileSystemUNCPathValidateExecute, FileSystemUNCPathValidateArgs, ValidateResult>
    {

        private static readonly AvePoint.GCommon.AveLogger Logger = AvePoint.GCommon.AveLogger.GetInstance(typeof(FileSystemUNCPathValidateHandler));
        ...
        public override ValidateResult Process(FileSystemUNCPathValidateArgs param)
        {
            Logger.Info($"Receive message....");
            try
            {
                var request = SerializerHelper.SerializeByJsonSerializer(param);
                var response = RetryPolicy.ExecuteAction(() => HybridBrowserUtil.Instance.Browse(HybridBrowserType.FileSystemUNCPathValidate, request));
                var result = SerializerHelper.DeserializeByJsonConvert<ValidateResult>(response);
