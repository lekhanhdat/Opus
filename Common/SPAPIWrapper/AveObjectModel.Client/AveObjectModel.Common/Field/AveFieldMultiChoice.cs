...
namespace AvePoint.ObjectModel.Common
{
    class AveFieldMultiChoice : AveField, IAveFieldMultiChoice
    {
        ...
        private bool needUpdateChoices;

        public AveFieldMultiChoice(IAveRequest request, AveList list, AveWeb web, string fieldSource, AveFieldCollection fieldCollection, IDictionary<string, object> contentTypeProp, IDictionary<string, object> prop)
        ...
        public override string GetFieldValueAsText(object uV)
        {
            ...
            if (uV is String[])
            {
                return string.Join(",", uV as String[]);
            }
            else
            {
                return uV.ToString();
        ...
        {
            if (base.DataCache.ChangedProperties.ContainsKey("Choices"))
            {
                int index = (base.DataCache.ChangedProperties["Choices"] as StringCollection).Add(value);
            }
            else
            {
                base.DataCache.AddChangedProperty("Choices", base.DataCache.GetProperty<StringCollection>("Choices"));
                int index = (base.DataCache.ChangedProperties["Choices"] as StringCollection).Add(value);
            }
        }
