...
namespace AvePoint.RA.SharePoint.Extension
{
    public static class AveListItemExtension
    {
        ...
        public static bool IsHaveRecordLabel(this IAveListItem item)
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiveBackUp.HaveRetentionLabel"))
            {
                ...
                if (!string.IsNullOrEmpty(currentLabelInfo?.ComplianceTag ?? string.Empty))
                {
                    ...
                    var availableTags = aveSite.GetAvailableTagsForSite().ToDictionary(r => r.TagName, StringComparer.OrdinalIgnoreCase);
        ...
        public static DateTime GetCustomUTCDateWithTimeZone(this IAveListItem item, string fieldName)
        {
            DateTime dateTime = DateTime.MinValue;
            object customDate = null;
            if(!item.FieldValues.TryGetValue(fieldName, out customDate))
            {
                try
                {
                    customDate = item[fieldName];
                }
                catch (Exception e)
                {
                    mLog.Error("An error occurred while getting utc time for field:{0} error:{1}", fieldName, e.ToString());
        ...
        public static bool IsStubItem(this IAveListItem item)
        {
            ...
            try
            {
                if (item.FieldValues.TryGetValue(LinkFileCommon.LinkFileFieldName, out object value) && value != null
        ...
        public static bool IsM365ArchivedItem(this IAveListItem item)
        {
            bool isM365Archived = false;
            try
            {
                if (item.FieldValues.TryGetValue("_FileArchiveStatus", out object value)
                    && value != null
            ...
            {
                mLog.Warn($"check m365 archived item error:{ex.ToString()}");
            }
            return isM365Archived;
        }
