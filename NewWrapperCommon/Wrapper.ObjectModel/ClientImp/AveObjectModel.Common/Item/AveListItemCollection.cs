...
namespace AvePoint.ObjectModel.Common
{
    class AveListItemCollection : AveAbstractCommonCollection<IAveListItem>, IAveListItemCollection
    {
        ...
        private void InitItemCollection(bool isNewCreated)
        {
            ...
            this.mPageInfo = base.DataCache.GetProperty<string>("PageInfo");
        }
        ...
        public IAveListItem GetItemByGuid(Guid guid, string parentFolderServerRelativeUrl = null)
        {
            return mListData.Find(
                delegate(IAveListItem item)
                {
                    return string.IsNullOrEmpty(parentFolderServerRelativeUrl) ? 
                        item.GetTPGuid().Equals(guid) : 
                        (item.GetTPGuid().Equals(guid) && 
                         item.FieldValues.ContainsKey("FileDirRef") &&
                         item.FieldValues["FileDirRef"].ToString().Equals(parentFolderServerRelativeUrl, StringComparison.OrdinalIgnoreCase));
                });
        }
