...
namespace SharpWordNet
{
	...
	/// </summary>
	public class DataFileEngine : WordNetEngine, IDisposable
	{
        ...
        public override IndexWord[] GetAllIndexWords(string partOfSpeech)
        {
            ...
            while (!searchFile.EndOfStream)
            {
                ...
                if (line != null && !line.StartsWith(space))
                {
...
//				}
        ...
        public void Dispose()
        {
            foreach(PosDataFileSet set in _dataFileDictionary.Values)
            {
				set.Dispose();
            }
        }

        private class PosDataFileSet : IDisposable
        {
