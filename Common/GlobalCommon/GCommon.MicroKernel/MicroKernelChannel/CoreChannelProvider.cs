...
namespace AvePoint.GCommon.MicroKernel
{
    ...
    internal class CoreChannelProvider : ICoreChannelPovider
    {
        ...
        public TChannel CreateChannel<TChannel>(EndpointInfo endpoint)
        {
            ...
            if (endpoint.Scheme.StartsWith("https", StringComparison.OrdinalIgnoreCase))
                channel = this.GetHttpsChannel<TChannel>(endpoint);
            else channel = this.GetNetTcpChannel<TChannel>(endpoint);
            return channel;
        }
        ...
        {
            var channelFactory = default(ChannelFactory<TChannel>);
            if (!String.IsNullOrEmpty(endpoint.EndpointConfigurationName))
            {
                ...
                if (endpoint.X509CertificateValidationThumbprintFindValue is X509Certificate2)
                {
                    channelFactory.Credentials.ClientCertificate.Certificate = (X509Certificate2)endpoint.X509CertificateValidationThumbprintFindValue;
                }
            }
            else
            {
                ...
                if (String.IsNullOrEmpty(endpoint.EndpointAddress))
                    channelFactory.Endpoint.Address = new EndpointAddress(defaultEndpointAddress);
                else channelFactory.Endpoint.Address = new EndpointAddress(endpoint.EndpointAddress);
