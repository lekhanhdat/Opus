...
{
    public class RMDiscoveryFSExecutionInfoDao : IRMDiscoveryFSExecutionInfoDao
    {
        public async Task<(long fileTotalSize, int executedCount, int currentYearCount)> CalculateAsync(LicenseType licenseType)
        {
            ...
            var fileTotalSize = (await efContext.FSExecutionInfoList
                .Where(item => item.LicenseType == licenseType)
                .ToListAsync()).Sum(item => item.FileTotalSize);

            var executedCount = await efContext.FSExecutionInfoList
                .Where(item => item.LicenseType == licenseType)
                .CountAsync();

            var currentYearCount = (await efContext.FSExecutionInfoList
                .Where(item => item.LicenseType == licenseType
        ...
        public async Task<(long fileTotalSize, int executedCount)> CalculateAllAsync(LicenseType licenseType)
        {
            ...
            var fileTotalSizeQuery = efContext.FSExecutionInfoList
                .Where(item => item.LicenseType == licenseType);
        ...
        public async Task<int> GetCurrentMonthExecuteCountAsync()
        {
            ...
            using var efContext = await RMDiscoveryDBManager.GetEFContextAsync();
            return await efContext.FSExecutionInfoList
                .CountAsync(item => item.Month == month);
        }
        ...
        public async Task GenerateByMainJobAsync(Guid mainJobId, LicenseType licenseType)
        {
            var month = Convert.ToInt32(DateTime.UtcNow.ToString("yyyyMM"));
            using var efContext = await RMDiscoveryDBManager.GetEFContextAsync();
            efContext.FSExecutionInfoList.Add(new RMDiscoveryFSExecutionInfo
            {
                Month = month,
                FileTotalSize = 0,
                MainJobId = mainJobId,
        ...
        {
            using var efContext = await RMDiscoveryDBManager.GetEFContextAsync();
            var info = await efContext.FSExecutionInfoList
                .FirstOrDefaultAsync(item => item.MainJobId == mainJobId);
            if (info != null)
            {
        ...
        public async Task DeleteByMainJobIdAsync(Guid mainJobId)
        {
            ...
            if (info != null)
            {
                efContext.FSExecutionInfoList.Remove(info);
                await efContext.SaveChangesAsync();
            }
        }
        ...
        {
            using var efContext = await RMDiscoveryDBManager.GetEFContextAsync();
            efContext.FSExecutionInfoList.RemoveRange(efContext.FSExecutionInfoList);
            await efContext.SaveChangesAsync();
        }
    }
}
