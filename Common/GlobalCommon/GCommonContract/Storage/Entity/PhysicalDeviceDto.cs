...
namespace AvePoint.GCommon.Contract.Storage.Entity
{
    ...
    public class PhysicalDeviceDto
    {
        ...
        private readonly static Regex lr = new Regex(LOCATION_PATTERN);
        ...
        private readonly static Regex sf = new Regex(START_FOLDER_PATTERN);
        ...
        private string BuildXRI(bool creation)
        {
            ...
            if (!sb.ToString().Contains("&assignedspace=") && Extension != null && Extension.AssignedSpace > 0)
            {
                sb.Append("&assignedspace=");
                sb.Append(Extension.AssignedSpace);
                sb.Append("&usedspace=");
        ...
        public bool IsSnapLock
        {
            get
            {
                if (ConnectionString.ToLower().Contains(PhysicalDeviceParameterKeys.IS_SnapLock_KEY))
                {
                    return true;
                }
                return false;
        ...
        public bool IsLUN
        {
            get
            {
                if (ConnectionString.ToLower().Contains(PhysicalDeviceParameterKeys.IS_LUN_KEY))
                {
                    return true;
                }
                return false;
        ...
        public bool IsDefaultDevice()
        {
            if (Id != null)
            {
                return string.Equals(Id, RecordsConstants.AVEPOINT_DEFAULT_STORAGEID, StringComparison.OrdinalIgnoreCase) || IsSystemStorage;
            }
            else
            {
                return false;
        ...
        public static PhysicalDeviceDto GenterateCacheDevice(string path)
        {
            ...
            * Sink Details:  未被调用
            * Ignore Reason: random用于PhusicalDevice name，但name不是唯一标识，不影响安全性
            */
            pd.Name = "DocAve-Cache-" + new Random().Next(0, 1000);
            pd.Type = 0;
            pd.ConnectionString = "DocAve".ToLower() + "-xam://fs_vim?location=" + XRIUtil.ValueEncode(path) + "&" + PARAM_KEY_CACHE + "=true";
        ...
        public static PhysicalDeviceDto GenterateFS(string path, string username, string password)
        {
            PhysicalDeviceDto pd = new PhysicalDeviceDto();
            pd.Id = "pd id:" + Guid.NewGuid().ToString();
            /* Fortify Issue Type: Insecure Randomness 
            * Sink Details:  AvePoint.GCommon.Contract.Server.ControlPanel.Object CacheSettingDtoExtension ConvertToMediaLogicalDeviceDto
            *                AvePoint.Media.Service.ArchiverBackup ArchiverInsertSiteMasterIndexCommand OpenLogicalDevice
            *                AvePoint.Media.Service.ArchiverBackup MergeIndexCommand OpenLogicalDevice
            *                AvePoint.Media.Service.ArchiverBackup ArchiverRebuildIndexCommand OpenIndexLogicalDevice
            *                AvePoint.GCommon.Contract.Server.Common CacheSettingService ConvertPhysicalDeviceInfo
            *                AvePoint.GCommon.Contract.Server.ControlPanel.Object CacheSettingDto ConvertToLogicalDeviceDto
            * Ignore Reason: random用于PhusicalDevice name，但name不是唯一标识，不影响安全性
            */
            pd.Name = "DocAve-Cache-" + new Random().Next(0, 1000);
            pd.Type = 0;
            pd.ConnectionString = "DocAve".ToLower() + "-xam://fs_vim?location=" + XRIUtil.ValueEncode(path) + "&name=" + XRIUtil.ValueEncode(username) + "&secret=" + XRIUtil.ValueEncode(password);
            pd.Usage = PhysicalDeviceUsage.All;
            pd.DeviceMode = (int)PhysicalDeviceStatus.Online;
            return pd;
        }
        ...
        /// <summary>
        /// 用来直接生成connecting string，目前仅支持Azure,add for DocAve Online
        /// </summary>
        /// <param name="deviceType">device type eg:net share; ftp...</param>
        /// <param name="parameters">containername,name,secret</param>
        /// <returns></returns>

        public static string GenerateConnectingString(StorageDeviceType deviceType, Dictionary<string, string> parameters)
        {
    ...
    public class PlysicalDeviceExtension
    {
        ...
        [DataMember]
        public long UsedSpaceForAchiver { get; set; }

        [DataMember]
        public long TotalSpace { get; set; }

        [DataMember]
        public long BackupSpace { get; set; }
    ...
    public class PhysicalDeviceParameterKeys // same as XRIParameterKeys class
    {
        ...
        public static readonly string MEDIASTORAGE_PROTOCOL = "DOCAVE-XAM://".ToLower();//connecting string的固定前缀
        ...
        public const string GOOGLE_VIM = "google_vim";
        /**************************For Vim Tpye**********************************/
        ...
        /**************************For NetApp*********************************/
        public static readonly string IS_LUN_KEY = "NetAppType=LUN".ToLower();
        public static readonly string IS_SnapLock_KEY = "SnapLock=true".ToLower();
        /**************************For NetApp*********************************/
    ...
    //online上使用的device type   2014.2.13 添加
    [DataContract(Namespace = ContractConstants.Namespace)]
