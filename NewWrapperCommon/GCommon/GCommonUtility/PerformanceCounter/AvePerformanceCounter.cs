...
namespace AvePoint.GCommon.Utility
{
    ...
    public class AvePerformanceCounterUtil
    {
        ...
        /// <summary>
        /// we will call this method when install to setup all performance counter category we will use
        /// </summary>
        public static void SetupAllCategory()
        {
            if (AvePerformanceCounterUtil.Frequency > 0)
            {
                AveSpeedPerformanceCounter.SetupCategory();
            }
        }
    }
    ...
    public class AveSpeedPerformanceCounterCatalogs
    {
        ...
        public static readonly string PRSendQueueWait = "DocAve Platform Send Queue Wait Time";

        public static readonly List<string> AllCatalogs = new List<string>() { SocketReadCatalog, SocketWriteCatalog, FileSenderWriteCatalog, FileReceiverReadCatalog, PRBackupCatalog, PRRestoreCatalog, PRReadFile, PRSendQueueWait };
    }
    ...
    public class AveSpeedPerformanceCounter
    {
        ...
        private static DateTime mLastReportTime = DateTime.MinValue;
        ...
        public static void RemoveAllCounterInstances()
        {
            try
            {
                if (mEnabled)
                {
                    lock (lockObj)
                    {
                        for (int i = 0; i < AveSpeedPerformanceCounterCatalogs.AllCatalogs.Count; i++)
                        {
                            mTotalBytesCounters[i].Close();
                            mTotalBytesCounters[i].RemoveInstance();
                            mTotalDurationCounters[i].Close();
                            mTotalDurationCounters[i].RemoveInstance();
                            mSpeedInLastMinuteCounters[i].Close();
                            mSpeedInLastMinuteCounters[i].RemoveInstance();
                        }
                        mEnabled = false;
                    }
                }
            ...
            {
                logger.Warn("RemoveAllCounterInstances() exception:{0}", e.ToString());
            }
        }

        private static string OutputPerfCounter(PerformanceCounter pc)
        {
            return string.Format("{0} : {1} . ", pc.CounterName, pc.RawValue.ToString());
        }
        ...
        public static void ReportToLog()
        {
            try
            {
                ...
                lock (lockObj)
                {
                    if (mLastUpdateTime != DateTime.MinValue && mLastUpdateTime > mLastReportTime)
                    {// need report
                        mLastReportTime = DateTime.Now;
                        for (int i = 0; i < AveSpeedPerformanceCounterCatalogs.AllCatalogs.Count; i++)
                        {
                            if (mTotalDurationCounters[i].RawValue > 0 || mTotalBytesCounters[i].RawValue > 0)
                            {
                                StringBuilder builder = new StringBuilder();
                                builder.Append(AveSpeedPerformanceCounterCatalogs.AllCatalogs[i]);
                                builder.Append(OutputPerfCounter(mTotalBytesCounters[i]));
                                builder.Append(OutputPerfCounter(mTotalDurationCounters[i]));
                                builder.Append(OutputPerfCounter(mSpeedInLastMinuteCounters[i]));
            ...
            {
                logger.Warn("Output performance information failed. Details {0}", ex.ToString());
            }
        }
    }
}
