        ...
        public void Produce(T obj)
        {
            lock (mLocker)
            {
                do
                {
                    ...
                    if (mProducerEnd)
                    {
                        throw new ObjectDisposedException("StorageOptimization13_SOARCOMPCContainerProduceObjectDisposedException");
                    }
                    if (mLists.Count == mMaxCount)
                    {
                        mLog.Info($"Current list count has reached the maximum:{mMaxCount} produce value, waiting for consume.");
                ...
                if (mLists.Count > mMaxCount)
                {
                    throw new SynchronizationLockException("StorageOptimization13_SOARCOMPCContainerProduceSynchronizationLockException");
                }
        ...
        public T Consume()
        {
            ...
            {   
                do
                {
                    if (mConsumerEnd)
                    {
                        throw new ObjectDisposedException("StorageOptimization13_SOARCOMPCContainerConsumeObjectDisposedException");
                    }
                    if (0 == mLists.Count)
                    {
                        if (mProducerEnd)
