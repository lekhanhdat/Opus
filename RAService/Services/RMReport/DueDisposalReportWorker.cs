...
{
    public class DueDisposalReportWorker : AbstractReportWorker
    {
        public DueDisposalReportWorker() 
        {
            InitCreateTableSQLString();
        }

        public override void SaveReportJobDatas(IEnumerable<BaseReport> baseReports, BaseJobDto jobInfo)
        {
            InitCreateTableSQLString();
            string reportFilePath = NeedCreateTable(jobInfo);
            ReportCenterDao.SaveReportJobDatas(reportFilePath, baseReports, this.INSERT_DATA_SQL);
        }
        ...
        {
            TABLE_NAME = ReportConstants.ReportDETAIL;
            CREATE_TABLE_SQL = string.Format(ReportConstants.CREATE_TABLE_DUE_DISPOSAL_REPORT, TABLE_NAME);
            INSERT_DATA_SQL = string.Format(ReportConstants.INSERT_DATA_DUE_DISPOSAL_REPORT, TABLE_NAME);
        }
