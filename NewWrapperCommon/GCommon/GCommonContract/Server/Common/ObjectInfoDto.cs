/********************************************************************
 *
 *  PROPRIETARY and CONFIDENTIAL
 *
 *  This file is licensed from, and is a trade secret of:
 *
 *                   AvePoint, Inc.
 *                   525 Washington Blvd, Suite 1400
 *                   Jersey City, NJ 07310
 *                   United States of America
 *                   Telephone: +1-201-793-1111
 *                   WWW: www.avepoint.com
 *
 *  Refer to your License Agreement for restrictions on use,
 *  duplication, or disclosure.
 *
 *  RESTRICTED RIGHTS LEGEND
 *
 *  Use, duplication, or disclosure by the Government is
 *  subject to restrictions as set forth in subdivision
 *  (c)(1)(ii) of the Rights in Technical Data and Computer
 *  Software clause at DFARS 252.227-7013 (Oct. 1988) and
 *  FAR 52.227-19 (C) (June 1987).
 *
 *  Copyright © 2017-2026 AvePoint® Inc. All Rights Reserved. 
 *
 *  Unpublished - All rights reserved under the copyright laws of the United States.
 */




using System;
using System.Collections.Generic;
using System.Text;
using System.Runtime.Serialization;
using AvePoint.GCommon.Contract.Common;

namespace AvePoint.GCommon.Contract.Server.Common
{
    [DataContract(Namespace = ContractConstants.Namespace)]
    public class ObjectInfoDto
    {
        [DataMember]
        public string Id { get; set; }

        [DataMember]
        public string CreateBy { get; set; }

        [DataMember]
        public string Version { get; set; }

        [DataMember]
        public long UpdateTime { get; set; }

        [DataMember]
        public string ModifyBy { get; set; }

        [DataMember]
        public long CreateTime { get; set; }

        [DataMember]
        public string CreatedUserId { get; set; }

        [DataMember]
        public string ModifiedUserId { get; set; }

        /// <summary>
        /// Object上User的Permissions
        /// </summary>
        [DataMember]
        public List<ObjectPermissionDto> ObjectPermissions { get; set; }

        /// <summary>
        /// Object上User的Roles
        /// </summary>
        [DataMember]
        public List<ObjectRoleDto> ObjectRoles { get; set; }

        /// <summary>
        /// 当前用户对Object的Permission
        /// </summary>
        [DataMember]
        public int ObjectPermission { get; set; }

        public bool HavePermission(EntityObjectPermissionType permissionType)
        {
            return (EntityObjectPermissionType)(ObjectPermission & (int)permissionType) == permissionType;
        }
    }
}
