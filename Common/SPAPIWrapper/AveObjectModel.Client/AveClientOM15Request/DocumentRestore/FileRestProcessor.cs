...
namespace AvePoint.ObjectModel.ClientOM
{
    ...
    {
        private static AveLogger mLogger = AveLogger.GetInstance(typeof(FileRestProcessor));
        ...
        public static void AddFileByRestApi(ClientContext context, ITokenProvider provider, string parentWebFullUrl, Guid parentFolderUniqueId, string fileServerRelativeUrl, Stream content, bool overwrite)
        {
            ...
            catch (Exception e)
            {
                ...
                if(webEx != null && (webEx.Response as HttpWebResponse)?.StatusCode == HttpStatusCode.Conflict)
                {
                    throw;
                }
            }
        ...
        private static void AddFileByRestApiWithParentFolderUrl(ClientContext context, ITokenProvider provider, string parentWebFullUrl, string fileServerRelativeUrl, Stream body, bool isOverwrite)
        {
            ...
            mLogger.Info("Add Large OneNote file request: {0}", $"{parentWebFullUrl}, {SensitiveLogExtension.FormatURLInLog(fileServerRelativeUrl)}, {isOverwrite}");
        ...
        private static void AddFileByRestApiWithParentFolderId(ClientContext context, ITokenProvider provider, string parentWebFullUrl, Guid parentFolderId, string fileName, Stream body, bool isOverwrite)
        {
            ...
            string methodCmd = string.Format("getfolderbyid(guid'{0}')/files/addUsingPath(decodedUrl='{1}', overwrite={2})", parentFolderId, Uri.EscapeDataString(realName), isOverwrite.ToString().ToLowerInvariant());
            string request = string.Format("{0}/_api/Web/{1}", parentWebFullUrl, methodCmd);
            mLogger.Info("Add Large file request: {0}", $"{parentWebFullUrl}, {parentFolderId}, {SensitiveLogExtension.FormatURLInLog(fileName)}, {isOverwrite}");
            ExecuteAddFileRequest(context, provider, parentWebFullUrl, body, request);
        }
        ...
        {
            ReliableHttpWebRequest webRequest = ReliableHttpWebRequest.CreateRequest(request);
            webRequest.RefreshDigestInfo(parentWebFullUrl, provider);
        ...
        private static void AddContentStream(ReliableHttpWebRequest webRequest)
        {
