    ...
    public class AveSPProject
    {
        ...
        private void CreateProjectData(AveProjectInfo projInfo)
        {
            if (projInfo.ProjectSiteInfo != null)
            {
                ...
                if (!projSite.Exists)
                {
                    projSite = mSite.SPSite.AllWebs.Add(projInfo.ProjectSiteInfo.Url, projInfo.ProjectSiteInfo.Title, string.Empty, (uint)projInfo.ProjectSiteInfo.Language, projInfo.ProjectSiteInfo.WebTemplate, false, false);
                }
                if (!string.IsNullOrEmpty(projInfo.TaskListTitle))
                {
                    Tuple<Guid, string> taskInfo = CreateTaskList(projSite, projInfo.TaskListTitle, 1);
                    projInfo.TaskListId = taskInfo.Item1;
                    projInfo.TaskListTitle = taskInfo.Item2;
                }
            }
        }

        private Tuple<Guid, string> CreateTaskList(IAveWeb projSite, string taskListTitle, int index)
        {
