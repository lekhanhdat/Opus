...
namespace AvePoint.Media.Storage.FTP
{
    ...
    #endregion
    [VIM(VIMName.FTP, typeof(AvePoint.Media.Storage.FTP.FTPVIM))]
    class FTPVIM : AbstractVIM
    {
        public override IXSystem CreateSystem(string xri, AbstractXSystem parentSystem)
        {
            return new FTPSystem(xri, parentSystem);
        }

        public override List<string> GetFeatureXML(int type)
        {
            return FTPFeature.Getstances(type).FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(int type)
        {
            return FTPFeature.Getstances(type).FeatureObjs;
        }

        public override List<StorageFeature> GetFeatureObj(int type, string culture)
        {
            return FTPFeature.Getstances(type, culture).FeatureObjs;
        }
    }
}
