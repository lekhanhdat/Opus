...
namespace AvePoint.Media.Core.Index
{
    ...
    internal class ConnectionLock
    {
        ...
        public bool Add(Guid id, ConnectionLockType lockType)
        {
            lock (connectionLocks)
            {
                connectionLocks.RemoveAll((lockInfo) =>
                {
                    ...
                    {
                        logger.Warn(MediaCoreIndexResource.ConnectionLockAddRemoveLockOfDeadThread, lockInfo.ToString());
                        return true;
                    }
                    return false;
                });
                if (lockType == ConnectionLockType.Download || lockType == ConnectionLockType.Upload)
                {
                    ...
                    {
                        logger.Info(MediaCoreIndexResource.ConnectionLockAddCancelLock, lockType.ToString());
                        ShowCurrentLocks();
                        return false;
                    }
                }
                else if (lockType == ConnectionLockType.ReadWrite)
                {
                    ...
                    {
                        logger.Info(MediaCoreIndexResource.ConnectionLockAddWaitingLock, lockType.ToString());
                        ShowCurrentLocks();
                        Monitor.Wait(connectionLocks);
                        logger.Info(MediaCoreIndexResource.ConnectionLockAddWaitingLockAwake, lockType.ToString());
                        ShowCurrentLocks();
                    }
                }
        ...
        private void ShowCurrentLocks()
        {
            ...
            logger.Info(MediaCoreIndexResource.ConnectionLockShowCurrentLocksAllLocks, sb.ToString());
        }
