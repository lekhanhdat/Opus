...
namespace AvePoint.Media.Storage.Cloud.Amazon
{
    ...
    [VIM(VIMName.Amazon, typeof(AvePoint.Media.Storage.Cloud.Amazon.AmazonVIM))]
    class AmazonVIM : AbstractVIM
    {
        public override IXSystem CreateSystem(string xri, AbstractXSystem parentSystem)
        {
            return new AmazonSystem(xri, XSystemConst.MODE_NOW_INITSYSTEM,parentSystem);
        }

        public override List<string> GetFeatureXML(int type)
        {
            return AmazonFeature.Getstances(type).FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(int type)
        {
            return AmazonFeature.Getstances(type).FeatureObjs;
        }

        public override List<StorageFeature> GetFeatureObj(int type,string culture)
        {
            return AmazonFeature.Getstances(type,culture).FeatureObjs;
        }
    }
}
