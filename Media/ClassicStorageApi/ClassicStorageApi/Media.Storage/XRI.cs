...
namespace AvePoint.Media.ClassicStorage.Util
{
    ...
    public class XRI : INotifyPropertyChanged
    {
        ...
        public static XRI ValueOf(string xriString)
        {
            ...
            if (!match.Success)
            {
                throw new Exception("Storage connection string isn't match xri pattern.");
            }
            XRI xri = new XRI();
        ...
        {
            return XRI.DocAve_PREFIX + "fs_vim?location=" + location + "&name=" + username + "&secret=" + encrptedPassword; 
        }
        /**
