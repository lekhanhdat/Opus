...
namespace AvePoint.Wrapper.Common
{
    ...
    internal class CabinetFileStreamContext : ICabinetCreateStreamContext, ICabinetExtractStreamContext
    {
        ...
        private static IAveLogger logger = AveLogger.GetInstance(typeof(CabinetFileStreamContext));
        ...
        public virtual void CloseFileWriteStream(string path, Stream stream, FileAttributes attributes, DateTime lastWriteTime)
        {
            ...
            if (fileName != null)
            {
                ...
                if (lastWriteTime != DateTime.MinValue)
                {
                    ...
                    {
                        logger.Error($"error occured when CloseFileWriteStream,error:{e}");
                    }
                    catch (IOException e)
                    {
                        logger.Error($"error occured when CloseFileWriteStream1,error:{e}");
                    }
                }
                try
                {
                ...
                {
                    logger.Error($"error occured when CloseFileWriteStream2,error:{e}");
                }
            }
        }
