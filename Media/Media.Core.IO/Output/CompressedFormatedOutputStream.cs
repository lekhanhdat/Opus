...
namespace AvePoint.Media.Core.IO.Output
{
    ...
    public class CompressedFormatedOutputStream : FilterGeneralOutputStream
    {
       // AveLogger logger = new AveLogger(MethodBase.GetCurrentMethod().DeclaringType);
        ...
        public static event Action<long> BeforeCompressEvent = (long size) => { };

        public static event Action<long> AfterCompressEvent = (long size) => { };
        ...
        public override void WriteHeaderXml(string headerXml)
        {
            ...
            {
                var headerBytes = System.Text.Encoding.UTF8.GetByteCount(headerXml);
                BeforeCompressEvent(headerBytes);
                AfterCompressEvent(headerBytes);
            }
        ...
        {
            if (!string.IsNullOrEmpty(tailXml))
            {
                var tailBytes = System.Text.Encoding.UTF8.GetByteCount(tailXml);
                BeforeCompressEvent(tailBytes);
                AfterCompressEvent(tailBytes);
            }
