...
using System.Text;
using System.Xml;
using AvePoint.RA.RACommonUtility.CAMLHelper.CAML.Base;
...
namespace AvePoint.RA.RACommonUtility.CAMLHelper.CAML
{
    ...
    public sealed class Query : BaseElement
    {
        ...
        const string CAML_IN = "<In><FieldRef Name='{0}'/><Values>{1}</Values></In>";

        const string CAML_IN_VALUE = "<Value Type='{1}'>{0}</Value>";
        ...
        public Query(string field, Types.FieldRefTypes fieldRefType, Types.FieldTypes fieldType, Types.QueryTypes queryType, string[] values)
            : base(field, fieldRefType)
        {
            _fieldType = fieldType;
            _queryType = queryType;
            ...
            if (!string.IsNullOrEmpty(this.Field))
            {
                ...
                else
                {
                    if (Validate())
                    {
                        ...
                        else if (this.QueryType == Types.QueryTypes.In) 
                        {
                            StringBuilder camlValue = new StringBuilder();
                            for (int i = 0; i < this.StrValues.Length; i++)
                            {
                                camlValue.Append(string.Format(CAML_IN_VALUE, this.StrValues[i], this.FieldType.ToString()));
                            }
                            caml = string.Format(CAML_IN, this.Field, camlValue);
                        }
