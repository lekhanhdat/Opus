...
{
    public class RMEXOSyncDataFullJob : RMEXOSyncDataJobBase
    {
        public RMEXOSyncDataFullJob(ExchangeOnlineTreeNodeDto treeNode, JobManagement jobManagement) : base(treeNode, jobManagement)
        {
        }
        ...
        protected override IEnumerable<ExchangeFolder> GetFolders(ExchangeFolder folder)
        {
            ...
            else
            {
                foreach (var f in folder.GetAllSubFolders().Where(f => f.FolderType == "IPF.Note"))
                {
                    ...
                    f.GenerateCurrentItemSyncState();
                    yield return f;
                }
            }
        }
