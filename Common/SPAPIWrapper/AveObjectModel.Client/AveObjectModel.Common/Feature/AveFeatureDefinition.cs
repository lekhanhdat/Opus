...
namespace AvePoint.ObjectModel.Common
{
    class AveFeatureDefinition : AveClientObject, IAveFeatureDefinition
    {
        ...
        public AveFeatureDefinition(AveFeature feature, IAveRequest request, IDictionary<string, object> featureDefProperties)
        {
            mFeature = feature;
            mRequest = request;
        ...
        public IAveFeatureDependencyCollection ActivationDependencies
        {
            get 
            {
                if (base.DataCache.IsPropertyNotLoaded("ActivationDependencies"))
                {
                    ...
                    base.DataCache.AddProperty("ActivationDependencies",featureDependencyCol);
        ...
        public bool ActiveStatus
        {
            get
            {
                if (base.DataCache.GetProperty<string>("Status").Equals("Deactivate", StringComparison.OrdinalIgnoreCase)
                    || base.DataCache.GetProperty<string>("Status").Equals("非アクティブ化", StringComparison.OrdinalIgnoreCase))
                {
                    return true;
                }
                else
