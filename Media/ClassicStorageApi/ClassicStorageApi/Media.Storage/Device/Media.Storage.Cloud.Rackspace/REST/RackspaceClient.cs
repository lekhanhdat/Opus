...
namespace AvePoint.Media.ClassicStorage.Cloud.Rackspace.REST
{
    ...
    using PathNotFoundException = global::Storage.Util.PathNotFoundException;
    #endregion
   ...
   "dapeng.zhang@avepoint.com",
    ...
    true)]
    #endregion
    public class RackspaceClient : AbstractRESTOprationExecutor, ICloudOprationExecutor, IHttpRequestPrepare, IHttpResponseHandler
    {
        ...
        {
            Logger = AveLogger.GetInstance(typeof(RackspaceClient));
        ...
        public override List<string> ListContainers()
        {
            ...
            {
                Logger.Error("List containers failed." + e.Message, e);
                throw;
            }
        ...
        public override bool Login(string xSetName)
        {
            ...
            try
            {
                RackspaceHttpRequest request = GetAuthRequest(RackspaceConstants.authenticationURL);
                request.Method = RESTCommands.GET;
            ...
            {
                Logger.Error("Login failed : " + t.Message, t);
                throw;
            }
            return result;
        }
        ...
        public override bool CreateContainer(string xSetName)
        {
            ...
            try
            {
                ...
                using (HttpWebResponse resp = DoExecute(request))
                {
                    ...
                    {
                        result = true;
                        Logger.Info("Create container successfully, name : " + xSetName);
                    }
                }
            }
            catch (Exception t)
            {
                Logger.Error(GetCreateCtnErrorMsg(xSetName), t);
                throw;
            }
            return result;
        }

        public override StorageOpenValidResult HasPermissions()
        {
        ...
        public override bool CheckContainer(string xSetName)
        {
            ...
            try
            {
                ...
                using (HttpWebResponse resp = DoExecute(request))
                {
                    ...
                    {
                        Logger.Info("Container already exists, container : " + xSetName);
                        result = true;
                    }
                }
            }
        ...
        {
            bool result = false;
            Logger.Debug("Try to delete container:" + xSetName);
            ...
            try
            {
                ...
                using (HttpWebResponse resp = DoExecute(request))
                {
                    ...
                    {
                        Logger.Debug("Delete container successfully:" + xSetName);
                        result = true;
                    }
                }
            }
            ...
            {
                Logger.Warn("Cannot find path, think it deleted successful : " + t.Message);
                result = true;
            }
            catch (Exception e)
            {
        ...
        public override HttpWebRequest GetUploadRequest(string xSetName, string xStreamName, string mimeType, HttpWebRequest webRequest, int blockNumber, long dataLength)
        {
            ...
            RackspaceHttpRequest request = GetOperateRequest(BuildURL(GetStorageUrl(), xSetName, xStreamName));
            request.Method = RESTCommands.PUT;
        ...
        private bool DeleteObject(string xSetName, string xStreamName)
        {
            ...
            try
            {
                ...
                using (HttpWebResponse resp = DoExecute(request))
                {
                    ...
                    {
                        Logger.Info("Delete object successfully : " + xStreamName);
                        result = true;
                    }
                }
            }
            ...
            try
            {
                if (this.isCDNEabled && !this.isCDNFalied)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        Logger.Error("Open object with cdn failed, object : " + xStreamName + ", xSetName : " + xSetName);
        ...
        private string InitCDNURL(string xSetName)
        {
            ...
            {
                Logger.Error("Init cdn failed, container : " + xSetName + ", " + e.Message, e);
                throw;
            }
        ...
        public static bool Validate(string username, string apiAccessKey)
        {
            ...
            try
            {
                request = new RackspaceHttpRequest(RackspaceConstants.authenticationURL);
        ...
        {
            return base.Parse2File(responseXmlString);
        }
        #endregion
    }
}
