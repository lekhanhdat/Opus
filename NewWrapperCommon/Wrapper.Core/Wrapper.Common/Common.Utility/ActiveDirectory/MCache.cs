...
namespace AvePoint.Common.ActiveDirectoryWrapper.MemeryCache
{
    public class MCache
    {
        private static object mlock = new object();
        private const long AvailableRange = 5L * 60 * 1000 * 10000;
        private static Dictionary<string, MCacheItem> _cache = new Dictionary<string, MCacheItem>();
        private static Dictionary<string, long> _record = new Dictionary<string, long>();
        ...
        public static T GetValue<T>(string key) 
        {
            ...
            lock (mlock) 
            {
                long lastUpdateTime = DateTime.UtcNow.Ticks;
        ...
        private static void ClearExpiredItemWorker(object state) 
        {
            lock (mlock)
            {
                ...
                foreach (string key in _record.Keys)
                {
