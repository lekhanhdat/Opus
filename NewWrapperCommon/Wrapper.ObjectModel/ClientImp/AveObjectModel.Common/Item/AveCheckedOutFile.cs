...
namespace AvePoint.ObjectModel.Common
{
    class AveCheckedOutFile : AveClientObject, IAveCheckedOutFile
    {
        ...
        public long Length
        {
            get
            {
                if (base.DataCache.IsPropertyNotLoaded("Length") && base.DataCache.IsPropertyAvailable("FileSize"))
                {
                    ...
                    double sizeBytes = 0;
                    if (mFileSize.StartsWith("LT",StringComparison.OrdinalIgnoreCase))
                    {
                        sizeBytes = 1;
                    }
                    else if (mFileSize.EndsWith("KB", StringComparison.OrdinalIgnoreCase))
                    {
                        mFileSize = mFileSize.TrimEnd('B').TrimEnd('K');
                        sizeBytes = Convert.ToDouble(mFileSize) * 1024;
                    }
                    else if (mFileSize.EndsWith("MB", StringComparison.OrdinalIgnoreCase))
                    {
                        mFileSize = mFileSize.TrimEnd('B').TrimEnd('M');
                        sizeBytes = Convert.ToDouble(mFileSize) * 1024 * 1024;
                    }
                    else if (mFileSize.EndsWith("GB", StringComparison.OrdinalIgnoreCase))
                    {
                        mFileSize = mFileSize.TrimEnd('B').TrimEnd('G');
                        sizeBytes = Convert.ToDouble(mFileSize) * 1024 * 1024 * 1024;
                    }
                    base.DataCache.PropertiesCache.Add("Length", Convert.ToInt64(sizeBytes));
                }
        ...
        public int ListItemId
        {
            get
            {
                return base.DataCache.GetProperty<int>("ListItemId");
            }
        }
