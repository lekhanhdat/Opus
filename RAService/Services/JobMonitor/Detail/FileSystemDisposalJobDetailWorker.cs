...
{
    public class FileSystemDisposalJobDetailWorker : AbstractJobDetailWorker
    {
        ...
        public override IEnumerable<JMJobDetails> GetData(int PageSize, int StartPage, string conditionFilter, BaseJobDto jobInfo)
        {
            ...
            foreach (JMFSDisposalJobDetails jobDetail in result.Cast<JMFSDisposalJobDetails>())
            {
                var tempSize = jobDetail.Size.ToInt64();
                if(tempSize != 0 || jobDetail.Size == "0")
                {
                    jobDetail.Size = JobDetailHelper.GetDataSizeToViewForFS(tempSize);
                }
            }
            return result;
        }

        public void InitGetDataSQL(string reportFilePath, int PageSize, int StartPage, string conditionFilter)
        {

            (var exactMatch, _, _) = SQLCommond.CheckColumnsExistenceInTable(reportFilePath, TABLE_NAME, [nameof(JMFSDisposalJobDetailV2.Depth), nameof(JMFSDisposalJobDetailV2.DirPath), nameof(JMFSDisposalJobDetailV2.DetailAction)]);
            if (exactMatch)
            {
                logger.Info("The table has the new columns, use V2 SQL string.");
                InitGetDataSQLString(PageSize, StartPage, conditionFilter);
            }
            else
            {
                base.InitGetDataSQLString(PageSize, StartPage, conditionFilter);
            }
        }

        public override void InitGetDataSQLString(int PageSize, int StartPage, string conditionFilter)
        {
            string groupCondition = $" {nameof(JMFSDisposalJobDetailV2.DetailAction)}, {nameof(JMFSDisposalJobDetailV2.DirPath)}, {nameof(JMFSDisposalJobDetailV2.Depth)}, {nameof(JMFSDisposalJobDetailV2.ObjectName)} ";
        ...
        {
            if (jobDetails.FirstOrDefault() is JMFSDisposalJobDetailV2 dataSyncV2detail && dataSyncV2detail != null)
            {
                InitCreateTableSQLStringV2();
            }
            else
            {
        ...
        public void InitCreateTableSQLStringV2()
        {
            TABLE_NAME = JobMonitorConstants.JOBDETAIL;
