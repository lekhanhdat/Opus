...
namespace AvePoint.RA.SharePoint.OneDrive.EnforceRetention
{
    ...
    public class RMOneDriveEnforceRetentionProcessor
    {
        private static readonly AvePoint.RA.Contract.Services.IRALogger logger = RALogger.GetInstance(typeof(RMOneDriveEnforceRetentionProcessor));
        ...
        private OneDriveRetentionDataCache RetentionCache = OneDriveRetentionDataCache.Instance;

        public RMOneDriveEnforceRetentionProcessor(string jobId)
        {
            CurrentJobId = jobId;
        ...
        public async System.Threading.Tasks.Task RunNowAsync()
        {
            using (var performance = new PerformanceScope("RMOneDriveEnforceRetentionProcesser.RunNow"))
            {
                ...
                //此处逻辑参考的为EXO，需要重新init JobType为EF，避免Job Detail出错
                    ...
                    using (CheckJobStopScope stopScope = new CheckJobStopScope())
                    {
                        ...
                        foreach (var site in siteNodes)
                        {
                            try
                            {
                                ...
                                var allTermChanged = RetentionCache.TermRetentionMapping.Count > 0;
                                var scanTermChanged = RetentionCache.TermRetentionMapping.Count > 0;
                                    //Values.Where(x => !string.IsNullOrEmpty(x.OneDriveRetentionLabel)).ToList().Count > 0;
                                logger.Warn($"All Term Changed count:{RetentionCache.TermRetentionMapping.Count}.One Drive Term Changed count:{RetentionCache.TermRetentionMapping.Values.Where(x => !string.IsNullOrEmpty(x.OneDriveRetentionLabel)).ToList().Count}.");
                                //var scanSPChanged = lastScanTime != DateTime.MinValue.Ticks;
                                //每次只处理term change 关联的OneDrive documents
                                //其余enforce retention 相关操作，在Explorer sync 和 reclassify action 中进行处理
                                ...
                                if (/*scanSPChanged || */scanTermChanged)
                                {
                                    ...
                                    using (discoverSite)
                                    {
                                        ...
                                        if (scanTermChanged)
                                        {
                                            ...
                                            var jobWorker = BuildJobWorker(SPDiscoverType.Full, discoverSite, site, jobContext, null);
        ...
        private RMOneDriveEnforceRetentionBase BuildJobWorker(SPDiscoverType discoverType, AveDiscoverSite discoverSite, SPTreeNodeDto treeNode, JobContext jobContext, List<AveCamlQuery> camlQueries = null)
        {
            using (var performance = new PerformanceScope("SP.RMEnforceRetentionProcesser.BuildJobWorker"))
            {
                ...
                var retentionWorker = new RMOneDriveEnforceRetentionBase(discoverSite, treeNode, jobContext);
                retentionWorker.Init(sPDiscover, camlQueries);
        ...
        {
            var collectionTime = RMNodeFlagDao.GetSPValidChangeTime((int)NodeFlagType.EnforceRetention, new Guid(gorupId), new Guid(nodeId));
        ...
        {
            var processingLabelName = OneDriveRetentionDataCache.Instance.LabelStateInfo.CurrentLabel.Name;
            if (OneDriveRetentionDataCache.Instance.SPSiteRetentionLables != null &&　!OneDriveRetentionDataCache.Instance.SPSiteRetentionLables.TryGetValue(processingLabelName, out AveComplianceTagInfo tagInfo))
            {
