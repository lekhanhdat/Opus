...
namespace AvePoint.RA.SharePoint.Archiver.CAMLHelper
{
    ...
    public sealed class Query : BaseElement
    {
        ...
        const string CAML_TEXT_IN = "<In><FieldRef Name='{0}'/><Values>{1}</Values></In>";

        const string CAML_TEXT_IN_VALUE = "<Value Type='{0}'>{1}</Value>";
        ...
        // for null or isNull, no need value
        public Query(string field, Types.FieldRefTypes fieldRefType, Types.FieldTypes fieldType, Types.QueryTypes queryType)
            : base(field, fieldRefType)
        {
            _fieldType = fieldType;
            _queryType = queryType;
            ...
            if (!string.IsNullOrEmpty(this.Field))
            {
                ...
                else
                {
                    if (Validate())
                    {
                        ...
                        else if (this.QueryType == Types.QueryTypes.In && this.FieldType == Types.FieldTypes.Text)
                        {
                            StringBuilder camlValue = new StringBuilder();
                            for (int i = 0; i < this.StringValues.Length; i++)
                            {
                                camlValue.Append(string.Format(CAML_TEXT_IN_VALUE, this.FieldType.ToString(), this.StringValues[i]));
                            }
                            caml = string.Format(CAML_TEXT_IN, this.Field, camlValue);
                        }
        ...
        protected override bool Validate()
        {
            ...
            switch (this.FieldType)
            {
                case Types.FieldTypes.YesNo:
                    bool boolValue;
                    if (!string.Equals(this.Value1, "0", StringComparison.OrdinalIgnoreCase) && !bool.TryParse(this.Value1, out boolValue))
