...
{
    public class RMArchiveFullTextIndexUpgradeDao : IDbUpgradeDao
    {
        ...
        public async Task UpgradeAsync(RMDbContext context)
        {
            try
            {
                if (context.Schedule.Count(item => item.JobCategory == (int)ScheduleType.ArchiveFullTextIndex) > 0)
                {
                    Logger.Info($"The tenant: [{TenantLocalValue.LogonGroupId}] has archive full text index job schedule.");
                    return;
                }
                ...
                var scheduleInfo = new RMSchedule
                {
                    ...
                    JobCategory = (int)ScheduleType.ArchiveFullTextIndex,
            ...
            {
                Logger.Error($"An error occurred while archiver full text index upgrade logic. Error: {e}");
            }
        }
