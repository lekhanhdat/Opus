...
{
    public class ExchangeExportProcessor
    {
        ...
        public ExchangeExportProcessor(ArchiverSiteBase archiverSite, IndexDatabaseHelper dbHelper, ExportOption exportOption, Reporter reporter, string groupAddress)
        {
        ...
        public void Execute()
        {
            ...
            try
            {
                logger.Info($"Starting execution process for mail box [{_archiverMailBox.GroupAddress}]");
                ...
                ExportQueue<ExchangeDiscoverNode> exportQueue = new ExportQueue<ExchangeDiscoverNode>();
                DiscoverContent(exportQueue);

                var siteExportPath = ExportInternal(_reporter, exportQueue);

                logger.Info($"Execution process completed for site [{_archiverMailBox.GroupAddress}]");

                //ExportUtility.AddUploadedTeamsToReport(_archiverMailBox.GroupAddress, I18NEntity.GetString("SATool_ExportPath_GroupMailBoxes"), siteExportPath, GlobalCache.TargetStorageType, NodeType.ExchangeOnlineMailbox);
            }
            catch (Exception e)
            {
                logger.Error($"An error occurred when executing export process for site [{_archiverMailBox.GroupAddress}]. Error: {e}");
                throw;
            }
            finally
            {
        ...
        private string ExportInternal(Reporter reporter, ExportQueue<ExchangeDiscoverNode> exportQueue)
        {
            ...
            MailBoxExportWorkerBase worker;
            switch (GlobalCache.TargetStorageType)
            {
                case StorageDeviceType.CloudAzure:
                case StorageDeviceType.SFTP:
                    worker = new MailBoxCloudExportWorker(reporter, exportQueue, _groupAddress);
                    break;
                case StorageDeviceType.None:
                    worker = new MailBoxLocalExportWorker(reporter, exportQueue, _groupAddress);
                    break;
                default:
            ...
            try
            {
                var mailBoxPath = worker.Process();
        ...
        private void DiscoverContent(ExportQueue<ExchangeDiscoverNode> exportQueue)
        {
            try
            {
                logger.Info($"Starting content discovery mailbox for [{_groupAddress}]");

                var discover = new MailBoxDiscover(_groupAddress, _dbHelper, IndexProcessor, _archiverMailBox.SiteUrl);
        ...
        private ArchiverBrowseInfo BuildRestoreRequest(string emailAddress, string siteUrl)
        {
            ...
            {
                IndexVolume = new ExchangeVolumeGenerator().GenerateIndexVolume(volumeParam),
                Path = siteUrl,
