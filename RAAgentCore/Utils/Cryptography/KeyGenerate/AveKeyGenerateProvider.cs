...
namespace AvePoint.Hybrid.Utility.Cryptography.KeyGenerate
{
        ...
        private void TestResult(byte[] result)
        {
            foreach(byte b in result) {
                ...
                if (stream.Length == stream.Capacity)
                {
                    ...
                    stream.Position = 0;
                    var read1 = stream.Read(buf1, 0, buf1.Length);
                    var read2 = stream.Read(buf2, 0, buf2.Length);

                    if(read1 != buf1.Length || read2 != buf2.Length)
                    {
