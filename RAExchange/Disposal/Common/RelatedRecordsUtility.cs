...
namespace AvePoint.RA.RAExchange.Disposal.Common
{
    public class RelatedRecordsUtility// : IDisposable
    {
        ...
        public RelatedRecordsUtility(EXOConfiguration config)
        {
            this.config = config;
        }
        ...
        private List<RMRelatedItemInfo> GetSPRelatedProperties(string recordValue)
        {
            try
            {
                ...
                foreach (var ele in xmlDoc.GetElementsByTagName("a"))
                {
                    ...
                    var relatedObjString = element.GetAttribute("rel");
                    relatedObjString = HttpUtility.UrlDecode(relatedObjString);
                    RMRelatedItemInfo relatedObj = SerializerHelper.DeserializeFromXmlString<RMRelatedItemInfo>(relatedObjString);
                    var relatedItemUrl = HttpUtility.UrlDecode(element.GetAttribute("href"));
                    //relatedObj.url = relatedItemUrl;
                    infos.Add(relatedObj);
        ...
        {
           var record = ExplorerDao.ReadById(Guid.Empty, relatedItemInfoBeforeMove.id);
        ...
        public void AddRelateColumnValue(RMRelatedItemInfo sourceRMRelatedItemInfo, Record desFile)
        {
            if (sourceRMRelatedItemInfo.SourceFlag == (int)SourceFlag.All || sourceRMRelatedItemInfo.SourceFlag == (int)SourceFlag.SharePoint)
            {
                ...
                var desRelatedProperties = RelatedRecordsUtility.DeserializeRelatedProperties(desFile.RelatedRecords);
                if (!desRelatedProperties.Exists(x => x.id == sourceRMRelatedItemInfo.id))
                {
                    ...
                    desFile.RelatedRecords = RelatedRecordsUtility.SerializeRelatedProperties(desRelatedProperties);
                    //2.源端Related SP Object添加目的端Folder RelatedInfo.
                    sourceObjectRelatedProperties.Add(new RMRelatedItemInfo()
                    {
                        id = desFile.Id,
                        recId = desFile.RecordsId,
                    ...
                    var id = IDGenerator.GetRecordId(sourceRMRelatedItemInfo.SiteId, sourceRMRelatedItemInfo.id);
                    var record = ExplorerDao.ReadById(sourceRMRelatedItemInfo.SiteId, id);
                    if (record != null)
                    {
            ...
            else if (sourceRMRelatedItemInfo.SourceFlag == (int)SourceFlag.Physical)
            {
                ...
                if (!desRelatedProperties.Exists(x => x.id == sourceRMRelatedItemInfo.id))
                {
                    ...
                    var record = ExplorerDao.ReadById(sourceRMRelatedItemInfo.SiteId, sourceRMRelatedItemInfo.id);
                    var relatedProperties = RelatedRecordsUtility.DeserializeRelatedProperties(record.RelatedRecords);
        ...
        public string ConvertRMRelatedItemInfosToColumnValueString(List<RMRelatedItemInfo> relatedItemInfos)
        {
            ...
            foreach (var relatedItemInfo in relatedItemInfos)
            {
                ...
                string rel = SerializerHelper.SerializeByDataContractSerializer(relatedItemInfo);
        ...
        {
            var remoteSite = RABrowserClient.GetRemoteSiteCollectionByUrl(siteUrl);
            bool useSpecialApp = false;
            var bposInfo = CommonPoolUserUtil.GetBPOSInfo(remoteSite); //PoolUserUtil.GetBPOSInfo(remoteSite);
            logger.Info($"Use Special App Profile: {useSpecialApp}");
            var mfactory = MultiAppUtil.CreateAveObjectModelFactory(siteUrl, bposInfo, AveContextKind.ClientObjectModel, useSpecialApp);
            var aveSite = mfactory.CreateSite(siteUrl);
            return aveSite;
        }
