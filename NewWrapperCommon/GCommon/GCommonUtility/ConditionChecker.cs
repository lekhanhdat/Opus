...
namespace AvePoint.GCommon
{
    ...
    public class ConditionChecker
    {
        ...
        public static bool RexMatch(string value, string criteria, bool ignoreCase = false)
        {
            if (null == value) 
            { 
            ...
            if (ignoreCase)
            {
                regexRule = new Regex(criteria, RegexOptions.IgnoreCase);
            }
        ...
        public static bool Contains(List<string> valueCollection, string criteria, bool ignoreCase = true)
        {
            if (null == valueCollection) { throw new ArgumentNullException(); }
            if (string.IsNullOrEmpty(criteria)) throw new ArgumentNullException();
             if (ignoreCase)
            {
                return valueCollection.Any(element => element.ToLowerInvariant().Contains(criteria.ToLowerInvariant()));
            }
            else
            {
                return valueCollection.Any(element => element.Contains(criteria));
            }
        }
        ...
        public static bool LessThanDays(DateTime value, int criteria)
        {
            CheckDataTimeKind(value);
            if (criteria <= 0) throw new ArgumentException("criteria must be greater than zero");
            return value > DateTime.UtcNow.AddDays(0 - criteria);
        ...
        public static bool LessThanWeeks(DateTime value, int criteria)
        {
            CheckDataTimeKind(value);
            if (criteria <= 0) throw new ArgumentException("criteria must be greater than zero");
            return value > DateTime.UtcNow.AddDays(0 - criteria * 7);
        ...
        public static bool LessThanMonths(DateTime value, int criteria)
        {
            CheckDataTimeKind(value);
            if (criteria <= 0) throw new ArgumentException("criteria must be greater than zero");
            return value > DateTime.UtcNow.AddMonths(0 - criteria);
        ...
        public static bool LessThanYears(DateTime value, int criteria)
        {
            CheckDataTimeKind(value);
            if (criteria <= 0) throw new ArgumentException("criteria must be greater than zero");
            return value > DateTime.UtcNow.AddYears(0 - criteria);
        ...
        private class RegexUtility
        {
            ...
            public static string ConvertWildcardPatternToRegex(string wildcardPattern)
            {
                ...
                //ADO-161225:在正则表达式开头加上(?s)，表示后面的字符串应用SingleLine正则表达式选项。
                //应用了此选项之后，点号（.）既可以匹配文字，还可以匹配换行符\n。所以，(?s)之后的.*就可以匹配“整个文本”。
                return String.Format("(?s)^{0}$", regexExpression);
            }
