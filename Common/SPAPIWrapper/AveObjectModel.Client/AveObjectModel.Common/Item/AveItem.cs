...
{
    class AveItem : AveClientObject, IAveItem
    {
        private AveWeb mWeb;
        ...
        [ThreadStatic]
        private static AveListItem CurrentListItemCaches;
        ...
        public AveItem(AveBaseItemInfo BasicItemInfo, IAveFolder folder, IAveWeb web, IAveList list)
        {
            ...
            try
            {
                if (mBasicItemInfo.RowId > 0 && mList != null)
                {                    
                    if (AveItem.CurrentListItemCaches != null)
                    {
                        ...
                        {
                            if (mBasicItemInfo.Item != null)
                            {
                                if (mBasicItemInfo.Item.ID == mBasicItemInfo.RowId)
                                {
                                    mListItem = (AveListItem)mBasicItemInfo.Item;
                                }
                                else
                                {
                                    mLogger.Warn("Logic error, the item id:{0} is not equal the id:{1}", mBasicItemInfo.Item.ID, mBasicItemInfo.RowId);
                                    mListItem = mList.GetItemById(mBasicItemInfo.RowId) as AveListItem;
                                }
                                //mBasicItemInfo.ItemProperties = null;
                            }
                            else
                            {
                                mListItem = mList.GetItemById(mBasicItemInfo.RowId) as AveListItem;
        ...
        public Dictionary<string, object> GetAttachmentInfo(AveBaseItemInfo baseItemInfo)
        {
            ...
            #region add in 3183
            attachmentInfo.Add("Id", file.UniqueId);
            #endregion
        ...
        {
            AvePerformanceTimerPool.Start("AvePoint.ObjectModel.Common.AveItem.GetUserDataJunction");
            if (this.UserDataJunction != null && this.UserDataJunction.Count > 0)
            {
                AvePerformanceTimerPool.Stop("AvePoint.ObjectModel.Common.AveItem.GetUserDataJunction");
                ...
                foreach (IAveField field in itemVersion.Fields)
                {
                    try
                    {
                        ...
                        if (field is IAveFieldUser)
                        {
                            ...
                            {
                                List<string> value = fieldValue as List<string>;
                        ...
                        else if (field is IAveFieldLookup)
                        {
                            ...
                            if (lookupField.AllowMultipleValues)
                            {
                                ...
                                foreach (string temp in value)
                                {
                                    ...
                                    {
                                        int tempValue = 0;
                                        bool result = int.TryParse(temp.Substring(0, index), out tempValue);
                                        if (result)
                                        {
                                            dataCache = new Dictionary<string, object>();
                                            ...
                                            {
                                                dataCache["tp_value"] = temp.Substring(index + 2);
                                            }
                                        }
                                    }
                                }
        ...
        {
            AvePerformanceTimerPool.Start("AvePoint.ObjectModel.Common.AveItem.GetDocInfo");
            ...
            if (currentVersionDocData != null && currentVersionDocData.Count > 0)
            {
                ...
                //}
                AvePerformanceTimerPool.Stop("AvePoint.ObjectModel.Common.AveItem.GetDocInfo");
                //return dataCache;
                return currentVersionDocData;
            }
        ...
        {
            AvePerformanceTimerPool.Start("AvePoint.ObjectModel.Common.AveItem.GetDocVersions");
            List<int> AllVersions = null;
            AveListItem listItem = null;
            if (mList == null || baseItemInfo == null || baseItemInfo.RowId <= 0)
            {
                AvePerformanceTimerPool.Stop("AvePoint.ObjectModel.Common.AveItem.GetDocVersions");
                return null;
            }
        ...
        public int SetAttachmentSize(AveBaseItemInfo info)
        {
            return 0;
        }
        ...
        public void UpdateFields(Dictionary<string, object> fieldData, AveBaseItemInfo info)
        {
            ...
            if (this.List != null && this.List.BaseTemplate == AveListTemplateType.UserInformation && fieldData != null && fieldData.ContainsKey("ContentType"))
            {
                fieldData.Remove("ContentType");
            }
            Dictionary<string, object> fields = AveList.ConvertFieldValuesToString(fieldData, null, mList != null ? (int)mList.BaseTemplate : -1);
            mListItem.DataCache.ChangedProperties.Add("ChangedFieldValues", fields);
        ...
        public IAveFile GetVirtualFile()
        {
            Dictionary<string,object> fileProp = new Dictionary<string,object>();
            fileProp["ServerRelativeUrl"] = mBasicItemInfo.ServerRelativeUrl;
            if (this.mListItem != null)
            {
                fileProp["UIVersion"] = this.mListItem.FieldValues["_UIVersion"];
                fileProp["UniqueId"] = this.mListItem.FieldValues["UniqueId"];
            }
            else
            {
                fileProp["UIVersion"] = mBasicItemInfo.Version;
                //是否需要考虑在此添加uniqueId
                fileProp["UniqueId"] = mBasicItemInfo.GUID;
            }
            fileProp["Exists"] = true;
            return new AveFile((this.mWeb.Site as AveSite).Request, this.mWeb, null, null, fileProp);
        }
        ...
        public int GetLookupIdByGUID(Guid lookupListId, Guid tempGuid)
        {
            throw new NotImplementedException();
        }
        ...
        public void SkipRestoreSpecialListColumnValues(AveBaseItemInfo info, List<string> fieldInternalNames)
        {
            if (info == null || info.FieldsInfo == null || info.FieldsInfo.Fields == null || fieldInternalNames == null)
            {
                return;
            }
            foreach (string internalName in fieldInternalNames)
            {
                if (info.FieldsInfo.Fields.ContainsKey(internalName))
                {
                    info.FieldsInfo.Fields.Remove(internalName);
                }
            }
        }
