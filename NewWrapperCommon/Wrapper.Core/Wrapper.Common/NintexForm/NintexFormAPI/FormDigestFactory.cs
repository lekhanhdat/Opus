    ...
    class FormDigestFactory
    {
        ...
        public FormDigestBase CreateFormDigestInstance(AveContextKind contextKind, string webUrl, ICredentials credentials)
        {
            switch (contextKind)
            {
                ...
                case AveContextKind.WebServiceObjectModel:
                case AveContextKind.Auto:
                    throw new System.Exception(string.Format("Can not support this contex kind {0}", contextKind));
                ...
                default://默认返回FormDigest2013 避免以后SharePoint再次升级导致nintex form失败
