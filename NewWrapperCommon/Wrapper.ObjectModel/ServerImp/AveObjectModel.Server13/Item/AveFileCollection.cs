...
namespace AvePoint.ObjectModel.Server13
{
    class AveFileCollection : AveAbstractCommonCollection<IAveFile>, IAveFileCollection,IDisposable
    {
        ...
        {
            return new AveFile(mWeb, mFiles.Add(parameters.Url, parameters.Content, parameters.Overwrite));
        }
        ...
        {
            return new AveFile(mWeb, mFiles.Add(urlOrFile, file, overwrite));
        }
        ...
        {
            return new AveFile(mWeb, mFiles.Add(urlOfFile, file, overwrite, checkInComment, checkRequiredFields));
        }
        ...
        {
            return new AveFile(mWeb, mFiles.Add(url, file, overwrite));
        }

        public IAveFile Add(string urlOfFile, byte[] file)
        {
            return new AveFile(mWeb, mFiles.Add(urlOfFile, file));
        }
        ...
        public IAveFile AddStreamInternal(string urlOfFile, Stream stream, bool bIsMigrate, bool bIsPublish, bool bcheckRequiredProps, bool bAutoCheckoutOnInvalidData, bool bForceCreateVersion, string lockIdMatch, IAveUser createdBy, IAveUser modifiedBy, DateTime timeCreated, DateTime timeLastModified, object varProperties, string checkinComment, bool bOverwrite, Stream formatMetadata, string etagToMatch, bool bSyncUpdate, out AveVirusCheckStatus virusCheckStatus, out string virusCheckMessage, out string etagNew)
        {
            ...
            Type tSPFileStreamManager = AveAssemblyUtility.GetType("Microsoft.SharePoint, Version=15.0.0.0, Culture=neutral, PublicKeyToken=71e9bce111e9429c", "Microsoft.SharePoint.SPFileStreamManager");
        ...
        {
            return new AveFile(mWeb, mFiles.Add(urlOfFile, file, properties, ((AveUser)createdBy).User, ((AveUser)modifiedBy).User, timeCreated, timeLastModified, overwrite));
        }
