...
using AvePoint.RA.Service.Services.Discovery.Office365.Work.Analyzer.V2.Inactive;
using AvePoint.RA.Service.Services.Discovery.Office365.Work.Analyzer.V2.Rot;
...
namespace AvePoint.RA.Service.Services.Discovery.Office365.Work.Analyzer.V2
{
    public class RMDiscoveryOffice365Analyzer
    {
        ...
        private readonly RMDiscoveryExclusionInfo _exclusionInfo;
        ...
        public RMDiscoveryOffice365Analyzer(
                ...
                RMDiscoveryExclusionInfo exclusionInfo,
            ...
            try
            {
                ...
                foreach(var tenantDiscoveryJobs in tenantGroupedDiscoveryJobs)
                {
                    ...
                    foreach(var contentSourceDiscoveryJobs in contentSourceGroupedDiscoveryJobs)
                    {
                        ...
                        var aggregateTotalDataAnalyer = new RMDiscoveryOffice365AggregateTotalDataAnalyzer(o365TenantId, contentSource, _sizeRangeIds, _dateRangeIds, _enableExpandQueryTest);
                        ...
                        foreach (var discoveryJob in contentSourceDiscoveryJobs.Value)
                        {
                            ...
                            var enumerableAnalysisJobs = _jobDao.GetAnalysisJobsByDiscoveryJobWithPaginationAsync(discoveryJob.Id, 1000);
                            ...
                            await foreach(var analysisJob in enumerableAnalysisJobs)
                            {
                                ...
                                if (analysisJob.Status != RMDiscoveryJobStatus.Pending)
                                {
                                    _logger.Info($"Skip this analysis job, id:[{analysisJob.Id}], url: [{analysisJob.Url}], status: [{analysisJob.Status}]");
                                    continue;
                                }
                            ...
                            _logger.Info($"Number of analysis jobs being processed: [{processedJobCount}]");
        ...
        )
        {
            try
            {
                ...
                var hidden = aggregateInfo.FileTotalSize <= (contentSource == SourceFlag.SharePoint ? _exclusionInfo.SharePointOnlineSiteSizeLimit : _exclusionInfo.OneDriveSiteSizeLimit);

                _logger.Info($"Current site total size [{aggregateInfo.FileTotalSize}]. Exclusion info: sharepoint size limit [{_exclusionInfo.SharePointOnlineSiteSizeLimit}], onedrive size limit [{_exclusionInfo.OneDriveSiteSizeLimit}]. Need hidden [{hidden}].");

                var siteDataAnalyzer = new RMDiscoveryOffice365SiteDataAnalyzer(jobType, contentSource, containerId, analysisJob);
                var (initSucceed, siteInfo) = await siteDataAnalyzer.InitAsync(aggregateInfo.FileTotalSize, aggregateInfo.FileSumCount, hidden);
                if(!initSucceed)
                {
                    await ChangeAnalysisJobToEndAsync(analysisJob, RMDiscoveryJobStatus.Failed);
                    _reportManager.Increase();
                ...
                if (!hidden)
                {
                    ...
                    var siteRotDataAnalyzer = new RMDiscoveryOffice365SiteRotDataAnalyzer(
                                                             ...
                                                             fileExtensionAnalyzer,
                                                             _enableExpandQueryTest);
                    var (rotSucceed, rotDataList) = await siteRotDataAnalyzer.AnalysisAsync();
                    res &= rotSucceed;

                    if (rotSucceed)
                    {
                        containerRotDataAnalyer.Increse(rotDataList);
                        basicRotDataAnalyer.Increse(rotDataList);
                    }

                    if (res)
                    {
                        containerDataAnalyer.Increse(aggregateInfo.FileTotalSize, aggregateInfo.FileSumCount);
                ...
                else
                {
                    containerDataAnalyer.Increse(aggregateInfo.FileTotalSize, aggregateInfo.FileSumCount);
                    aggregateTotalDataAnalyer.Increse(aggregateInfo);
                    _logger.Info($"The hidden site holds the total number of files and the total size, [{analysisJob.Id}] [{analysisJob.Url}]");
                }
