...
namespace AvePoint.Wrapper.Restore.NintexForm.Online
{
    class ManagedMetadataControl : BaseControl
    {
        ...
        public override void ProcessControl(bool isPost)
        {
            ...
            var rootPrefix = GetNodePrefixAndAddNameSpace(mControlNode);
            if (!EnsureTermStore(isPost, rootPrefix) ||
                !EnsureGroup(isPost, rootPrefix) ||
                !EnsureTermSet(isPost, rootPrefix) ||
                !EnsureTerm(isPost, rootPrefix))
            {
                return;
            }
            UpdateControlTypeUniqueId();
        }
        private bool EnsureTermStore(bool isPost,string rootPrefix)
        {
            XmlNode termStoreNode;
            var termStoreId = TryGetNodeId(GetXPath(rootPrefix, "TermStoreId"), out termStoreNode);
        ...
        private bool EnsureGroup(bool isPost, string rootPrefix)
        {
            XmlNode termGroupNode;
            var termGroupId = TryGetNodeId(GetXPath(rootPrefix, "GroupId"), out termGroupNode);
        ...
        private bool EnsureTermSet(bool isPost, string rootPrefix)
        {
            XmlNode termSetNode;
            var termSetId = TryGetNodeId(GetXPath(rootPrefix, "TermSetId"), out termSetNode);
        ...
        private bool EnsureTerm(bool isPost, string rootPrefix)
        {
            XmlNode termNode;
            var termId = TryGetNodeId(GetXPath(rootPrefix, "AnchorId"), out termNode);
        ...
        private void UpdateControlTypeUniqueId()
        {
            ...
            var controlId = TryGetNodeId(GetXPath("FormControlTypeUniqueId"), out uniqueControlIdNode);
        ...
        {
            string prefix = node.GetPrefixOfNamespace("http://schemas.datacontract.org/2004/07/Nintex.Forms.SharePoint.FormControls._properties");
