    ...
    public abstract partial class BaseQuery<Q> : AbstractQuery where Q : BaseQuery<Q>
    {
        ...
        public Q NewChild()
        {
            ...
            this.Clauses.ForEach(
                c => {
                    if (c.Component == "from" || c.Component == "as")
                    {
                        newQuery.Clauses.Add(c.Clone());
                    }
                }
                );
            return newQuery;
        ...
        public Q FromParent(string table, string path)
        {
            return AddOrReplaceComponent("from", new FromParentClause
            {
                Table = table,
                Path = path
            });
