...
namespace AvePoint.Media.Storage.GoogleDrive
{
    ...
    [VIM(VIMName.GoogleDrive, typeof(AvePoint.Media.Storage.GoogleDrive.GoogleDriveVIM))]
    class GoogleDriveVIM : AbstractVIM
    {
        public override IXSystem CreateSystem(string xri, AbstractXSystem parentSystem)
        {
            return new GoogleDriveSystem(xri, parentSystem);
        }

        public override List<string> GetFeatureXML(int type)
        {
            return GoogleDriveFeature.Getstances(type).FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(int type)
        {
            return GoogleDriveFeature.Getstances(type).FeatureObjs;
        }

        public override List<StorageFeature> GetFeatureObj(int type, string culture)
        {
            return GoogleDriveFeature.Getstances(type, culture).FeatureObjs;
        }
    }
}
