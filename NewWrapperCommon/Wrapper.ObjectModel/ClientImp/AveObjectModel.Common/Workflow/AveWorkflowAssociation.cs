...
namespace AvePoint.ObjectModel.Common
{
    class AveWorkflowAssociation : AveClientObject, IAveWorkflowAssociation
    {
        ...
        static private AveLogger mLogger = AveLogger.GetInstance(typeof(AveWorkflowAssociation));

        public AveWorkflowAssociation() 
        {

        }
        public AveWorkflowAssociation(IAveWeb web, IAveList list, string workflowSource, Dictionary<string, object> prop)
        {
            mList = (AveList)list;
            mWeb = (AveWeb)web;
        ...
        public Guid HistoryListId
        {
            get
            {
                if ((!base.DataCache.IsPropertyAvailable("HistoryListId") || base.DataCache.GetProperty<Guid>("HistoryListId") == Guid.Empty)
                    && !string.IsNullOrEmpty(HistoryListTitle))
                {
                    try
                    {
                        IAveList historyList = this.ParentWeb.Lists[HistoryListTitle];
                        base.DataCache.PropertiesCache["HistoryListId"] = historyList.ID;
                    }
                    catch (Exception ex)
                    {
                        mLogger.Debug("Can not find workflow history list by Title. Message: {0}", ex.ToString());
                    }
                }
        ...
        public Guid TaskListId
        {
            get
            {
                if ((!base.DataCache.IsPropertyAvailable("TaskListId") || base.DataCache.GetProperty<Guid>("TaskListId") == Guid.Empty)
                    && !string.IsNullOrEmpty(TaskListTitle))
                {
                    try
                    {
                        IAveList taskList = this.ParentWeb.Lists[TaskListTitle];
                        base.DataCache.PropertiesCache["TaskListId"] = taskList.ID;
                    }
                    catch (Exception ex)
                    {
                        mLogger.Debug("Can not find workflow task list by Title. Message: {0}", ex.ToString());
                    }
                }
        ...
        public Wrapper.Common.AveWorkflowAssociationCollection.Configuration Configuration
        {
            get
            {
                ...
                if (!this.Enabled) 
                {
                    cfg = cfg | Wrapper.Common.AveWorkflowAssociationCollection.Configuration.NoNewWorkflows;
                }
                return cfg | (Wrapper.Common.AveWorkflowAssociationCollection.Configuration)131072;
            }
            set
            {
        ...
        public IAveContentTypeId ContentTypeId
        {
            get
            {
                if (base.DataCache.PropertiesCache.ContainsKey("ContentTypeId"))
                {
                    var contentTypeId = base.DataCache.GetProperty<IAveContentTypeId>("ContentTypeId");
                    if (contentTypeId != null) 
                    {
                        return contentTypeId;
                    }
                }
                if (base.DataCache.PropertiesCache.ContainsKey("ContentTypeIdString"))
                {
                    var stringId = base.DataCache.GetProperty<string>("ContentTypeIdString");
                    if (!string.IsNullOrEmpty(stringId))
                    {
                        var id = new AveContentTypeId(stringId);
        ...
        public Guid ParentAssociationId
        {
            ...
            set
            {
                mLogger.Info("cannot set parent association id on Client API mode. value:{0}", value);
            }
        }
        ...
        internal void Update() 
        {
            ...
            {
                mRequest.UpdateWorkflowAssociation(mWeb.ServerRelativeUrl, string.Empty, Guid.Empty, ContentTypeId.ToString(), Id, "web.workflows", base.DataCache.ChangedProperties);
            }
