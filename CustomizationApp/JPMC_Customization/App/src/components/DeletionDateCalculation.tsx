...
import { getCustomColumns, getRetentionPeriod as getRetentionPeriod } from '../config/AppConfigs';
import { ICustomColumns } from '../model/IAppConfigs';
import { allRetentionTypes, retentionPeriodUnits } from '../common/Constants';
import { localDateToSpDate } from '../common/DateUtil';

export interface IDeletionDateCalculationProps {
  context: FieldCustomizerContext;
  spItem: ListItemAccessor;
}

interface IDeletionDateCalculationState {
  text: string
}

export default class DeletionDateCalculation extends React.Component<IDeletionDateCalculationProps, IDeletionDateCalculationState> {
  private customColumns: ICustomColumns;
  constructor(props: IDeletionDateCalculationProps) {
    super(props);
    ...
    try {
      ...
      Logger.error(error, "initData fails.", LOG_SOURCE_EndDateCustomizer);
    }
  ...
  private getText(): string | undefined {
    ...
    if (!classCodeId) {
      Logger.warn("classCode is empty", LOG_SOURCE_EndDateCustomizer);
      return;
    }
    ...
    if (!recordCode || !countryCode || !retentionType) {
      Logger.warn("one of recordCode, countryCode, retentionType is empty", LOG_SOURCE_EndDateCustomizer);
      return;
    }
    ...
    if (retentionType === allRetentionTypes.event) {
      if (!startDateVal) {
        Logger.warn("It's event retention. but createDate is empty.", LOG_SOURCE_EndDateCustomizer);
        return;
      }
    ...
    const retentionPeriod = getRetentionPeriod(recordCode, classCodeId, countryCode, retentionType);
    if (!retentionPeriod) {
      Logger.warn("no retentionPeriod", LOG_SOURCE_EndDateCustomizer);
      return;
    }

    switch (retentionPeriod.unit) {
      case retentionPeriodUnits.Year:
        calcDate.setFullYear(calcDate.getFullYear() + retentionPeriod.value);
        break;
      case retentionPeriodUnits.Month:
        calcDate.setMonth(calcDate.getMonth() + retentionPeriod.value);
        break;
      case retentionPeriodUnits.Day:
        calcDate.setDate(calcDate.getDate() + retentionPeriod.value);
        break;
      default:
        return;
    }
