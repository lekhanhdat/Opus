...
{
    public class AveSPMySite : RestoreableObject, AvePoint.Wrapper.Restore.IAveSPMySite, IDisposable
    {
        ...
        {
            if (!AveSPUtility.IfServiceAvailable(webApp, AveServiceApplicationType.UserProfileService))
            {
            ...
            {
                //case ADO-137210  uncheck window authenication in webapp. SP API will get windows login user as secondary user for creating personal site.
                userProfileManager.SetAccountForCreatePersonalSite(userProfile["AccountName"].Value.ToString());
        ...
        public void Dispose()
        {
            mUserProfile.Dispose();
        }

        #region IAveSPMySite Members

        IAveSPSite IAveSPMySite.ParentSite
        {
            get { return mAveParentSite; }
        }
