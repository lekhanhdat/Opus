    ...
    class AveBrowserQuery : IAveBrowserQuery
    {
        ...
        public List<AveSiteBrowserInfo> GetBrowserSites(IAveWebApplication webApp, List<string> usernames, int startIndex, uint perPage, ref int childrenCount, ref bool hasError, bool needFilterInfo = false)
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveBrowserQuery.GetBrowserSites"))
            {
                ...
                //     {"c:0(.s|true",null},
                //     {"c:0!.s|windows",null},
                //     {@"NT AUTHORITY\LOCAL SERVICE" ,null},
                //     {@"NT AUTHORITY\authenticated users",null},
                //     {@"SHAREPOINT\system",null}
                //};
                ...
                try
                {
                    ...
                    var SiteMasterInfos = new List<Guid>();
                    ...
                    foreach (var contentDatabase in webApp.ContentDatabases)
                    {
                        try
                        {
                            ...
                            #region Get SiteMaster Info
                            FillSiteMasterInfos(sqlConn, SiteMasterInfos);
                            #endregion
                        }
                        catch (Exception e)
                        {
                    ...
                    sites = sites.Where
                                 (siteInfo => AveUrlUtility.CheckManagedPath(webApp, siteInfo.Url, siteInfo.IsHostHeader)
                                              && !SiteMasterInfos.Contains(siteInfo.ID)).ToList();
        ...
        private void FillSiteMasterInfos(AveSqlConnection sqlConn, List<Guid> siteMasterInfos)
        {
            try
            {
                string cmdText = @"SELECT SiteId FROM SiteMasters With(nolock)";
                using (SqlDataReader reader = sqlConn.ExecuteReader(cmdText))
                {
                    while (reader.Read())
                    {
                        siteMasterInfos.Add(reader.GetGuid(0));
                    }
                }
            }
            catch (Exception ex)
            {
                logger.Log(AveLogLevel.WARN, "Cannot get site master info.Error: {0}", ex);
            }
        }
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveBrowserQuery.GetBrowserWebs"))
            {
                ...
                try
                {
                    using (var connect = new AveSqlConnection(mConnectString))
                    {
                        ...
                        string cmdText = "SELECT FullUrl FROM AllWebs WHERE SiteId=@siteId AND Id=@ParentWebId";
                        string parentUrl = connect.ExecuteScalar(cmdText) as string;

                        cmdText = string.Format("SELECT count(Id) FROM AllWebs WHERE SiteId=@siteId AND ParentWebId=@ParentWebId {0} AND DeleteTransactionId=0x", strFilterAppWeb);
...
SELECT top {0} Id,FullUrl,Title,Language,WebTemplate,ProvisionConfig,FirstUniqueAncestorWebId,AppInstanceId,ROW_NUMBER() OVER (ORDER BY FullUrl) AS RowNumber
FROM AllWebs 
WHERE SiteId=@siteId AND ParentWebId=@ParentWebId {1} AND DeleteTransactionId=0x) As W
...
SELECT top {0} al.tp_ID, al.tp_Title, al.tp_BaseType, al.tp_ServerTemplate, ad.DirName, ad.LeafName, al.tp_Flags,al.tp_ScopeId,w.ScopeId,ROW_NUMBER() OVER (ORDER BY al.tp_Title) AS RowNumber
FROM (AllLists al with(nolock) 
INNER JOIN AllDocs ad with(nolock) ON al.tp_SiteId=@SiteId And ad.SiteId=al.tp_SiteId AND al.tp_WebId=@WebId AND al.tp_DeleteTransactionId=0x AND ad.Id=al.tp_RootFolder AND ad.Level=1 AND ad.DeleteTransactionId=0x) 
INNER JOIN AllWebs w with(nolock) ON al.tp_SiteId=w.SiteId AND al.tp_WebId=w.Id AND w.DeleteTransactionId=0x ) AS temp
                    ...
                    using (SqlDataReader dr = connect.ExecuteReader(cmdText))
                    {
                        try
                        {
                            while (dr.Read())
                            {
                                ...
                                string serverRelativeUrl = option.ParentFolderServerRelativeUrl.TrimEnd('/') + "/" + leafName; // System.Web.HttpUtility.UrlPathEncode(leafName.Replace("#","%23")); 
                                bool hidden = dr.IsDBNull(3) ? true : false;
                                Guid scopeId = dr.GetGuid(4);
                                string url = new Uri(new Uri(mSiteUrl), serverRelativeUrl).ToString();
                                bool hasUniqueRoleAssignments = !scopeId.Equals(parentFolderScopeId);
...
INNER JOIN AllLists al with(nolock) on ad.SiteId=al.tp_SiteId AND al.tp_WebId=ad.WebId AND ad.ListId=al.tp_ID 
                            ...
                            try
                            {
                                while (dr.Read())
                                {
                                    ...
                                    if (parentFolderListId == Guid.Empty)
                                    {
                                        ...
                                        Guid scopeId = dr.GetGuid(3);
                                        string webServerRelativeUrl = dr.GetString(4);
                                        string url = dirName.Substring(webServerRelativeUrl.Length + 1) + "/" + System.Web.HttpUtility.UrlPathEncode(leafName.Replace("#", "%23"));
                                        bool hasUniqueRoleAssignments = !scopeId.Equals(parentScopeId);
                                        itemInfo = new AveItemBrowserInfo
                                        {
                                            UniqueId = uniqueId,
        ...
        public AveWebBrowserInfo GetBrowserRootWeb(AveBrowserOption option)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveBrowserQuery.GetBrowserRootWeb"))
            {
                ...
                try
                {
                    ...
                    {
                        string platformVersion = GetSitePlatformVersion(option.ParentSiteId, connect);
                        string cmdText = string.Format("SELECT Id,FullUrl,Title,Language,WebTemplate,ProvisionConfig,FirstUniqueAncestorWebId FROM AllWebs WHERE SiteId=@SiteId AND ParentWebId IS NULL ORDER BY FullUrl");
                        connect.AddParameter("@SiteId", option.ParentSiteId);
                        using (SqlDataReader sr = connect.ExecuteReader(cmdText))
                        {
                            while (sr.Read())
