...
public class FullDiscoverV2 : BaseDiscover, IBatchDiscoverV2
{
    public IEnumerable<IExchangeItemGroup> GetGroupedItems(IExchangeFolder folder, SearchFilter extraFilter = null)
    {
        if (ArchiverCommonStaticMethod.IsNestleCustomizeSearchFilter && ArchiverCommonStaticMethod.NestleCustomizeSearchFilterDays > 0)
        {
            ...
            return FindGroupedItems(folder, currentSOFilterPolicySearchFilter).GetConsumingEnumerable();
        }
        return GetGroupedItemsAsync(folder, string.Empty).GetConsumingEnumerable();
