...
const Action = ({ isSelectAll, selectedItems, callback, filterOptions, searchText, limitNumberForAction }) =>{
    ...
    const onComplete = () =>{
        let messageboxContent = RMResx.RM_MT_PickList_CompleteLoanActionMsg;
        if(!isSelectAll){
    ...
    const onCompleteForRequest = () =>{
        $$.loading(true);
        let option = {
            url: "/api/PickListApi/LoanRequestCompelte",
            method: "Post",
            data: getActionParam()
        };
        fetchUtility(option).then((res) => { 
            ...
            if(isSelectAll || isContainerLevel){
                ...
                showToast.success(RMResx.RM_MT_PickList_CompleteLoanSuccessTip); 
            }
            callback();
        }).catch((e) => {
    ...
    const allowShowCompleteBtn = () =>{
        ...
        if(selectedItems.length > 0){
            allSelectItemsStatusIsPedding = selectedItems.every((item)=>{ 
                return item.Status == PickListForLoanStatusType.Pendding;
            });
        }
        ...
        return <React.Fragment>
            {
                ...
                <R.Button 
                    ...
                    text={RMResx.RM_MT_PickList_CompleteloanBtn} 
                    onClick={onComplete}
                />
            }
        </React.Fragment>;
