    ...
    public class RMSPDashboardCalculator
    {
            ...
            try
            {
                ...
                if (!this.hasErrorNode)
                {
                    ...
                    if(total != null)
                    {
                        ...
                        await BoardTotalDao.UpdateAsync(total);
        ...
        private void InnerProcessSite(RMSiteCollectionSize currentSize)
        {
            using (var performance = new PerformanceScope("RMSPDashboardCalculator.Work.Existing"))
            {
                try
                {
                    ...
                    string sql = "SELECT VALUE COUNT(1) FROM c WHERE c.scopeId = @scopeId AND c.nodeType = @nodeType AND c.recordStatus = @recordStatus";

                    var parameters = new Dictionary<string, object>
                    {
                        { "scopeId", currentSize.ScopeId },
                        { "nodeType", 500 },
                        { "recordStatus", 1 }
                    };

                    int itemCount = ExplorerDao.QueryCount(sql, parameters);
                    if (itemCount > 0)
                    {
        ...
        /*private void SaveOrUpdateSCCount(RMNodeFlag flag, int count)
        {
            ...
            else
            {
                ...
                newSize.Size = count;
                newSize.SourceFlag = (int)SourceFlag.SharePoint;
