...
{
    public class SharePointSOSettingCsv(string filePath, BaseJobDto baseJobDto) : SettingCsv<ExportSPSOSettingData>(filePath, baseJobDto)
    {
        ...
        {
            headers.AddRange([ExportSettingHeaderColumnContract.ContainerCol, ExportSettingHeaderColumnContract.SiteCollectionCol, ExportSettingHeaderColumnContract.SiteCol,
                ExportSettingHeaderColumnContract.ListOrLibraryCol, ExportSettingHeaderColumnContract.FolderCol]);
        }

        protected override List<string> ConvertSettingToList(ExportSPSOSettingData setting)
        {
        ...
        private void ConvertOther(List<string> exportSetting, ExportSPSOSettingData setting)
        {
            string isInheritSetting = setting.IsInheritSetting ? TRUE : FALSE;
            exportSetting.AddRange([ProcessCol(isInheritSetting)]);
        }

        private void ConvertArchivingSetting(List<string> exportSetting, ExportSPSOSettingData setting)
        {
        ...
        private void ConvertGeneralSetting(List<string> exportSetting, ExportSPSOSettingData setting)
        {
        ...
        private void ConvertPath(List<string> exportSetting, ExportSPSOSettingData setting)
        {
            exportSetting.AddRange([ProcessCol(setting.ContainerName), ProcessCol(setting.SiteCollectionUrl), ProcessCol(setting.SiteUrl), ProcessCol(setting.ListUrl), ProcessCol(setting.FolderUrl)]);
        }
