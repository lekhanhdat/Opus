...
{
    internal class RMDiscoveryFSInactiveSizeRangeQuerier : RMDiscoveryFSInactiveDataQuerier<List<RMDiscoverySizeRangeDataInfo>>
    {
        public RMDiscoveryFSInactiveSizeRangeQuerier(RMDiscoveryFSQueryParameter queryParameter) : base(queryParameter)
        {
        }
        ...
        public override async Task<List<RMDiscoverySizeRangeDataInfo>> QueryAsync()
        {
            ...
            var sql = $@"SELECT sizeRange.Id AS Id, sizeRange.DisplayName AS Name, sizeRange.[Order] AS [Order], Sum(data.FileTotalSize) AS FileTotalSize 
                FROM [{_schemaName}].[{dataTable}] AS data
                JOIN [dbo].[RMFSSizeRange] AS sizeRange ON data.SizeRange = sizeRange.Id ";
