...
{
    public abstract class BaseContainsFilterBuilder : IFilterBuilder
    {
        public Query Filter(Query query, ExplorerFilterOptionV2 filterOption)
        {
            if (CanFilter(filterOption))
            {
                return query.WhereContains(GetFilterColumnName().FormatColumnName(), GetFilterValue(filterOption));
            }
            return query;
        }
