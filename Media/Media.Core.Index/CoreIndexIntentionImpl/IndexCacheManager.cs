...
namespace AvePoint.Media.Core.Index
{
    ...
    public class IndexCacheManager : IIndexCacheManager
    {
        ...
        public IControlStubService ControlStubService { get; set; }
        ...
        public StorageResult UploadIndex(IndexCacheManagerParameter param, Boolean isFailedIndex = false)
        {
            ...
            logger.Info(MediaCoreIndexResource.IndexCacheManagerUploadIndexBegin, param.IndexVolume, param.IndexName, indexFileFullPath);
                ...
                try
                {
                    if (!isFailedIndex)
                    {
                        if (ShouldUploadIndex(param.IndexVolume, param.IndexName, param.CacheSystem))
                        {
                            ...
                            XFileInfo fileInfo = param.CacheSystem.OpenFile(XConvert.FromNames(param.IndexVolume, param.IndexName));
                            logger.Info(MediaCoreIndexResource.IndexCacheManagerUploadIndexFileInfo, fileInfo.LastWriteTimeUtc);
        ...
        private async Task<(bool,long)> QueryLastModifyTimeFromControlAsync(string indexVolume, string indexFileName)
        {
            ...
            try
            {
                ...
                List<MediaDataDto> mediaDataDtoList = await MediaDao.GetMediaDatasAsync(key);
                if (mediaDataDtoList == null || mediaDataDtoList.Count == 0)
                {
                    ...
                    logger.Info($"Control does not contain the key {key}");
                }
                else
                {
            ...
            {
                logger.Error(MediaCoreIndexResource.IndexCacheManagerQueryLastModifyTimeFromControlError, ex.ToString());
            }
            stopwatch.Stop();
        ...
        private void SaveIndexFileNameAndLastModifyTimeToControl(string filePath, long time)
        {
            try
            {
                ...
                logger.Info($"UpdateOrInsertMediaData key:{key},time:{time.ToString()}");
                MediaDao.UpdateOrInsertMediaDataAsync(key, time.ToString()).Wait();
            }
            catch (Exception ex)
            {
                logger.Error(MediaCoreIndexResource.IndexCacheManagerSaveIndexFileNameAndLastModifyTimeToControlError, ex.ToString());
            }
        }
        ...
        private StorageResult UploadToRealXSystem(IndexCacheManagerParameter param, Boolean isFailedIndex)
        {
            try
            {
                logger.Info(MediaCoreIndexResource.IndexCacheManagerUploadToRealXSystemBegin, param.IndexName, param.IndexVolume);
                ...
                logger.Info(MediaCoreIndexResource.IndexCacheManagerUploadToRealXSystemOpen, info.ToString());
                ...
                using (XStream cacheStream = param.CacheSystem.OpenStream(encryptedFile, FileMode.Open))
                {
                    ...
                    if (workSystem.StorageType == XStorageType.Azure)
                    {
                        ...
                        storageResult = workSystem.CommitStream(cacheStream, azureInfo);
                    }else if(workSystem.StorageType == XStorageType.GoogleCloud)
                    {
                    ...
                    {
                        logger.Info($"this index device is not azure device,set index to hot tier ,device type: {workSystem?.StorageType}");
                        storageResult = workSystem.CommitStream(cacheStream, info);
                    }
                }
                logger.Info(MediaCoreIndexResource.IndexCacheManagerUploadToRealXSystemEnd, param.IndexName, param.IndexVolume);
                return storageResult;
            }
            catch (Exception ex)
            {
                logger.Error(MediaCoreIndexResource.IndexCacheManagerUploadToRealXSystemError, ex.ToString());
                throw;
            }
        }
        ...
        private void DownloadFromRealXSystem(IndexCacheManagerParameter param)
        {
            ...
            if (storageSystem != null && storageSystem.StorageType == XStorageType.Azure)
            {
                ...
                var containerClient = RAStorageUtil.GetBlobContainerClientByStorageXRI(storageSystem?.ConnectionString);
        ...
        private Boolean ShouldUploadIndex(String indexVolume, String indexFileName, IXSystem cacheSystem)
        {
            ...
            logger.Info($"ShouldUploadIndex: {shouldUpload}");
            return shouldUpload;
        }
