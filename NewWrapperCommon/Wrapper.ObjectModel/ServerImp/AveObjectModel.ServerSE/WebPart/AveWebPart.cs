            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("Server.AveWebPart.RealRestore"))
            {
                ...
                try
                {
                    ...
                    if (isIListWebPart)
                    {
                        ...
                        UpdateView();
                        if (this.internalWebPart is XsltListViewWebPart && (this.internalWebPart as XsltListViewWebPart).InplaceSearchEnabled)
                        {
                            this.manager.NeedResaveListViewWebpart.Add(new Tuple<Guid, bool>(this.webPartId, isShared));
                        }
                    }
    ...
    public class SpecialWebPartUpdater
    {
        ...
        public static SpecialWebPartUpdater GetWebPartUpdater(System.Web.UI.WebControls.WebParts.WebPart webPart, AveWebPart aveDoc)
        {
            try
            {
                ...
                if (updater == null)
                {
                    Type subType = Type.GetType(string.Format("AvePoint.ObjectModel.ServerSE.Ave{0}Updater", webPart.GetType().Name), false, true);
                    if (subType != null)
                    {
    ...
    class AveExcelWebRendererUpdater : SpecialWebPartUpdater
    {
        ...
        public override bool DoUpateAfterAdd(AveWebPartBaseInfo webPartInfo)
        {
            try
            {
                ...
                {
                    AveAssemblyUtility.SetPropertyValue(this.mWebPart, "WorkbookUri", AveReplaceProcessor.UrlReplace(url, mAveWebPart.Manager.Cache.SiteManagedMappings, new ReplaceOption(true, true), mAveWebPart.Manager.Cache.SourceSiteInfo, mAveWebPart.Manager.Cache.DestSiteInfo.ServerRelativeUrl,true));
                }
            }
            catch (Exception e)
            {
