...
{
    public sealed class DoNotDestoryActivity : NativeActivity
    {
        ...
        public InArgument<DisposalReviewRequestInfo> RunningRequestInfo { get; set; }

        protected override void Execute(NativeActivityContext context)
        {
            var requestInfo = RequestInfo.Get(context);
            var runningRequest = RunningRequestInfo.Get(context);

            WFThreadHelper.SetForCurrentThread(runningRequest); //set the setting for current thread

            ManualApprovalService.SetWorkflowIsComplete(runningRequest.InstanceId, SOApproveDBStatus.Rejected);
            logger.Info($"DoNotDestory record. request id : {requestInfo.RequestId}, instance id : {context.WorkflowInstanceId}");
            //to be add the business logic...
        }
    }
}
