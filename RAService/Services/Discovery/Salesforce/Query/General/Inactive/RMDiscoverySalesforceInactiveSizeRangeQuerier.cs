...
public class RMDiscoverySalesforceInactiveSizeRangeQuerier(RMDiscoverySalesforceQueryParameter salesforceQueryParameter)
    : RMDiscoverySalesforceInactiveDataQuerier<List<RMDiscoverySizeRangeDataInfo>>(salesforceQueryParameter)
{
    public override async Task<List<RMDiscoverySizeRangeDataInfo>> QueryAsync()
    {
        ...
        var sql =
            $@"SELECT sizeRange.Id AS Id, sizeRange.DisplayName AS Name, sizeRange.[Order] AS [Order], Sum(data.TotalFileSize) AS FileTotalSize 
FROM [{_schemaName}].[{dataTable}] AS data
JOIN [dbo].[RMSalesforceSizeRange] AS sizeRange ON data.SizeRange = sizeRange.Id ";
        ...
        var dataList = await _dataQueryDao.GetDataListAsync<RMDiscoverySizeRangeDataInfo>(sql, sqlParams);
