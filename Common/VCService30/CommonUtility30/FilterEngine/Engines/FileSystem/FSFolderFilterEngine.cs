...
namespace AvePoint.Common.FilterEngine
{
    ...
    {
        public FSFolderFilterEngine(List<FilterPolicy> policyLists, Dictionary<PolicyLevel, string> filterConditionExpressionLists, FilterEngine engine)
        ...
        protected override bool IsQualified(ObjectInfoBase objectInfo, GCommon.Contract.CommonFilter.FilterPolicy policy)
        {
            ...
            if (policy.Rule is NameRule)
            {
                ...
                RecordFilterLog("fsfolder",isQualified, folderInfo.Name, policy);
                return isQualified;
            }
            ...
            else if (policy.Rule is FSTermRule || policy.Rule is TermRule)
            {
                ...
                RecordFilterLog("fsfolder", isQualified, columnValue, policy);
                return isQualified;
            }
            else if (policy.Rule is OwnerRule)
            {
                ...
                RecordFilterLog("fsfolder", isQualified, folderInfo.Owner, policy);
                return isQualified;
            }
            else
            {
