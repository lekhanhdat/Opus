...
{
    class ChoiceDataFormatForHSMConnector : BaseDataFormatForHSMConnector
    {

        private const string splitChar = ";#";

        public ChoiceDataFormatForHSMConnector(AveXmlField xmlField, IAveField destField, IAveListItem mItem) :
        ...
        public override object CheckFieldValue(object value)
        {
            if (xmlField.Type==AveFieldType.MultiChoice)
            {
                ...
                {
                    values = value.ToString().Split(new string[] { splitChar }, StringSplitOptions.RemoveEmptyEntries);
                }
                bool hasValue = false;
