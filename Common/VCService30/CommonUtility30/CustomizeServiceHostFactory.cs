...
namespace AvePoint.Common
{
    public class CustomizeServiceHostFactory
    {
        ...
        public static ServiceHost CreateServiceHost(Type serviceType, string schema, string address, int port, string relatedBaseUri, string jobId)
        {
            ...
            if (sh.Description.Behaviors.Find<ServiceCredentials>() == null)
                sh.Description.Behaviors.Add(BuildCredentialsBehavior(AvePoint.GCommon.Utility.Cloud.GCommonRoleConfiguration.WCF_Certificate));
        ...
        public static IServiceBehavior BuildCredentialsBehavior(Object thumbprint = default(Object))
        {
            ...
            result.ClientCertificate.Authentication.CertificateValidationMode = System.ServiceModel.Security.X509CertificateValidationMode.Custom;
            result.ClientCertificate.Authentication.CustomCertificateValidator = new CustomX509CertificateValidator();
            return result;
        }
    }
}
