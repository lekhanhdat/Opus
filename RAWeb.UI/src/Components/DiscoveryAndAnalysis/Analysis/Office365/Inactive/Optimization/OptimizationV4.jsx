...
import { CalculateUtil, JobUtil, MappingContainerName, NumberUtil } from "../../../Utils";
...
const buildInColumns = new Map([
    [
        ...
        [
            ...
            {
                displayName: RMResx.RM_FA_Inactive_SummaryTab_PHL + " (" + RMResx.RM_JS_RDM_CreateRule_Unit_GB + ")",
                internalName: "phlTotalSize",
                isAggregateField: true,
                width: 200,
            },
            {
...
const InactiveOptimizationV4 = ({ o365TenantId, jobInfo }) => {
    ...
    useEffect(() => {
        const fetchAggregateInfo = async () => {
            var res = await ProfileInactiveDataRequester.queryAggregateInfo(queryParameter);
    ...
    const queryNodeDataInfo = async (queryParameter) => {
        const res =
            await ProfileInactiveDataRequester.queryOptimizationNodesDataV3(
                queryParameter
            );
        let items = MappingContainerName(res.items);
        res.items = await CalculateUtil.CalculateInactivesNodesDataV3(items);
        return res;
    };

    const queryNodeTotalAggregateInfo = async (queryParameter) => {
        const res =
            await ProfileInactiveDataRequester.queryOptimizationNodeTotalAggregateInfoV3(
                queryParameter
            );
        return await CalculateUtil.CalculateInactivesNodeTotalAggregateInfoV3(
            res
        );
    };
    ...
    const handleExportProfileBtn = () => {
        Messagebox({ content: RMResx.RM_JS_Common_ExportMsg, actionFun: handleProfileExport });
    }

    const handleProfileExport = async (e) => {
        const isDesc = queryParameter.nodeQueryParameter.isDesc;
        const requestOption = {
            url: "/api/RMDiscoveryOffice365ProfileApi/RunExportDataAnalysisProfileJob",
            method: "POST",
            data: {
                profileType: DiscoveryProfileType.Inactive,
                profileId: selectedProfileInfo.id,
        ...
        <div className="reco-inactive-optimization-container">
            <div className="flex gap-s">
                ...
                {!JobUtil.isRunning(jobInfo) && (
                    <R.Button
                        id="raExportInactiveProfileButton"
                        classify="default"
                        icon="fia-export-settings"
                        text={RMResx.RM_FA_DataOptimize_ExportProfileBtn}
                        onClick={handleExportProfileBtn}
                    />
                )}
            </div>
...
export default InactiveOptimizationV4;
