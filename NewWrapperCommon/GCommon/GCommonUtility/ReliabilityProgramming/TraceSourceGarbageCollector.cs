...
namespace AvePoint.GCommon.Utility
{
    ...
    /// </remarks>
    /// </summary>
    [SuppressMessage("FxCopCustomRules", "C100007:SpellCheckStringValues", Justification = "tracesources is ok.")]
    public class TraceSourceGarbageCollector
    {
        static Int32 lastTraceSourcesCollectionCount;
        static Int32 lastSwitchesCollectionCount;
        ...
        static TraceSourceGarbageCollector()
        {
            ...
            var switchInternalField = typeof(Switch).GetField("switches", BindingFlags.NonPublic | BindingFlags.Static);
            if (switchInternalField != null)
                switches = switchInternalField.GetValue(null) as List<WeakReference>;
        }
        ...
        public static void Collect()
        {
            if (tracesources != null)
            {
                lock (tracesources)
                {
                    if (lastTraceSourcesCollectionCount != GC.CollectionCount(2))
                    {
                        ...
                        lastTraceSourcesCollectionCount = GC.CollectionCount(2);
                    }
                }
            }
            if (switches != null)
            {
                lock (switches)
                {
                    if (lastSwitchesCollectionCount != GC.CollectionCount(2))
                    {
                        var collection = new List<WeakReference>(switches.Count);
                        ...
                        lastSwitchesCollectionCount = GC.CollectionCount(2);
                    }
                }
            }
        }
