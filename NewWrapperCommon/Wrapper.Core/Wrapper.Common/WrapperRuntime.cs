...
namespace AvePoint.Wrapper.Common
{
    public sealed class WrapperRuntime
    {
        ...
        private static object InstanceCreateLock = new object();
        ...
        public static void SetGlobalRuntimeSetting(bool optimized, AveWrapperRunningAccountInfo trimmingAccount, ContextInstanceMode instanceMode)
        {
            WrapperRuntime.SetGlobalRuntimeSetting(typeof(AveLogger), optimized, trimmingAccount, instanceMode);
        }
        ...
        public static void SetGlobalRuntimeSetting(Type loggerType, bool optimized, AveWrapperRunningAccountInfo trimmingAccount, ContextInstanceMode instanceMode)
        {
            ...
            if (WrapperRuntime.GlobalContext == null)
            {
                lock (InstanceCreateLock)
                {
                    ...
                    {
                        throw new WrapperStateException("Global context can be configured only once.");
                    }
                }
            }
            else
        ...
        public static void SetGlobalContextModelFactorySetting(AveObjectModelFactory modelFactory)
        {
            if (GlobalContext == null)
            {
                GlobalContext = DefaultContext;
            }
            GlobalContext.ModelFactory = modelFactory;
        }
            ...
            get
            {
                ...
                switch (ContextInstanceMode)
                {
                    ...
                    case ContextInstanceMode.Thread:
                        ...
                        var threaContext = Thread.GetData(key) as WrapperContext;
                        if (threaContext == null)
                        {
                            threaContext = GlobalContext.Clone() as WrapperContext;
