...
namespace AvePoint.GCommon.Contract.Server.ControlPanel.PlanGroup
{
    public interface IMPlanGroupServiceForOtherModule
    {
        ...
        /// <summary>
        /// 更新plan时使用（该方法需要在各个模块调用update plan方法之后被调用）
        /// </summary>
        /// <param name="dto">PlanGroupDtoForOtherModule中的属性PlanId，Category需要赋上值</param>
        /// <returns>返回更新的状态</returns>
        PlanGroupResult UpdateScheduleStateForPlanGroup(PlanGroupDtoForOtherModule dto);

        PlanGroupDto GetPlanGroupById(string planGroupId);
    }
}
