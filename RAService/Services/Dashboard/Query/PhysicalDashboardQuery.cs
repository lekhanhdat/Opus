...
{
    public class PhysicalDashboardQuery : DashboardQueryable
    {
        ...
        {
            var dataSourcePermission = permission.ScopePermissionInfo.Find(item => item.DataSourceType == SourceFlag.Physical);
            var isEndUser = dataSourcePermission?.SubPermission == SubPermissionType.EndUser;
            if (isEndUser)
            {
                return 0;
            }
            ...
            {
                return DashboardDao.GetPhysicalSettingCount();
            }
            var permissionTopLocationIds = dataSourcePermission?.ScopeIds ?? new List<Guid>();
            var permissionLocationIds = RMLocationDao.LoadAllLocationIdUnderTopLocation(permissionTopLocationIds);
            return DashboardDao.GetPhysicalSettingCount(permissionTopLocationIds);
        }
        ...
        public override long GetSourceActiveCount(SecurityUserPermissionsDto permission)
        {
            ...
            return DashboardDao.GetSourceStatusCountWithScopeId(Flag, bottomLocationIdsUnderTopLocations, item => item.Active);
        }
        ...
        public override long GetSourceDestroyedCount(SecurityUserPermissionsDto permission)
        {
            ...
            var permissionTopLocationIds = dataSourcePermission?.ScopeIds ?? new List<Guid>();
            var bottomLocationIdsUnderTopLocations = RMLocationDao.LoadAllLocationBottomIdUnderTopLocation(permissionTopLocationIds).Select(id => id.ToString());
            return DashboardDao.GetSourceStatusCountWithScopeId(Flag, bottomLocationIdsUnderTopLocations, item => item.Destroyed);
        }
    }
}
