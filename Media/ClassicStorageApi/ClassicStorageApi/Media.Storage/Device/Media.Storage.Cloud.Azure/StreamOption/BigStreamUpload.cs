...
{
    public class BigDBHttpUploadStream : HttpUploadStream
    {
    ...
    "dapeng.zhang@avepoint.com",
     ...
     true)]
    #endregion
    public class BlockBlobUploadStream : HttpUploadStream
    {
        ...
        private void CommitMemoryStream(MemoryStream mStream)
        {
            if (mStream.Position > 0)
            {
                ...
                asOption.Azureclient.PutBlock(asOption.FullURL, Convert.ToBase64String(blockIdBytes), blockContent, 0, readLen);
                blockIdBase64s.Add(Convert.ToBase64String(blockIdBytes));
                blockId++;
                mStream.SetLength(0);
                mStream.Position = 0;
            }
        ...
        public override StorageResult Commit(bool closeParent)
        {
            ...
            if (!this.IsCommited)
            {
                ...
                //Ignore blob metadata for Records only, use file level to store content.  RECO-14317
