...
import { DeployTermMethod, SavedTermSettingType } from "../DocumentTermSettingPanel";
...
export default {
    getContext() {
        return {
            configurations: {
                ...
                existClassificationSetting: true,
            },
            itemId: "fs",
            saveTermSettings: this.saveTermSettings,
            getSavedTerm: this.getSavedTerm,
            termSettingsTitle: isEnableJPMCFeature ? RMResx.RM_JS_FS_EditTitle_ClassCodeSettings : RMResx.RM_JS_EXO_EditTitle_TermSettings,
    ...
    saveTermSettings(that, callback) {
        ...
        currNode.DeployTermMethod = isEnableJPMCFeature ? DeployTermMethod.NoDefaultValue : stateData.deployTermMethod;
        ...
        if(isEnableJPMCFeature && (currNode.TermSetId !== that.termSetId)) {
            currNode.ClassCode = null;
        }
        ...
        let sendUrl = CRMCommonUtil.isGroup(currNode) ? "/api/FSSettingApi/SaveFSGroupDocLevelSetting" : "/api/FSSettingApi/SaveFSDocLevelSetting";
        let option = {
            url: sendUrl,
            method: "Post",
            data: currNode
    ...
    getSavedTerm(that) {
        ...
        paramObj.ConnGroupId = groupNode.ConnGroupId;
        paramObj.perPageCount = that.treePageSize;
        let option = {
            url: "/api/FSSettingApi/GetFSSavedTerm",
            method: "Post",
            data: paramObj
        };
