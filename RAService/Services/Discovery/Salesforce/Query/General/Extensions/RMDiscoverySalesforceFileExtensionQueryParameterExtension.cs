...
public static class RMDiscoverySalesforceFileExtensionQueryParameterExtension
{
    public static bool TryGetSqlDefinition(this RMSFFileExtensionQueryParameter parameter, string dataTableAlias, out RMDiscoverySqlDefinition sqlDefinition)
    {
        sqlDefinition = null;
        if (parameter.FileExtensions.IsNullOrEmpty())
        {
            return false;
        }
        ...
        if (parameter.FileExtensions!.Count <= 3)
        {
            ...
            parameter.FileExtensions = parameter.FileExtensions.Select(fileExtension => fileExtension == "Empty" ? RMConstants.UNKNOW : fileExtension).ToList();
