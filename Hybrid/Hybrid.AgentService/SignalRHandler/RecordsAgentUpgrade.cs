...
{
    internal class RecordsAgentUpgrade : EndpointHandlerBase<RecordsAgentUpgradeExecute, RecordsAgentUpgradeArgs, RecordsAgentUpgradeResult>
    {
        ...
        public override RecordsAgentUpgradeResult Process(RecordsAgentUpgradeArgs param)
        {
            ...
            try
            {
                logger.Info($"Start RecordsAgentUpgrade process for AgentId: {param.AgentId}");
                var upgrader = new RecordsAgentUpgrader(param.AgentId, param.TargetVersion);
                Task.Run(async () =>
                {
                    try
                    {
            ...
            {
                logger.Error($"RecordsAgentUpgrade process failed for AgentId: {param.AgentId}, Error: {ex}");
                res.Result = RMAgentUpgradeResult.Failed;
                res.AgentId = param.AgentId;
