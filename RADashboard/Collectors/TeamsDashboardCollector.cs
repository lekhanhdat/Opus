...
{
    public class TeamsDashboardCollector : DashboardCollector
    {
        public override SourceFlag Flag => SourceFlag.Teams;

        private const long UNIXEPOCH_TICKS = 621355968000000000;

        private static readonly IRMScopeDao RMScopeDao = PlatformWindsorManager.GetService<IRMScopeDao>();

        private static readonly string NodeTypeConditionStr = $" (c.nodeType = {(int)NodeLevel.Item} or c.nodeType = {(int)NodeLevel.Document} or c.nodeType = {(int)NodeLevel.Folder}) ";
        ...
        protected override async Task<List<RMDashboardDataUsage>> CollectDataUsageAsync()
        {
            ...
            var createdSql = $"SELECT c.aveSiteId, COUNT(1) AS siteUsageCount FROM items c where c.sourceFlag = {(int)Flag} and  c.recordStatus = {(int)RMRecordStatus.Active} and {NodeTypeConditionStr} GROUP BY c.aveSiteId";
            var createdDataUsageDic = ExplorerDao.QuerySiteCollectionUsageCount(createdSql);

            var destoryedSql = $"SELECT c.aveSiteId, COUNT(1) AS siteUsageCount FROM items c where c.sourceFlag = {(int)Flag} and c.recordStatus = {(int)RMRecordStatus.Destroyed} and {NodeTypeConditionStr} GROUP BY c.aveSiteId";
            var destoryedDataUsageDic = ExplorerDao.QuerySiteCollectionUsageCount(destoryedSql);

            var archivedSql = $"SELECT c.aveSiteId, COUNT(1) AS siteUsageCount FROM items c where c.sourceFlag = {(int)Flag} and c.recordStatus = {(int)RMRecordStatus.Archived} and {NodeTypeConditionStr} GROUP BY c.aveSiteId";
        ...
        protected override Dictionary<string, int> CollectTermUsage()
        {
            ...
            AND c.termId != {Guid.Empty} AND {NodeTypeConditionStr} GROUP BY c.termId";
        ...
        {
            var activeSql = $@"
            SELECT LEFT(TicksToDateTime(c.timeCreated - {UNIXEPOCH_TICKS}), 10) AS date , COUNT(1) AS count FROM c WHERE
            c.sourceFlag = {(int)Flag}
            AND c.timeCreated >= {startTicks}
            AND ARRAY_CONTAINS([{(int)NodeLevel.Document}, {(int)NodeLevel.Item}, {(int)NodeLevel.Folder}], c.nodeType)
            GROUP BY LEFT(TicksToDateTime(c.timeCreated - {UNIXEPOCH_TICKS}), 10)
            ";

            var destroyedSql = $@"
            SELECT LEFT(TicksToDateTime(c.destroyedTime - {UNIXEPOCH_TICKS}), 10) AS date , COUNT(1) AS count FROM c WHERE
            c.sourceFlag = {(int)Flag}
            AND c.recordStatus = {(int)RMRecordStatus.Destroyed}
            AND c.destroyedTime >= {startTicks}
            AND ARRAY_CONTAINS([{(int)NodeLevel.Document}, {(int)NodeLevel.Item}, {(int)NodeLevel.Folder}], c.nodeType)
            GROUP BY LEFT(TicksToDateTime(c.destroyedTime - {UNIXEPOCH_TICKS}), 10)
            ";

            var waitingSql = $@"
            SELECT LEFT(TicksToDateTime(c.manual_collectionTime - {UNIXEPOCH_TICKS}), 10) AS date , COUNT(1) AS count FROM c WHERE
            c.manual_isManualSynced
            AND c.sourceFlag = {(int)Flag}
            AND c.manual_collectionTime >= 637865381843477781
            GROUP BY LEFT(TicksToDateTime(c.manual_collectionTime - {UNIXEPOCH_TICKS}), 10)
            ";
