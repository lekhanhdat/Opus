...
{
    public class AveO365MasterPageDocumentRestore : AveO365BaseDocumentRestore
    {
        public AveO365MasterPageDocumentRestore(AveClientContext context, AveClientOMOffice365Request request, FederationToken tokenProvider, AveDocumentInfo docInfo, Stream fileStream)
        ...
        protected override bool CheckFileLockStatus(ClientFile file)
        {
            ...
            try
            {
                ...
                var path = ResourcePath.FromDecodedUrl(htmlFileUrl);
                ClientFile htmlFile = ParentWeb.GetFileByServerRelativePath(path);
                Context.Load(htmlFile, f => f.Exists);

                string htmFileUrl = ReplaceExtention(FileServerRelativeUrl, ".htm");
                var htmpath = ResourcePath.FromDecodedUrl(htmFileUrl);
                ClientFile htmFile = ParentWeb.GetFileByServerRelativePath(htmpath);
