...
{
    [VIM(VIMName.CAStor, typeof(AvePoint.Media.Storage.CAStor.CAStorVIM))]
    class CAStorVIM : AbstractVIM
    {
        #region VIM Members

        public override IXSystem CreateSystem(string xri, AbstractXSystem parentSystem)
        {
            return new CAStorSystem(xri, parentSystem);
        }

        public override List<string> GetFeatureXML(int type)
        {
            return CAStorFeature.Getstances(type).FeatureXMLs;
        }

        public override List<StorageFeature> GetFeatureObj(int type)
        {
            return CAStorFeature.Getstances(type).FeatureObjs;
        }

        public override List<StorageFeature> GetFeatureObj(int type,string culture)
        {
            return CAStorFeature.Getstances(type,culture).FeatureObjs;
        }
        #endregion
    }
}
