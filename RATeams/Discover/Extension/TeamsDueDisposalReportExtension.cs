...
namespace RATeams.Discover.Extension
{
    public static class TeamsDueDisposalReportExtension
    {
        private static RALogger _logger = RALogger.GetInstance(typeof(TeamsDueDisposalReportExtension));
        public static void BuildRelatedRecords(this DueDisposalReport report, IAveListItem item, string siteUrl, RelatedRecordOption relatedRecords)
        {
            ...
            {
                _logger.Warn("get related record info error{0}", e.ToString());
            }
