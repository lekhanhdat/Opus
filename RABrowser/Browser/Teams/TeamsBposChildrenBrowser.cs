...
namespace AvePoint.RA.Browser.Browser.Teams
{
    public class TeamsBposChildrenBrowser : SPOBaseBrowser
    {
        ...
        private BrowserType mBrowserType = BrowserType.Teams;
        public TeamsBposChildrenBrowser(BrowserType browserType)
        {
        ...
        protected override async Task<BrowseResult> BrwoseAsync(SPTreeNodeDto node)
        {
            ...
            try
            {
                ...
                {
                    if (nodeTemp.Level == NodeLevel.SiteCollections || nodeTemp.Level == NodeLevel.Office365GroupEntire)
                    {
                ...
                if (node != null && (node.Level == NodeLevel.SiteCollections || node.Level == NodeLevel.Sites || node.Level == NodeLevel.Lists || node.Level == NodeLevel.Folders))
                {
