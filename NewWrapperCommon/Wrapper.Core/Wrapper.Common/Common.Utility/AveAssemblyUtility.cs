...
namespace AvePoint.Wrapper.Common
{
    public static class AveAssemblyUtility
    {
        ...
        {
            Func<object, object> property = GetMethodInCache(objType, propertyName);
            if (property != null)
            {
        ...
        static Dictionary<string, Func<object, object>> PropetyGetterCache = new Dictionary<string, Func<object, object>>();        

        private static Func<object, object> GetMethodInCache(Type objType, string propertyName)
        {
            string key = string.Format("{0}, {1}", propertyName, objType.FullName);
            lock (lockObject)
            {
                if (!PropetyGetterCache.ContainsKey(key))
                {
                    PropertyInfo propInfo;
                    Func<object, object> converted = null;
                    try
                    {
                        propInfo = GetPropertyInternal(objType, propertyName);
                        converted = FastReflection.Getter(propInfo);
                    }
                    catch (Exception e)
                    {
                        logger.Warn(e.ToString());
                    }
                    PropetyGetterCache.Add(key, converted);
                }
            }
            return PropetyGetterCache[key];
        }
        ...
        static class FastReflection
        {
            ...
            {
                return Convert<Func<object, object>>(property, "GetterHelper");
            }

            public static Action<object, object> Setter(PropertyInfo property)
            {
                return Convert<Action<object, object>>(property, "SetterHelper");
            }

            static T Convert<T>(PropertyInfo property, string helperName)
            {
                var genericHelper = typeof(FastReflection).GetMethod(helperName, BindingFlags.Static | BindingFlags.NonPublic);
                var constructedHelper = genericHelper.MakeGenericMethod(property.DeclaringType, property.PropertyType);
                object ret = constructedHelper.Invoke(null, new object[] { property });
                return (T)ret;
            }

            static Func<object, object> GetterHelper<TTarget, TProperty>(PropertyInfo property)
            {
                var getter = property.GetGetMethod(true);
                if (getter.IsStatic)
                {
                    var func = (Func<TProperty>)Delegate.CreateDelegate(typeof(Func<TProperty>), getter);
                    return t => func();
                }
                else
                {
                    var func = (Func<TTarget, TProperty>)Delegate.CreateDelegate(typeof(Func<TTarget, TProperty>), getter);
                    return t => func((TTarget)t);
                }
            }

            static Action<object, object> SetterHelper<TTarget, TProperty>(PropertyInfo property)
            {
                var setter = property.GetSetMethod(true);
                if (setter.IsStatic)
                {
                    var action = (Action<TProperty>)Delegate.CreateDelegate(typeof(Action<TProperty>), setter);
                    return (t, p) => action((TProperty)p);
                }
                else
                {
                    var action = (Action<TTarget, TProperty>)Delegate.CreateDelegate(typeof(Action<TTarget, TProperty>), setter);
                    return (t, p) => action((TTarget)t, (TProperty)p);
                }
            }
        }
        ...
        public static object InvokeStaticMethod(string assemblyName, string typeFullName, string methodName, Type[] paramTypes, params object[] args)
        {
        ...
        public static Type GetGenerticType(Type aveType, string typeMapping)
        {
            lock (typeSearchAssemblies)
            {
                ...
                foreach (Assembly assembly in typeSearchAssemblies)
                {
                    ...
                    {
                        logger.Error("An error occurred while getting type in assembly:{0}. Error:{1}.", assembly.FullName, e);
                        if (e.LoaderExceptions != null)
                        {
                            foreach (var item in e.LoaderExceptions)
                            {
                                if (item != null)
                                {
                                    logger.Error("More details:{0}.", item.ToString());
                                }
                            }
                        }
                    }
                    ...
                    {
                        logger.Error("An error occurred while getting generic type. Assembly name:{0}. Error;{1}", assembly.FullName, ex);
                    }
                }
    ...
    public class AveNullResultException : AveWrapperBaseException
    {
        ...
        public AveNullResultException(AveInternalResourceKey key, params object[] args)
            : base(key, args)
        { }
    }
