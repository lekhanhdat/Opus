...
namespace AvePoint.Media.Storage.Centera
{
    ...
    class CenteraInputStream : XStream
    {
        ...
        public CenteraInputStream(StorageInfo info, CenteraClient client, AbstractXSystem sys)
            : base(sys)
        {
            this.client = client;
            this.Info = info;
        ...
        public void InitReadStream(StorageInfo info)
        {
            ...
            this.tag = client.FetchTag(info.ClipId, client.CheckName(info.LowName));
            if (innerStream != null)
            {
                innerStream.Close();
                innerStream = null;
