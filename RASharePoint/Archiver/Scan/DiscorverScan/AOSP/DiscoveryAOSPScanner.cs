...
namespace AvePoint.RA.SharePoint.Archiver.Scan.DiscorverScan.AOSP
{
    public class DiscoveryAOSPScanner : ISharePointScanner
    {
        ...
        public static IEAOSPDataOptimizationService discoverDBService;
        ...
        public List<RMDiscoveryFileDataInfo> fileInfos = new List<RMDiscoveryFileDataInfo>();
        ...
        private AnalysisOptimizationAOSPDiscoverNodeWorker mDiscoverWorker = null;
        ...
        public IDiscoverNodeWorker discoverWorker
        {
            get
            {
                ...
                {
                    mDiscoverWorker = new AnalysisOptimizationAOSPDiscoverNodeWorker(jobSettings, mConfiguration, mDependencyObjs);
                }
                return mDiscoverWorker;
            }
            set { }
        }
        public static IEAOSPDataOptimizationService Instance
        {
            get
            {
        ...
        public PCContainer<ArchiveApproveReport> GetPCContainer()
        {
            if (mDiscoverWorker == null)
            {
                return (discoverWorker as AnalysisOptimizationAOSPDiscoverNodeWorker).pcContainer;
            }
            else
            {
        ...
        private RMDiscoveryAOSPOptimizeDataSettingDto mRMDiscoveryOptimizeDataSettingDto = null;
        private List<DiscoverTagRule> discoverTagRules = new List<DiscoverTagRule>();
        private ITenantService TenantService => PlatformWindsorManager.GetService<ITenantService>();

        public DiscoveryAOSPScanner(ScanJobSettings scanJobSettings, RMDiscoveryAOSPOptimizeDataSettingDto dataDto)
        {
        ...
        private void InitDiscoverDBService()
        {
            ...
            {
                discoverDBService = new IEAOSPDataOptimizationService(mRMDiscoveryOptimizeDataSettingDto, true, sourceFlag);
                if (mConfiguration.InactiveDiscoveryAOSPRuleInfos != null && mConfiguration.InactiveDiscoveryAOSPRuleInfos.Count > 0)
                {
                    mConfiguration.InactiveAndRotVerisonAOSPRuleInfos.AddRange(mConfiguration.InactiveDiscoveryAOSPRuleInfos ?? null);
                }
                if (mConfiguration.ROTDiscoveryAOSPRuleInfos != null && mConfiguration.ROTDiscoveryAOSPRuleInfos.Count > 0)
                {
                    mConfiguration.InactiveAndRotVerisonAOSPRuleInfos.AddRange(mConfiguration.ROTDiscoveryAOSPRuleInfos?.Where(r => r.AnalyseMethod == RMDiscoveryRuleAnalyseMethod.Version));
                }
                if (mConfiguration.InactiveAndRotVerisonAOSPRuleInfos != null && mConfiguration.InactiveAndRotVerisonAOSPRuleInfos.Count > 0)
                {
                    discoverTagRules = mConfiguration.InactiveAndRotVerisonAOSPRuleInfos?.ConvertAll(rule =>
                    {
            ...
            {
                discoverDBService = new IEAOSPDataOptimizationService(mRMDiscoveryOptimizeDataSettingDto, false, sourceFlag);
            }
        }
        ...
        public async System.Threading.Tasks.Task RunAsync()
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("SharePointScanner.Run"))
            {
                try
                {
                    ...
                    ArchiverNodeItem selectNodeItem = new ArchiverNodeItem(ruleNode);
                    selectNodeItem.Name = ruleNode.FullPath;
                    JobExecutionProcessStatisticExecutor.Instance.StartCalculateRuleAndSummary(selectNodeItem.SPNodeLevel.ToString(), selectNodeItem.FullPath);
                    ...
                    await ProcessSiteCollectionForArchiverModAsync(selectNodeItem);
                    mDependencyObjs.Flush();
                }
                catch (JobStopException)
                {
        ...
        private async Task ProcessSiteCollectionForArchiverModAsync(ArchiverNodeItem sitecollection)
        {
            try
            {
                        ...
                        using (AveDiscoverSite discoverySite = sitecollection.DiscoverSPObject as AveDiscoverSite)
                        {
                            using (AveDiscoverWeb rootWeb = discoverySite.GetRootWeb())
                            {
                                using (ArchiverNodeItem webnode = sitecollection.GenerateSiteNodeItem(rootWeb, mConfiguration, true))
                                {
                                    ...
                                    await ProcessWebForArchiverModAsync(webnode);
                                }
                            }
                        }
        ...
        public async System.Threading.Tasks.Task ProcessWebForArchiverModAsync(ArchiverNodeItem web, bool needInitInfo = false)
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.ProcessWeb"))
            {
                try
                {
                    ...
                    await ProcessListCollectionForArchiverModAsync(web, result == ProcessResult.SkipListNode);
                    //Process web
                    await ProcessWebCollectionForArchiverModAsync(web);
                }
                catch (AveWrapperI18NException IUPEx)
                {
        ...
        private async System.Threading.Tasks.Task ProcessWebCollectionForArchiverModAsync(ArchiverNodeItem web)
        {
            using (new CheckJobStopScope()) { }
        ...
        private async System.Threading.Tasks.Task ProcessListCollectionForArchiverModAsync(ArchiverNodeItem web, bool isSkipListNode)
        {
            using (new CheckJobStopScope()) { }
                ...
                foreach (AveDiscoverList list in discoveryLists.Values)
                {
                    ...
                    try
                    {
                        ...
                        if (list.Title.Equals("{System Folder}"))
                        {
                            ...
                            await ProcessListForArchiverModAsync(listnode);
                        }
                        else
                        {
        ...
        private async System.Threading.Tasks.Task ProcessListForArchiverModAsync(ArchiverNodeItem list)
        {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.ProcessList"))
            {
                try
                {
                    ...
                    await ProcessFolderForArchiverModAsync(foldernode);
                }
                catch (AveWrapperI18NException IUPEx)
                {
                ...
                {
                    mLog.Error("[ProcessListForArchiverModAsync]An unexpected error occurred while processing list node.Path:{0}.Message:{1}.", list.FullPath, e.ToString());
        ...
        public async System.Threading.Tasks.Task ProcessFolderForArchiverModAsync(ArchiverNodeItem folder, bool needInitInfo = false, List<int> itemIDs = null)
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.ProcessFolder"))
            {
                try
                {
                    ...
                    await ProcessItemsAndSubfoldersForArchiverModAsync(folder, folder.Cache_NodeType, itemIDs);
        ...
        public async System.Threading.Tasks.Task ProcessItemsAndSubfoldersForArchiverModAsync(ArchiverNodeItem folderNode, int folderLevel, List<int> itemIDs = null)
        {
            using (new CheckJobStopScope()) { }
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.RealProcessItemsAndSubfolders"))
            {
                ...
                bool isSiteAssetsFolder = rootFolder.LeafName.Equals("SiteAssets", StringComparison.OrdinalIgnoreCase);
                ...
                try
                {
                    ...
                    foreach (var items in rootFolder.GetItemsWithStructureForArchiver())
                    {
                        ...
                        await ProcessDataForArchiverModAsync(items, itemIDs, rootFolder, folderNode, discoverWorker);
                ...
                try
                {
                    ...
                    {
                        var tempFolders = folders;
                        if (isSiteAssetsFolder)
                        {
                            tempFolders = folders.Where(f => f.LeafName.Equals("Lists")).ToList();
                        }
                        mLog.Info("Curent GetFoldersWithStructure folders Count:{0}.", folders.Count);
                        var folderIds = tempFolders.Where(x => x.ID != null).Count() != 0 ? tempFolders.Where(x => x.ID != null).Select(x => x.ID.Value).ToList() : new List<int>();
                        await ProcessDataForArchiverModAsync(tempFolders, itemIDs, folderNode, discoverWorker);
        ...
        private async System.Threading.Tasks.Task ProcessDataForArchiverModAsync(List<AveDiscoverFolder> folders, List<int> itemIDs, ArchiverNodeItem folderNode, IDiscoverNodeWorker discoverWorker)
        {
            foreach (AveDiscoverFolder folder in folders)
            {
                ...
                await ProcessItemsAndSubfoldersForArchiverModAsync(subFolderNode, subFolderNode.Cache_NodeType);
                folder.Dispose();
            }
        }
        private async System.Threading.Tasks.Task ProcessDataForArchiverModAsync(List<AveDiscoverItem> items, List<int> itemIDs, AveDiscoverFolder rootFolder, ArchiverNodeItem folderNode, IDiscoverNodeWorker discoverWorker)
        {
            using (new CheckJobStopScope()) { }
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.ProcessData"))
            {
                foreach (AveDiscoverItem item in items)
                {
                    ...
                    try
                    {
                        ...
                        await ProcessVersionAndAttachmentsForArchiverModAsync(item, rootFolder, folderNode, discoverWorker);
                    }
                    catch (JobStopException)
                    {
                        throw;
        ...
        internal async virtual System.Threading.Tasks.Task ProcessVersionAndAttachmentsForArchiverModAsync(AveDiscoverItem item, AveDiscoverFolder rootFolder, ArchiverNodeItem folderNode, IDiscoverNodeWorker discoverWorker)
        {
            using (new CheckJobStopScope()) { }
        ...
        public bool ListSkipCheck(ArchiverNodeItem listNode)
        {
            try
            {
                ...
                if (discoverList != null)
                {
                    if ((!string.IsNullOrEmpty(mConfiguration.ForceFitTeamsRuleID) || !string.IsNullOrEmpty(listNode.Parent.RuleId))
                        && CombineListUrlAndTemplate(discoverList).Equals("SiteAssets101", StringComparison.OrdinalIgnoreCase))
                    {
                        mLog.Info("No need to skip the design list {0} with TeamsGroup or SC level rule.", discoverList.Name);
                        return false;
                    }
                    ...
                    if (CheckIsDesignList(CombineListUrlAndTemplate(discoverList)))
                    {
                    ...
                    if (listNode.SPList != null && listNode.SPList.BaseType == AveBaseType.GenericList)
                    {
        ...
        {
            return ArchiverSettingDao.LoadBreakInheritNodeUrls(scopeUrl);
        }
            ...
            if (rootFolder.SubFolders != null)
            {
                foreach (SPOFolder folder in rootFolder.SubFolders)
                {
                    ...
                    foreach (int id in failedItemsId)
                    {
                        try
                        {
                            var info = fileInfos.Where(f => f.ItemId.Equals(id)).FirstOrDefault();
                            if (info != null)
                            {
                                ...
                                mLog.Error($"[AnalyzeFolderStructureV3]Cannot find ID:{info.ItemId}.Name:{info.FullUrl} when AnalyzeFolderStructureV3.");
                                mConfiguration.JobReportDto.AddScanReport(info.FullUrl, 0, (int)CacheNodeType.Item, "", Contract.RMWeb.JobMonitor.JobDetailsStatus.Failed, "StorageOptimization_SOARRecordManagerFileNotExist");
        ...
        void AnalyzeListItems(List<RMDiscoveryFileDataInfo> items, SPOFolder rootFolder)
        {
            foreach (var item in items)
            {
                string decodeFullPath = Uri.UnescapeDataString(item.FullUrl);
        ...
        internal async virtual System.Threading.Tasks.Task ProcessVersionAndAttachmentsAsync(AveDiscoverItem item, AveDiscoverFolder rootFolder, ArchiverNodeItem folderNode, IDiscoverNodeWorker discoverWorker)
        {
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.ProcessVersionAndAttachments"))
            {
                using (ArchiverNodeItem itemNode = folderNode.GenerateItemNodeItem(item, rootFolder, mConfiguration))
                {
                    var fileRecord = fileInfos.Where(s => s.ItemUniqueId == itemNode.ID.ToString()).FirstOrDefault();

                    if (fileRecord != null && fileRecord.ModifiedTime.Ticks + TimeSpan.FromMinutes(1).Ticks < itemNode.Modified)//由于sharepoint不同modified time之间可能存在差异，这里暂时先加一分钟的容错
                    {
            ...
            using (AvePerformanceScope pc = new AvePerformanceScope("ArchiverScan.ProcessListCollection"))
            {
                ...
                foreach (AveDiscoverList list in discoveryLists.Values)
                {
                    ...
                    try
                    {
                        ...
                        string siteIDString = mConfiguration.SiteCollectionID.ToString();
                        string siteUrlString = mConfiguration.SiteCollectionUrl;
                        await foreach (var tempFileInfo in discoverDBService.GetAllFilesAsync(siteIDString, web.WebId.ToString(), list.ListId.ToString()))
                        {
                            fileInfos.AddRange(tempFileInfo);
                        }
                        mLog.Info($"discover optimization current list file count is:{fileInfos.Count}");
        ...
        private AveBPOSAccountInfo GetBposInfoBySite(string siteUrl)
        {
            lock (locker)
            {
                ...
                {
                    AveBPOSAccountInfo aveBPOSAccountInfo = PoolUserUtil.GetAOSPBPOSInfoAsync(jobSettings.AppProfileId, jobSettings.SiteAdminUrl).Result;
