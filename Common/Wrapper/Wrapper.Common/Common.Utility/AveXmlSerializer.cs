    ...
    public class AveXmlSerializer
    {
        ...
        private static void SerializeInternal(XmlWriter writer, object name, object obj)
        {
            ...
            try
            {
                if (name != null)
                {
                    if (name is string)
                    {
                        writer.WriteAttributeString(AveWrapperConstants.COLUMN_NAME, (string)name);
                    }
                    else
                    {
                        string sname;
        ...
        private static string RemoveSurrogateCharacters(string input, object key)
        {
            using (new AvePerformanceScope("AvePoint.Wrapper.Common.AveXmlSerializer.RemoveSurrogateCharacters"))
            {
                ...
                {
                    log.Log(AveLogLevel.WARN, "Replace surrogate in char, key:{0}, from:{1}, to:{2}", key.ToString(), input, output);
                }
                return output;
            }
        }
        ...
        private static void WriteOneElement(XmlWriter writer, string key, string type, string value, string keyType)
        {
            ...
            try
            {
                if (key != null)
                {
                    writer.WriteAttributeString(AveWrapperConstants.COLUMN_NAME, key);
                }
                if (keyType != null)
                {
                    writer.WriteAttributeString(AveWrapperConstants.COLUMN_KEY_TYPE, keyType);
                }
        ...
        public static object Deserialize(string path, Type type = null)
        {
            ...
            using (var stream = File.OpenRead(SecurityUtils.SafeCombinePath(path)))
        ...
        private static object DeserializeInternal(XmlReader reader, Type type, out bool isMovedToNext)
        {
            ...
            if (type == null)
            {
                string stype = reader.GetAttribute(AveWrapperConstants.COLUMN_TYPE);
            ...
            if (type.IsGenericType)
            {
                ...
                if (type.FullName.StartsWith("System.Collections.Generic.Dictionary`2", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    while (isDicItemMovedNext || dicSubReader.ReadNextElement())
                    {
                        ...
                        string keyname = dicSubReader.GetAttribute(AveWrapperConstants.COLUMN_NAME);
                        if (keyname == null)
                        {
                            continue;
                        }
            ...
            if (type.GetInterface("System.Collections.IDictionary") != null)
            {
                ...
                while (isIdicMovedNext || idicSubReader.ReadNextElement())
                {
                    ...
                    string keyname = idicSubReader.GetAttribute(AveWrapperConstants.COLUMN_NAME);
                    string keytype = idicSubReader.GetAttribute(AveWrapperConstants.COLUMN_KEY_TYPE);
                    if (keyname == null)
                    {
                        continue;
                    }
            ...
            while (isNextObjReaded || subReader.ReadNextElement())
            {
                ...
                string childFieldName = subReader.GetAttribute(AveWrapperConstants.COLUMN_NAME);
                ...
                try
                {
                    string keytype = subReader.GetAttribute(AveWrapperConstants.COLUMN_TYPE);
                    object value = null;
        ...
        public static void Deserialize(XmlElement xmlEle, IDictionary dictionary)
        {
            foreach (XmlElement childEle in xmlEle.ChildNodes)
            {
                ...
                string stype = childEle.GetAttribute(AveWrapperConstants.COLUMN_TYPE);
                try
                {
        ...
        private static object GetValueFromType(string stype, string svalue)
        {
            switch (stype)
            {
                case AveWrapperConstants.TYPE_STRING:
                case AveWrapperConstants.TYPE_SYSTEM_STRING:
                    return svalue;
                ...
                case AveWrapperConstants.TYPE_FLOAT: return float.Parse(svalue);
                case AveWrapperConstants.TYPE_DOUBLE: return double.Parse(svalue);
