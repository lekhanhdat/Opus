...
import SPNodeContent from '../../NodeContents/SPDestinationTreeNodeContent';
...
class SPDestinationTree extends Component {
    constructor(props) {
        super(props);
        ...
        this.treeContext = {
            ...
            transToTreeNodeObject(oitem) {
                ...
                const isLeafNode = oitem.Level === NodeLevel.List;
                const isLeaf = oitem.Level === NodeLevel.List || oitem.Level === NodeLevel.SiteCollection || oitem.Level === NodeLevel.Site;
                return {
                    origin: oitem,
                    nodeKey: oitem.Id,
    ...
    initRequestUrl() {
        let apiPrefix = this.getRequestApiPrefix();
        this.browseTreeReqUrl = `${apiPrefix}/BrowseAllTree`;
        this.getInitTreeDataReqUrl = `${apiPrefix}/GetSPTreeInitData`;
        this.getDesignListsReqUrl = `${apiPrefix}/GetSPDesignLists`
    }

    getDesignLists() {
        let reqUrl = this.getDesignListsReqUrl;
        $.ajax({
            type: "POST",
            url: reqUrl,
    ...
    filterItems(items) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (!item.Hidden && item.Level != NodeLevel.Apps) {
                ...
                if (item.Level == NodeLevel.List && ($.inArray(item.TemplateId, listTemplateIdWhiteList) == -1 || $.inArray(uniqueName, this.DesignLists) != -1))//过滤designLists
                {
                    continue;
                }
    ...
    relateTreeItemSearchChildren(item) {
        ...
        if (item && item.ChildrenIds) {
            for (let childId of item.ChildrenIds) {
                ...
                if (text.indexOf(this.treeContext.searchKey) > -1
...
SPDestinationTree.propTypes = {
    treeData: PropTypes.array,
    onSelectedNodeChanged: PropTypes.func
};
SPDestinationTree.defaultProps = {
    treeData: [],
    onSelectedNodeChanged: null
};

export default SPDestinationTree;