...
{
    public class PredictTermIdQueryBuilder : IFilterBuilder, IAdvancedQueryBuilder
    {
        #region BaseArrayFilterBuilder
        private bool CanFilter(ExplorerFilterOptionV2 filterOption)
        {
            return filterOption != null && filterOption.PredictTermIds != null;
        }

        private string GetFilterColumnName()
        {
            return CosmosConst.C_PredictTermId;
        }

        private List<Guid> GetFilterValue(ExplorerFilterOptionV2 filterOption)
        {
            return filterOption.PredictTermIds;
        }
        #endregion
        ...
        public Query Filter(Query query, ExplorerFilterOptionV2 filterOption)
        {
            if (CanFilter(filterOption))
            {
                ...
                if (values?.Count == 0)
                {
                    query.Where(q =>
                    {
                        q.WhereNot(GetFilterColumnName().FormatColumnName(), Guid.Empty);
                        return q;
                    });
                }
                else
        ...
        {
            return QueryCloumnIds.PredictTermId;
        }
        ...
        private ExplorerFilterOptionV2 Convert2SearchOptionV2(ExplorerQueryColumn column, string objJson, ExplorerSearchColumnOperationLogic columnOperationLogic, ExplorerSearchKeyOperationLogic keyOperationLogic)
        {
            ...
            {
                PredictTermIds = JsonConvert.DeserializeObject<List<Guid>>(objJson)
            };
        }
