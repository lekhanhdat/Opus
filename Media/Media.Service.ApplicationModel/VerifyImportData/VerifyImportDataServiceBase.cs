...
namespace AvePoint.Media.Service
{
    ...
    public abstract class VerifyImportDataServiceBase<TParameter>
        : ApplicationModelServiceBase
        , IVerifyImportDataService
        where TParameter : class, IUpgradeInfo, new()
    {
        AveLogger logger = AveLogger.GetInstance(MethodBase.GetCurrentMethod().DeclaringType);

        public void Verify(IUpgradeInfo upgradeInfo)
        {
            ThreadPool.QueueUserWorkItem(upgradeRequest =>
            {
                InternalVerify(upgradeInfo as TParameter);
            }, upgradeInfo);
        }

        void InternalVerify(TParameter upgradeInfo)
        {
            try
            {
                this.Open(upgradeInfo);
                this.Verify(upgradeInfo);
            }
            catch (Exception e)
            {
        ...
        public abstract void Verify(TParameter upgradeInfo);
