...
{
    public class RMDiscoveryOffice365OptimizationCalculateJobRunner
    {
        private readonly RALogger _logger = RALogger.GetInstance(typeof(RMDiscoveryOffice365OptimizationCalculateJobRunner));
        
        private readonly IRMDiscoveryOffice365OptimizationSettingsInfoDao _settingInfoDao = new RMDiscoveryOffice365OptimizationSettingsInfoDao();
        ...
        public RMDiscoveryOffice365OptimizationCalculateJobRunner(string jobId, Guid settingId, Guid o365TenantId)
        {
            _settingId = settingId;
            _o365TenantId = o365TenantId;
            _reportManager = ReportMangerFactory.Instance.ReportManager;
            ReportMangerFactory.Instance.Init(jobId, Contract.JobMonitor.JobType.DiscoveryOptimizationCalculate);
            ...
            try
            {
                using (var cts = new CancellationTokenSource())
                {
                    ...
                    await foreach (var site in siteCollection)
                    {
                        ...
                        var calculator = new RMDiscoveryOffice365OptimizationCalculator(_o365TenantId, site, settingInfo);
