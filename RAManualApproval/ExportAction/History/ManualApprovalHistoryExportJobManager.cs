...
{
    public class ManualApprovalHistoryExportJobManager
    {

        private static readonly IRMReportManager ReportManager = ReportMangerFactory.Instance.ReportManager;

        private static List<JMManualApprovalJobDetails> JobDetailList = new();
    ...
    new Dictionary<RMNodeLevel, string>
    {
        ...
        {
            ReportMangerFactory.Instance.Init(jobId, AvePoint.RA.Contract.JobMonitor.JobType.ManualExportHistoryDatasJob);
            ReportManager.StartUpdateJobProgress(60);
        }

        public static void AddSucceedJobDetail(RMManualApproveHistoryTableEntity record, string reviewers)
        {
            HasSucceed = true;
            JobDetailList.Add(new JMManualApprovalJobDetails
            {
                TitleOrName = record.LeafName,
                Url = record.FullPath,
                ObjectLevel = ReportNodeLevelI18ns.TryGetValue((RMNodeLevel)record.Level, out var objectLvel) ? objectLvel : "",
                ApprovalStatus = ApprovalStatusI18ns.TryGetValue((SOApproveDBStatus)record.ApprovedStatus, out var approvalStatus) ? approvalStatus : "",
                RuleCriteria = record?.RuleCriteria,
                RecordOwner = reviewers,
                Status = JobDetailsStatus.Successful,
            });
        }

        public static void AddFailedJobDetail(RMManualApproveHistoryTableEntity record, string reviewers, string comment)
        {
            HasFailed = true;
            JobDetailList.Add(new JMManualApprovalJobDetails
            {
        ...
        public static void AddJobDetail(string objectName, string url, bool isSuccess = true, string comment = "")
        {
