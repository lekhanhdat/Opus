...
namespace ExchangeUtility
{
    ...
    class EWSTraceListener : ITraceListener,ISingleton
    {
        private EWSTraceListener() { }

        static RALogger logger = RALogger.GetInstance(typeof(EWSTraceListener));
    ...
    static class EWSTraceUtil
    {
        public static void EnableTraceLog(this ExchangeService self)
        {
            self.TraceEnabled = true;
            self.TraceFlags = TraceFlags.EwsRequest | TraceFlags.EwsResponse;//| TraceFlags.EwsRequestHttpHeaders | TraceFlags.EwsResponseHttpHeaders
