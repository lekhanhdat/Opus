...
{
    public class GoogleDashBoardQuery : DashboardQueryable
    {
        public override SourceFlag Flag => SourceFlag.Google;

        public override long GetApplySettingCount(SecurityUserPermissionsDto permission)
        {
            if(permission.IsAdmin || permission.ScopePermissionInfo.Any(i => i.IsScopeAdmin && i.DataSourceType == Flag))
            {
                return DashboardDao.GetGoogleSettingCount();
            }
            return 0;
        }
