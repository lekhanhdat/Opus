...
namespace AvePoint.ObjectModel.Common
{
    class AveGroup : AvePrincipal, IAveGroup
    {
        ...
        public AveGroup(IAveRequest request, AveWeb web, Dictionary<string, object> groupProperties)
        {
            mWeb = web;
            mRequest = request;
        ...
        public IAveMember Owner
        {
            get
            {
                if (base.DataCache.IsPropertyNotLoaded("Owner") && base.DataCache.IsPropertyAvailable("OwnerId"))
                {
                    ...
                    {
                        base.DataCache.PropertiesCache["Owner"] = this.mWeb.SiteGroups.GetByID(memberId);
                    }
                    else //if (type.Equals("user"))
                    {
                        base.DataCache.PropertiesCache["Owner"] = this.mWeb.SiteUsers.GetByID(memberId);
                    }
                }
            ...
            set
            {
                base.DataCache.PropertiesCache["Owner"] = value;
        ...
        public IAveUserCollection Users
        {
            get
            {
                ...
                {
                    Dictionary<string, object> userCollectionProperties = new Dictionary<string, object>();
                    if (base.DataCache.IsPropertyAvailable("Users" + AveObjectModelConstant.ObjectPropertySuffix))
                    {
                        List<Dictionary<string, object>> usersProperties = new List<Dictionary<string, object>>();
                        foreach (string loginName in base.DataCache.PropertiesCache["Users" + AveObjectModelConstant.ObjectPropertySuffix] as List<string>)
                        {
                            Dictionary<string, object> userProperties = new Dictionary<string, object>();
                            IAveUser gropUser = mWeb.SiteUsers.GetByLoginName(loginName);
                            if (gropUser != null) //SAAS-202
                            {
                                userProperties = (gropUser as AveUser).DataCache.PropertiesCache;
                                usersProperties.Add(userProperties);
                            }
                        }
                        userCollectionProperties.Add(AveObjectModelConstant.ChildrenProperties, usersProperties);
                    }
                    else
                    {
                    ...
                    base.DataCache.PropertiesCache["Users"] = users;
                    return users;
                }
        ...
        {
            this.Users.Add(user.LoginName, user.Email, user.Name, user.Notes);
