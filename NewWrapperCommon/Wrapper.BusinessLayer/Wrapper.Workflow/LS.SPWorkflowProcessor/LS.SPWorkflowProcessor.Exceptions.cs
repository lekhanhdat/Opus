...
namespace LS.SPWorkflowProcessor
{
    ...
    { 
        ...
        SkipNonBuiltinAssociationException=151,
        SkipOnlineListField = 152,
        AssociationDependencyFeatureNotActivatedError =600,
        ...
        InstanceIsRunningException = 1125,
        UpdateTableNameValuePair = 1126,
        ...
        WorkflowServiceNotAvailableException = 3001,//13mode workflow service manager is not available
        CustomWorkflowNotSupportedException=3002,//we donot support to restore custom workflows example: nintex workflow at current sharepoint version.

        PutIntoPostAction=9999,
    }


    public class SPWFProcessorException:AveWrapperBaseException
    {
        ...
        private SPWFProcessorErrorCode mErrorCode = 0;
        public SPWFProcessorErrorCode ErrorCode
        {
        ...
        public override string Message
        {
            get
            {
                //Message InnerExceptionMessage  ErrorCode ErrorCodeString
                EnsureFinalMessage();
                return message;
            }
        }

        private void EnsureFinalMessage()
        {
            if (message == null)
            {
                string errorCodePart = "";
                string messagePart = "";
                string innerExceptionPart = "";
                var messageBuilder = new StringBuilder();
                messageBuilder.Append("An error occurred while handing workflow processor.");
                var _message = AveAssemblyUtility.GetFieldValue(this, "_message");
                if (_message != null)
                {
                    messagePart = _message.ToString();
                }
                if (ErrorCode != 0)
                {
                    errorCodePart = string.Format("{0}({1})", ErrorCodeString, ErrorCode);
                }
                if (InnerException != null)
                {
                    innerExceptionPart = InnerException.Message;
                }
                if (!string.IsNullOrEmpty(messagePart))
                {
                    messageBuilder.AppendFormat("ErrorMesssage:{0}.", messagePart);
                }
                if (!string.IsNullOrEmpty(errorCodePart))
                {
                    messageBuilder.AppendFormat("ErrorCode:{0}.", errorCodePart);
                }

                if (!string.IsNullOrEmpty(innerExceptionPart))
                {
                    messageBuilder.AppendFormat("Inner Error:{0}.", innerExceptionPart);
                }
                if (!string.IsNullOrEmpty(InnerMessage))
                {
                    messageBuilder.AppendFormat("Details:{0}.", InnerMessage);
                }
                messageBuilder.Append("Please refer to agent logs for more details.");
        ...
        public SPWFProcessorException(SPWFProcessorErrorCode errorCode, AveInternalResourceKey key, params object[] args)
            :base(key,args)
        {
            mErrorCode = errorCode;
            mErrorCodeString = errorCode.ToString();
            mInnerMessage = WrapperExceptionResource.ResourceManager.GetString(key.ToString(), WrapperExceptionResource.Culture);
        }
    }
    ...
    public class Logs
    {
        ...
        public const string IP_BackupInstanceSelfException = "IP_BackupInstanceSelfException";
        ...
        public const string IP_RestoreInstanceSelfException = "IP_RestoreInstanceSelfException";
        ...
        public const string Markup_FoundFolderByUrl = "Markup_FoundFolderByUrl";
        ...
        public const string Markup_MissingFolder = "Markup_MissingFolder";
