...
namespace AvePoint.Media.ClassicStorage.Cloud.Azure.REST
{
    ...
    class MSAzureUtils
    {
        ...
        public static string AuthorizationHeader(HttpWebRequest request, string storageAccount, string storageKey)
        {
            ...
            using (var hmacSha256 = new System.Security.Cryptography.HMACSHA256(Convert.FromBase64String(storageKey)))
            {
                return "SharedKey " + storageAccount + ":" + Convert.ToBase64String(hmacSha256.ComputeHash(SignatureBytes));
            }
        }
        ...
        public static void signRequest(HttpWebRequest request, MSAzureRequest config)
        {
            ...
            string message = canonicalizeHttpRequest(request, config);
        ...
        public static void signRequest(HttpWebRequest request, CloudOpenParameter openParam)
        {
            ...
            {
                string message = canonicalizeHttpRequest(request, openParam);
        ...
        //{
        //    var hash = new System.Security.Cryptography.HMACSHA256(Convert.FromBase64String(storageKey));
        //    byte[] value = hash.ComputeHash(Encoding.UTF8.GetBytes(signature));
        //    return Convert.ToBase64String(value);
        //}

        public static string canonicalizeHttpRequest(HttpWebRequest request, MSAzureRequest config)
        {
            ...
            canonicalizedString.Append("/").Append(config.UserName).Append(getCanonicalizedResource(request.Address));
            return canonicalizedString.ToString();
        }

        public static string canonicalizeHttpRequest(HttpWebRequest request, CloudOpenParameter openParam)
        {
            StringBuilder canonicalizedString = new StringBuilder();
            string verb = request.Method;
            string contentEncoding = getHeaderValue(request, MSAzureConstants.ContentEncoding);
            string contentLanguage = getHeaderValue(request, MSAzureConstants.ContentLanguage);
            string contentLength = request.ContentLength.ToString();
            if (request.ContentLength == -1 || (request.ContentLength == 0 && request.Headers["x-ms-version"].CompareTo("2017-04-17") >= 0))
            {
                contentLength = string.Empty;
            }
            string contentMD5 = getHeaderValue(request, MSAzureConstants.ContentMD5);
            ...
            canonicalizedString.Append(Range).Append("\n");
            canonicalizedString.Append(getCanonicalizedHeaders(request.Headers));
            canonicalizedString.Append("/").Append(openParam.UserName).Append(getCanonicalizedResource(request.Address));
            return canonicalizedString.ToString();
        }
