...
using System.Text;
//using AvePoint.Application.Security.Cryptography;

namespace AvePoint.Wrapper.Common
{
    public class GAPolicyHelper
    {
        public static List<string> keysNeedToDecryption = new List<string>() { "Gov Auto Policy" };
        public const string GA_PolicyForGranularBackup = "GA_PolicyForGranularBackup";
        ...
        private static string Decrypt(string _str, Guid _key)
        {
            ...
            byte[] iv = new byte[8];
            Array.Copy(_key.ToByteArray(), 0, key, 0, 8);
            Array.Copy(_key.ToByteArray(), 8, iv, 0, 8);
            MemoryStream ms = new MemoryStream(Convert.FromBase64String(_str));
            byte[] buffer = new byte[4];
            ms.ReadExInternal(buffer, 0, 4);
            int len = 0;
            int offset = 3;
            for (int i = 0; i < 4; ++i)
            {
            ...
            ms.ReadExInternal(value, 0, value.Length);
    ...
    internal class GAEncryptInternal
    {
        ...
        public static byte[] CommunicationEncryptionKey
        {
            ...
            set
            {
                ...
                communicationEncryptionKeyHash = HashAlgorithmFactory.CreateHashAlgorithm(GCommon.Utility.Cryptography.HashAlgorithm.SHA1, new byte[0]).ComputeHash(communicationEncryptionKey);
            }
        }
