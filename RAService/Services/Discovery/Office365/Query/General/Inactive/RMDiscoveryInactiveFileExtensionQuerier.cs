...
{
    public class RMDiscoveryInactiveFileExtensionQuerier : RMDiscoveryInactiveDataQuerier<List<RMDiscoveryFileExtensionDataInfo>>
    {
        public RMDiscoveryInactiveFileExtensionQuerier(RMDiscoveryOffice365QueryParameter queryParameter) : base(queryParameter)
        {
        }

        public override async Task<List<RMDiscoveryFileExtensionDataInfo>> QueryAsync()
        {
            using var performance = new PerformanceScope("QueryInactiveFileExtensionsAsync");
            var dataTable = GetDataTable();
            var sql = $@"SELECT TOP 20 fileType.Id AS Id, fileType.Name AS Name, SUM(data.FileTotalSize) AS FileTotalSize FROM [{_schemaName}].[{dataTable}] AS data
