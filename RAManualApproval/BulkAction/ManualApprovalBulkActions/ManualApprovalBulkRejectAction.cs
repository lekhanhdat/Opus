...
{
    public class ManualApprovalBulkRejectAction : ManualApprovalBulkAction
    {
        ...
        private static readonly IGControlPlatformApprovalProcessService _gControlPlatformApprovalProcessService = PlatformWindsorManager.GetService<IGControlPlatformApprovalProcessService>();
        ...
        public override ManualApprovalBulkActionType ActionType
        {
            get
            {
                return ManualApprovalBulkActionType.Reject;
            }
        }
        ...
        public ManualApprovalBulkRejectAction()
        {
        ...
        protected override async Task ProcessAction(Record item)
        {
            try
            {
                if (ManualApprovalInfos.ExtendType == ManualApprovalExtendType.Custom)
                {
                    var extendTime = ManualApprovalInfos.CustomeExtendDate.Ticks;
                    if (extendTime <= DateTime.UtcNow.Ticks)
                    {
                        throw new Exception("RM_MA_ExtendDisposalTime_Valid_EarlierThanNow");
                    }
                }
                ...
                {                
                    ...
                    await ManualApprovalAzureTableManager.RebuildAuditsAsync(item, (SOApproveDBStatus)ActionType, ApprovalAccount, ManualApprovalInfos.ExtendType, 0 , ManualApprovalInfos.CustomeExtendDate);
                    ...
                    {
                        item.ManualExtendTime = await CalculationExtendTimeAsync(ManualApprovalInfos.ExtendType, ManualApprovalInfos.CustomeExtendDate);
        ...
        private async Task ApprovalOrRejectForWorkflowNewAsync(Record item)
        {
            ...
            if (!nextStep.IsEnd)
            {
                ...
                item.ManualLastExtendType = ManualApprovalInfos.ExtendType;
                item.ManualLastCustomeExtendDate = ManualApprovalInfos.CustomeExtendDate;
                if ((SOApproveDBStatus)ActionType == SOApproveDBStatus.Approved)
                {
                    item.ManualLastExtendType = ManualApprovalExtendType.After1Month;
                }
            }

            if (nextStep.IsEnd || (FromGControl && (SOApproveDBStatus)ActionType == SOApproveDBStatus.Rejected))
            {
                if (FromGControl)
                {
            ...
            if ((!nextStep.IsEnd || item.SourceFlag > (int)SourceFlag.Connector) && !FromGControl)
            {
                _historyCache.TryAdd(item.Id, historyData);
            } 
