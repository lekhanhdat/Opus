...
namespace AvePoint.Wrapper.Core.SPRestore.Mapping
{
    ...
    #region Field Mapping Condition-- Internal use
    /// <summary>
    /// 用于存放并构造Condition信息，对应界面上的Condition(If these conditions are met)部分
    /// </summary>
    class FieldMappingCondition
    {
        internal List<FieldMappingConditionEntry> SiteCondition {get;private set;}
        internal List<FieldMappingConditionEntry> ListCondition { get; private set; }
        internal List<FieldMappingConditionEntry> ItemCondition { get; private set; }

        #region 保证一个FieldMappingCondition对象上最多有两个condition checker
        private object listLevelCheckerLock = new object();
        private object siteLevelCheckerLock = new object();
        private FieldMappingConditionChecker listLevelConditionChecker;
        private FieldMappingConditionChecker siteLevelConditionChecker;

        private FieldMappingConditionChecker GetConditionChecker(FieldMappingConditionInfo conditionInfo)
        {
            if (conditionInfo is WebFieldMappingConditionInfo)
            {
                return GetSiteLevelConditionChecker(this);
            }
            if (conditionInfo is ListFieldMappingConditionInfo)
            {
        ...
        private FieldMappingConditionChecker GetSiteLevelConditionChecker(FieldMappingCondition fieldMappingCondition)
        {
            if (this.siteLevelConditionChecker==null)
            {
                lock (this.siteLevelCheckerLock)
                {
                    if (this.siteLevelConditionChecker == null)
                    {
                        this.siteLevelConditionChecker = new WebLevelFieldMappingConditionChecker(fieldMappingCondition);
                    }
                }
            }
        ...
        private FieldMappingConditionChecker GetListLevelConditionChecker(FieldMappingCondition fieldMappingCondition)
        {
            if (this.listLevelConditionChecker == null)
            {
                lock (this.listLevelCheckerLock)
                {
                    if (this.listLevelConditionChecker == null)
                    {
                        this.listLevelConditionChecker = new ListLevelFieldMappingConditionChecker(fieldMappingCondition);
                    }
                }
            }
        ...
        {
            this.SiteCondition = new List<FieldMappingConditionEntry>();
            this.ListCondition = new List<FieldMappingConditionEntry>();
            this.ItemCondition = new List<FieldMappingConditionEntry>();
        }
        ...
        internal bool IsQualified(FieldMappingConditionInfo conditionInfo)
        {
        ...
        private void LoadConditions(List<FieldMappingConditionEntry> conditions, XmlNode conditionsNode)
        {
            ...
            foreach (XmlNode n in conditionsNode.ChildNodes)
            {
                ...
                var conditionInfo = new FieldMappingConditionEntry();
    ...
    /// </summary>
    class FieldMappingConditionEntry
    {
