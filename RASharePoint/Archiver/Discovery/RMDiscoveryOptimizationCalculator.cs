...
{
    public class RMDiscoveryOptimizationCalculator
    {
        private readonly RALogger _logger = RALogger.GetInstance(typeof(RMDiscoveryOptimizationCalculator));
        ...
        private RMDiscoveryOffice365OptimizationSetting _optimizationSettingInfo;

        private RMDiscoveryOffice365OptimizationSettingsInfo _optimizationAllSettingInfo;

        public RMDiscoveryOptimizationCalculator(Guid o365TenantId, RMDiscoveryOffice365SiteInfo siteInfo, long beforeScheduleTicks)
        {
        ...
        {
            var settingInfoDao = new RMDiscoveryOffice365OptimizationSettingsInfoDao();
            ...
            if(settingInfo != null)
            {
                ...
                _optimizationSettingInfo = SerializerHelper.DeserializeByDataContractSerializer<RMDiscoveryOffice365OptimizationSetting>(RMDiscoveryOffice365OptimizationSetting.XMLCompatibleConvert(settingInfo.Setting));
            }
        }
        ...
        private async Task<(long FileTotalSize, long VersionTotalSize)> CalculateOptimizableAsync()
        {
            ...
            else
            {
                ...
                {
                    var rules = await DiscoverUtil.GetInactiveRuleAsync(_optimizationSettingInfo.InactiveRuleQueryParameter, _optimizationSettingInfo.ArchiveDataType);
                    versionRules.AddRange(rules);
                }

                if (_optimizationSettingInfo.ROTRuleQueryParameter.Enable)
                {
                    var rules = await DiscoverUtil.GetROTRuleAsync(_optimizationSettingInfo.ROTRuleQueryParameter, _optimizationSettingInfo.ArchiveDataType);
        ...
        && _optimizationSettingInfo.SizeRangeQueryParameter.QueryMode != RMDiscoverySizeRangeQueryMode.None
