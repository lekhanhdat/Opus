...
namespace AvePoint.ObjectModel.Common.Office
{
    ...
    {
        private static AveLogger mLog = AveLogger.GetInstance(typeof(AveORecords));
        ...
        public void UndeclareItemAsRecord(IAveListItem item)
        {
            try
            {
                ...
                (((item.Web.Site as AveSite).Request) as IAveRequest).UndeclareItemAsRecord(item.Web.ServerRelativeUrl, item.ParentList.ID, item.ID);
            }
            catch(Exception e)
            {
                mLog.Error($"Undeclare record has errors{e}");
            }
        }
        ...
        public void DeclareItemAsRecord(IAveListItem item)
        {
            try
            {
                ...
                (((item.Web.Site as AveSite).Request) as IAveRequest).DeclareItemAsRecord(item.Web.ServerRelativeUrl, item.ParentList.ID, item.ID);
            }
            catch(Exception e)
            {
                mLog.Error($"Declare record has errors{e}");
            }
        }
