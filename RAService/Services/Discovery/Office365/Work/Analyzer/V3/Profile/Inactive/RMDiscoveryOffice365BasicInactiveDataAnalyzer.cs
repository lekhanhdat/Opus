...
namespace AvePoint.RA.Service.Services.Discovery.Office365.Work.Analyzer.V3.Profile.Inactive
{
    public class RMDiscoveryOffice365BasicInactiveDataAnalyzer
    {
        ...
        public async Task<bool> AnalysisAsync()
        {
            try
            {
                _logger.Info($"Start analysis o365 tenant [{_o365TenantId}] cotnent source [{_contentSource}] profile [{_profileInfo.Id} {_profileInfo.Name}] inactive data.");

                var customColumns = _rules.ConvertAll(item => item.ToCustomColumn());

                await _profileDataDao.DeleteBasicInactiveDataByContentSourceAsync(_o365TenantId, _profileInfo.Id, _contentSource);

                var containerDataEnumerable = _profileDataDao.GetContainerInactiveDataByContentSourceAsync(_o365TenantId, _profileInfo.Id, _contentSource, customColumns);

                var basicDataInfo = new RMDiscoveryProfileBasicInactiveData
                {
                ...
                await _profileDataDao.AddBasicInactiveDataListAsync(_o365TenantId, _profileInfo.Id, basicDataInfo);

                _logger.Info($"End analysis o365 tenant [{_o365TenantId}] cotnent source [{_contentSource}] profile [{_profileInfo.Id} {_profileInfo.Name}] inactive data.");

                return true;
            }
            catch(Exception e)
            {
                _logger.Error($"An error occurred while analysis o365 tenant [{_o365TenantId}] cotnent source [{_contentSource}] profile [{_profileInfo.Id} {_profileInfo.Name}] inactive data. Error: {e}");
                return false;
            }
        }
    }
