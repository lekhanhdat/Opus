...
namespace AvePoint.GCommon.Transfer.Data
{
    ...
    public class BaseDataTransferLogic : IDisposable
    {
        ...
        public Object DataTransferWorkStatusLock = new object();
        ...
        public BaseDataTransferLogic(string localIdentifier, string remoteIdentifier, bool filterByServerFlag = true, int bufferSize = 100)
        {
            ...
            //this.reconnectTimeout = DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.DefaultReconnectTimeout;
            this.performanceTimerPool = new CommonPerformanceTimerPool(DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.DisablePerformanceLogger);
        ...
        public virtual Boolean Open(DataTransferSetting setting, string sessionId)
        {
            ...
            this.performanceCounter.Init(DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.EnablePerformanceCounter, this.sessionId);
            //启动连接方法
        ...
        /// 关闭所有线程。
        /// </summary>
        /// <param name="force">是否强制关闭线程</param>
        /// <param name="clearAllSession">是否清除相关的session中的内容</param>
        /// <param name="clearStatus">是否清除dataTransferResultStatus</param>
        /// <param name="checkClose">是否等待close</param>
        protected void Reset(bool force, bool clearAllSession, bool checkClose, bool clearStatus, bool throwException = false)
        {
            ...
            try
            {
                ...
                if (transferChannel != null)
                {
                    ...
                    var closeMessage = transferChannel.Close();
                    transferChannel.Dispose();
                    if (!string.IsNullOrEmpty(closeMessage))
                    {
                        logger.Warn("The message from close method:{0}", closeMessage);
                    }
        ...
        private void InitDataTransferWorkerThread()
        {
            ...
            inputQueue.TimeOut = DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.TakeDataBlockTimeOut;
            if (DataTransferGlobalConfig.DataTransferConfiguration.DataConfig.ProcessDataWithNewThread)
            {
            ...
            {
                processor = new DataBlockProcessorV3(dataTransferSetting.IsEncryption, dataTransferSetting.DataEncryptionInfo, dataTransferSetting.IsCompression, dataTransferSetting.CompressionLevel, filterByServerFlag, localIdentifier, this.SessionId);
            }
            processor.DataProcessorExceptionCallback = ChangeWorkStatus;
            processor.PerformanceTimerPoolProperty = this.performanceTimerPool;
            ...
            logger.Info("{0}\r\nReconnectTimeoutMinutes:{1}, Encryption:{2},Compression:{3}, CompressionLevel:{4}, FilterByServerFlag:{5}, BufferSize:{6}, SessionId:{7}",
        ...
        protected bool SetupConnection(bool isFirst)
        {
            ...
            long timers = 0;
            DataTransferLogger.Logger(AveLogLevel.INFO, "start to setup connection for sessionId:{0}, identifier:{1}, remoteIdentifier:{2}, isSender:{3}", sessionId, localIdentifier, remoteIdentifier, filterByServerFlag);
            while (true)
            {
                try
                {
                    ...
                    {
                        //避免太多log
                        if (currentTime.AddMinutes(timers) < DateTime.UtcNow)
                        {
                            timers = (long)(DateTime.UtcNow - currentTime).TotalMinutes + 1;
                            logger.Warn("Cannot open data channel:{0} when the session is {1}.", DataTransferErrorMessage, this.SessionId);
                        }
                        if (DataTransferErrorMessage.Equals(DataTransferConstants.StreamModeReconnectionError, StringComparison.OrdinalIgnoreCase))
                        {
                            DataTransferWorkStatus = DataTransferWorkStatus.ReconnectionError;//can not find data block, so stop reconnection and failed job
                            break;
                        }
            ...
            DataTransferLogger.Logger(AveLogLevel.INFO, "end to setup connection for sessionId:{0}, identifier:{1}, remoteIdentifier:{2}, isSender:{3}, keep alive timeout:{4}", sessionId, localIdentifier, remoteIdentifier, filterByServerFlag, transferChannel.KeepAliveTimeout);
        ...
        {
            if (transferChannel == null || !dataTransferSetting.CommunicationSettings.IsStreamMode)
            {
        ...
        private void InitSession()
        {
            ...
            if (filterByServerFlag)
            {
                while (true)
                {
                    var result = transferChannel.InitSession(sessionId, remoteIdentifier, false, reconnectTimeout);
            ...
            {
                var result = transferChannel.InitSession(sessionId, localIdentifier, true, reconnectTimeout);
        ...
        //    {
        //        ObjectUtility.Dispose(TransferChannel);
        //        TransferChannel = null;
        //    }
        //}

        /// <summary>
        /// 结束时候的等待操作,不需要强制关闭线程，等待线程自己关闭
        /// </summary>
        protected void WaitClose(bool clearSession)
        {
            this.PerformanceTimerPool.Action("WaitClose", true);
            Reset(false, clearSession, NeedRelativeNodeHandShake, false, true);
        ...
        protected void CheckDataTransferIsValid()
        {
            ...
            switch (DataTransferWorkStatus)
            {
                ...
                case DataTransferWorkStatus.ReconnectionError:
                default:
                    if ((!string.IsNullOrEmpty(DataTransferErrorMessage)) && DataTransferErrorMessage.Length > 101)
                    {
                        DataTransferErrorMessage = DataTransferErrorMessage.Substring(0, 100);
        ...
        {
            CheckDataTransferIsValid();
            if (this.PerformanceTimerPool.IsEnable)
            {
            ...
            else
            {
                if (ReconnectTimeout != 0)
                {
                    ...
                    if (LastCommunicationTime.AddMinutes(TransferChannel.KeepAliveTimeout) < DateTime.Now)
                    {
                    ...
                    if (shouldKeepAlive)
                    {
                        ...
                        while (true)
                        {
                            ...
                            {
                                //DataTransferWorkStatus = DataTransferWorkStatus.Running;
                                logger.Info("start to keep alive with session id:{0}, local identifier:{1}, remote identifier:{2}", sessionId, localIdentifier, remoteIdentifier);
                                if (filterByServerFlag)
                                {
                    ...
                    //else
                    //{
                    //    logger.Info("cannot keep alive with session id:{0}, local identifier:{1}, remote identifier:{2}", sessionId, localIdentifier, remoteIdentifier);
                    //}
                }
            }
            if (this.PerformanceTimerPool.IsEnable)
        ...
        protected AveDataBlock TakeFreeBlock(bool checkRunningStatus)
        {
            ...
            while (true)
            {
                ...
                {
                    logger.Warn("Exception occurred while taking free block. {0}", e.ToString());
        ...
        internal void ChangeWorkStatus(object obj)
        {
            ...
            {
                if (DataTransferWorkStatus == DataTransferWorkStatus.Running && obj != null && obj is Tuple<DataTransferWorkStatus, string>)
                {
    ...
    public static class DataTransferDynamicKeyResolver
    {
        private static Func<DataEncryptionInfo, DataEncryptionInfoWrapper> innerResolver = null;

        public static void SetDynamicKeyResolver(Func<DataEncryptionInfo, DataEncryptionInfoWrapper> resolver)
        {
            innerResolver = resolver;
        }

        public static DataEncryptionInfoWrapper ResolverDynamicKey(DataEncryptionInfo encryptionInfo)
        {
            if (innerResolver == null) return null;
            if (encryptionInfo == null) return null;

            return innerResolver(encryptionInfo);
        }
    }
