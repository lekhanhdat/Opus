...
namespace AvePoint.RA.RAExchange.Discover
{
    ...
    public class ExchangeItemGroup
    {
        ...
        public IEnumerable<ExchangeItem> Items
        {
            get { return items; }
        }
        ...
        public ExchangeItemGroup(IEnumerable<ExchangeItem> items)
        {
        ...
        public ExchangeItemGroup(int maxItemsCount, int maxSizeLimit, ExchangeItem item)
        ...
        /// <param name="maxItemsCount">集合中Item数量上限</param>
        /// <param name="maxSizeLimit">集合中Item Size总和上限</param>
        public ExchangeItemGroup(int maxItemsCount, int maxSizeLimit)
        {
            this.items = new List<ExchangeItem>();
            this.TotalSize = 0L;
            this.maxItemsCount = maxItemsCount;
            this.maxSizeLimit = maxSizeLimit;
        }

        private bool IsFull(ExchangeItem item)
        {
        ...
        private bool Add(ExchangeItem item)
        {
        ...
        private void AddInternal(ExchangeItem item)
        {
            this.items.Add(item);
        ...
        /// <returns></returns>
        public static List<ExchangeItemGroup> GroupCachedItems(IEnumerable<ExchangeItem> items, int maxItemCount, int maxSizeLimit)
        {
            ...
            var groupedItems = new List<ExchangeItemGroup>();
        ...
        private static void AddBigSizeItems(IEnumerable<ExchangeItem> cachedItems, List<ExchangeItemGroup> groupedItems)
        {
            foreach (var item in cachedItems)
            {
                groupedItems.Add(new ExchangeItemGroup(1, -1, item));
            }
        }

        private static void AddSmallSizeItems(IEnumerable<ExchangeItem> cachedItems, List<ExchangeItemGroup> groupedItems, int maxItemCount, int maxSizeLimit)
        {
            ExchangeItemGroup currentCollection = null;
            foreach (var item in cachedItems)
            {
                if (currentCollection == null || currentCollection.IsFull(item))
                {
                    currentCollection = new ExchangeItemGroup(maxItemCount, maxSizeLimit);
