...
{
    public class FSDiscoveryDataQueue
    {
        private static BlockingCollection<string> _queue = new BlockingCollection<string>();

        public FSDiscoveryDataQueue() { }

        public bool Enqueue(string dirPath)
        {
            return _queue.TryAdd(dirPath);
        }

        public IEnumerable<string> EnumerateDequeue()
        {
