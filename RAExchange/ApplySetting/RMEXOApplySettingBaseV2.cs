...
{
    public class RMEXOApplySettingBaseV2 : RMEXODiscoverBaseV2
    {
        private static readonly RALogger logger = RALogger.GetInstance(typeof(RMEXOApplySettingProcesser));
        private Dictionary<string, Guid> ruleTermIdMapping = new();
        private ConcurrentDictionary<string, string> termNameDic = new();
        ...
        protected Dictionary<Guid, string> RetentionLabels = null;
        ...
        public RMEXOApplySettingBaseV2(RMExchangeOnlineSetting setting, ExchangeOnlineTreeNodeDto treeNode, JobManagement jobManagement)
            : base(treeNode)
        {
            Setting = setting;
            ...
            using (var performance = new PerformanceScope("EXO.RMEXOApplySettingBase.Init", "", true))
            {
                try
                {
                    using (CheckJobStopScope jScope = new CheckJobStopScope())
                    {
                        ...
                        RetentionLabels = mConfiguration.ExoRetentionLabelCache;
                    }
                }
        ...
        private void ProcessGroupedItems(IExchangeFolder folder)
        {
            using (AveAppendableTaskExecutor taskExecutor = new AveAppendableTaskExecutor(MaxBackupItemsThreads))
            {
                ...
                IEnumerable<IExchangeItemGroup> items = null;
        ...
        private void ProcessGroupedItem(IExchangeItemGroup itemGroup, IExchangeFolder folder)
        {
            try
            {
                using (CheckJobStopScope jScope = new CheckJobStopScope())
                {
                    ...
                    if (Setting.EnableRecordManagement != (int)EnableRecordManagementSetting.Enable)
                    {
                        ...
                        {
                            RemoveTermForGroupedItem(itemGroup, bulkHelper, folder.FolderId, folder.MailBoxId, def);
                        }
                    }
                    else
                    {
                        ...
                        {
                            AddTermForGroupedItem(itemGroup, bulkHelper, folder.FolderId, folder.MailBoxId, def, Setting.AutoJobOption == (int)AutoJobOption.Override);
                        }
                    }
                    // }
                    #region No use code
        ...
        private void AddTermForGroupedItem(IExchangeItemGroup itemGroup, IExchangeItemBulkHelper bulkHelper, string folderId, string mailboxId, ExtendedPropertyDefinition def, bool isOverwrite)
        {
            //Key 是Item，Value是对应的TermId. 表示批处理的Item 集合
            Dictionary<IExchangeItem, string> itemAndTermIdMapping = new ();
            ...
            if (!isOverwrite)
            {
                using (new PerformanceScope("AddTermForGroupedItem.LoadExtendProperties", $"AddTermForGroupedItem.LoadExtendProperties{itemGroup.ItemsCount}", true))
                {
                    try
                    {
                        bulkHelper.LoadExtendProperties(itemGroup.Items, false);
                    }
                    catch (Exception e)
                    {
                        JobManagement.HasErrorNode = true;
                        ...
                        if (itemGroup != null && itemGroup.ItemsCount > 0)
                        {
                            foreach (var item in itemGroup.Items)
                            {
                                EXOCommonUtil.AddDetailItem("RM_JS_JMD_Action_SetAutoClassification", item, MailboxAddress, Contract.RMWeb.JobMonitor.JobDetailsStatus.Failed, e.Message,string.Empty);
                            }
                        }
                        throw;
                    }
            ...
            else
            {
                try
                {
                    bulkHelper.LoadExtendProperties(itemGroup.Items, true);
                }
                catch (Exception e)
                {
            ...
            foreach (var item in itemGroup.Items)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    EXOCommonUtil.AddDetailItem("RM_JS_JMD_Action_SetAutoClassification", item, MailboxAddress, Contract.RMWeb.JobMonitor.JobDetailsStatus.Failed, ex.Message, string.Empty);
                }
            }
            ...
            try
            {
                if (itemAndTermIdMapping != null && itemAndTermIdMapping.Count > 0)
                {
                    ...
                    {
                        updateResult = bulkHelper.BatchAddExtendProperty(itemAndTermIdMapping, folderId, mailboxId, def);
                    }
                }
                else
                {
            ...
            using (var performance = new PerformanceScope("EXO.RuleManagement.AnalyzeResult", addToStatistics: true))
            {
                foreach (var item in itemAndTermIdMapping.Keys)
                {
                    var status = AvePoint.RA.Contract.RMWeb.JobMonitor.JobDetailsStatus.Successful;
                    var comment = string.Empty;
                    ExchangeUpdateItemResult result;
                    if (updateResult.TryGetValue(IsSupportGraphApi ? item.ItemId.ToRestId() : item.ItemId, out result))
                    {
                        if (result.IsFailed)
                        {
                            JobManagement.HasErrorNode = true;
                    ...
                    EXOCommonUtil.AddDetailItem("RM_JS_JMD_Action_SetAutoClassification", item, MailboxAddress, status, comment, GetTermNameFromDicByFileId(item.ItemId));
                }
            }
        }
        ...
        private void RemoveTermForGroupedItem(IExchangeItemGroup itemGroup, IExchangeItemBulkHelper bulkHelper, string folderId, string mailboxId, ExtendedPropertyDefinition def)
        {
            logger.Info($"Start batch update items.");
            var updateResult = new Dictionary<string, ExchangeUpdateItemResult>();
            try
            {
                mWorkerThreads.WaitOne();
                updateResult = bulkHelper.BatchRemoveExtendProperty(itemGroup.Items.ToList(), folderId, mailboxId, def);
            }
            finally
            {
                mWorkerThreads.Release();
            ...
            foreach (var item in itemGroup.Items)
            {
                ...
                EXOCommonUtil.AddDetailItem("RM_EXO_RemoveTerm", item, MailboxAddress, status, comment, string.Empty);
            }
        }


        private Guid GetTermId(IExchangeItem item)
        {
            using (var performance = new PerformanceScope("EXO.RuleManagement.GetTermId", addToStatistics: true))
            {
                ...
                try
                {
                    var rule = RuleManagement.CheckItemCriteria(item, RetentionLabels);
