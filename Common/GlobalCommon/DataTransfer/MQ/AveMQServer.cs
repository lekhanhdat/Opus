    ...
    internal class AveMQServer: IDisposable
    {
        ...
        public void PutMessage(AveMessage msg)
        {
            lock (mMessages)
            {
                if (msg.TimeOut < 0)
                {
                    msg.TimeOut = AveMQConfigure.MaxMessageTimeout;
                }
        ...
        private void Init()
        {
            ...
            performanceCounter = new MQPerformanceCounter(AveMQConfigure.EnablePerformanceCounter, System.Diagnostics.Process.GetCurrentProcess().ProcessName);
        }
        ...
        private void DumpMessage()
        {
            ...
            {
                AveThreadWrapper currentThreadWrapper = AveThreadUtility.CurrentThreadWrapper;
                const string DumpFileName = "C:\\DumpMQMessageQueue.AvePoint";
                while (currentThreadWrapper.KeepRunning)
                {
                    Thread.Sleep(2000);
                    ...
                    {
                        string fileName = Path.Combine(AveMQConfigure.TempFolder, "MQDUMP " + AveDateTimeUtility.ConvertToType013(DateTime.Now) + ".txt");
                        lock (mMessages)
                        {
