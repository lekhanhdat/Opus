...
{
    internal class ExchangeGraphExportController : IBackupController
    {
        private static readonly AvePoint.RA.Contract.Services.IRALogger logger = RALogger.GetInstance(typeof(ExchangeGraphExportController));
        private EXOConfiguration config = null;
        private EXOExportBeforeArcInfo EXOExportBefArcInfo = null;
        public ExchangeGraphExportController(EXOConfiguration mConfig, EXOExportBeforeArcInfo EXOExportBefArcInfo)
        {
            config = mConfig;
        ...
        public void Process(EXOArchiveData archiveData)
        {
            ...
            {
                ExchangeItemExportV2 exoItemExport = new ExchangeItemExportV2(logger) { Configuration = config };
