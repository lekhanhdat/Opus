...
namespace RAManualApprovalCommon
{
    public class ManualApprovalOwnerManager
    {
        ...
        private static bool TryGetOwnerDisplayName(int userIntId, out string displayName)
        {
            ...
            {
                var owner = AccountDao.Find(item => item.IsRemoved == 0 && item.Id == userIntId);
                if (owner == null)
                {
                    throw new Exception("RM_MA_NoOwner");
                }
        ...
        public static bool TryGetOwnerId(string userId, out int ownerId)
        {
            ...
            {
                var owner = AccountDao.Find(item => item.IsRemoved == 0 && item.UserId == userId);
                if (owner == null)
                {
                    throw new Exception("RM_MA_NoOwner");
                }
