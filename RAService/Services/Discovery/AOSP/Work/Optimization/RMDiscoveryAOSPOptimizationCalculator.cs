...
using AvePoint.RA.Service.Services.Discovery.AOSP.Work.Analyzer;
...
namespace AvePoint.RA.Service.Services.Discovery.AOSP.Work.Optimization
{
    public class RMDiscoveryAOSPOptimizationCalculator
    {
        ...
        private readonly RMDiscoveryAOSPOptimizationSettingsInfo _settingInfo;

        public RMDiscoveryAOSPOptimizationCalculator(Guid o365TenantId, RMDiscoveryAOSPSiteInfo siteInfo, RMDiscoveryAOSPOptimizationSettingsInfo settingInfo)
        {
        ...
        public async Task CalculateAsync()
        {
            try
            {
                await using (await RMRedisLockHandler.LockAsync(RMRedisLockKey.DiscoveryOptimizationCalculate, _siteInfo.SiteId.ToString(), TimeSpan.FromMinutes(20)))
                {
                    ...
                    var nextRunTime = 0L;
                    var optimizationSettingInfo = SerializerHelper.DeserializeByDataContractSerializer<RMDiscoveryAOSPOptimizationSetting>(RMDiscoveryAOSPOptimizationSetting.XMLCompatibleConvert(_settingInfo.Setting));
        ...
        private async Task<(long FileTotalSize, long VersionTotalSize)> CalculateOptimizableAsync()
        {
            ...
            var optimizationSettingInfo = SerializerHelper.DeserializeByDataContractSerializer<RMDiscoveryAOSPOptimizationSetting>(RMDiscoveryAOSPOptimizationSetting.XMLCompatibleConvert(_settingInfo.Setting));
            if (optimizationSettingInfo.ArchiveDataType == (int)Contract.Discovery.Model.Configuration.AOSP.ArchiverDataType.All)
            {
                var sql = $"{RMDiscoveryAOSPAnalysisConfiguration.ODATA_URI[_siteInfo.ContentSource]}?$apply=filter({string.Join(" and ", filters)})" +
            ...
            else
            {
                ...
                {
                    var rules = await DiscoverUtil.GetInactiveRuleAsync(_o365TenantId.ToString(), optimizationSettingInfo.InactiveRuleQueryParameter, optimizationSettingInfo.ArchiveDataType);
                    versionRules.AddRange(rules);
                }

                if (optimizationSettingInfo.ROTRuleQueryParameter.Enable)
                {
                    var rules = await DiscoverUtil.GetROTRuleAsync(_o365TenantId.ToString(), optimizationSettingInfo.ROTRuleQueryParameter, optimizationSettingInfo.ArchiveDataType);
                ...
                if (fileRules.Any())
                {
                    ...
                    var sql = $"{RMDiscoveryAOSPAnalysisConfiguration.ODATA_URI[_siteInfo.ContentSource]}?$apply=filter({string.Join(" and ", fileFilters)})" +
                    $"/aggregate(FileSize with sum as file_total_size, HistoryVersionsSize with sum as version_total_size)";
                    fileTotalSize = await CalculateFileTotalSizeAsync(sql);
                }
        ...
        && optimizationSettingInfo.SizeRangeQueryParameter.QueryMode != RMDiscoveryAOSPSizeRangeQueryMode.None
