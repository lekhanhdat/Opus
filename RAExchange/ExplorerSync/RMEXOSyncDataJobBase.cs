...
{
    public class RMEXOSyncDataJobBase : RMEXODiscoverBase, IEXOSyncDataJob
    {
        private static readonly AvePoint.RA.Contract.Services.IRALogger logger = RALogger.GetInstance(typeof(RMEXOSyncDataJobBase));
        ...
        private Dictionary<Guid, ExchangeFolder> mSubFolders = new Dictionary<Guid, ExchangeFolder>();
        ...
        public RMEXOSyncDataJobBase(ExchangeOnlineTreeNodeDto treeNode, JobManagement jobManagement)
            : base(treeNode)
        {
        ...
        private async System.Threading.Tasks.Task ProcessFailedItemsAsync(ExchangeFolder folder)
        {
            try
            {
                ...
                if (failedItems.Count > 0)
                {
                    ...
                    List<string> notExistIds = new List<string>();
                    ...
                    {
                        var result = folder.GetItemsByIds(failedItems.Select(i => i.NodeId).ToList());
                        items = result.Item1;
                        notExistIds = result.Item2;
                    }
                    ...
                    using (new RA.Common.PerformanceScope(string.Format("EXO.RMEXOSyncDataJobBase.ProcessFailedItems.LoadExtendPropery", "", true)))
                    {
        ...
        public virtual async System.Threading.Tasks.Task ProcessFailedItemAsync(ExchangeItem item, SyncFailureItemEntity failedItem, UniqueIdUtil idUtil, CancellationTokenSource cts = null)
        {
            var def = new Microsoft.Exchange.WebServices.Data.ExtendedPropertyDefinition(TermColumnInfo.WellKnowTermColumnGuid, TermColumnInfo.WellKnowTermColumnId, Microsoft.Exchange.WebServices.Data.MapiPropertyType.String);
            ...
            try
            {
                using (new RA.Common.PerformanceScope(string.Format("EXO.RMEXOSyncDataJobBase.ProcessFailedItem", "", true)))
                {
                    ...
                    string value;
                    if (item.TryGetExtendProperties(def, out value))
                    {
            ...
            //    //JobManagement.HasErrorNode = true;
            //    logger.Error($"An error occur in ProcessItem, item path {item.ItemPath}, reason : {ex.ToString()}.");
            ...
            catch (Exception e)
            {
                ...
                AnalyzeStatus(JobDetailsStatus.Failed);
            }
            finally
            {
        ...
        //protected BlockingCollection<ExchangeItem> GetSubItemsAsync(ExchangeFolder folder, string syncState)
        //{
        //    var result = new BlockingCollection<ExchangeItem>();
        //    ThreadPool.QueueUserWorkItem(obj =>
        //    {
        //        using (var performance = new PerformanceScope("EXO.RMEXOSyncDataJobBase.GetSubItemsAsync"))
        //        {
        ...
        //                logger.Info("Start to sync items, incrementalSync: {0}, total item count:{1}.", incrementalSync, folder.ItemsCount);
        ...
        private async System.Threading.Tasks.Task ProcessChangedTermAsync(ExchangeFolder currentFolder, long CollectionTime)
        {
        ...
        private void ProcessTermChangeGroupedItems(ExchangeFolder folder, List<BaseRecordDto> records, UniqueIdUtil idUtil)
        {
            try
            {
                ...
                using (AveAppendableTaskExecutor taskExecutor = new AveAppendableTaskExecutor(MaxBackupItemsThreads))
                {
                    ...
                    {
                        IEnumerable<ExchangeItemGroup> exchangeItems = null;
                        using (var performance1 = new PerformanceScope("EXO.RMEXODataSync.GetGroupItems", "", true))
                        {
                            ...
                            Discover.DiscoverImpl.FullDiscover fullDiscover = new Discover.DiscoverImpl.FullDiscover(help);
        ...
        protected async System.Threading.Tasks.Task ProcessTermChangeItemAsync(ExchangeItem item, UniqueIdUtil idUtil)
        {
            using (new PerformanceScope("RMEXOSyncDataJobBase.ProcessItem", addToStatistics: true))
            {
                ...
                try
                {
                    using (CheckJobStopScope jScope = new CheckJobStopScope())
                    {
                        var sensitivityLabelDef = new Microsoft.Exchange.WebServices.Data.ExtendedPropertyDefinition(Microsoft.Exchange.WebServices.Data.DefaultExtendedPropertySet.InternetHeaders, "msip_labels", Microsoft.Exchange.WebServices.Data.MapiPropertyType.String);
                        var def = new Microsoft.Exchange.WebServices.Data.ExtendedPropertyDefinition(TermColumnInfo.WellKnowTermColumnGuid, TermColumnInfo.WellKnowTermColumnId, Microsoft.Exchange.WebServices.Data.MapiPropertyType.String);
                        var properties = item.LoadExtendProperties(def, sensitivityLabelDef);
                        if (properties.ContainsKey(def))
                        {
                            ...
                            RMRuleItemCollection rules = null;
                            Guid termId;
                            if (Guid.TryParse(properties[def], out termId))
                            {
                                if (!TermExistInTermManagement(termId))
                                {
                                    logger.Info($"Term id no longer exists in term management for item : {item.ItemId}. Current term id:{termId}");
                                ...
                                //AddRecordToDB(recoInfo, recordInDB, recordInDB != null);
                                //JobManagement.ReportManager.SendJobDetail(new AvePoint.RA.Contract.RMWeb.JobMonitor.JMEXODataSyncJobDetails()
                                //{
                                //    ObjectName = item.ItemName,
                                //    FullPath = MailboxAddress + item.ItemPath + "_" + item.SendDateUTC.ToString("R"),
        ...
        private async System.Threading.Tasks.Task ProcessTermChangeItemsByGroupAsync(ExchangeItemGroup itemGroup, List<BaseRecordDto> records, UniqueIdUtil idUtil)
        {
        ...
        private async System.Threading.Tasks.Task ProcessFolderAsync(ExchangeFolder folder)
        {
            ...
            using (var performance = new PerformanceScope("EXO.RMEXODataSync.ProcessFolder", "", true))
            {
                try
                {
                    ...
                    discoverHelper.GetGroupedDeleteItems(folder, nodeInfo?.ItemSyncState);
                    var deleteItemIds = discoverHelper.deleteItemIds;
        ...
        private void AddFailureItem2Cache(ExchangeItem exoItem, ExchangeFolder folder, Exception e)
        {
        ...
        private Microsoft.Exchange.WebServices.Data.SearchFilter GenerateSearchFilter(ExchangeFolder folder)
        {
            DateTime collectionTime = DateTime.MinValue;
            var nodeInfo = EXONodeInfoDao.GetEXONodeInfo(folder.FolderId.ToMd5(), GroupId, (int)NodeFlagType.ExplorerSync);
            if (nodeInfo != null)
        ...
        //private void ProcessItems(ExchangeFolder folder)
        //{
        //    var logonGroupId = TenantLocalValue.LogonGroupId;
        //    var logonUserEmail = TenantLocalValue.LogonUserEmail;
        //    using (AveAppendableTaskExecutor taskExecutor = new AveAppendableTaskExecutor(25))
        //    {
        //        taskExecutor.StartExecute();
        //        ////目前准备采用Batch方式操作，如果需要操作single item，此处的discover应该是IDiscover 接口对象
        //        foreach (var item in discover.GetItems(folder))
        //        {
        //            taskExecutor.AddTask(() =>
        //            {
        //                TenantLocalValue.LogonGroupId = logonGroupId;
        //                TenantLocalValue.LogonUserEmail = logonUserEmail;
        //                JobManagement.ReportManager.Increase();
        //                logger.Info($"Begin processing item : {item.ItemPath}.");
        //                ProcessItem(item);
        //            });
        //        }
        //        logger.Info($"Add items to task executor finished.");
        ...
        private async System.Threading.Tasks.Task ProcessItemsAsync(ExchangeItemGroup itemGroup, ExchangeFolder folder, int retryCount = 0)
        {
            List<ExchangeItem> nonePropertyItems = new List<ExchangeItem>();
            try
            {
                logger.Info($"Begin process grouped item, item count: {itemGroup.ItemsCount}.");
                var def = new Microsoft.Exchange.WebServices.Data.ExtendedPropertyDefinition(TermColumnInfo.WellKnowTermColumnGuid, TermColumnInfo.WellKnowTermColumnId, Microsoft.Exchange.WebServices.Data.MapiPropertyType.String);
                var sensitivityLabelDef = new Microsoft.Exchange.WebServices.Data.ExtendedPropertyDefinition(Microsoft.Exchange.WebServices.Data.DefaultExtendedPropertySet.InternetHeaders, "msip_labels", Microsoft.Exchange.WebServices.Data.MapiPropertyType.String);
                using (new RA.Common.PerformanceScope("EXO.RMEXOSyncDataJobBase.ProcessItems.LoadExtendPropery", addToStatistics: true))
                {
                    ExchangeItemBulkHelper bulkHelper = new ExchangeItemBulkHelper(folder, "");
            ...
            finally
            {
                if (nonePropertyItems.Count > 0)
                {
                    try
                    {
                        ...
                        if (retryCount < 2)
                        {
                            await ProcessItemsAsync(new ExchangeItemGroup(nonePropertyItems.AsEnumerable()), folder, ++retryCount);
                        }
                        else
                        {
        ...
        private void AddFailedDetail(ExchangeItem item, string errorMessage)
        {
            JobManagement.ReportManager.SendJobDetail(new AvePoint.RA.Contract.RMWeb.JobMonitor.JMEXODataSyncJobDetails()
            {
                ObjectName = item.ItemName,
        ...
        protected async System.Threading.Tasks.Task ProcessItemAsync(ExchangeItem item, UniqueIdUtil idUtil)
        {
            using (new PerformanceScope("RMEXOSyncDataJobBase.ProcessItem", addToStatistics: true))
            {
                logger.Info($"Begin processing item : {item.ItemId}.");
        ...
        private async System.Threading.Tasks.Task AddRecordToDBAsync(Record recoEntity, Record dbRec, bool getDBRecord = true)
        {
            ...
            //if (useBatchUpdate)
            //{
            //    cacheList.Add(recoEntity);
            //    if (cacheList.Count() >= BatchUpdateCount)
            //    {
            //        using (new RA.Common.PerformanceScope(string.Format("CollectionData.Scan.BatchSaveItem:{0}", recoEntity.FullPath)))
            //        {
            //            ExplorerDao.BatchAddRecords(cacheList);
            //            //var details = cacheList.ConvertAll(m => { return new JMCollectionDataJobDetails() { ObjectName = m.LeafName, FullPath = m.FullPath, Status = JobDetailsStatus.Successful }; });
            //            //finalDetails.AddRange(details);
            //            //ReportManager.BatchSendJobDetail(finalDetails);
            //            //finalDetails.Clear();
            //            //ClearBatchCache();
            ...
            //    if (ExplorerDao.AddOrUpdateRecord(recoEntity, false))
            //    {
            //        //finalDetails.Add(new JMCollectionDataJobDetails() { ObjectName = recoEntity.LeafName, FullPath = recoEntity.FullPath, Status = JobDetailsStatus.Successful });
            //        //ReportManager.BatchSendJobDetail(finalDetails);
            //        //finalDetails.Clear();
            //    }
            //}
        ...
        private async System.Threading.Tasks.Task ProcessSucceedRecordAsync(Record item)
        {
            ...
            AnalyzeStatus(JobDetailsStatus.Successful);
        ...
        /*public void FinallyBatchUpdate(Guid siteId)
        {
            try
            {
                if (useBatchUpdate && cacheList.Count() > 0)
                {
                    //if (CollectionDataDao.BatchAddRecords(cacheList))
                    //{
                    //ExplorerDao.BatchAddRecords(cacheList);
                    //var details = cacheList.ConvertAll(m => { return new JMCollectionDataJobDetails() { ObjectName = m.LeafName, FullPath = m.FullPath, Status = JobDetailsStatus.Successful }; });
                    //finalDetails.AddRange(details);
                    ...
                    //else
                    //{
                    //    logger.Warn("batch update failed:{0}", cacheList.Count);
                    //}
                }
            }
            catch (Exception ex)
            {
                //if (siteId != Guid.Empty && !NeedRevertCollecDateTime.Contains(siteId))
                //{
                //    NeedRevertCollecDateTime.Add(siteId);
                //}
                //logger.Warn("error when batch add items {0}:{1}", siteId, ex.ToString());
                //HasErrorNode = true;
                //foreach (var item in cacheList)
                //{
                //    AddFailedDetail(item, ex.Message);
                //}
        ...
        private Record GenerateRecordInfo(ExchangeItem item, Guid termId, Rule disposalRule, string dueDisposalTime, Record recordInDB, UniqueIdUtil idUtil)
        {
        ...
        private EXONodeFlag GenerateNodeFlag(ExchangeFolder folder, DateTime collectionTime)
        {
        ...
        private Dictionary<string, CustomColumn> GetEXOCustomMetadata(ExchangeItem item, Record record)
        {
            ...
            foreach (var mapping in _customIndexMetadatas)
            {
                try
                {
                    ...
                    var value = GetEWSItemPropertyValue(item, mapping.SourceColumnName);
                    if (value == null)
                    {
                        logger.Warn($"Cannot get value for EWS column [{mapping.SourceColumnName}].");
