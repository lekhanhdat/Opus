...
{
    public class RecordDestructionPickStatusQueryBuilder : IFilterBuilder, IAdvancedQueryBuilder
    {
        #region BaseArrayFilterBuilder
        private bool CanFilter(ExplorerFilterOptionV2 filterOption)
        {
            return filterOption != null && filterOption.DestructionStatus != null && filterOption.DestructionStatus.Count > 0;
        }

        private string GetFilterColumnName()
        {
            return CosmosConst.C_DestructionPickStatus;
        }

        private List<PickStatusType> GetFilterValue(ExplorerFilterOptionV2 filterOption)
        {
            return filterOption.DestructionStatus;
        }
        #endregion
        ...
        {
            return Contract.TemplateManagement.QueryCloumnIds.DestructionPickStatus;
        }
        ...
        private ExplorerFilterOptionV2 Convert2SearchOptionV2(ExplorerQueryColumn column, string objJson, ExplorerSearchColumnOperationLogic columnOperationLogic, ExplorerSearchKeyOperationLogic keyOperationLogic)
        {
            ...
            {
                DestructionStatus = JsonConvert.DeserializeObject<List<PickStatusType>>(objJson)
            };
        }
