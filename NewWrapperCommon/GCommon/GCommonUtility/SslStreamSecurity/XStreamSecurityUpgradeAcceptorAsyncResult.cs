...
{
    internal abstract class XStreamSecurityUpgradeAcceptorAsyncResult : XTraceAsyncResult
    {
        private SecurityMessageProperty remoteSecurity;

        private Stream upgradedStream;

        private static AsyncCallback onAuthenticateAsServer = XDiagnosticUtility.ThunkAsyncCallback(new AsyncCallback(OnAuthenticateAsServer));

        protected XStreamSecurityUpgradeAcceptorAsyncResult(AsyncCallback callback, object state) : base(callback, state)
        {
        }
        ...
        public void Begin(Stream stream)
        {
            ...
            try
            {
                asyncResult = this.OnBegin(stream, onAuthenticateAsServer);
            }
        ...
        {
            XStreamSecurityUpgradeAcceptorAsyncResult streamSecurityUpgradeAcceptorAsyncResult = End<XStreamSecurityUpgradeAcceptorAsyncResult>(result);
        ...
        private static void OnAuthenticateAsServer(IAsyncResult result)
        {
            ...
            XStreamSecurityUpgradeAcceptorAsyncResult streamSecurityUpgradeAcceptorAsyncResult = (XStreamSecurityUpgradeAcceptorAsyncResult)result.AsyncState;
