...
namespace LS.SPWorkflowProcessor
{
    ...
    public class AveSPFieldCollection
    {
        ...
        public void Initialize(string srcSchema, IAveFieldCollection dstFields,bool createIfNotExist)
        {
            ...
            try
            {
                ...
                foreach (XmlNode node in doc.FirstChild.ChildNodes)
                {
                    if (node is XmlElement)
                    {
                        ...
                        {
                            logger.Log(AveLogLevel.DEBUG, WrapperWorkflowResource.GetWorkflowFieldTypeError, e.ToString());
                        }
                        ...
                        if (fieldObj == null && createIfNotExist)
                        {
                            ...
                            {
                                var exception = ex as SPWFProcessorException;
                                if (exception != null && exception.ErrorCode == SPWFProcessorErrorCode.SkipOnlineListField)
                                {
                                    logger.Log(AveLogLevel.DEBUG, "Skip restore specific online field. {0}", exception.InnerException);
                                }
                                else
                                {
                                    logger.Log(AveLogLevel.WARN, WrapperWorkflowResource.FieldCreatedFailed, ex);
                                }
                            }
                        }
                        aveField.SetAveFieldBySPField(spField);
                        if (!mAveFields.ContainsKey(aveField.SerializableData.mSrcInternalName))
                        {
                            mAveFields.Add(aveField.SerializableData.mSrcInternalName, aveField);
                        }
                        if (!mAveFieldIdDic.ContainsKey(aveField.SerializableData.mSrcId))
                        {
            ...
            catch (Exception e)
            {
                ...
                logger.Warn("An exception occurred while initial field collection. exception:{0}", e.ToString());
        ...
        public IAveField CreateSPField(XmlElement schemaXML,bool addToDefaultView,AveAddFieldOptions op)
        {
            ...
            try
            {
                string origId = schemaXML.GetAttribute("ID");
                var id = new Guid(origId);
                if (!mDstCollection.Web.Site.IsOnlineSite 
                    && OnlineFieldId.NeedSkipOnlineField.FirstOrDefault(f => f == id) != Guid.Empty)
                {
                    throw new SPWFProcessorException(SPWFProcessorErrorCode.SkipOnlineListField, new Exception(string.Format("FieldId: {0}, FieldName: {1}", origId, origName)));
                }
            ...
            catch (Exception e)
            {
                ...
                logger.Warn("An exception occurred while create field. exception:{0}", e.ToString());
        ...
        public void Output(string outputFile)
        {
            ...
            {
                logger.Log(AveLogLevel.DEBUG, WrapperWorkflowResource.WriteToFileError, e.ToString());
            }//need not to log
            finally 
            {
                if (sw != null)
    ...
    public class SPFieldProcessor
    {
        ...
        //TODO 这个是备份过程需要的缓存，但是如果备份的东西比较大，可能会导致内存比较大，需要以后考虑的，比如只缓存最近使用的10个
        private static Dictionary<Guid, Dictionary<string, string>> mDBFieldToSPFieldDic;
        ...
        {
            Dictionary<string, string> fieldDic = null;
            lock (logger)
            {
                if(mDBFieldToSPFieldDic == null)
                {
                    mDBFieldToSPFieldDic = new Dictionary<Guid, Dictionary<string, string>>();
                    fieldDic = GetDBFieldToSPFieldDic(list.Fields.SchemaXml);
                    mDBFieldToSPFieldDic[list.ID] = fieldDic;
                }
                else
                {
                    if(!mDBFieldToSPFieldDic.TryGetValue(list.ID, out fieldDic))
                    {
                        fieldDic = GetDBFieldToSPFieldDic(list.Fields.SchemaXml);
                        mDBFieldToSPFieldDic[list.ID] = fieldDic;
                    }
        ...
        private Dictionary<string, string> GetDBFieldToSPFieldDic(string schema)
        {
            ...
            try
            {
                ...
                foreach (XmlNode node in doc.FirstChild.ChildNodes)
                {
                    if ((node is XmlElement) && (node.Name.Equals("Field", StringComparison.OrdinalIgnoreCase) || node.Name.Equals("FieldRef", StringComparison.OrdinalIgnoreCase)))
                    {
                        ...
                        string colName = field.GetAttribute("ColName").ToLower(CultureInfo.CurrentCulture);
            ...
            catch(Exception e)
            {
                ...
                logger.Warn("An exception occurred while convert database field to SPField. exception:{0}", e.ToString());
        ...
        public void ConvertPropsToDBField(Hashtable props, string srcSchemaXML, IAveList dstList,bool isNewItem)
        {
            ...
            foreach (DictionaryEntry de in cloneProps)
            {
                try
                {
                    ...
                    //nvarchar的长度不能超过255.
                    if(realName.ToString().Contains("nvarchar", StringComparison.OrdinalIgnoreCase))
                    {
                        var stringValue = value as string;
                        if (stringValue != null && stringValue.Length > 255)
                        {
                            logger.Warn("Cut off the value of {0}", key);
                ...
                catch (Exception e)
                {
                    ...
                    logger.Warn("An exception occurred while convert properties. exception:{0}", e.ToString());
                }
            }
