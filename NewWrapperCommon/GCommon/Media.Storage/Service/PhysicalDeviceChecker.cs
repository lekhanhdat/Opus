...
namespace AvePoint.Media.Storage.Service
{
    ...
    public class PhysicalDeviceChecker : IPhysicalDeviceChecker
    {
        public PhysicalDeviceDescription CheckDevice(PhysicalDeviceDto pdDto)
        {
            try
            {
                using (IXSystem system = XFactory.InstanceSystem(pdDto.ConnectionString))
                {
                    StorageOpenValidResult rs = system.Validate();
                    return new PhysicalDeviceDescription()
                    {
                        IsOnline = rs.SystemHealth == Util.XSystemHealth.Available || rs.SystemHealth == Util.XSystemHealth.AvailableAndNotFull,
                        Message = rs.Message,
